...
// a package repository (apt/dnf/yum/apk; a safe declarative MVP).
//
// States:
...
func (m *Module) Validate(_ context.Context, req *pluginv1.ValidateRequest) (*pluginv1.ValidateReply, error) {
	...
	gpgKey, gkErr := util.OptStringParam(req.Params, "gpg_key")
	if gkErr != nil {
		errs = append(errs, gkErr.Error())
	}
	// gpg_key_path (variant B): reference an existing on-host key, no copy.
	// Path must be absolute+traversal-free; mutually exclusive with inline gpg_key.
	if gpgKeyPath, gpErr := util.OptStringParam(req.Params, "gpg_key_path"); gpErr != nil {
		errs = append(errs, gpErr.Error())
	} else if gpgKeyPath != "" {
		if perr := validateAbsPath("gpg_key_path", gpgKeyPath); perr != nil {
			errs = append(errs, perr.Error())
		}
		if gpgKey != "" {
			errs = append(errs, `params "gpg_key" and "gpg_key_path" are mutually exclusive`)
		}
	}
	if dest, dErr := util.OptStringParam(req.Params, "dest"); dErr != nil {
		errs = append(errs, dErr.Error())
	} else if dest != "" {
		if perr := validateAbsPath("dest", dest); perr != nil {
			errs = append(errs, perr.Error())
		}
	}
...
func (m *Module) readParamsFromPlan(req *pluginv1.PlanRequest) (repoParams, error) {
	...
	if p.gpgKeyPath, err = util.OptStringParam(req.Params, "gpg_key_path"); err != nil {
		return p, err
	}
	if p.dest, err = util.OptStringParam(req.Params, "dest"); err != nil {
		return p, err
	}
...
func (m *Module) planPresent(stream grpc.ServerStreamingServer[pluginv1.PlanEvent], mgr util.PkgMgr, p repoParams) error {
	...
	case util.PkgMgrApt:
		...
		// Inline-key drift only: variant B references an external key the
		// module does not own (existence guarded above, content not tracked).
...
func (m *Module) planAbsent(stream grpc.ServerStreamingServer[pluginv1.PlanEvent], mgr util.PkgMgr, p repoParams) error {
	switch mgr {
	case util.PkgMgrApt:
		_, existed, err := readFile(m.aptListPath(p))
		if err != nil {
			return util.PlanFailed(err.Error())
		}
		return util.SendPlanFinal(stream, existed)
	case util.PkgMgrDnf, util.PkgMgrYum:
		_, existed, err := readFile(m.yumRepoPath(p))
		if err != nil {
			return util.PlanFailed(err.Error())
		}
		return util.SendPlanFinal(stream, existed)
...
// validateAbsPath ensures a caller-supplied filesystem path (gpg_key_path /
// dest) is absolute and free of ".." traversal — the module reads/writes it
// directly, so a relative or traversing path is a security error.
func validateAbsPath(field, p string) error {
	if !filepath.IsAbs(p) {
		return fmt.Errorf("param %q: must be an absolute path, got %q", field, p)
	}
	if strings.Contains(p, "..") {
		return fmt.Errorf("param %q: must not contain %q, got %q", field, "..", p)
	}
	return nil
}
