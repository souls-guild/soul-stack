...
- **Plugins & SDK** — gRPC-over-stdio plugin infrastructure (SoulModule / CloudDriver / SshProvider) with a public `sdk/`. The companion web UI lives in the `soul-stack-web` repo.
...
- **Cloud integration:** module `keeper.cloud`, **Provider** and **Profile** live in Postgres (managed via API/MCP). Cloud-create is a script step `module: core.cloud.provisioned` with `on: keeper` ([ADR-017](docs/adr/0017-keeper-side-core.md), keeper-side core) calling the `CloudDriver` plugin (`soul-cloud-<provider>`). The old "destiny `cloud-provision`" design has been abandoned - this is not a task package for Soul, but a keeper-side operation. A service's default vars live in git; a fleet that needs different ones forks the service repo ([ADR-0082](docs/adr/0082-service-vars.md)).
