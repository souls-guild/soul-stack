...
- **Documentation and ADRs** — 69 ADRs in [docs/adr/](docs/adr/README.md) (one ADR = one file `NNNN-<slug>.md`); [docs/architecture.md](docs/architecture.md) — overview + stub links to ADR; `docs/`-areas cover input / templating / destiny / scenario / soul[+soulprint] / keeper / destiny-output / migrations and more.
...
- **`LICENSE`** BSL 1.1 (fair-code; core → Apache 2.0 after 2 years); `sdk/`, `examples/` and the plugin contracts stay Apache 2.0 ([ADR-016](docs/adr/0016-parity-license.md), Amendment 2026-07-09).
...
- [docs/architecture.md](docs/architecture.md) - architecture review + stub links to ADR, end-to-end script, sections about connection/push/Reaper, open questions. Full ADRs - in [docs/adr/](docs/adr/README.md) (69 files + index with statuses).
...
- **ADR-016. parity strategy + Soul Stack license:** **BSL 1.1** for the core (this repo — `keeper`/`soul`/`soul-lint`/`shared`) and the web UI (`soul-stack-web`), converting to **Apache 2.0** after 2 years (Change License); `sdk/`, `examples/` and the plugin contracts stay **Apache 2.0** (fair-code Amendment 2026-07-09; `LICENSE` at the root). Parity strategy - **hybrid without wrapper**: core MVP - our rewrite in Go, exotic - community plugins `soul-mod-*`/`soul-cloud-*`/`soul-ssh-*` via Go SDK. **Wrapping GPLv3 modules is prohibited** (copyleft + a foreign language runtime on the host contradicts "security first" + external template engines do not match CEL+text/template ADR-010). CLA - required from the first external contributor. Staged map: Phase 0 core MVP → Phase 1 SDK + `soul-mod-template` → Phase 2 first 10 official `soul-mod-*` → Phase 3 community-onboarding → Phase 4 cloud parity (3 CloudDrivers in MVP).
