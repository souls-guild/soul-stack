...
All source changes are in **English** — code, comments, log/error strings, tests, godoc, `examples/`, ADR/README/CONTRIBUTING, and these instructions. **No Cyrillic in sources.** Exception: i18n product content of public sites (docs prose and landing RU/EN translations) stays bilingual for readers. Replies to the user are not a source change — they follow the user's preferred language. Remaining migration across the other repos is tracked in NIM-118.

## Repository status

Soul Stack is a working configuration-management system, built out across several release trains. Its four binaries — `keeper`, `soul`, `soulctl`, `soul-lint` — carry real logic across implemented `internal/` subsystems. In the repo:
...
- **Go workspace** per [ADR-011](docs/adr/0011-go-layout.md): `go.work` + 8 product modules (`proto/`, `proto/plugin/`, `shared/`, `sdk/`, `keeper/`, `soul/`, `soul-lint/`, `soulctl/`) plus the four test modules under `tests/`, each with implemented `internal/` subsystems.
- **`keeper`** — stateless, horizontally-scalable cluster over Postgres + Redis: the gRPC Keeper↔Soul stream ([ADR-012](docs/adr/0012-keeper-soul-grpc.md)), Destiny render (CEL + Go text/template), Soul- and Keeper-side core modules, the Reaper, RBAC and operator identity (Archon / JWT), machine provisioning through ordinary `side: keeper` SoulModule plugins, Vault integration, plus OpenAPI + MCP surfaces.
...
- **Plugins & SDK** — gRPC-over-stdio plugin infrastructure (SoulModule / SshProvider — CloudDriver was removed in NIM-761) with a public `sdk/`. The companion web UI lives in the `soul-stack-web` repo.
- **`LICENSE`** BSL 1.1 (fair-code; production use is Internal Use only, each version turns Apache 2.0 two years after its release); `sdk/`, `proto/plugin/` and `examples/` each carry their own Apache 2.0 `LICENSE` ([LICENSING.md](LICENSING.md), [ADR-016](docs/adr/0016-parity-license.md) Amendment 2026-07-09).
- **`Makefile`** — `gen` / `build` / `test` / `check` / packaging & signing targets; `make check` is the local CI gate (fmt, vet, build, tests, doc-link + OpenAPI + template checks, vuln scan, lint). Distribution ships as binary / deb / docker / apt artifacts with cosign signing.

Work follows a release-train model: a `release/<REL>` branch per train, `feat/<TASK>` branches per ticket, each developed in its own git worktree and squash-merged back into the release, then the release into `main`. The doc-first ethos still holds — an architectural decision lands in an ADR before or with the code (see "Documentation ahead of code") — but the system is built; implementing a component no longer waits on a separate "start coding" gate.
...
- **New ADR** - new file in `docs/adr/` + line in [docs/adr/README.md](docs/adr/README.md) (with status) + stub link in [docs/architecture.md](docs/architecture.md); if ADR introduces a new name, also in [docs/naming-rules.md](docs/naming-rules.md).
- **A new ADR is written WITHOUT a number**, as `docs/adr/draft-<slug>.md`, and cited by that path while the ticket is in flight. The number is stamped **in the release worktree, at squash-merge** - the point where the release tip is known and tickets merge one at a time: rename to `NNNN-<slug>.md` and write the index row with that number. The `NNNN-<slug>.md` filename format and the `ADR-062` citation form are **unchanged**; only the moment of allocation moved. What it fixes: two parallel sessions each read "the highest is 0085" from their own worktree and both take 0086, and the clash surfaces only at rebase - in the filename and in the index row at once, the one place a merge cannot silently pick a side. Renumbering the 81 existing ADRs is **not** decided - do not propose a scheme.
...
The naming convention ([docs/naming-rules.md](docs/naming-rules.md)) is required everywhere: in the names of packages, types, files, endpoints, CLI flags, metrics, labels, log messages and user documentation. Use **Keeper**, **Souls**, **Destiny**, **Soulprint** (see table in the "Intent" section). A service's parameters are its **vars** (`<service>/vars/`, CEL root `vars.*`, [ADR-0082](docs/adr/0082-service-vars.md)). The primary terms are ours; use them consistently across code and docs.
...
- [docs/keeper/modules.md](docs/keeper/modules.md) - specification of all seven keeper-side core modules (`core.soul` / `core.vault` / `core.choir` / `core.bootstrap` / `core.cert` / `core.ssh` / `core.state`).
- [docs/keeper/rbac.md](docs/keeper/rbac.md) - RBAC and Bootstrap of the first Archon (ADR-013).

## Intent

Soul Stack is a configuration management system with its own dictionary of names in the "soul" metaphor.

| Soul Stack | Meaning |
|---|---|
| Keeper | Guardian, central hub |
| Souls | Managed Agents |
| Destiny | What will be applied to the host after the run |
| Soulprint / Prints | Facts about the system |
...
- **ADR-007. Artifact versioning:** The Service/Destiny/Module version is a **git ref** (tag or branch), not a field in the manifest. Top level field `version:` in `service.yml`/`destiny.yml`/`manifest.yaml` **missing**; dependencies are written via `ref: v2.0.0` (or `ref: main`), no semver-range. Exception (this is **not** an "artifact version"): `protocol_version` in the module manifest (SoulModule API compat flag). The former `state_schema_version` exception is **retired** (amendment 2026-09-01 / NIM-735): the manifest key is gone, the state-schema version is derived from the migration ladder. The runtime column `incarnation.state_schema_version` and the API/MCP fields of that name are unchanged.
- **ADR-008. Coven = stable tags only:** Coven - stable logical tags of the host (cluster / project / environment / data center). Role (master/replica) **NOT Coven**, former convention `{incarnation.id}-{role}` sub-covens **removed**. `incarnation.id` is **not** a Coven either (amendment 2026-07-17 / NIM-124; the CEL root was spelled `incarnation.name` until ADR-0085 / NIM-730, and the offline rule `on_incarnation_id` matches both spellings for the length of that compatibility window): incarnation **membership** is the first-class relation `incarnation_membership`. A label is never inherited — a host carries only the covens and traits an operator attached to it, and belonging to an incarnation attaches nothing (amendment 2026-08-05 / NIM-281). The volatile role is determined only by the inline-probe step in the script (`core.exec.run`, `register:`) and `where:` in this register. The declared role lives only in the host's Choir Voice (`incarnation_choir_voices.role`; `incarnation.spec.hosts[]` was removed by ADR-044 amendment 2026-07-30 / NIM-330), is reflected in `soulprint.hosts[].role` (`null` for a host with no Voice) and is written on bootstrap `create` by a `core.choir.present` step (`on: keeper`), where probe is not possible. service vars are role-agnostic (there is no `role/<Y>.yaml` stage in the assembly).
- **ADR-009. Scenario = full DSL destiny + orchestration:** scenario gets all destiny task blocks (`module:` including modifying / `templates` / `vars` / `register` / `changed_when` / `onchanges` / `onfail` / `require` / `retry` / …) + orchestration delta (`on:` / `where:` / `serial:` / `run_once:` / `apply:`; the state write is a `core.state.<verb>` keeper-side step since [ADR-0084](docs/adr/0084-explicit-state-capture.md), not a `state_changes:` block). The boundary destiny↔scenario is **recommendation** ("reused / critical / isolated → put in destiny", otherwise you can inline). Two-level resolution of resources `templates/`/`vars.yml`/`tests/`/`include:` - locally in `scenario/<name>/`, then service-level. Specification - [docs/scenario/](docs/scenario/README.md). Output destiny: declared top-level `output:` in `destiny.yml` (symmetrically `input:`, [docs/destiny/output.md](docs/destiny/output.md)), read through `register:` on the applier task - **design only, unimplemented**: a task-level `output:` is refused (`output_unsupported`) and `register.<applier>.<field>` does not resolve. New entities: `soulprint.hosts` (scenario-only accessor of run hosts with stable facts - [orchestration.md §4.1](docs/scenario/orchestration.md)); keeper-side core modules (dispatcher - `on: keeper`; first - `core.soul.registered`, [docs/keeper/modules.md](docs/keeper/modules.md)).
...
- **ADR-011. Go code layout:** `go.work` with seven modules as decided — **eight** in the tree today (`soulctl/` joined after the ADR), plus four test modules under `tests/`. `proto/` (internal Keeper↔Soul, Operator API, committed `proto/gen/go/`), `proto/plugin/` (separate go.mod submodule with the service contracts SoulModule/SshProvider/soul_beacon + handshake — CloudDriver was removed in NIM-761, `KIND_CLOUD_DRIVER` is a reserved enum value; plugin authors only pull it), `shared/` (transverse code of all binaries: `obs`/`log`/`config`/`vault` - client only, Soul-safe/`tlsx`/`cel`/`tmpl`), `sdk/` (public SDK: `module`/`sshprovider`/`beacon`/`schema`/`handshake`; `clouddriver` went with the contract, `cloudutil` stayed), `keeper/` (`cmd/keeper` + `internal/` with all server-side subsystems including `vault` server-side), `soul/` (`cmd/soul` + `internal/`; **NOT** `require .../keeper` - Soul isolation is guaranteed by the compiler, not the CI linter), `soul-lint/` (`cmd/soul-lint` + `internal/`). The core modules of both sides implement the common interface from `sdk/module/`. Module path placeholder `github.com/soul-stack/soul-stack/<module>` (renaming by sed when pushing to remote hosting). Shared tags (one git-tag on the root repo = one logical version of all modules; third exception in ADR-007 for Go-library dependencies). `examples/` - non-Go artifacts only. Full layout with ASCII tree - [ADR-011](docs/adr/0011-go-layout.md).
...
- **ADR-015. Core MVP modules:** basic MVP - 12 Soul-sides (`core.pkg`/`core.file`/`core.service`/`core.user`/`core.group`/`core.exec`/`core.cmd`/`core.cron`/`core.mount`/`core.git`/`core.archive`/`core.sysctl`), post-MVP for real requests added `url`/`line`/`repo`/`firewall`/`http` (total 17 Soul-sides in ADR) + 3 Keeper-sides (`core.soul.registered` was already there, `core.cloud.provisioned` and `core.vault.kv-read` are introduced by ADR-017 — ⛔ `core.cloud` was removed in NIM-761). `core.template` is NOT highlighted - the rendering is done by `core.file.rendered` (drift in the architecture is fixed). `core.copy` is NOT highlighted - covered by `core.file.present` with inline-content. `core.line` (lineinfile) accepted in stripped-down safe MVP (in-place line-by-line edit pilot) - no backrefs, replace first match. The actual registry fact (with `core.augur` ADR-025 and `core.choir` ADR-044) is in [docs/module/README.md](docs/module/README.md#catalog-status). Closes open Q No. 5 regarding core MVP.
- **ADR-016. parity strategy + Soul Stack license:** **BSL 1.1** for the core (this repo — the `keeper`/`soul`/`soulctl`/`soul-lint` binaries + the built-in `core.*` modules) and the web UI (`soul-stack-web`); each version turns **Apache 2.0** two years after its own first public release (Change License). The Additional Use Grant covers **Internal Use only** — running Soul Stack for your own or your organization's infrastructure, commercial internal operations included; providing it to third parties (hosted/managed service, product, white-label, OEM) needs a separate commercial license. `sdk/`, `proto/plugin/` and `examples/` each carry their own **Apache 2.0** `LICENSE` and stay permissive — every plugin statically links the generated `pluginv1` stubs, so BSL there would leak into third-party plugin binaries (fair-code Amendment 2026-07-09). Binding text — [`LICENSE`](LICENSE); the readable version, incl. the managed-provider boundary — [LICENSING.md](LICENSING.md). Check the license boundary against those two files, not against this summary. Parity strategy - **hybrid without wrapper**: core MVP - our rewrite in Go, exotic - community plugins via Go SDK (SoulModule artifacts, named after the module they serve since NIM-851, and `soul-ssh-*` providers). **Wrapping GPLv3 modules is prohibited** (copyleft + a foreign language runtime on the host contradicts "security first" + external template engines do not match CEL+text/template ADR-010). CLA - [CLA.md](CLA.md) is in the tree and is required from the first external contributor. Staged map: Phase 0 core MVP → Phase 1 SDK + `template` → Phase 2 first 10 official plugins → Phase 3 community-onboarding → Phase 4 cloud parity (3 CloudDrivers in MVP).
- **ADR-017. Keeper-side core expanded:** ⛔ `core.cloud.provisioned` (`created`/`destroyed`) — CloudDriver call from scenario, replaces the "destiny `cloud-provision`" pattern; **removed in NIM-761** together with the contract, a machine is created by an ordinary `side: keeper` SoulModule plugin with its own address. `core.vault.kv-read` (verb `read`) - explicit reading of Vault KV on the keeper side for audit-accurate cases; implicit `${ vault(...) }` in CEL remains. The SoulModule contract is the same for both parties (ADR-009).
...
- **ADR-019. State_schema migration DSL:** grammar flat (`rename`/`set`/`delete`/`move`) + CEL expressions in `set.value` through `${ … }` + structured `foreach` (`in:`/`as:`/`do:`) - extension of the "flat DSL" ADR-009. Conditional `if:` key - deferred until the first real request (extension without breaking change). Forward-only in MVP (`down:` is not supported, recovery via `state_history`). Escape module `state.migrate` rejected (name out of dictionary, grammar covers 90%+ cases; candidate `core.incarnation.state-migrate` - separate ADR if necessary). Atomicity - one PG transaction for the entire chain of migrations (SELECT FOR UPDATE → in-memory in-Go application → snapshot per-step in state_history → COMMIT; if there is a failure, rollback + status: migration_failed). Migration-CEL sandbox: available `state.*` (mutable) + `<as-name>` inside foreach; prohibited `vault(...)`/`now()`/`register.*`/`soulprint.*`/`vars.*`/`input.*` (migration = pure function from old state). Tests - `migrations/<NNN>_<slug>/tests/<case>.yml` (state_before → migration → assert state_after, symmetrically destiny/scenario). Full spec - [`docs/migrations.md`](docs/migrations.md), commit - [ADR-019](docs/adr/0019-state-migration-dsl.md). Closes open Q No. 18.
...
- **Module model:** basic MVP set - **12 Soul-side core by [ADR-015](docs/adr/0015-core-modules-mvp.md)** (`core.pkg`/`core.file`/`core.service`/`core.user`/`core.group`/`core.exec`/`core.cmd`/`core.cron`/`core.mount`/`core.git`/`core.archive`/`core.sysctl`) - statically built into `soul`-binary. **The registered count is 21 Soul-side + 7 Keeper-side.** The authority for the count — derived from `soul/internal/coremod.Default` and `keeper/internal/coremod.Default` — is [docs/module/README.md → Catalog status](docs/module/README.md#catalog-status). Check a figure there before quoting it: every copy of that list in the tree had drifted at least once by NIM-884. **File rendering is done by `core.file.rendered`** (NOT `core.template`, which is deliberately NOT highlighted - drift is removed). **Keeper-side core**: the side follows from the ADDRESS since NIM-747 (the catalog is `shared/coremanifest.KeeperSideAddrs()`), so `on: keeper` on a core address is refused as redundant and stays only as a keeper-side **plugin**'s one spelling. ★ An address the catalog does not know is routed **Soul-side**, so a removed core address does not error — it silently changes side (NIM-863). See [ADR-0087](docs/adr/0087-task-side-derived-from-module-address.md), [docs/keeper/modules.md](docs/keeper/modules.md). Custom modules are separate artifacts, named after the module they serve, launched as a sub-process using **gRPC-over-stdio** (HashiCorp `go-plugin` model). The same `soul` binary works in pull (daemon) and push (oneshot) - the modules are used in the same way. In push: Keeper transfers **all registered modules** en masse, cached on the host using SHA-256 in `/var/lib/soul-stack/{bin,modules}/`.
- **Plugin infrastructure:** single gRPC-stdio handshake (HashiCorp-style) for plugins with different service contracts: **SoulModule** (Destiny steps; a cloud provider is one of these, declaring `side: keeper`, since NIM-757 removed the separate CloudDriver contract) and **SshProvider** (SSH for `keeper.push`, binaries `soul-ssh-*`). A SoulModule artifact carries no family prefix — it is named after the module it serves (NIM-851).
- **Service / Incarnation:** Service = type (git repo with scenario/, vars/00-base.yaml, migrations/, manifest with `state_schema`). Incarnation = runtime instance in Postgres with spec/state/status. Scripts are operations on state (create/add_user/update_acl/restart/…), each with an `input:` schema and its own `core.state.<verb>` capture steps ([ADR-0084](docs/adr/0084-explicit-state-capture.md)). The database is updated only after successful apply on the hosts; otherwise `status: error_locked` and blocking.
- **State_schema versioning:** a step is a directory `migrations/<NNN>_<slug>/` with `main.yml` + `tests/`; `<NNN>` is the version the step leads to, the "from" is derived (the ladder is forward-only and goes by one). The version is **not stored** - it is the top of the ladder, an empty `migrations/` means version 1; `state_schema_version:` is gone from `service.yml` (amendment 2026-09-01 / NIM-735 - the runtime column of that name is unchanged). `schema.lock` beside the ladder holds the version + a fingerprint of the parsed `state_schema`, written by `soul-lint schema-stamp <service dir>` and checked on every `soul-lint validate-service` (NIM-737). DSL by [ADR-019](docs/adr/0019-state-migration-dsl.md): flat `rename`/`set`/`delete`/`move` + CEL expressions in `set.value` via `${ … }` + structural `foreach`; forward-only; migration-CEL sandbox prohibits `vault/now/register/soulprint/vars/input`. Upgrade is an explicit operator step through the UI (`keeper.incarnation.upgrade to_version=...`), not lazy, atomically in one PG transaction. `state_history` - snapshot per-change. Tests - `migrations/<NNN>_<slug>/tests/<case>.yml`. Full spec - [docs/migrations.md](docs/migrations.md).
- **Targeting and host association:** membership in an incarnation is the relation `incarnation_membership`, not a Coven label (ADR-008 amendments NIM-124 / NIM-281); Coven = stable tags only (ADR-008, NOT Coven role), attached by an operator and never inherited. In the scenario - `on:` (`keeper` / `[coven,…]` / omitted = all incarnation) + `where:` (per-host predicate by `register.*` from probe or by stable facts - `soulprint.self.*`). Cross-incarnation targeting is prohibited by grammar. Cross-host data - `soulprint.hosts` (scenario-only accessor of run hosts with stable facts, [orchestration.md §4.1](docs/scenario/orchestration.md)) and `soulprint.where(coven=...)`; destiny does NOT see these accessors directly; it receives values ​​only through `apply: input:`. Full scenario-DSL spec - [docs/scenario/](docs/scenario/README.md).
- **Cloud integration:** ⛔ `keeper.cloud`, `core.cloud` and the **Provider** / **Profile** registries were all removed in NIM-761 with the CloudDriver contract ([known-limitations.md](docs/known-limitations.md)). A machine is created by an ordinary SoulModule plugin declaring `side: keeper`, addressed `<alias>.<object>.<action>` and carrying `on: keeper` in the scenario — its credentials arrive as ordinary step params, so no cloud-specific channel is left in keeper. The old "destiny `cloud-provision`" design stays abandoned - this is not a task package for Soul, but a keeper-side operation. A service's default vars live in git; a fleet that needs different ones forks the service repo ([ADR-0082](docs/adr/0082-service-vars.md)).
- **What's in git, what's in the database:** Service / Destiny / Module - git (code, versioning, review). Incarnation / Coven - Postgres (runtime-state, API/MCP). ⛔ The Provider and Profile registries went with the CloudDriver contract in NIM-761.
