...
- **Plugin gRPC** (`proto/plugin/v1/*`) - consumers: `sdk/*` (module/sshprovider/beacon/cloudutil — `clouddriver` went with the contract in NIM-761), `shared/pluginhost`, every plugin artifact (SoulModule ones carry no family prefix since NIM-851; `soul-ssh-*`/`soul-beacon-*` still do) in companion-repo.
