...
//
// It scaffolds a new SoulModule bundle per [ADR-016 amendment 2026-05-27]
// and [ADR-020]. All artifacts (main.go / handler.go / Makefile / README.md /
// tests/) are embedded via go:embed from this package's `template/` directory —
// the single source of truth for the CLI and, via a periodic copy, for the git
// template repo `github.com/souls-guild/soul-stack-plugins`.
//
// There is no manifest.yaml in the tree it writes. Since NIM-377 the module
// declares itself as a `module.Def` value next to its code and the schema
// document is GENERATED from it by `soul-mod stamp`, so a scaffold that shipped
// a hand-written manifest would be shipping a file nothing reads.
//
...
type TemplateVars struct {
	...
	GoModulePath string
	// AuthorName fills README.md. Defaults to `EXAMPLE`.
	AuthorName string
	// Description is the user-supplied description; it also becomes the
	// module's own Description in the generated schema document.
	Description string
}
...
func printNextSteps(w io.Writer, outDir string, vars TemplateVars) {
	...
	fmt.Fprintf(w, "  cd %s\n", outDir)
	fmt.Fprintf(w, "  # 1. declare your states and params in Module (internal/%s/handler.go)\n", vars.NamePackage)
	...
	fmt.Fprintln(w, "  make check    # gofmt + vet + test + stamp + verify")
	fmt.Fprintln(w, "")
	fmt.Fprintf(w, "artifact: %s/dist/%s (+ dist/schema.json, generated by `soul-mod stamp`)\n", outDir, vars.BinaryName)
	fmt.Fprintln(w, "")
