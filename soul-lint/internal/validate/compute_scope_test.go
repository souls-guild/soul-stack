...
// captureScn builds a scenario whose only task is one `core.state.<verb>` capture.
func captureScn(verb string, params map[string]any, names ...string) *config.ScenarioManifest {
	return computeScn([]config.Task{{
		Name:   "capture",
		On:     "keeper",
		Module: &config.ModuleTask{Module: "core.state." + verb, Params: params},
	}}, names...)
}

// TestComputeUnknownName_CaptureParamsAreWalked — a capture is an `on: keeper`
// task, and its params render in the run-level context compute itself resolved in
// (render.keeperVars). So the namespace is there and a misspelling is the author's
// mistake, not a scope one. Each param is its own CEL cell.
func TestComputeUnknownName_CaptureParamsAreWalked(t *testing.T) {
	cases := []struct {
		name   string
		verb   string
		params map[string]any
	...
	}{
		{
			name:   "set value",
			verb:   "set",
			params: map[string]any{"field": "cfg", "value": "${ compute.node_cont }"},
			want:   "compute_unknown_name $.tasks[0].params.value",
		},
		{
			name:   "nested value",
			verb:   "set",
			params: map[string]any{"field": "cfg", "value": map[string]any{"a": []any{"${ compute.node_cont }"}}},
			want:   "compute_unknown_name $.tasks[0].params.value.a[0]",
		},
		{
			name:   "map add key",
			verb:   "add",
			params: map[string]any{"field": "users", "key": "${ compute.node_cont }", "value": "x"},
			want:   "compute_unknown_name $.tasks[0].params.key",
		},
		{
			name:   "modify patch",
			verb:   "modify",
			params: map[string]any{"field": "users", "patch": map[string]any{"acl": "${ compute.node_cont }"}},
			want:   "compute_unknown_name $.tasks[0].params.patch.acl",
		},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := codesOf(t, captureScn(tc.verb, tc.params, "node_count"))
...
// TestComputeScope_CaptureMatchIsOutOfScope — `match:` is the one capture param
// merge evaluates a second time, per collection element, over elem/key/value alone
// (render.Pipeline.StateOpEvaluators). A BARE reference there reads a namespace
// that is absent at that point; an interpolated one is substituted by the render
// long before merge sees the text, and must stay silent.
func TestComputeScope_CaptureMatchIsOutOfScope(t *testing.T) {
	got := codesOf(t, captureScn("add", map[string]any{
		"field": "replicas", "value": "x", "match": "elem == compute.node_count",
	}, "node_count"))
	want := "compute_out_of_scope $.tasks[0].params.match"
	if len(got) != 1 || got[0] != want {
		t.Fatalf("bare reference: got %v, want [%s]", got, want)
	}

	for _, verb := range []string{"modify", "remove"} {
		if got := codesOf(t, captureScn(verb, map[string]any{
			"field": "replicas", "match": "elem == compute.node_count",
		}, "node_count")); len(got) != 1 || got[0] != want {
			t.Fatalf("%s match: got %v, want [%s]", verb, got, want)
		}
	}

	// Interpolated — the param render substitutes it with the namespace in scope.
	if got := codesOf(t, captureScn("add", map[string]any{
		"field": "replicas", "value": "x", "match": "elem == ${ compute.node_count }",
	}, "node_count")); len(got) != 0 {
		t.Fatalf("interpolated match: got %v, want none", got)
	}
}

// TestComputeScope_MatchOnAnOrdinaryModuleIsNotJudged — the rule keys on the module
// base address, so a `match:` param that belongs to some other module keeps the
// ordinary in-scope reading. Matching on the param NAME would silently reinterpret
// every module that happens to spell one that way.
func TestComputeScope_MatchOnAnOrdinaryModuleIsNotJudged(t *testing.T) {
	scn := computeScn([]config.Task{{
		Name:   "grep",
		Module: &config.ModuleTask{Module: "core.exec.run", Params: map[string]any{"match": "elem == compute.node_count"}},
	}}, "node_count")
	if got := codesOf(t, scn); len(got) != 0 {
