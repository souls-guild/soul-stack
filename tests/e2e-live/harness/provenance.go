...
// answers "does the path exist", and `make e2e-live` depends on `build-linux`,
// which produces `keeper/bin/keeper-linux-amd64` - not the NATIVE
// `keeper/bin/keeper` this tier actually spawns on the host. So L3b ran
// whatever the last `make build` happened to leave behind, and reported the
// verdict as if it were about the tree. (`make e2e-live-gate` depended on
// `build` too and was the one entry point without the hole.)
//
...
// answer is not this tree. Absence of the binary is loud (t.Skipf in NewStack),
// staleness used to be silent - the same asymmetry NIM-342 found in the dev
// stand, and the one NIM-456 paid for twice in one session.
//
// Deliberately OUTSIDE the `e2e_live` tag, for the reason setupdecl.go is: so
// the docker-free guard tier can test it. tests/e2e carries its own copy
// (tests/e2e/harness/provenance.go) - the two harnesses are separate Go modules
// and already duplicate `locateKeeperBinary` for the same reason. Fix both or
// neither.
...
// standing up a privileged container and hoping.
...
// the commit matches. `make e2e-live` closes it by building; a bare
// `go test -tags=e2e_live` after a revert is the residue.
	...
	if commitOf(p.reported) != commitOf(p.expected) {
		return fmt.Sprintf(
			"the keeper binary is STALE - this run would test code that is not in the tree.\n"+
				...
				"  it says:  %s\n"+
				"  tree is:  %s\n"+
				"Rebuild and re-run: `make build`. Note that `make build-linux` does NOT\n"+
				"cover this - it cross-compiles keeper-linux-amd64 for the soul container,\n"+
				"while L3b runs the NATIVE keeper on the host.\n"+
	...
	if p.newestSrc != "" && p.newestSrcMTime.After(p.binaryMTime) {
		return fmt.Sprintf(
			"the keeper binary is STALE - an uncommitted source file is younger than it is.\n"+
				...
				"Rebuild and re-run: `make build` (NOT `make build-linux` - that one builds the\n"+
				"container's keeper, not the host's).",
...
// and must say so.
//
// Called INSIDE the bring-up declaration region (setupdecl.go) - below the
// defer, above `infraUp = true`, the position NIM-547 settled for the presence
// check one line above. Outside it the same refusal reaches the reader as a
// bare `--- FAIL:` on every test in the tier at once; inside, the label does
// not replace this message, which prints either way. What the label DOES cost
// is its standing advice, "rerun this test alone" - false here, since a rerun
// re-runs the same binary. scripts/classify-e2e-live-failure.py answers that by
// keying on the wording below and prescribing `make build` first; its self-test
// requires BOTH stale paths to carry the phrase, so rewording one of them here
// breaks `make check` rather than silently restoring the wrong advice.
func assertKeeperBinaryMatchesTree(t *testing.T, tier, binaryPath string) {
	...
	if err != nil {
		t.Fatalf("%s: cannot establish which code the keeper binary carries: %v\n"+
			...
			"  KEEPER_EXPECTED_VERSION=<the version you believe you built> go test -tags=e2e_live ...\n"+
...
// Makefile sets it from $(VERSION) so that `make e2e-live VERSION=v1.2.3` - the
...
// for a test in tests/e2e-live and wrong for anything else.
...
// leaves BOTH axes quiet on an keeper binary that still contains code the tree no
...
func newestUncommittedKeeperSource(root string) (string, time.Time, error) {
	...
	// tests/e2e-live/harness/keeper/. A pathspec is resolved against the working
