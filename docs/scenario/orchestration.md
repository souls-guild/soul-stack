...
| `serial:` | int (1..M) ИЛИ string `"<N>%"` | module/apply/`block:`-задаче | опционально (опущен = вся ширина таргета) *Гранулярность в MVP — per-RUN min-width, см. подраздел §2.2.1 ниже* |
...
> **Гранулярность `serial:` в MVP — per-RUN, не per-task.** Грамматика разрешает писать `serial:` на каждой задаче независимо, но в MVP ширина волны — **одна на весь прогон**: минимальная положительная `serial:`-ширина среди всех задач прогона (самое узкое окно). Задачи без `serial:` не сужают окно; если `serial:` не несёт ни одна задача — прогон идёт одной волной (вся ширина таргета). Причина — текущая dispatch-модель: один `ApplyRequest` на хост со всеми его задачами скопом (ADR-012(d)) поверх composite PK `(apply_id, sid)` таблицы `apply_runs` — задачи едут хосту одним сообщением, поэтому катить разные задачи разными волнами в рамках одного прогона нельзя. Выбор минимума (а не максимума) — fail-closed: окно сужается до самого узкого намерения автора, blast radius при падении минимален. Следствие, которое автор должен учитывать: задача без собственного `serial:` поедет узкими волнами, если в прогоне есть хоть одна узкая задача. **Per-task волны (каждая задача — своя ширина) — post-MVP** (см. §8 и open Q в architecture.md): требуют per-task dispatch (несколько `ApplyRequest` / строк `apply_runs` на хост) — переделки узла Keeper↔Soul. Инварианты fail-stop и barrier/state-commit (§7) min-width **не нарушает**: волны по-прежнему последовательны, state коммитится один раз после всех волн.
...
([destiny/input.md](../destiny/input.md)). Изоляция destiny этим **не
меняется**: `soulprint.hosts` в `tasks/main.yml` destiny — ошибка валидации.
...
### 7.1. Грамматика `state_changes` — поле → источник значения

`state_changes` декларирует, **что** сценарий пишет в `incarnation.state` после
барьера, **и откуда берётся значение** каждого поля. Три ключа:

| Ключ | Форма | Семантика |
|---|---|---|
| `sets` | map `<поле>: <CEL-выражение>` | присвоить полю `incarnation.state.<поле>` вычисленное значение (перезапись). |
| `appends` | list of field-path (`[...]`) | **future** — дописать в коллекцию (per-host свёртка). Не реализовано, см. ниже. |
| `modifies` | list of field-path (`[...]`) | **future** — точечная мутация вложенного поля. Не реализовано, см. ниже. |

Пустой блок (`state_changes: {}`) валиден — сценарий state не меняет.

#### `sets` — поле получает значение из CEL-выражения

```yaml
state_changes:
  sets:
    greeting_file: "${ vars.greeting_path }"   # значение → incarnation.state.greeting_file
    redis_version: "${ input.version }"
    created_at:    "static-literal"             # литерал без ${ } — присваивается как есть
```

Значение каждого поля `sets` — **строка-выражение**, рендерится Keeper-side
после барьера (§7) в том же CEL-окружении, что и `params:` задач ([ADR-010](../adr/0010-templating.md#adr-010-шаблонизатор-cel-для-yaml-выражений-go-texttemplate-для-файлов),
маркер `${ … }`).
Литерал без `${ … }` присваивается как строковое значение. Non-string результат
CEL (число/bool/list/map) — по правилам интерполяции [ADR-010](../adr/0010-templating.md#adr-010-шаблонизатор-cel-для-yaml-выражений-go-texttemplate-для-файлов)
(вся ячейка = один `${…}` → нативный тип).

**Доступный контекст `sets` — `input.*`, `incarnation.*`, `soulprint.self.*`
и `register.*` (per-host, см. ниже).** Это фактический CEL-env (sets-only форма,
`stateChangesVars` в `keeper/internal/render/cel_render.go`). Остальные
namespace — **future (полная грамматика)**, недоступны и дают ошибку
рендера (прогон → `error_locked`):

- `vars.*` / `essence.*` — scenario-локалы и essence-иерархия (требуют resolve до
  state-commit, отдельный slice);
- `state.*` — старый state до коммита (read-old-state, future);
- `soulprint.hosts` (cross-host топология) — только через полную грамматику;
- `vault(...)` — future в sets (маскинг секретов в state — отдельное решение).

**Cross-host свёртка `sets` — last-wins.** Когда таргет шага — несколько хостов,
выражение может дать разные значения per-host (`${ soulprint.self.* }`).
Для `sets` (скаляры) применяется **last-wins по сортировке `SID`**: значение,
вычисленное для хоста с лексикографически последним `SID`, побеждает
(детерминированно, как «последняя запись побеждает» в [output.md](../destiny/output.md)).
Per-host карты/коллекции — это `appends`/`modifies` (future, ниже).

#### `register.*` как источник `sets`

`sets` видит `register.<task>.<поле>` — результат probe-задачи прогона:

```yaml
state_changes:
  sets:
    leader_host: "${ register.elect.stdout }"   # из probe-шага register: elect
```
...
Рендер `sets` идёт per-host в том же last-wins-порядке, что и для `soulprint.self.*`:
register адресует **результат именно того хоста**, для которого вычисляется
ячейка.

`register.*` в `sets` — это **стабильный** post-barrier-снимок (значения уже
...
`task_idx → register-имя` пропускает no_log-задачи. Следствие — `sets`,
ссылающийся на `register.<no_log-task>.*`, получает «no such key» (прогон
...
#### `appends` / `modifies` — **future** (per-host коллекции)

`appends` (дописать элемент в коллекцию state) и `modifies` (точечная мутация
вложенного поля) остаются декларацией-плейсхолдером (`list of field-path`).
Per-host свёртка коллекций (объединение вкладов хостов) и грамматика
field-path-таргетов — расширение полной грамматики, отдельный slice. До его
реализации непустые `appends`/`modifies` движком **не применяются**.

## 8. Открытые вопросы (расширения, не закрывать молча)

- **Per-task гранулярность `serial:`.** MVP — per-RUN min-width (см. §2.2.1).
  Per-task волны требуют per-task dispatch (несколько `ApplyRequest`/строк
  `apply_runs` на хост) — переделка узла Keeper↔Soul. Отложено; новый ADR при
  реальном запросе.
