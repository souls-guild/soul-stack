...
| The **top** step | at `T-1` — the NEW top | **Nothing, at the moment of upgrade**: `target == current`, the chain is empty, and it completes as a plain ref-bump, with `upgrade-paths` reporting `direction: same-schema, reachable: true`. The ladder check has nothing to say either — a shorter gapless ladder is a legitimate ladder. It is caught **offline, by `schema.lock`**: the stamped `version` is above the new top (`schema_lock_version_stale`). |
...
The **third** row was the hole, and it is narrower than it looks: the incarnation's state really is at `T-1` and the ladder really does top out at `T-1`, so the two agree. What no longer agrees is `state_schema`, which still describes the shape the deleted step produced — the service's declaration says `T`, its ladder says `T-1`, and nothing in the *engine* reconciles the two. That is the same "described twice, by hand, with nothing comparing them" gap [`schema.lock`](#the-schema-lock) exists for, and the stamped `version` is what closes it: it remembers where the ladder used to end, so the removal is refused on the service repo's own `make validate` before the ref is ever pushed. It stays an OFFLINE answer — a repository that never stamped, or a ref pushed past the lint, still reaches the keeper with the row above unchanged.
...
`schema.lock` is generated next to the ladder: `version` (the top of the ladder at stamp time) and `fingerprint` (a hash of the **parsed and canonicalized** `state_schema`, never of the file text — a comment would break a text hash). Written by **`soul-lint schema-stamp <service dir>`** and read by `soul-lint` on every `validate-service` (and so on every `validate-service-tree`, which walks the manifest through the same checks). A service repository wraps the two as `make schema-stamp` and `make validate`; both targets live there, in the tree holding `service.yml` and `migrations/`, not in this core repo.

```yaml
# GENERATED by `soul-lint schema-stamp` - do not edit by hand.
version: 7
fingerprint: sha256:cab827ce6ae1cfe2d68eb646f625b2a4aee9f8b84a187b0cc90df676d01e6369
```

**The hash is over the parsed schema, so a comment, a reordered key and a re-quoted value do not move it** — that is what makes a changed lock worth reading. Every key of the schema node does move it, `description:` included: a `#` comment is outside the schema, a `description:` is part of it. `$type` references are resolved first: an unresolved one is half a schema, so the fingerprint half says it could not run (`schema_lock_unchecked`, an error) rather than hashing what is left, and the stamp refuses for the same reason.

**When a lock is required.** A lock that is there is always checked. A lock that is absent is an error once the ladder holds at least one step (`schema_lock_missing`), and says nothing at all otherwise. A service adopts the lock with one `schema-stamp` — which stamps a version-1 service too, creating `migrations/` to hold it — and from then on it cannot quietly stop being checked: deleting the file is an error, and the only way back to silence is deleting the ladder itself. Requiring one from every directory that merely holds a `service.yml` would instead fail a loose manifest that owns no repository.

**`schema-stamp` refuses to stamp while the manifest or the ladder is red.** A stamp is the claim that this schema and this ladder were, at one moment, both sound and in agreement; generated over a ladder with a gap it would freeze a version nobody meant and then match, silently, until the schema is touched again. Those two and no more — a broken scenario is not evidence about the shape of `incarnation.state`.
...
| `state_schema` changed with no step added | `schema.lock` — the stamped fingerprint no longer matches the parsed schema (`schema_lock_stale`, with both hashes in the message). |
| The top step of a published ladder deleted | `schema.lock` — the stamped `version` is above the ladder's new top (`schema_lock_version_stale`). It is the only thing in the repository that remembers where the ladder used to end. |
| A step added without re-stamping | The same code, the other way round: the stamped `version` is below the ladder's top. |
...
The version half is not judged when the ladder's layout or continuity is broken. A gap moves the derived version while the author's fix is to renumber — and the number moves again with it — so reporting the lock as stale on top of a `migration_chain_broken` would send them to re-stamp something they have to repair first. A step whose `main.yml` is missing does *not* skip it: the rung is on disk, so the top is where it says it is. The fingerprint half never depends on the ladder, and the version half runs even when the schema cannot be hashed.

**The stamp bypass is procedural, not mechanical.** `schema-stamp` re-stamps whenever it is run, so an author can edit `state_schema` and re-stamp without adding a step. That is deliberate, and it is how `atlas migrate hash` behaves in Atlas: the bypass shows up in the diff — `schema.lock` changed, no new directory under `migrations/` — and the reviewer reads it there. A mechanical ban (refusing to stamp until the top of the ladder moves) was considered and rejected, because it would force an empty migration step for every harmless schema edit.
...
To iterate through the map, the native macro `.map()` **above the map itself** is used: it bypasses the **keys** (iteration element = key), the value is obtained by the index `m[k]`. This is how `map → array` is collapsed without `keys()`: `state.redis_users.map(n, {'name': n, 'perms': state.redis_users[n].perms, ...})`. No ladder in the tree demonstrates it any more — the one that did was `examples/service/redis/migrations/006_acl_users_map_to_array`, removed with that service (NIM-871).
...
- [`examples/service/dragonfly/migrations/`](../examples/service/dragonfly/migrations/) - the worked ladder (six forward-only steps with per-step `tests/`, plus the `schema.lock` `soul-lint schema-stamp` writes).
