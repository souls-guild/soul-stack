...
> **Soul-side metric-naming note.** The `soul_` prefix refers to the **component role** (Soul agent), not to the module namespace dictionary (`core` / `acme` / ...). These are different spaces: `soul_apply_tasks_total` - agent metric; `core.pkg.installed` — module address. There is no intersection.
