...
| `shell` | if specified - **absolute path** (starting with `/`), without leading `-`. The existence of the file is **not** checked. | `useradd` does not require the existence of a shell (hybrid flexibility rule); absoluteness + prohibition `-` cut off argument confusion. |
