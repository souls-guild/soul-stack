...
| `enabled` | bool | optional (tri-state) | Managing autostart **in one step**: **omitted** — do not touch autostart (we only control activity); **`true`** - additionally `enable`; **`false`** - additionally `disable`. enable/disable are idempotent (check via `is-enabled`). |
