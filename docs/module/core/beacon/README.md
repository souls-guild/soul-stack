...
- Vigil addressing — **one level: the registration alias itself** (`zfs-degraded`). Settled by **NIM-770** against the code, and it is the artifact's shape rather than an implementation accident: a beacon declares no `modules[]` — the schema **refuses** the block on any kind but `soul_module` (`modules_not_allowed`, [`sdk/schema/validate.go`](../../../../sdk/schema/validate.go)) — so it serves a single endpoint and there is no second level to name. [`Discovered.Address()`](../../../../shared/pluginhost/discovery.go) returns the bare alias whenever `Module == ""`, and that is the key [`beacon.NewPluginRegistry`](../../../../soul/internal/beacon/pluginregistry.go) stores it under.
  - ⚠ The `<namespace>.<name>` form this line used to promise (for example `community.zfs-degraded`) described the **pre-NIM-377 manifest**, which carried a `namespace:` and a `name:` of its own; both fields went with that mechanism, and `community` is additionally the origin-grouping level struck by the [ADR-020 amendment 2026-09-02](../../../adr/0020-plugin-infrastructure.md#amendment-2026-09-02-nim-764--nim-765-a-plugin-address-is-pluginobjectaction-and-the-origin-grouping-level-is-removed). Such an address is not merely stale but **unregistrable**: [`plugin.AliasPattern`](../../../../shared/plugin/reserved.go) admits no dot, because an alias is one address level and one cache-directory name. The three-level `<plugin>.<object>.<action>` grammar does not reach here either — it rules on a plugin **step**'s address, and a beacon has neither an object set nor an action set (`Check` is its one operation; "state" is edge-triggered at the scheduler).
  - The Soul-side dispatcher separates built-in from plugin by **lookup order, not by reading a namespace**: [`CompositeRegistry.Lookup`](../../../../soul/internal/beacon/composite.go) tries core first, plugin second. The two key spaces cannot overlap anyway — a core beacon is keyed by the full `core.beacon.<name>`, a plugin beacon by a dotless alias.
