...
|---|---|---|
| [`community.redis`](redis/README.md) | `command` / `pinged` / `role` / `replica-synced` / `offset-synced` / `config` / `acl` / `cluster` / `replica` / `detached` / `sentinel` (12) | ОСНОВНОЙ интерфейс к **живому Redis** (концепция Ansible-роли `redis`): scenario оркеструет порядок/rolling, плагин исполняет одну операцию над одним инстансом. Backend — `go-redis/v9`. Покрывает hot-reload (`config`/`acl`), сборку и day-2-эволюцию кластера (`cluster` create/add-node/remove-node/reshard), live-миграцию между кластерами (`cluster` join-external/failover-takeover/forget-external), репликацию и миграцию из внешнего источника (`replica source_external` → `offset-synced` → `detached`), Sentinel-реконсиляцию. |
