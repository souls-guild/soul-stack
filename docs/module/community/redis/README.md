...
pinged/role/replica-synced, `cluster.go` — cluster-state, `replica.go` — replica-state (REPLICAOF),
...
Текущий срез — девять state. Остальные **планируются** следующими батчами и пока в
...
Все states (`command`/`pinged`/`role`/`replica-synced`/`config`/`acl`/`cluster`/`replica`/`sentinel`)
принимают **общие** TLS-параметры коннекта. По умолчанию TLS выключен (plaintext,
back-compat); при `tls: true` плагин коннектится к Redis по TLS.
...
- [examples/service/redis/](../../../../examples/service/redis/) — сервис redis:
  ...
  force re-read SSL_CTX) вызывают states этого плагина.
