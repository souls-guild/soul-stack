...
MAIN interface to **live Redis** in redis consolidation (concept
Ansible roles): the service scenario orchestrates the order/targeting/rolling, and the plugin
...
| `sentinel` | Reconcile Redis Sentinel (`SENTINEL MONITOR`/`REMOVE`/`SET`/`CONFIG SET`). Algorithm 1:1 from Ansible `redis_sentinel_update.py`. | `true` when changing monitor/parameters; `false` (no-op), if everything matches. |
...
(top-level `CONFIG` is not supported in Sentinel mode). Algorithm moved 1:1
from Ansible `library/redis_sentinel_update.py`
...
## TLS connection (redis Ansible role concept: `redis_tls_*`)
