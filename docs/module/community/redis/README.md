...
(`impl.go` — диспетчер + states command/config, `probe.go` — read-probe states
...
Текущий срез — восемь state. Остальные **планируются** следующими батчами и пока в
...
Все states (`command`/`pinged`/`role`/`replica-synced`/`config`/`cluster`/`replica`/`sentinel`)
