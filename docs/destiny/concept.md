...
| Layer | Who | What does |
|---|---|---|
| **Service** | `service.yml` + `scenario/<name>/main.yml` in the service git repo | Service type (Redis HA), set of operations (`create`, `add_user`, `restart`, …). Glues destiny to different hosts through the `tasks:` script, reads/writes `incarnation.state` to the database. |
...
| **Module** | `core.pkg.installed`, `wb.haproxy.reloaded`, … | Implementation of one "verb" (install the package, raise the service, render the file). See [architecture.md → "Module model"](../architecture.md). |
...
- [vars.md](vars.md) - `vars.yml` destiny locales (which is available in task templates as `vars.*`).
