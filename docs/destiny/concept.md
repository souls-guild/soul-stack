...
- **Without runtime-state.** Destiny does not know about the Keeper database, about other hosts, about the current state of the cluster, **and does not see the service's own `vars/`**. Everything she needs from the outside world comes through the **`input:`-contract**; The author sets local values in **`vars.yml`** (isolated, not overridden from outside). This is how destiny differs from scenario, see below.
