...
- **`checksum` обязателен и fail-closed (нормативный default).** В `input:`-контракте поле хэша — `required: true` **без `default`**. Нет хэша → честный отказ, а не fetch с placeholder-ом. `core.url.fetched` верифицирует хэш **до** материализации файла — неверный хэш не попадает на диск. Так сделано на **service-уровне**: сценарии [`monitoring`](../../examples/service/monitoring/) и [`redis-monitored`](../../examples/service/redis-monitored/) объявляют `node_exporter_sha256` / `redis_exporter_sha256` как `required: true` без default и прокидывают значение в destiny через `apply: input:`.
