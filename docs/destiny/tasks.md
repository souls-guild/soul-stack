...
| `when:` | string (template-expr) | всем (на include — только **статический** предикат, см. §4) | опционально |
| `parallel:` | bool | всем | опционально, default `false` |
| `loop:` | map | module-задаче | опционально (на include — **отложено**, см. §7) |
...
- **Правила:**
  ...
  - на include-задаче допустимы **только** `include:`/`name:`/`when:`; любой другой
    модификатор (`vars:`/`loop:`/`register:`/`parallel:`/scenario-дельта) — ошибка
    `include_modifier_unsupported` (whitelist раскрытия, чтобы scope не терялся молча).
- **Условный include (`when:` на include-задаче) — render-phase group-drop.** На
  include-задаче допустим `when:` — тогда подключённая группа задач включается в план
  **только** если предикат истинен. Когда `when:` ложен — ВСЕ задачи подключённого
  файла **физически отсутствуют** в плане (реальное исключение, не placeholder): они
  не эмитятся, индекс не резервируется. Это отличается от static-false `when:` на
  обычной/block-задаче (placeholder-skip с резервированием индекса):
  - **скоуп предиката — только статический** (`input.*`/`essence.*`/`incarnation.*`/
    `vars.*`). include раскрывается **до** стратификации, когда `register:` предыдущих
    задач ещё не собран, а per-host `soulprint` неизвестен. Динамический include-when
    (`register.*`/`soulprint.*`) → ошибка `include_when_dynamic_unsupported` (ловит и
...
- **Параллелизм:** `parallel: true` на block-задаче — **пост-pilot** (parallel целиком отложен — нет прод-потребителя; гейт fail-closed, см. код ошибки `parallel_on_block_invalid`). Дизайн-намерение: `parallel: true` помечает **всю группу** как члена parallel-группы текущего уровня, внутри блока задачи выполняются между собой по своим правилам. В pilot C1 не реализовано.
- **`loop:`** на block-задаче — **пост-pilot** (block+loop комбинация отложена). Дизайн-намерение: block раскрывается N раз с разными значениями переменной итерации, внутри `${ <as>.* }` (или голая `<as>.*` в expression-keys) доступен. В pilot C1 не реализовано.
...
### Граница: реализовано в pilot C1 / пост-pilot

Спецификация выше — полное дизайн-намерение `block:`. Реализация pilot C1 покрывает не всё; явная граница, чтобы при currency-аудите спека не читалась как drift:

**Реализовано в pilot C1:**
...
### Семантика `loop` на `include:` — **отложено**

> **Не реализовано.** `loop:` на include-задаче отвергается раскрытием как
> `include_modifier_unsupported` (на include допустимы только `include:`/`name:`/
> `when:`). Дизайн-намерение ниже сохраняется как ориентир будущего слайса, но в
> текущем пилоте недоступно. Для «N раз по элементам» используйте `loop:` на
> module-задаче (реализовано) либо повторяемую структуру внутри подключённого файла.

Дизайн-намерение (не реализовано):
...
Содержимое `ensure-user.yml` исполнялось бы N раз, на каждой итерации `${ user.* }` (в строках) / `user.*` (в expression-keys) доступен в задачах файла.
...
#### `has()`-guard для optional-input при не-статическом `when:`

Если `when:` задачи **не статический** (ссылается на `register.*` — probe-результат, известный только Soul-side; либо смешанный `input.x && register.y`), Keeper **обязан** отрендерить `params:`/`vars:`/`loop.items:` этой задачи: он не может статически доказать skip, потому что `register.*`-часть условия известна только во время прогона на Soul. Поэтому **optional-input, отсутствующий при текущем прогоне**, упадёт `no such key` ещё на render-фазе — до того, как Soul вычислит `when:` и пропустит задачу.

Паттерн-решение — авторский `has()`-guard на каждом обращении к optional-input в `params:`/`vars:`/`loop.items:` такой задачи:

```yaml
- name: Tune maxmemory on master only
  when: register.role.stdout == 'master'        # НЕ статический → params рендерятся всегда
  module: core.exec.run
  params:
    command: "redis-cli CONFIG SET maxmemory ${ has(input.maxmemory) ? input.maxmemory : '256mb' }"
```

Здесь `${ has(input.maxmemory) ? input.maxmemory : '256mb' }` подставляет default, когда `maxmemory` не передан в `input:`, — render не падает, а на не-master хостах задача всё равно скипнется по `when:`.

**Контраст со статическим `when:`.** Для **статического** `when:` (register-/soulprint-независимый, вычислим на Keeper — например `input.action == 'update_acls'`) guard **не нужен**: Keeper доказывает `when: false` ещё до рендера и скипает задачу целиком вместе с её `params:` (static-when placeholder-skip, Вариант b — [ADR-012(d)](../adr/0012-keeper-soul-grpc.md), см. §7 «Static-when предшествует loop-fan-out»). Поэтому задача неактивной ветки multi-action destiny безопасно читает отсутствующий optional-input без `has()` — её `params:` вообще не рендерятся. `has()`-guard нужен ровно там, где static-skip недоступен: register-зависимый или смешанный `when:`.
...
### Include
- **Динамический `when:` на include — отложено.** Условный include (§4) поддерживает
  только **статический** предикат (`input.*`/`essence.*`/`incarnation.*`/`vars.*`),
  потому что include раскрывается **до** стратификации — `register:` предыдущих задач
  ещё не собран, per-host `soulprint` неизвестен. Динамический include-when
  (`register.*`/`soulprint.*`) → `include_when_dynamic_unsupported`. Open Q: вводить ли
  «отложенное раскрытие include» (резолв группы в пост-probe Passage), что дало бы
  `when: register.X` на include — но усложнило бы фазовую модель (раскрытие перестаёт
  быть до-render-фазой). Пока отвергнуто как преждевременное.
- **`loop:` на include — отложено.** «Подключить файл N раз по элементам» (§7) не
  реализовано (`include_modifier_unsupported`); обходится `loop:` на module-задаче либо
  повторяемой структурой внутри файла. Open Q: реализовывать ли render-time fan-out
  include-группы по `loop:` (симметрия с `loop:` на module-задаче).
