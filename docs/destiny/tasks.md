...
- **Not on a keeper-side task.** `async: true` on a keeper-side task is rejected (`async_on_keeper_invalid`). The side is read off the module ADDRESS since NIM-749, not off a written `on: keeper` — the flag is equally a no-op on a `core.state.set` carrying no `on:` at all: `async:` is Soul-side task concurrency and a keeper task is executed by the keeper's own runner, never reaching a Soul - honouring the flag would be a no-op. `require:` on a keeper task is a different matter and stays legal (§8).
...
- **A keeper-side task with `async: true`** - **rejected** (the side comes from the module address, not from a written `on: keeper`). `async:` is Soul-side task concurrency; a keeper-side task ([keeper/modules.md](../keeper/modules.md)) executes in the Keeper's scenario runner and never reaches a Soul, so the flag would be a silent no-op. `require:` on a keeper task is accepted and simply redundant (see §8).
...
- **`onchanges:` / `onfail:` / `require:` block tasks** - inherited by **every** descendant (the block itself emits no task, so a requisite left on it would reach nothing) and merged with the descendant's own as a **union of names**. For a descendant that declares none of its own, this reads as "the requisite applies to the whole group": no named source changed/failed → every descendant is skipped. For a descendant that declares its OWN, the two sets merge rather than intersect - see the composition rule below.
...
- **★ Requisites compose as OR and therefore WIDEN - `when:` composes as AND and narrows.** This is the normative behaviour of both group constructs (`block:` here, `apply:` in [orchestration.md §2.1.2](../scenario/orchestration.md#212-the-appliers-own-keys---what-reaches-the-group-and-what-decides-before-it)), and the asymmetry is deliberate, not an oversight. `onchanges:`/`onfail:` on a single task already mean "**any** of the named sources changed/failed" ([§8](#8-requisites---inter-task-dependencies)), and the union inherits that meaning: a descendant carrying `onchanges: [inner]` under a block carrying `onchanges: [outer]` ends up with `[outer, inner]` and runs when **either** changed. So a group-level requisite makes such a descendant run **more** often, not less - the opposite of how the outer line reads. A descendant with no requisite of its own is unaffected: for it the group requisite is a pure narrowing, which is the case the key was built for.

  **There is no way to spell "outer AND inner" today**, and no workaround reproduces it: `when:` on an applier must be static, and `where:` selects hosts rather than reacting to a source's outcome. If a descendant's own requisite must stay authoritative, do not put a requisite on the group - gate the group with `when:`/`where:`, or split it so that the tasks carrying their own requisites sit outside. Introducing AND needs a grouped requisite on the wire (a conjunction of disjunctions - the flat `onchanges_idx` cannot express two brackets) plus a decision about a bracket whose sources are all filtered out on a host; that is deferred to NIM-351 and would be a breaking change to the behaviour fixed here.
...
The same holds for every **requisite** naming a fanned-out register, and it is the reason the name resolves to a **set** of task indices rather than one (NIM-246): `require: [<name>]` waits for all iterations, `onchanges: [<name>]` fires if **any** iteration changed, `onfail: [<name>]` if any failed. Resolving to a single iteration - which is what a name-to-one-index map silently did - turned "restart if any of the N files changed" into "restart if the last one did", and released a barrier while its siblings were still writing.
...
A name that **fanned out** - a `loop:` task, whose N iterations share one `register:` - resolves to all of its indices, so the barrier waits for every iteration (§7).
...
> **Keeper-side tasks** (judged by the module address, not by a written `on: keeper`). `require:` is accepted and redundant - the keeper executor runs its tasks in plan order. `async:` is **rejected**: it is Soul-side task concurrency, a keeper task never reaches a Soul runner, and honouring it would be a silent no-op.
