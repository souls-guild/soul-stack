...
- **Not on a keeper-side task.** `async: true` next to `on: keeper` is rejected (`async_on_keeper_invalid`): `async:` is Soul-side task concurrency and a keeper task is executed by the keeper's own runner, never reaching a Soul - honouring the flag would be a no-op. `require:` on a keeper task is a different matter and stays legal (§8).
