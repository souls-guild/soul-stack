...
|---|---|---|---|
| `fixtures:` (input/vars/soulprint/vault/state + `default_destiny_source` for apply:destiny + `service` — see below) / `mocks:` / `assert.{rendered_tasks,state_after,state_absent}` | **L0** (render-only, hermetic, single-host sugar) | fixtures for the render pipeline + expected task plan / final `incarnation.state` (fields present and fields gone) | **MVP, executing** `soul-trial run` |
| `fixtures.hosts: [...]` (roster of N hosts) instead of `fixtures.soulprint` + the same `assert.*` | **L0** (render-only, hermetic, multi-host roster) | list of run hosts → `soulprint.hosts`-projection / `.where(...)` / `size()` / nodes-determinism / a `core.state.<verb>` capture on multi-host | **closed** (amendment 2026-06-22, ADR-023), in progress `soul-trial run` |
...
`fixtures.service` — the name the own-namespace Vault fence runs against ([ADR-0083](../adr/0083-declared-secret-state-fields.md) §7): the name the service is **registered** under. Optional. Since [NIM-726](../adr/0085-entity-id-and-label.md) `service.yml` states no name — a service is named once, at registration — so with the key absent the harness derives one from the service DIRECTORY (a `<destiny>/_trial/` wrapper takes the destiny above it).

That default is a convention, not an authority. A checkout laid out as `service-<name>/` derives `service-redis` where the registry says `redis`; the name is non-empty, so it clears the fence's own `service == ""` guard, and then matches no path — the fence runs and finds nothing, which looks exactly like a clean scenario. **`soul-trial run` therefore prints the name it fenced on for every case**, marked `(from directory)` when nobody stated it. If that word is not the name your service is registered under, set `fixtures.service:` — nothing offline can detect the mismatch for you, because there is no registry to ask.

`assert.state_after` — L0 section: the expected final `incarnation.state` = base `fixtures.state` + the run's state writes on top, applied in run order — the plan's `core.state.<verb>` capture steps, in plan order. There is no end-of-run commit left to fold in ([ADR-0084](../adr/0084-explicit-state-capture.md)). The writes go through the production merge engine (`keeper/internal/stateop`), and each capture op is built by the same code the module runs, so a verb, a `match:` or an `on_conflict:` means offline exactly what it means in the run.

The comparison is **by key**: every field the case names must be there and match, a field the case does not name is not a discrepancy ([ADR-0084](../adr/0084-explicit-state-capture.md) F-C/C4 — it was a full reconciliation before that). The L1 migration form keeps the full reconciliation. A capture step whose params are invalid is reported as an **error**, not as a failed assert — the run would refuse the same task at load — and it is checked in every case, including one that asserts no state at all.

```yaml
assert:
  state_after:            # these fields must be there, with these values
    owner: alice
    users: [alice, bob]
  state_absent:           # these fields must be gone
    - migrating
```

`assert.state_absent` — the other half of the same subset: a list of fields the run must **not** leave in `incarnation.state`. Naming no field says nothing about it, so a subset `state_after` on its own cannot assert a removal — `core.state.unset`, a `remove` that empties a field and a declared secret stripped on the way out all need this section. The check is strict key absence: a field that is still there holding `null` is reported *with* its value rather than counted as gone, because dropping a key and blanking it are two different states — a case that means the second one writes `owner: null` in `state_after`.

`assert.dispatch` - L3 section (meant only on multi-host), the L0 loader rejects it with a strict decode.
...
fixtures:
  hosts:
    - sid: node-1.example.com
      covens: [prod, redis]          # real stable tags only (no incarnation.id); membership = presence in fixtures.hosts (see below)
