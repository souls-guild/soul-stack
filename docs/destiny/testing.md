...
|---|---|---|---|
| `fixtures:` (input/vars/soulprint/vault/state + `default_destiny_source` for apply:destiny) / `mocks:` / `assert.{rendered_tasks,state_changes,state_after}` | **L0** (render-only, hermetic, single-host sugar) | fixtures for the render pipeline + expected task plan / delta sets / final `incarnation.state` | **MVP, executing** `soul-trial run` |
