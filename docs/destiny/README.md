...
| [output.md](output.md) | `output:`-destiny contract (top-level): symmetrical `input:` block declaration of what result destiny publishes to the caller; read from scenario via `register:` on the applier task. ★ **Declaration only:** nothing fills it yet - a task-level `output:` is refused (`output_unsupported`) and `register.<applier>.<field>` does not resolve. |
