...
> **Статус: accepted + end-to-end (LDAP, стадия 1 / OIDC, стадия 2) + веб-логин cookie→Bearer (NIM-77, см. (f-bis)).** LDAP- и OIDC-аутентификация **реализованы и доведены end-to-end**. LDAP: search-bind, auto-provision по группам **с посевом строки `operators`**, cookie-доставка JWT — блокер посева системного оператора `archon-system` **закрыт** (`created_via`-релакс bootstrap-индекса, миграция 086; см. ниже в (d)). OIDC (стадия 2): authorization-code flow с **обязательным PKCE (S256)**, discovery (`go-oidc/v3`) + JWKS-валидация `id_token` (подпись/iss/aud/exp/nonce), cluster-shared server-side flow-state store на Redis (state→nonce/PKCE-verifier, single-use GETDEL, TTL 5m), эндпоинты `GET /auth/oidc/{login,callback}`, cookie-доставка JWT (`SameSite=Strict`, унифицировано с LDAP — security-fix 2026-06-24, см. (g)/(№4)). Mapper обобщён под оба метода (`auth_method`/`created_via` = `ldap`|`oidc` из `cfg.Method`). Все развилки разрешены: enum-имя — `oidc` (закреплено), PKCE — обязателен (config-флага нет, оператору не оставлен).
>
> **Security-fix-проход 2026-06-24 (consolidated):** (1) **account-takeover (CRIT)** — federated-login существующего оператора отвергается, если его `auth_method` ≠ метод mapper-а (`ErrAuthFailed`, anti-oracle): bootstrap/system/mTLS/другой-federated оператор НЕ присваивается совпавшим derived-AID. Federated-путь обслуживает только операторов, заведённых тем же методом. См. (d) revocation-инвариант. (2) **scoped role-revoke (HIGH)** — реконсиляция ролей теперь снимает membership-роли, ушедшие из групп (не только grant), scoped по объединению `values(group_role_map)` (Synod/ручные роли вне домена НЕ трогаются), grant+revoke в одной транзакции — реализация замысла (d). (3) **anti-bruteforce (HIGH)** — публичные login-эндпоинты получили per-IP+per-username throttle + lockout после серии неудач + лимит флуда OIDC flow-state (`auth.rate_limit`-блок, default-ON, Redis, fail-closed на lockout). (4) **SameSite унифицирован Strict** для LDAP и OIDC; **aid_claim дефолт `sub`** (иммутабельный) + WARN на user-mutable `email`/`preferred_username`.
...
- **(b) OAuth2/OIDC — authorization-code flow с PKCE (логин человека-Архонта). РЕАЛИЗОВАНО (стадия 2).**
  ...
  4. **Маппинг** на `operators(aid)` + роли (общий `auth.DBMapper`, `Method=oidc`; auto-provision по группам, см. (e)/(i)). Выпускает **внутренний JWT** через `jwt.Issuer`, кладёт в HttpOnly+Secure cookie `soul_session` (`SameSite=Strict`, унифицировано с LDAP — security-fix 2026-06-24, см. (g)/(№4)) и редиректит на `/ui` (302). JSON-токена в теле нет (cookie-only, parity LDAP). PKCE **обязателен** — config-флага нет (развилка №6 разрешена в пользу «обязателен»). OIDC требует живого Redis (flow-store cluster-shared, ADR-053 OPTIONAL-tier): без Redis эндпоинты не монтируются.
...
- **(d) Маппинг внешней identity → `operators(aid)` + roles. РЕАЛИЗОВАНО (LDAP, `keeper/internal/auth/mapper.go`).**
  ...
  - **Role-mapping (РЕШЕНО — роли из групп, развилка №2; РЕКОНСИЛЯЦИЯ — security-fix 2026-06-24):** LDAP/OIDC-группы → RBAC-роли через `group_role_map: {group: [role,...]}`. Источник ролей — внешние группы (а не реестр), и для нового, и для существующего оператора. Для существующего оператора membership **РЕКОНСИЛИРУЕТСЯ** (а не только grant): роли из текущих групп выдаются, роли, **ушедшие из групп**, — снимаются (`rbac.RevokeOperator`), grant+revoke в ОДНОЙ транзакции. Снятие **scoped по объединению `values(group_role_map)`** — реконсиляция владеет только своим доменом; роли, выданные Synod / вручную / иным путём ВНЕ `group_role_map`, НЕ трогаются. Это реализация замысла «роли = внешние группы, смена групп отражается в RBAC» (раньше реконсиляция была grant-only → вечная эскалация: снятие из группы у IdP не отзывало роль). `granted_by_aid=NULL` (federated-membership без инициатора); авторитет RBAC — таблица membership, не JWT-claim `roles` (ADR-028(c)). Роли дедуплицируются и стабильно сортируются.
  ...
  - **Auth-method-match (account-takeover-инвариант, CRIT-fix 2026-06-24):** federated-login существующего оператора отвергается, если его `operators.auth_method` ≠ метод mapper-а (`ErrAuthFailed` → 401, anti-oracle — наружу причина не утекает). Federated-путь обслуживает ТОЛЬКО операторов, заведённых ТЕМ ЖЕ federated-методом. Bootstrap-Архонт (`auth_method=jwt`), `archon-system` (`jwt`), mTLS-операторы и оператор, заведённый ДРУГИМ federated-методом, — НЕ присваиваются совпавшим derived-AID. Без этой проверки контролирующий внешний IdP мог бы выпустить себе derived-AID, равный AID привилегированного оператора (например, bootstrap cluster-admin), и присвоить его сессию.
...
- **(e) Config-схема (новые блоки `auth.ldap` / `auth.oidc`, секреты — Vault KV).** Симметрично существующим блокам ([keeper.go](../../shared/config/keeper.go): `KeeperRedis.password_ref`, `KeeperVault`, `KeeperAuthJWT.signing_key_ref`). Все секреты — через `*_ref` (`vault:<mount>/<path>[#field]`, резолв load-time как у `password_ref`/`dsn_ref`). TLS-required.
  ...
  auth:
    ...
    oidc:                                 # НОВОЕ (опц. блок, требует Redis)
      ...
      aid_claim: sub                       # claim → AID (дефолт sub — иммутабельный; email/preferred_username → semantic-WARN, identity-spoofing-риск)
      groups_claim: groups                # claim → список групп для role-map (дефолт groups)
      group_role_map:
        /keeper-admins: [cluster-admin]
        /keeper-ops: [operator]

    rate_limit:                           # НОВОЕ (опц., anti-bruteforce login, HIGH-fix; default-ON, требует Redis)
      enabled: true                       # default-ON (footgun-guard как Tempo/Toll)
      rate: 0.5                           # token-bucket: попыток/сек на принципал (IP/username)
      burst: 10                           # глубина бакета попыток
      lockout_threshold: 5                # неудач в окне → блокировка
      lockout_window: 15m                 # окно подсчёта неудач
      lockout_backoff: 15m                # длительность блокировки
  ```
...
- **(f) API-поверхность (новые публичные `/auth/*` эндпоинты вне `/v1`).** Регистрируются на root-уровне chi-роутера (parity `/healthz`/`/docs`/`/ui` — БЕЗ `RequireJWT`/RBAC, см. [router.go](../../keeper/internal/api/router.go)):
  ...
  |---|---|---|
  | `GET /auth/methods` | публичный | **РЕАЛИЗОВАНО (NIM-77):** backend-driven каталог способов логина ([ADR-042](0042-backend-driven-ui.md)) — booleans `{password,ldap,oidc}` для UI login-формы (`password` всегда true = вставка operator-JWT; `ldap`/`oidc` по факту сконфигурированности) |
  | `POST /auth/token` | публичный (cookie) | **РЕАЛИЗОВАНО (NIM-77):** обмен session-cookie `soul_session` на короткий Bearer в JSON (resolve cookie-gap SPA — см. (f-bis)) |
  ...
  `POST /auth/ldap/login` (`huma_auth.go`) и `GET /auth/oidc/{login,callback}` (`huma_oidc.go`) реализованы: ROOT-mount вне `/v1` (parity `/healthz`, без `RequireJWT`/RBAC/metrics; anti-DoS body-limit стоит), монтируются только при non-nil `LDAPAuth`/`OIDCAuth` (opt-in, как `pushH`/`errandH`; OIDC дополнительно требует Redis). FULL-TYPED huma-операции; в committed `docs/keeper/openapi.yaml` (группы `prefix /auth` в `fullSpecGroups`). Защита: TLS-required, body-лимит, OIDC — `state`(single-use)/`nonce`/PKCE. Успешный login пишет audit `operator.login` (`source: api`, payload `{method, aid, provisioned}` — без секретов). `GET /auth/methods` (backend-driven каталог) реализован (NIM-77, см. (f-bis)); rate-limit/lockout по username (anti-bruteforce) — реализовано (см. (g)). MCP-поверхность **не трогаем** — MCP принимает уже выпущенный внутренний JWT. OpenAPI — `/auth/*` регистрируются huma-операциями, попадают в `/openapi.yaml` автоматически (ADR-054).

- **(f-bis) Amendment 2026-07-09 (NIM-77): веб-логин — обмен session-cookie → короткий Bearer (resolve cookie-gap).** Федеративный login кладёт внутренний JWT в HttpOnly-cookie `soul_session`, но `/v1` (`RequireJWT`/`extractToken`) читает только `Authorization: Bearer` — SPA не доходит до API (cookie-gap). Разрешение (**Вариант B**, propose-and-wait): **`POST /auth/token`** (публичный, вне `/v1`, без `RequireJWT`, [huma_auth_token.go](../../keeper/internal/api/huma_auth_token.go)) читает cookie **тем же `jwt.Verifier`**, что и `/v1`, и выпускает короткий Bearer в JSON-теле `{token, expires_at}`; SPA кладёт его в localStorage и шлёт как `Authorization: Bearer`. `RequireJWT`/`extractToken`/`/v1`/MCP **НЕ трогаются** — cookie на `/v1` по-прежнему не читается (инвариант периметра, guard-тест). Решения:
  - **Субъект строго из cookie-claims:** `subject`/`roles`/`bootstrap_initial` выданного токена — только из `Verify(cookie)`, тело запроса пустое (нет privilege-escalation; RBAC-право не требуется). `roles` в токене косметические — права на `/v1` RBAC резолвит из БД-снимка по `aid` (ADR-028/047).
  - **Короткий TTL с cap:** `auth.jwt.exchange_ttl` (отдельный от `ttl_default=24h`, дефолт **10m**, floor **1m**); эффективный `ttl = min(exchange_ttl, remaining_до_cookie.exp)`, `≤0 → 401`. Cookie остаётся refresh-credential для silent re-exchange до своего истечения.
  - **Revoked-чек на обмене:** по in-memory RBAC-снимку (`Enforcer/Holder.IsRevoked`, map-lookup, не SQL) — ревокнутый оператор не обменяет ещё живую cookie; без PG-зависимости, свежесть как на `/v1`.
  - **Anti-oracle:** нет/невалид/протух/revoked → generic `401` (detail только из `jwt.ClassifyVerifyErr`, как RequireJWT); cross-site (`Sec-Fetch-Site: cross-site`) → `403` (defense-in-depth; primary — `SameSite=Strict` + отсутствие CORS). `expires_at` — из фактически выданного токена (self-verify).
  - **Cookie сужена `Path=/` → `Path=/auth`** (`newSessionCookie`, общее LDAP+OIDC): 24h-credential браузер шлёт только на `/auth/*` (единственный читатель — `POST /auth/token`), не на `/v1`/`/mcp`/`/docs`.
  - **Обмен в audit не пишется** (high-freq refresh; вход уже фиксирует `operator.login`); `/auth/token` под `AuthLoginLimit` (per-IP throttle, `recordFailures=false` — протухшая cookie ≠ подбор пароля). Guard-тесты — `huma_auth_token_test.go` (периметр/round-trip/subject-from-claims/revoked/cross-site/ttl-cap/bootstrap_initial) + `is_revoked_test.go`.
...
- **(g) Модель безопасности (security-critical, «безопасность на первом месте»).**
  ...
  - **Anti-bruteforce (РЕАЛИЗОВАНО, HIGH-fix 2026-06-24):** публичные login-эндпоинты (`POST /auth/ldap/login`, `GET /auth/oidc/{login,callback}`) защищены middleware `AuthLoginLimit` поверх Redis-примитива `LoginGuard` (cluster-shared): (a) **throttle частоты** попыток (token-bucket) per-IP + per-username — гасит флуд, включая flood flow-state на `/auth/oidc/login`; (b) **lockout** после N неудачных логинов в окне (для IP и username независимо) на backoff-интервал. Параметры — `auth.rate_limit`-блок (`rate`/`burst`/`lockout_threshold`/`lockout_window`/`lockout_backoff`, default-ON, дефолты `config.DefaultAuthLogin*`). **Fail-CLOSED на lockout** (Redis-ошибка → трактуется как «заблокировано»: login — security-периметр, недоступность Redis не должна открывать брутфорс — в ОТЛИЧИЕ от Tempo fail-open), **fail-open на throttle** (доступность login-страницы). Успешный логин счётчик неудач не трогает. Отказ — `429` + `Retry-After` + problem-type `auth-throttled` (anti-oracle: detail без scope/причины). client-IP берётся из `RemoteAddr` (X-Forwarded-For не доверяется без trusted-proxy-конфигурации — пока её нет, за L7-proxy основную защиту несёт per-username-слой). nil-Redis → passthrough (login без throttle, OPTIONAL-tier). Guard-тесты — `keeper/internal/api/middleware/authlimit_test.go` + integration `keeper/internal/redis/loginguard_integration_test.go`.
  ...
  - **Доставка JWT (РЕШЕНО — cookie-only, развилка №4; SameSite унифицирован 2026-06-24):** внутренний JWT отдаётся **только** в `Set-Cookie` `soul_session` с `HttpOnly` + `Secure` + **`SameSite=Strict`** + `Path=/` — ЕДИНОЙ фабрикой `newSessionCookie` для LDAP И OIDC (симметрия способов обязательна). **Strict безопасен и для OIDC-callback** (прежний рассинхрон Lax↔Strict устранён): `SameSite` ограничивает ОТПРАВКУ cookie на cross-site-запрос, а не её УСТАНОВКУ; на cross-site top-level redirect от IdP cookie СТАВИТСЯ (Set-Cookie на ответе callback-а), следующий шаг — same-site top-level навигация на `/ui` (302), на которой Strict-cookie ОТПРАВЛЯЕТСЯ. Прежний `Lax` на OIDC был избыточным послаблением. JSON-токена в теле ответа НЕТ (минимизация XSS-эксфильтрации). TTL cookie = `auth.jwt.ttl_default`. Программная выписка токена (CI/скрипты) остаётся через существующий `POST /v1/operators/{aid}/issue-token` (ADR-014).
...
- **Развилки.**
  ...
  4. **Форма доставки JWT → РЕШЕНО: HttpOnly+Secure+SameSite=Strict cookie `soul_session`** (LDAP И OIDC симметрично, ЕДИНОЙ фабрикой — рассинхрон Lax↔Strict устранён 2026-06-24, см. (g)). JSON-токена в теле нет. Программная выписка — через существующий `operator issue-token`.
