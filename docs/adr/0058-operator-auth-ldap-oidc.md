...
> **Статус: accepted (LDAP, стадия 1) / proposed (OIDC, стадия 2).** LDAP-аутентификация **реализована** (search-bind, auto-provision по группам, cookie-доставка JWT) — решения зафиксированы ниже. OIDC остаётся черновиком стадии 2 (скелет `auth/oidc` + config `auth.oidc` присутствуют, реальная логика не написана). Развилки LDAP-части разрешены (см. блок «Развилки»); развилки, специфичные для OIDC (имя enum-значения, PKCE), остаются открытыми до стадии 2.
...
- **(d) Маппинг внешней identity → `operators(aid)` + roles. РЕАЛИЗОВАНО (LDAP, `keeper/internal/auth/mapper.go`).**
  ...
  - **`created_by_aid` для federated (bootstrap-инвариант ADR-013 — как разрешено):** partial unique index `operators_first_archon_idx` (миграция 003) гарантирует ровно одну строку с `created_by_aid IS NULL` (первый bootstrap-Архонт); CHECK `self_reference_ok` запрещает `created_by_aid = aid`. Поэтому federated-provision **не может** ставить `NULL` (сломал бы инвариант на втором юзере) и не может ставить self. Решение: `created_by_aid` = служебный AID **`archon-system`** (та же конвенция, что у push auto-import `push.AutoImportSystemAID`) — не-self существующий маркер federated-источника. ⚠️ **Зависимость:** строка `operators(archon-system)` должна существовать до первого provision (FK `created_by_aid_fk`). Её посев — отдельный шаг (см. блок «Открытый вопрос посева `archon-system`» ниже); без неё provision вернёт FK-violation, а existing-оператор-логин и так работает.
  ...
  > **Открытый вопрос посева `archon-system` (needs_architect).** Системный оператор `archon-system` сам должен иметь `created_by_aid` — а оно не может быть `NULL` (конфликт с bootstrap-индексом) и не может быть self (CHECK). Корректный посев требует решения, затрагивающего инвариант ADR-013 (например: ослабить partial-unique до `WHERE created_by_aid IS NULL AND bootstrap_initial`, или seed `archon-system` особой миграцией с пересмотром индекса). Это **архитектурное решение**, вынесено отдельно; до него auto-provision не доведён end-to-end (existing-оператор-логин, маппинг ролей, cookie, revoked-инвариант — работают и протестированы).
...
- **Развилки.**
  ...
  Открыто (блокирует end-to-end auto-provision, **needs_architect**): посев системного оператора `archon-system` без нарушения bootstrap partial-unique-индекса ADR-013 (см. блок в (d)).
