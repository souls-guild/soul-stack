...
> **Status: accepted + end-to-end (LDAP, stage 1 / OIDC, stage 2) + web-login cookie→Bearer (NIM-77, see (f-bis)).** LDAP and OIDC authentication are **implemented and delivered end-to-end**. LDAP: search-bind, auto-provision by groups **with seeding the `operators` row**, cookie delivery of the JWT — the blocker of seeding the system operator `archon-system` is **closed** (`created_via` relax of the bootstrap index, migration 086; see (d) below). OIDC (stage 2): authorization-code flow with **mandatory PKCE (S256)**, discovery (`go-oidc/v3`) + JWKS validation of `id_token` (signature/iss/aud/exp/nonce), a cluster-shared server-side flow-state store on Redis (state→nonce/PKCE-verifier, single-use GETDEL, TTL 5m), endpoints `GET /auth/oidc/{login,callback}`, cookie delivery of the JWT (`SameSite=Strict`, unified with LDAP — security fix 2026-06-24, see (g)/(#4)). The mapper is generalized across both methods (`auth_method`/`created_via` = `ldap`|`oidc` from `cfg.Method`). All open forks are resolved: enum name — `oidc` (locked in), PKCE — mandatory (no config flag, not left to the operator).
>
...
- **(d) Mapping external identity -> `operators(aid)` + roles. IMPLEMENTED (LDAP, `keeper/internal/auth/mapper.go`).**
  ...
  - **Reconciliation may not empty the cluster's admin set (Amendment 2026-07-29, NIM-320).** The scoped revoke above obeys the IdP, and it was the one revoke path in the cluster below the self-lockout guard: it takes the package-level `rbac.RevokeOperator` because it must write inside its own transaction alongside the grants, and that function enforced nothing — the invariant lived in `rbac.Service`, which this path cannot use. So a managed role granting `*` could be stripped from the last operator holding it, leaving the cluster with no administrator and no route back through the API.

    What made this reachable without anyone erring: the IdP can be wrong in exactly one direction that matters — answering with **fewer** groups than it should. An outage, a directory reorganisation, a group whose membership has not propagated yet. Map a group to a `*`-granting role, which is the first thing anyone configures, and a bad day at the IdP demotes whoever logs in during it.

    **The revoke is now guarded, and a refusal fails the login** (`ErrReconcileWouldLockOutCluster`): nothing is revoked, and the deferred rollback discards the grants made in the same transaction, so membership is never left half-synced. A failed login is recoverable by fixing the IdP or the map; an empty admin set is not. The probe is the cluster-state precondition of [ADR-078 §n](0078-rbac-derived-roles.md), **not** a permission check — this path has no caller, and its grants are equally caller-less (`granted_by_aid=NULL`), so measuring it against an operator's rights would be meaningless and inventing a system caller to satisfy the least-privilege floor would mint an unlimited subject for no reason.

    Scope of the fix, stated because the obvious reading is wrong: an IdP answering with **no** usable groups was already safe and always had been — `Map` rejects an identity that maps to zero roles (`ErrNoRoleMapping`) before reconciliation begins, so nothing is revoked. The hole was the **partial** answer: groups that still map to something, minus the one granting `*`. The names in `rbac` were changed so the arrangement no longer depends on a comment — `RevokeOperator` runs the probe, and the bare DELETE is `RevokeOperatorRow`, reached only by asking for it.

    A cluster whose only administrator is an IdP-managed role is now a configuration that fails loudly instead of silently, which is the intended pressure: keep one administrator outside the federated domain.
...
- **(f) API surface (new public `/auth/*` endpoints outside `/v1`).** Registered at the root level of the chi router (parity with `/healthz`/`/docs`/`/ui` - WITHOUT `RequireJWT`/RBAC, see [router.go](../../keeper/internal/api/router.go)):
  ...
  |---|---|---|
  | `GET /auth/methods` | public | **IMPLEMENTED (NIM-77):** backend-driven catalog of login methods ([ADR-042](0042-backend-driven-ui.md)) — booleans `{password,ldap,oidc}` for the UI login form (`password` always true = paste-in operator-JWT; `ldap`/`oidc` reflect actual configuration) |
  | `POST /auth/token` | public (cookie) | **IMPLEMENTED (NIM-77):** exchange the `soul_session` session cookie for a short-lived Bearer in JSON (resolves the SPA cookie-gap — see (f-bis)) |
  | `GET /auth/oidc/login` | public | **IMPLEMENTED:** starts the OIDC code-flow with PKCE (302 redirect to the IdP) |
  | `GET /auth/oidc/callback` | public | **IMPLEMENTED:** OIDC callback, id_token validation, issues an internal JWT in a cookie (302 to /ui) |
  | `POST /auth/ldap/login` | public (HTTPS) | **IMPLEMENTED:** LDAP search-bind, issues an internal JWT in a cookie |

  `POST /auth/ldap/login` (`huma_auth.go`) and `GET /auth/oidc/{login,callback}` (`huma_oidc.go`) are implemented: ROOT-mount outside `/v1` (parity with `/healthz`, no `RequireJWT`/RBAC/metrics; anti-DoS body-limit is in place), mounted only when `LDAPAuth`/`OIDCAuth` is non-nil (opt-in, like `pushH`/`errandH`; OIDC additionally requires Redis). FULL-TYPED huma operations; committed in `docs/keeper/openapi.yaml` (groups `prefix /auth` in `fullSpecGroups`). Protection: TLS-required, body limit, OIDC — `state`(single-use)/`nonce`/PKCE. A successful login writes an `operator.login` audit event (`source: api`, payload `{method, aid, provisioned}` — no secrets). `GET /auth/methods` (backend-driven catalog) is implemented (NIM-77, see (f-bis)); rate-limit/lockout by username (anti-bruteforce) — implemented (see (g)). The MCP surface is **left untouched** — MCP accepts an already-issued internal JWT. OpenAPI — `/auth/*` are registered as huma operations and land in `/openapi.yaml` automatically (ADR-054).

- **(f-bis) Amendment 2026-07-09 (NIM-77): web login — exchange session-cookie → short-lived Bearer (resolve cookie-gap).** The federated login flow puts the internal JWT in an HttpOnly cookie `soul_session`, but `/v1` (`RequireJWT`/`extractToken`) only reads `Authorization: Bearer` — the SPA can't reach the API (cookie-gap). Resolution (**Option B**, propose-and-wait): **`POST /auth/token`** (public, outside `/v1`, no `RequireJWT`, [huma_auth_token.go](../../keeper/internal/api/huma_auth_token.go)) reads the cookie with **the same `jwt.Verifier`** as `/v1`, and issues a short-lived Bearer in the JSON body `{token, expires_at}`; the SPA stores it in localStorage and sends it as `Authorization: Bearer`. `RequireJWT`/`extractToken`/`/v1`/MCP **are left untouched** — the cookie is still not read on `/v1` (perimeter invariant, guard test). Decisions:
  - **Subject strictly from cookie claims:** `subject`/`roles`/`bootstrap_initial` of the issued token come only from `Verify(cookie)`, the request body is empty (no privilege-escalation; no RBAC permission required). `roles` in the token are cosmetic — `/v1` RBAC resolves permissions from the DB snapshot by `aid` (ADR-028/047).
  - **Short TTL with a cap:** `auth.jwt.exchange_ttl` (separate from `ttl_default=24h`, default **10m**, floor **1m**); effective `ttl = min(exchange_ttl, remaining_until_cookie.exp)`, `≤0 → 401`. The cookie remains a refresh-credential for silent re-exchange until its own expiration.
  - **Revoked check on exchange:** against the in-memory RBAC snapshot (`Enforcer/Holder.IsRevoked`, map-lookup, not SQL) — a revoked operator cannot exchange a still-live cookie; no PG dependency, freshness matches `/v1`.
  - **Anti-oracle:** missing/invalid/expired/revoked → generic `401` (detail only from `jwt.ClassifyVerifyErr`, same as RequireJWT); cross-site (`Sec-Fetch-Site: cross-site`) → `403` (defense-in-depth; primary is `SameSite=Strict` + absence of CORS). `expires_at` comes from the actually issued token (self-verify).
  - **Cookie scope narrowed `Path=/` → `Path=/auth`** (`newSessionCookie`, shared by LDAP+OIDC): the browser sends the 24h credential only to `/auth/*` (the only reader — `POST /auth/token`), not to `/v1`/`/mcp`/`/docs`.
  - **The exchange is not written to audit** (high-freq refresh; login already records `operator.login`); `/auth/token` is under `AuthLoginLimit` (per-IP throttle, `recordFailures=false` — an expired cookie ≠ a password guess). Guard tests — `huma_auth_token_test.go` (perimeter/round-trip/subject-from-claims/revoked/cross-site/ttl-cap/bootstrap_initial) + `is_revoked_test.go`.
