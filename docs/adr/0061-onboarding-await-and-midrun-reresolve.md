...
>
> **Implementation progress.** S1 (`await_online` pilot) is implemented: blocking poll of the Redis SID-lease + B1-strict fail + list-SID + the `keeper.yml::max_await_timeout` ceiling. S2 (Stratify passage boundary for `refresh_soulprint`) and S3 (actual re-resolve roster in run.go) are **implemented**: `refresh_soulprint: true` makes the step a passage-defining emitter, the scenario-runner re-resolves the roster at the refresh boundary (live snapshot, see §S3), `register.<name>.refreshed` echoes the flag value. WB cloud e2e is a separate slice.
