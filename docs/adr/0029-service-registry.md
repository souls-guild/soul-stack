...
- **Decision.**
  ...
  **(a) Storage — table `service_registry` + key-value `keeper_settings`.**
  ...
  | Table | Role |
  |---|---|
  | **`service_registry`** | Catalog of Services. `id` PK (kebab-case, immutable; the identifier is MINTED here — the manifest states none since NIM-726, so there is nothing to match it against. Spelled `name` until [ADR-0085](0085-entity-id-and-label.md) / NIM-729, migration `118`), `git`, `ref` ([ADR-007](0007-versioning-git-ref.md#adr-007-artifact-versioning--via-git-ref-not-a-manifest-field): version = git ref), `refresh` (opt. duration for auto-fetch, NULL-able), `created_at` / `updated_at`, `created_by_aid` / `updated_by_aid` FK→`operators(aid)` NULL-able (seed / no initiating Archon). |
...
- **ADR reconciliation.**
  - [ADR-007](0007-versioning-git-ref.md#adr-007-artifact-versioning--via-git-ref-not-a-manifest-field) — a Service's `ref` is now the column `service_registry.ref`, not the field `keeper.yml::services[].ref`; the "version = git ref" principle is unchanged.
  - [ADR-009](0009-scenario-dsl.md#adr-009-scenario--the-full-destiny-task-dsl-the-boundary-with-destiny-is-a-recommendation) — `default_destiny_source` for resolving `apply:destiny` is read from `keeper_settings`, not from `keeper.yml`; the hybrid resolve rule is unchanged.
  - [ADR-021](0021-hot-reload-config.md#adr-021-hot-reload-of-config-with-write-back-yaml) — the Service registry is **excluded** from the hot-reload-config path: an update = a DB mutation + `service:invalidate`, not `SIGHUP` / config-swap (like RBAC in ADR-028(d)).
