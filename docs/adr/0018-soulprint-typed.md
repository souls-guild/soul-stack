...
- **Decision.**
  ...
  **(d) The canonical CEL form — `soulprint.self.<path>`.** The bare form `soulprint.<path>` (without `.self`) — a **validation error** in `soul-lint`. Symmetric to `register.self.*` ([destiny/tasks.md §10](../destiny/tasks.md#10-template-context)). The existing examples (`examples/destiny/redis/tasks/` etc.) are rewritten by a batch task to match the canon.
