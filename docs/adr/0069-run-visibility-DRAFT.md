...
**RBAC** — like `/mcp/events` `authorizeSSE` ([`sse.go`](../../keeper/internal/mcp/sse.go)), WIDER by one action: the run initiator OR `incarnation.get`/`incarnation.history` here, where `/mcp/events` grants on `incarnation.get` alone. A nonexistent/foreign apply_id → **403** (anti-enum). Backend RBAC **does not depend** on the auth method (A0).

**Amendment 2026-09-12 (NIM-844): the decision is re-asked every 15 s, not taken once.** A stream runs for up to `sseMaxLifetime` (30 min) and `RejectRevoked` refuses only the *next* request, which an open stream never makes — so a revoked Archon went on receiving task payloads for the rest of the lifetime. `streamRunEvents` now re-evaluates the same rule on its own ticker and ends the body when the answer changes: revocation first (the token is no longer trusted, so being the initiator stops earning anything), then `incarnation.get`/`incarnation.history` for a watcher who is not the initiator — which is also what catches a *narrowing* short of revocation. What cannot change is not re-read: the run's incarnation and its initiator are fixed once the row exists, so the re-check costs no database round trip and the only per-tick work is the permission evaluation. The stream ends without a closing frame — the event kinds here are the bus's, and the client learns the way it does after any disconnect: it re-opens and is answered **403**.

**Amendment 2026-09-14 (NIM-858): `/mcp/events` has the same re-check, and the mechanism is now shared.** The amendment above closed the duplicate and left the original — this closes the original too, and moves the interval and its reasoning into [`keeper/internal/api/middleware/longlived.go`](../../keeper/internal/api/middleware/longlived.go), which all three long-lived channels (console socket, `/v1` run-events, `/mcp/events`) now call. `/mcp/events` was the worst-placed of the three: `RejectRevoked` does not run on the MCP listener at all (NIM-551), so it had no next request to be refused on either. Deciding "duplicate narrowly" per §A3 is what let one fix reach one channel and not the other, so the inventory is now derived from the source rather than remembered — `middleware/longlived_guard_test.go` is red the day a fourth channel of this class is written without a re-check.
...
- A cookie session; MCP `/mcp/events` is not touched — *by this slice*. Its re-authorization was closed later by NIM-858 (§A3 amendment 2026-09-14); the rest of the endpoint is still untouched.
