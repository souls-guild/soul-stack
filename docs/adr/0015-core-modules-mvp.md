...
- **Decision.**
  ...
  **Soul-side core MVP (18 modules):**
  ...
  | `core.http` | `probe` / `request` (verbs) | `probe` remains the read-only GET/HEAD contour (`changed=false` always). `request` is the explicit mutating contour: required method `{POST, PUT, PATCH, DELETE}`, optional request `body`/`content_type`, and `changed=true` only when transport succeeds and status matches `status_codes` (default `[200]`). Both share `url`/`headers`/`status_codes`/`timeout`, the three independent guards (`allow_http`/`allow_private`/`insecure_skip_verify`), and diagnostic response `{status, body, truncated, elapsed_ms, headers_keys}` with a final post-redaction 64 KiB rune-safe body cap. Probe retains guarded redirects; request never follows them, so one Apply is exactly one wire request. Header values are sensitive-by-construction (including `X-Consul-Token`): sent on the wire and redacted from echoed body/diagnostics, never emitted in output/log/audit. API idempotency and retry are scenario responsibilities. Capability is exactly `network_outbound`, without subprocess or filesystem writes. See Amendment 2026-08-08 below. |
  ...
  **`core.http` (read-probe HTTP) — accepted (2026-05)** on a real request, as a separate slice after `core.repo`/`core.firewall`. The original scope is deliberately narrowed to **read-only** (verb `probe`, methods `GET`/`HEAD`): health-check / API-readiness / reading a version are pure and `changed=false` by nature. The object `http` is separate from `core.url`: "`url` puts bytes on disk, `http` returns a response into a register". The shared HTTP infrastructure (`util.ValidateFetchURL`/`util.CheckRedirect`/`util.NewHTTPClient`/`util.HTTPDoer`) is reused by both modules. The mutating extension is now fixed by Amendment 2026-08-08 without widening `probe`.

  **Amendment 2026-08-08 (NIM-608, `core.http.request`).** A second verb adds the mutating API contour required by local Consul Agent lifecycle calls. The boundary is structural: `probe` accepts only GET/HEAD and keeps its default GET plus `changed=false`; `request` requires an explicit POST/PUT/PATCH/DELETE and rejects GET/HEAD. A successful request is one whose transport completed and whose status is in `status_codes` (default `[200]`); only it returns `changed=true`. A status mismatch is `failed` with diagnostic output and `changed=false`; transport/timeout failure is `failed` without response output. Request does not follow redirects: the first 3xx is checked against `status_codes`, so a 307/308 cannot replay the mutation. There is no hidden retry: scenario `retry` owns every repeated mutation. Request params add optional string `body` and `content_type` (the dedicated value overrides `headers[Content-Type]`). Response shape and 64 KiB cap are shared with probe, with the cap enforced after UTF-8 sanitation and redaction. All request-header values are sensitive-by-construction, notably `X-Consul-Token`; output contains keys only, and an echoed value in response/transport diagnostics is masked before register/log/audit. All three guards remain independently armed by default and must both be explicitly lifted for `http://127.0.0.1:8500` (`allow_http: true`, `allow_private: true`). The module retains exactly `network_outbound`, no subprocess/fs-write. Because sdk `ErrandReadSafe` is module-wide, `core.http` no longer carries that marker: the runner admits exact state `core.http.probe`, while `core.http.request` and future states remain default-deny (see ADR-033 Amendment 2026-08-08).
...
- **Consequences.**
  ...
  - Besides the 17 MVP core modules above, the `soul` binary provides an **infrastructure** core module `core.module.installed` for delivering custom modules to the host (Keeper → server-streaming RPC `FetchModule` → the catalog cache `paths.modules`, Sigil-verify). This is **not counted among the 17**: `core.module.installed` is a function of the Soul daemon itself, implemented as a core module for uniformity with the Destiny DSL (the operator writes an explicit step `module: core.module.installed` with `params: {name: <alias>}` in their Destiny if they want a custom module — the registration alias alone, not the two-level address a task uses: one artifact serves every module it declares, so the artifact is what gets installed). The behavior specification and input schema are **fixed by [ADR-065](0065-core-module-installed.md)** (transport `FetchModule`, the registry `plugins.soul_modules[]`, Sigil verification, hot-register). ⚠ **`FetchModule` is one of TWO transports as of the [ADR-065 amendment 2026-09-04](0065-core-module-installed.md#amendment-2026-09-04-nim-794-the-fetch-step-goes-to-the-source-and-fetchmodule-stays-as-the-egress-free-path) (NIM-794, not implemented — the sentence above describes what ships).** A host that can reach the artifact source fetches from it directly; `FetchModule` remains the path for hosts without egress and is **not deprecated**. Nothing else in this bullet moves: the step is still a core module, still not counted among the 17, still written with the bare registration alias.
- **Trade-offs.**
  - 17 Soul-side modules — more than the bare minimum (it could be squeezed to 6, replacing the rest via `core.exec.run`), but poor DX and idempotency cannot be verified. The base 12 are the necessary minimum for a production-ready first service; `core.url` was added on top as a safe replacement for the `core.cmd.shell` workaround for download, `core.line` — post-hoc on a real request (a trimmed safe MVP), `core.repo`/`core.firewall` — as the next slice on a real request, `core.http` — as a separate slice, with `request` added post-MVP without widening `probe`.
  - `core.line` is **accepted** (a trimmed safe MVP, without backrefs, replace of the first match) — a pilot of the in-place per-line editing pattern. `core.repo`/`core.firewall` are **accepted** (2026-05) as one slice following the pilot: both reuse `util.AtomicWritePreserving`/`util.DetectPkgMgr`/the new `util.DetectFirewall`, both `present`/`absent`. `core.http.probe` is **accepted** (2026-05), and the post-MVP mutating `request` extension is accepted by the 2026-08-08 amendment with a separate method set and changed-contract. Both reuse the HTTP infrastructure jointly with `core.url`. The early candidate name `core.uri` was rejected (uri/url confusion) — the object `http` was chosen.
