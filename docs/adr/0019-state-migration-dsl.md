...
- **Consequences.**
  ...
  - The redis-service migration-file example is rewritten for grammar (a) (a structural `foreach` instead of `{% for %}` Jinja). Implemented in [`examples/service/redis/migrations/002_acl_users_list_to_map/main.yml`](../../examples/service/redis/migrations/002_acl_users_list_to_map/main.yml) after the redis consolidation (`redis_users` from a list of names to a map `name → {perms, state}`).
