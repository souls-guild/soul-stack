...
- **Consequences.**
  ...
  - `examples/service/service-redis-cluster/migrations/001_to_002.yml` переписывается под грамматику (a) (вместо `{% for %}`-Jinja — структурный `foreach`).
  - Open Q №18 закрыт.
