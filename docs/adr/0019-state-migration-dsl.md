...
- **Consequences.**
  ...
  - The redis-service migration-file example is rewritten for grammar (a) (a structural `foreach` instead of `{% for %}` Jinja). Implemented in `examples/service/redis/migrations/002_acl_users_list_to_map/main.yml` after the redis consolidation (`redis_users` from a list of names to a map `name → {perms, state}`).
...
**Status:** accepted, **partly implemented** (epic NIM-734; this amendment is NIM-735). The **engine** — the directory layout, the version derived from the ladder, dropping the manifest key, and the `soul-lint` ladder check — landed with **NIM-736**, which also relocated the bundled `examples/` ladders and answered the derived-version edge case in the Consequences below. `schema.lock`, the `schema-stamp` command and the `soul-lint` lock check landed with **NIM-737**; NIM-738 is left with the one thing the relocation did not touch — the `015_system_acl_users` description comment corrected in clause 7.
...
**2. `from_version:` and `to_version:` leave the file.** `main.yml` carries `description:` and `transform:`, nothing about its own position. "The ladder goes by one" is **not a new invariant** introduced here: `to == from+1` was already enforced by the parser before this amendment, and the amendment merely single-sources the number that check was defending — which is why the check itself could go with it (NIM-736): [`statemigrate.Parse`](../../keeper/internal/statemigrate/parse.go) now takes the target version as an argument and derives the source, so there is nothing left for the two to disagree about.
...
`schema.lock` is generated next to the ladder: `version` (the top of the ladder at stamp time) and `fingerprint` (a hash of the **parsed and canonicalized** `state_schema`, never of the file text — a comment would break a text hash). Written by `soul-lint schema-stamp <service dir>` and read by `soul-lint` on every `validate-service`. A service repository wraps the two as `make schema-stamp` and `make validate`; both targets live there, in the tree holding `service.yml` and `migrations/`, not in this core repo.
...
`schema-stamp` re-stamps whenever it is run, so an author can edit `state_schema` and re-stamp without adding a step. That is deliberate, and it is exactly how `atlas migrate hash` behaves in Atlas: the bypass is visible in the diff — `schema.lock` changed, no new directory under `migrations/` — and a reviewer reads it there. The mechanical ban (the stamp refuses until the top of the ladder moves) was **rejected**, because it would force an empty migration step for every harmless schema edit, and a ladder padded with no-op directories costs more than a two-line diff a reviewer can see.

**7. A migration brings an incarnation up to the new schema, but not to new secrets.**

The migration DSL is a pure function of state and cannot reach Vault (the (b) sandbox above). A step that re-points the path a secret is derived under therefore leaves the incarnation in one of two wrong states, and **which one depends on the route, not on the verb**: a day-2 step re-resolving an already-stored record **fails closed, loudly** — `config.StripDeclaredSecrets` ([`shared/config/secret_field.go:533-547`](../../shared/config/secret_field.go)) removes the secret marker on the way into state, so the stored value carries no mint intent ([`keeper/internal/coremod/state/state.go:597-600`](../../keeper/internal/coremod/state/state.go)) and the resolve refuses ([`state.go:679-681`](../../keeper/internal/coremod/state/state.go), and [`state.go:682`](../../keeper/internal/coremod/state/state.go) for a field with no value and no marker); a create-class step carrying `generate_secret({…})` **silently mints a fresh password at the new path** that the running service does not know ([`state.go:207-216`](../../keeper/internal/coremod/state/state.go) pre-read → [`state.go:684-689`](../../keeper/internal/coremod/state/state.go)). `core.state.present` guards neither: its semantics are "an existing value is kept, a missing one is minted", not "verify the secret is reachable". That is why a step that changes a derived secret path must carry a plain-language item: hand the new passwords to consumers after the first run. For the **shape** of such a change see `examples/service/redis/migrations/015_system_acl_users/main.yml` — v14 minted under `secret/redis/<inc>/users/<name>`, v15 derives `secret/redis/<inc>/system_acl_users/<name>`. Cite it for the path shape only: its own description comment overstates the outcome of that particular step, and is corrected by NIM-738.

**Consequences of the amendment.**

- The diag codes `migration_version_missing` and `migration_version_invalid` are retired (NIM-736) — there is no header left to be missing or inconsistent. In their place is one code for the header still being there: `migration_version_key` ([`keeper/internal/statemigrate/errors.go`](../../keeper/internal/statemigrate/errors.go)), raised by the parser and, offline and with a line number, by [`config.ValidateMigrationStepFile`](../../shared/config/migration_ladder.go).
- The derived number is **arithmetically identical** to today's for every service that satisfies the documented completeness rule (version N ⟺ a gapless chain 1→…→N), so [ADR-0068](0068-service-upgrade-v2.md)'s `upgrade-paths` keeps working unchanged — only the source of the target integer moved (NIM-736): both readers now take `ServiceArtifact.StateSchemaVersion`, stamped once at load from [`config.ScanMigrationLadder`](../../shared/config/migration_ladder.go), instead of `Manifest.StateSchemaVersion` ([`keeper/internal/incarnation/upgrade_prepare.go`](../../keeper/internal/incarnation/upgrade_prepare.go), [`keeper/internal/api/handlers/incarnation_upgrade_paths.go`](../../keeper/internal/api/handlers/incarnation_upgrade_paths.go)). The runtime column `incarnation.state_schema_version` is untouched.
- **Answered by NIM-736 (deleting a step), and the answer is partly "nothing yet".** The question was: deleting a migration file used to yield `ErrMigrationChainBroken` ([`upgrade_prepare.go`](../../keeper/internal/incarnation/upgrade_prepare.go), surfaced as `reachable: false`), but under a derived version deleting the **top** step lowers the target instead. It turns out to be three cases, not two, because the outcome depends on where the incarnation sits as well as where the deletion is:
  - A step **below** the top leaves a gap. The chain still asks for the missing version, and [`ServiceLoader.LoadMigrationChain`](../../keeper/internal/artifact/service.go) still returns `ErrMigrationChainBroken` naming `migrations/<NNN>_*/main.yml` — unchanged behaviour, `reachable: false` still reaches the operator as data, and `config.ValidateMigrationLadder` reports it offline per missing version before the ref is pushed.
  - The **top** step (call the pre-deletion top `T`), for an incarnation at `T`: the derived version is now `T-1`, below the incarnation's, the transition reads as a **downgrade**, and the forward-only guard (`ErrDowngradeViaRef`) refuses it. Loud.
  - ★ The **top** step, for an incarnation at `T-1` — the NEW top: `target == current`, the chain is empty, and the upgrade **completes silently as a ref-bump**. `upgrade-paths` reports `direction: same-schema, reachable: true`. The linter does not help either, and deliberately so — a ladder one rung shorter is still gapless, still internally consistent, and refusing it would fire on every legitimate step removal (guarded as such by `TestLadder_DeletedTopStepLowersTheVersion`).
  - The **top** step, for an incarnation **below** `T-1`: a normal forward migration up to `T-1`. Complete chain, nothing skipped — not a case that needs an answer.
  
  **That third case was left open by NIM-736 and is closed by `schema.lock` (NIM-737).** Note what is and is not wrong in it: the incarnation's state is at `T-1` and the ladder tops out at `T-1`, so those two agree — what disagrees is `state_schema`, which still describes the shape the deleted step produced. It is therefore not a new defect but an instance of the one clause 4 above already names: the structure is described twice, by hand, and nothing reconciles the two. `schema.lock` is the only artifact that remembers where the ladder used to top out, so a stamped `version` above the current top is a removal and the service repo's own `make validate` refuses it (`schema_lock_version_stale`). That answer is OFFLINE: a repository that never stamped, or a ref pushed past the lint, still reaches the keeper with this case unchanged. Recorded here rather than papered over: the engine cannot tell a deleted rung from a rung that was never written, because nothing in the repository states the version twice any more. That is the cost of the derivation, and it is the same gap the fingerprint exists to cover.
