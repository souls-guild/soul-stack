...
- **Amendment (2026-07-09, pivot to fair-code / BSL — monetization).** Ahead of the public release, a monetization decision was made: the fully open Apache-forever model (decision (a), Amendment (n)) is replaced by a **fair-code** model. This revises the licensing part (a) and the (n) conclusion of "Apache across the whole lifecycle"; the parity strategy (b-m: hybrid without a wrapper, SDK, official plugins) is unchanged.
  ...
  - **(o) License map (3 buckets).**
    ...
    - **SDK** (`sdk/*`), **`examples/`**, and **official/community plugins** (`soul-stack-plugins`, `soul-mod-*`) → **Apache 2.0** (unchanged). The split is legally clean: plugins are separate processes over gRPC-stdio, not a derivative work of the core. Third-party authors are free to choose their own license, including proprietary paid plugins.
    - **Premium packs and enterprise modules** (later) → a separate **commercial** license (open-core on top of the BSL core).

  - **(p) Additional Use Grant (starting).** Production use is allowed, **except** offering the Licensed Work to third parties as a **Hosted/Managed Service** or **white-label**. Allowed: (a) internal use (incl. commercial operation of one's own/corporate infrastructure); (b) dev/test/eval/demo; (c) professional/managed services to clients where the client gets a **result**, not access to Soul Stack as a product. **Embedding/OEM** is not part of the starting grant (requires a commercial license; can be expanded later). The strategy is to start with a clear grant and **expand** over time (loosening is safe; tightening is a reputational tax + fork risk). The exact legal grant text and the definition of "Hosted/Managed Service" — in the `LICENSE` file; **final text to be checked by counsel**.
