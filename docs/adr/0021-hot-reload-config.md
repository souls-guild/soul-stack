...
- **Amendment 2026-07-24 (NIM-139) — cross-host coordination for the reload-able class, via Postgres + Redis pub/sub.** Normalized in [ADR-0073](0073-keeper-runtime-config-pg.md) (the `SettingsStore` overlay); this amendment records what changes in ADR-021 itself.

  - **(f) is closed for the reload-able class.** Cross-host "reload by cluster-wide event" is no longer post-MVP for keys admitted to the overlay: the value lives in `keeper_settings` (Postgres) and every instance re-merges on the `service:invalidate` pub/sub event, with the TTL-poll as the fallback. (f) continues to hold unchanged for everything else — the require-restart class, the bootstrap floor, and `soul.yml` on the Soul side are still per-host, still `SIGHUP`-driven.
  - **The file↔Postgres boundary.** Three layers with a fixed precedence, per key: built-in default < Postgres < `keeper.yml` (amended 2026-07-27 by NIM-141 — the local file outranks the cluster value; the original order had Postgres on top). A key with no row in `keeper_settings` falls through to the file — there is **no** seeding of file values into Postgres, so Postgres carries only deliberate operator overrides and `DELETE` of a row is a clean revert. Which keys are admissible is enumerated by the five rules of [ADR-0073(j)](0073-keeper-runtime-config-pg.md) — notably, security gates are excluded, since the overlay is fail-soft. Break-glass: `KEEPER_CONFIG_SOURCE=file` makes an instance ignore the overlay entirely.
  - **(d) write-back is split by class.** Write-back YAML stays exactly as specified for the file path. A key that lives in the overlay is written to `keeper_settings` instead — **never** back into `keeper.yml` — and the merge itself patches only an in-memory Document, which never reaches disk. Otherwise Postgres values would leak into the file layer and the precedence above would stop being observable.
  - **(c) is unchanged and is the load-bearing part.** The merged (file + Postgres) config passes the same parse → schema-validate → semantic-validate → atomic-swap pipeline as a plain file reload; the overlay adds a write-gate (validate + range bounds *before* the row is published) and an all-or-nothing read path that keeps the last-good snapshot.
  - **(g) audit is unchanged.** The `source` enum stays closed: an overlay-driven swap on a receiving node emits `config.reload_succeeded` with `source: keeper_internal` and `archon_aid: NULL`; the operator's identity is recorded once, by the settings write-path on the mutating node.
  - **(i) `config_history` remains deferred** — the audit trail plus git-blame on the file is still the history story.
  - **Documentation reconciliation.** The line "**Multi-host coordination** — not in MVP" in [`docs/keeper/config.md → Hot-reload`](../keeper/config.md#hot-reload) is superseded for the reload-able class and now points here and at ADR-0073.
