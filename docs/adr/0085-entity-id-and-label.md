...
**Status:** accepted, **partly implemented** (epic NIM-725; this ADR is NIM-727)
...
**Implemented by:** NIM-726 (the manifest key `name:` is deleted) and **NIM-728** — the `label` half, on all ten entities of the eight `NamePattern` registries: migration `117` adds a nullable, non-unique, un-CHECKed `label TEXT` to `incarnation`, `service_registry`, `providers`, `profiles`, `push_providers`, `omens`, `heralds`, `tidings`, `vigils`, `decrees`; the field is settable at creation and mutable through `PUT /v1/<collection>/{name}/label` (MCP `keeper.<resource>.label-set`) under the new permission family `<resource>.label-set` and the audit events `<resource>.label_changed`; every read surface that returns the identifier returns it. **THE INVARIANT is guarded behaviourally**, at each of the derivations it names — see "Where the invariant is guarded" in the package doc of [`keeper/internal/registrylabel`](../../keeper/internal/registrylabel/registrylabel.go), and the executable statement of what a caption in a Vault path would cost in that package's tests.

**NIM-729 renames the identifier** on the same ten entities, all ten of them, landed registry by registry (`service_registry`, then `providers` / `profiles` / `push_providers`, then `omens` / `heralds` / `tidings` / `vigils` / `decrees`, and `incarnation` last): the DB column (migration `118`, a
`RENAME COLUMN` plus a `RENAME CONSTRAINT` so each CHECK survives character-for-character), the Go
field and its `NamePattern`/`ValidName` pair (`IDPattern`/`ValidID`), the wire field `name` → `id`,
the URL path template `{name}` → `{id}` on those registries' routes, the MCP tool argument, and the
audit payload key. **The grammar does NOT move with it** — each registry keeps its current regex, so
the rename is behaviour-preserving and provable as one thing; adopting the single `IDPattern` this
ADR decides is a NARROWING on six of the eight and is scheduled apart from the rename. NIM-729 also
draws one boundary explicitly: `artifact.ServiceRef.Name` and the other internal consumer structs
that merely carry a copy of an identifier keep their spelling, because they are neither a column, a
wire field, an MCP argument nor a URL — the rename follows the identifier across the surfaces an
operator reads, not across every Go struct that holds one.

Still outstanding: the tail beyond the eight (NIM-732 — `rbac_roles`, `synods`, the remaining
`{name}` URL templates, and every `*_name` FK column: `state_history.incarnation_name`,
`apply_runs.incarnation_name`, `incarnation_choirs.incarnation_name`,
`incarnation_membership.incarnation_name` and `decrees.incarnation_name`, which NIM-729 left
spelled as they were because a foreign key follows column identity and nothing breaks by waiting), the grammar unification above, the CEL reach and its compatibility window
(NIM-730), and `operators.display_name` → `label` (NIM-733).
