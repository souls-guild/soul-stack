...
Still outstanding: the `name` → `id` rename itself (NIM-729, the same eight registries), the tail beyond them (NIM-732 — `rbac_roles`, `synods`, the 49 `{name}` URL templates), the CEL reach and its compatibility window, and `operators.display_name` → `label` (NIM-733).
