...
## Amendment (2026-07-01, «channel-types — 6 новых типов канала в двух транспортных классах»; зафиксирован ретроспективно 2026-07-03)

**Решение пользователя 2026-07-01** («Heralds 6 типов ОК»; дизайн — architect; реализовано, merge `f3739d03`, миграция 091). Погашение долга NIM-18 — фиксация ретроспективно.

### (n) Расширение enum `heralds.type` — шесть новых типов поверх MVP-`webhook`.

Closed-enum `heralds.type` (§a) расширен **only-add** шестью типами (итоговый enum — 7 значений): `webhook` (MVP, как был) + **`telegram` / `slack` / `mattermost` / `discord` / `custom`** (HTTP-класс) + **`email`** (SMTP-класс). Существующие `webhook`-строки не затронуты (миграция 091 пересоздаёт CHECK `heralds_type_enum` расширенным набором). Это закрывает «Отложено post-MVP → Herald-типы `slack`/`email`» из базового ADR — и шире заявленного (плюс messenger-семейство и generic `custom`).

### (o) Двухклассовая транспортная модель — `channelDriver` (HTTP) + отдельная SMTP-ось.

- **HTTP-класс** (`webhook`/`telegram`/`slack`/`mattermost`/`discord`/`custom`): каждый тип — драйвер **`channelDriver`** ([keeper/internal/herald/channel.go](../../keeper/internal/herald/channel.go)) с тремя обязанностями — `validateConfig` (CRUD-валидация config по дескриптору полей, Vault не читается), `secretRequired` (использует ли тип top-level `secret_ref`), `resolveDelivery` (сборка **`httpDelivery`** на момент доставки: URL + метод + тело + заголовки + SSRF-opt-out-флаги + опц. HMAC signing-key). **Единый SSRF-контур by construction:** драйвер ТОЛЬКО строит `httpDelivery`; guard (`shared/netguard`, §e) и `client.Do` зовёт сам `DeliveryWorker.deliver` — новый HTTP-тип не может обойти guard. У фиксированных публичных endpoint-ов мессенджеров opt-out-флаги (`httpAllowed`/`allowPrivate`) всегда `false`.
- **SMTP-класс** (`email`): **НЕ** `channelDriver` (нет httpDelivery/HTTP-транспорта) — отдельная ось `net/smtp` ([email.go](../../keeper/internal/herald/email.go)), своя ветка в `DeliveryWorker.deliver`, свой SSRF-guard по резолвнутому IP.
- Имена `channelDriver` / `httpDelivery` / `HeraldFieldSpec` / `FieldKind` закреплены пользователем (propose-and-wait пройден) — [naming-rules.md → Модули и подсистемы внутри keeper](../naming-rules.md#модули-и-подсистемы-внутри-keeper).

### (p) Семантика тел и размещение секретов по типам.

- **Тела:** `webhook`/`custom` шлют структурный `webhookPayload` (§d + `annotations` §i); мессенджеры `telegram`/`slack`/`mattermost`/`discord` — **человекочитаемый текст-дайджест** (их приёмники не парсят наш payload); `email` — RFC5322-письмо. `custom` = произвольный HTTP-endpoint с **фиксированным** телом `webhookPayload` (оператор-заданного `body_template` НЕТ — шаблонный движок в теле уже отвергнут, §h), настраиваемым методом и опц. `header_secret_ref` в `Authorization`.
- **Секреты:** top-level `secret_ref` (§a) использует **только `webhook`** (HMAC-подпись тела, `X-SoulStack-Signature: sha256=<hex>`); у остальных типов credential (bot-token / SMTP-пароль / auth-header) — **vault-ref-поле ВНУТРИ `config`** (дескриптор `Secret=true` ⟹ `Kind=vault_ref`). Инвариант §e «секрет не в PG cleartext» сохранён для всех типов.

### (q) Каталог типов — `GET /v1/herald-types`, единый источник.

Канонический реестр `channelDrivers` (+ `email`) — **единственный источник набора типов**: из него выводятся (1) generic-валидатор config по дескрипторам `HeraldFieldSpec` (`validateBySpec`), (2) huma-enum API, (3) PG-CHECK (091), (4) каталог-эндпоинт **`GET /v1/herald-types`** (`TypeCatalog` → `HeraldTypeDescriptor{Type, Fields, SecretRequired}`; auth-only, без отдельной permission). UI строит форму per-type из каталога, не хардкодит ([ADR-042](0042-backend-driven-ui.md)); каталог и валидатор рассинхрониться не могут (один источник), три внешних места (CHECK/huma-enum/каталог) сверяются guard-тестом с `AllHeraldTypes`. Добавление нового HTTP-типа = одна запись в `channelDrivers` + CHECK-миграция + huma-enum.

**Contract-impact:** всё additive (§f) — enum-расширение only-add, новый read-only эндпоинт `/v1/herald-types`, тело webhook-контракта (§d/§i) не менялось; Tiding/dispatcher/worker-механика (§c/§d) не затронута — расширился только резолв доставки по типу.
