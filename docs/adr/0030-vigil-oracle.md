...
- **Entities (propose-and-wait passed, entered into [naming-rules.md](../naming-rules.md)).**
  ...
  - **Decree** — a reactor rule (Postgres registry; managed via OpenAPI/MCP). **Default-deny** (like a Rite [ADR-025](0025-augur.md#adr-025-augur--keeper-side-broker-for-soul-external-access)): no matching Decree → the event triggers no action. Subject binding — `coven` **XOR** `sid` (like a Rite); **widened to exactly one of four dimensions by the 2026-08-05 / NIM-280 amendment below** ([ADR-008](0008-coven-stable-tags.md#amendment-2026-08-05-nim-280-a-rules-subject-reads-both-levels--targeting-only)). **A mandatory `incarnation_name`**: a scenario operates on `incarnation.state` (ADR-009), so a Decree explicitly carries the incarnation over which the action runs — guessing it from the subject is not allowed (a host may be a member of several incarnations; guessing = an amplification vector from a compromised Soul). `ServiceRef` is resolved **from** the incarnation (ADR-007 — the git-ref is not duplicated in the Decree); the reaction is enqueued against the **single SID sender** of the Portent (not against the whole coven); before enqueue — a **membership-check** (the subjectSID must belong to the Decree's incarnation, otherwise skip — fail-closed, no fire/audit written; read from `incarnation_membership`, never from the subject's covens — see the amendment below). The optional `where` predicate — a **keeper-local CEL sandbox** with the single root `event` (a narrow predicate over an untrusted payload; `shared/cel` is not extended, a deny-list as in migration-CEL ADR-019 in spirit). **Action = ONLY a named scenario** (whitelist) via the work-queue; a raw command is rejected (see the security invariant).
...
- **Amendment 2026-07-28 (NIM-224) — the subject's covens are the EFFECTIVE label set. REVOKED 2026-08-05 by NIM-281 (below).** It resolved both halves of the subject binding (`vigils.coven`, `decrees.subject_coven`) against the host's own `souls.coven[]` unioned with the covens and names of every incarnation it belonged to, making `coven: [<incarnation>]` the supported way to scope a rule to an incarnation's hosts.

- **Amendment 2026-08-05 (NIM-281) — the subject's covens are `souls.coven[]`; membership is not a label and never becomes one.** Both halves of the subject binding (`vigils.coven`, `decrees.subject_coven`) match against the tags an operator attached to that host, and nothing else ([ADR-008 / NIM-281](0008-coven-stable-tags.md#amendment-2026-08-05-nim-281-a-label-is-never-inherited)). Belonging to an incarnation attaches no tag, so `coven: [<incarnation-name>]` reaches only hosts somebody tagged with that string — it is an ordinary tag that happens to be spelled like an incarnation, not a way to say "the hosts of that incarnation".

  To scope a rule to a set of hosts, tag those hosts (`POST /v1/souls/coven`) and bind the rule to that tag. There is no declarative "members of incarnation X" subject — see **NIM-280**, which answers that gap below, on the same day, for both Decrees and Rites.

  The Vigil half matters as much as the Decree half, and for a quieter reason: a Vigil that is not broadcast to a host means the check never runs there, no Portent is ever emitted, and the Decree side is never consulted at all. An untagged host is therefore silently unwatched — the failure mode is absence, not an error.

  **The membership-check stays exactly where it was, and its rationale gets stronger.** It reads `incarnation_membership` directly, never the subject's covens: `incName ∈ coven` is true for any host somebody tagged with that string and false for a member nobody tagged — wrong in both directions. Treating a tag as membership would let a coven tag hand a non-member the right to have that incarnation's scenarios enqueued on it, which the cross-incarnation escalation invariant (b) exists to refuse. The subject binding answers *may this rule see the host*; the relation answers *does the host belong here*.

- **Amendment 2026-08-05 (NIM-280) — the subject binding is EXACTLY ONE of four dimensions.** Both halves (Vigil and Decree) drop `coven` XOR `sid` for:
  ...
  This answers the gap named just above, and answers it **without** re-introducing inheritance. Nothing is written to `souls`: a host still carries only what an operator attached to it, and the union of the two levels lives for the duration of one selector match — no other consumer (RBAC scope, `soulprint.self.covens`, push provider choice) sees it. Membership is read from the relation, and an incarnation's **name** is still not one of its labels, so `coven: ["<incarnation-name>"]` keeps reaching only hosts somebody tagged with that string. The dimension names are the RBAC scope vocabulary ([rbac.md](../keeper/rbac.md)) minus its boolean algebra: a subject stays a flat one-of-four, so it remains a set of indexed columns and "which rule reached this host" has a one-line answer. The incarnation address is the **pair** `<service>.<name>`, never the bare name — incarnation names are unique today (migration 005), and spelling the address in full is what keeps dropping that uniqueness a migration rather than a re-spelling of every stored rule.

  **The Decree now carries two incarnation-shaped fields, and they are not the same field.** `subject.incarnation` asks *may this host trigger the rule*; the top-level `incarnation_name` names the incarnation the **action** runs against, and its membership-check is untouched — same relation, same fail-closed skip, same rationale as the NIM-281 amendment above. A Decree with `subject.incarnation = redis.redis-a` and `incarnation_name: redis-b` is legal and fires for nobody: a host on A's roster passes the subject and fails the membership-check for B. The DB spells the difference with the `subject_` column prefix, and the API nests the subject (`subject.incarnation.{service,name}`) so the two can never be confused for one key.
  ...
  **⚠ The label namespace is shared.** Putting `prod` on an incarnation widens every existing `coven: ["prod"]` subject to that incarnation's members with no rule edited — the audit trail records an incarnation update, not a rule change. That is the declared meaning of a label subject, not a defect, but it cuts both ways here: a widened Vigil starts watching hosts nobody added to it, and a widened Decree lets hosts nobody listed trigger a reaction (still gated by the membership-check on `incarnation_name`). Where that is not wanted, address the roster explicitly (`incarnation`) or name the hosts (`sid`).

  The quiet half of the NIM-281 amendment still holds and now has a second edge: a host no Vigil reaches runs no check, emits no Portent and is silently unwatched — and under two-level resolution, **unbinding a host from an incarnation withdraws every Vigil it was reached by through that incarnation**. Membership changes are therefore monitoring-affecting. Storage — migration 113: `vigils.sid` and `decrees.subject_sid` become `TEXT[]` (the coven halves already were), each registry gains four columns for the two new dimensions (`service` / `incarnation` / `trait_key` / `trait_value`, `subject_`-prefixed on `decrees` as its existing columns are), and `*_subject_xor` becomes `*_subject_one_of` — a CHECK counting an empty array as ABSENT, so `coven: []` is a rule with no subject and is refused rather than stored as one that binds to everything.
