...
- **Amendment 2026-07-28 (NIM-224) — the subject's covens are the EFFECTIVE label set; membership is not a label.** Both halves of the subject binding (`vigils.coven`, `decrees.subject_coven`) resolve against a host's **own `souls.coven[]` unioned with the covens it inherits from every incarnation it belongs to** — each incarnation contributing its `covens[]` plus its name ([ADR-080](0080-label-inheritance-union.md)). So `coven: [<incarnation>]` is the supported way to scope a rule to an incarnation's hosts.

  This restores a reading that used to come free. When this ADR was written, `incarnation.name` was physically injected into `souls.coven[]`, so an incarnation-scoped rule matched its hosts as a side effect of the copy. [NIM-124](0008-coven-stable-tags.md) moved membership into `incarnation_membership` and stripped that value out; the Oracle was not converted along with the other readers, and every incarnation-scoped Vigil and Decree **silently stopped matching anything** — the column still existed and the query still succeeded, so nothing failed, nothing was logged, and the reaction simply never fired.

  The Vigil half matters as much as the Decree half, and for a quieter reason: a Vigil that is not broadcast to a host means the check never runs there, no Portent is ever emitted, and the Decree side is never consulted at all.

  **The membership-check is deliberately NOT part of this.** It reads `incarnation_membership` directly, because the union above admits host-attached tags: `incName ∈ effectiveCovens` is also true for a host merely tagged with the incarnation's name, and treating that as membership would let a coven tag hand a non-member the right to have that incarnation's scenarios enqueued on it — the cross-incarnation escalation invariant (b) exists to refuse. The subject binding answers *may this rule see the host*; the relation answers *does the host belong here*.
