...
- **Amendment 2026-08-05 (NIM-281) — the subject's covens are `souls.coven[]`; membership is not a label and never becomes one.** Both halves of the subject binding (`vigils.coven`, `decrees.subject_coven`) match against the tags an operator attached to that host, and nothing else ([ADR-008 / NIM-281](0008-coven-stable-tags.md#amendment-2026-08-05-nim-281-a-label-is-never-inherited)). Belonging to an incarnation attaches no tag, so `coven: [<incarnation-name>]` reaches only hosts somebody tagged with that string — it is an ordinary tag that happens to be spelled like an incarnation, not a way to say "the hosts of that incarnation".
  ...
  To scope a rule to a set of hosts, tag those hosts (`POST /v1/souls/coven`) and bind the rule to that tag. There is no declarative "members of incarnation X" subject — see **NIM-280**, which is where that gap is tracked for both Decrees and Rites.
