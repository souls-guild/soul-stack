...
- **Decision.**
  ...
  **(b) `source` — closed enum, 5 MVP values.** The source category of the event — who initiated it:
  ...
  | Value | When |
  |---|---|
  | `signal` | SIGHUP file-edit-path hot-reload ([ADR-021](0021-hot-reload-config.md#adr-021-hot-reload-of-config-with-write-back-yaml)). |
  ...
  | `soul_grpc` | Events from a Soul via the gRPC EventStream ([ADR-012](0012-keeper-soul-grpc.md#adr-012-keepersoul-grpc-contract-one-eventstream-with-oneof-keeper-side-render-forward-compat-only-add)), forwarded by Keeper. |
  ...
  **(d) Retention — via a new Reaper rule `purge_audit_old`.** Cleanup of `audit_log` is built into the existing Reaper mechanism ([reaper.md](../keeper/reaper.md), [ADR-006(d)](0006-cache-redis.md#adr-006-cache-and-coordination--redis)) — not a separate worker, not a cron inside `shared/audit`. A new rule in `keeper.yml → reaper.rules`:
  ...
  `action: delete` (per the rule-grammar table — [reaper.md → Rule structure](../keeper/reaper.md#rule-structure)), `max_age` mandatory. The exact `max_age` value is tuned to the installation's compliance requirements via hot-reload.
  ...
  **(g) Write-path — who writes audit.** A closed set of initiators inside the Keeper process (all write into the single `audit_log` via the shared helper `shared/audit`):
  ...
  | Hot-reload pipeline | `signal` or `api` | `config.reload_succeeded` / `config.reload_failed` ([ADR-021(g)](0021-hot-reload-config.md#adr-021-hot-reload-of-config-with-write-back-yaml)). |
  | `keeper.cloud` | `api` (if the initiator is an Archon via a scenario) or `keeper_internal` | `cloud.vm.created` / `cloud.vm.destroyed` ([ADR-017](0017-keeper-side-core.md#adr-017-keeper-side-core-modules-extended-corecloudprovisioned-corevaultkv-read)). |
  | `keeper.push` | `api` or `keeper_internal` | `push.applied` / `push.failed`. |
  | Bootstrap (`keeper init`) | `keeper_internal` | The first `operator.created` with `archon_aid: NULL` ([ADR-013](0013-bootstrap-archon.md#adr-013-bootstrap-the-first-archon)). |
  ...
  **(h) Event-types catalog.** The name convention is `<area>.<action>` (lowercase, dots), like RBAC permissions ([rbac.md → Permissions format](../keeper/rbac.md#format-permissions)). The catalog is open and extended by an ordinary PR in [`docs/naming-rules.md → Audit-events`](../naming-rules.md#audit-events) **as the subsystem is implemented** — we do not try to enumerate all 50+ names in advance. By the time ADR-022 was fixed, the catalog was filled with principles (5 categories by the `source` enum) and examples by area (`config.*`, `operator.*`, `incarnation.*`, `push.*`, `cloud.*`, `reaper.*`, `task.*`).
  ...
  **Amendment 2026-07-27 (NIM-194) — how the three fields are enforced.** Until this amendment the block had no consumer at all: `audit.enabled: false` turned nothing off, and `otel_export` switched a dual-write that was never wired. Four rules now fix what the fields mean, because a compliance control that is merely documented is worse than an absent one.

  1. **One gate, not a check per initiator.** The toggle is a decorator over the shared `audit.Writer` (`keeper/internal/auditgate`), applied where the writer is assembled (`setupAudit`). The write-path of (g) is a closed set only by convention; a single choke-point makes it closed by construction, so a new initiator cannot forget the flag. A gated event is dropped and reported as success — returning an error would fail the initiator's own operation over a deliberate setting.

  2. **★ Disabling the audit is itself audited, and the record precedes the gap.** `config.reload_succeeded` / `config.reload_failed` / `audit.disabled` / `audit.enabled` are written regardless of the toggle. This is load-bearing rather than cosmetic: `config.Store` swaps the snapshot **before** it emits the reload event, so the very reload that disables audit would otherwise be journaled through an already-closed gate and leave no trace of who stopped the trail or when — the flag would buy the ability to work without traces. The `audit.disabled` marker is written by the gate in-line on the write path, ahead of the first event it suppresses, and NOT from an `OnReload` subscriber: `Store.notify` runs subscribers in their own goroutines, so a marker written there races the drops it must precede.

  3. **`otel_export` presupposes the `otel:` block.** The secondary writer (`auditotel` under `auditmulti`) is assembled at startup only when OTel itself is enabled, and the flag gates the export per event on top of that. With no OTel stack configured there is nothing to export to and the flag has nothing to switch — the dependency is stated in [config.md → audit](../keeper/config.md#audit) rather than hidden behind a no-op.

  4. **`retention_days` materializes the rule it aliases.** With `reaper:` configured and no `purge_audit_old` declared, the alias creates the rule (`enabled: true`, `action: delete`) so the number is actually applied; a declared rule always wins, and a divergent pair stays a hard `audit_retention_mismatch` error. With no `reaper:` block nothing is fabricated — there would be no Reaper to run it.

  **`keeper init` is deliberately outside the gate.** Bootstrap of the first Archon builds its own writer before the daemon exists, and (ADR-013) requires that `operator.created` with `bootstrap_initial: true`. A one-shot administrative command that mints cluster-admin credentials must not be silenceable by a config flag; ignoring `audit.enabled` there is the lesser evil.

  **The block stays out of the `SettingsStore` overlay** ([ADR-0073(j.2)](0073-keeper-runtime-config-pg.md)) — `audit.enabled` is a security gate, and a fail-soft overlay degrades toward the more permissive value. Turning audit off remains an edit to `keeper.yml` on a host, not an API call.

  **(j) Read API `GET /v1/audit` — a separate task.** Not normalized in this ADR: implementation of the Operator API extension on the first real request. Permission — `audit.read` (one name), filters — `event_type` / `aid` / `correlation_id` / `date_range`, pagination — standard for the Operator API. Adding the endpoint to the Operator API + the permission in [rbac.md → Permissions catalog](../keeper/rbac.md#permissions-directory) — without breaking changes ([ADR-014](0014-operator-identity.md#adr-014-operator-identity-model-archon) allows extension without breaking).

  **(k) PII / Secret masking — shared mechanism.** The same rules normalized in [operator-api.md → Secret masking](../keeper/operator-api.md#secret-masking-in-logs-and-traces): JWT tokens / private_key / password / credentials_ref / Vault-ref values are masked with `***` before being written into `payload` or into OTel span attributes. The concrete list of masked fields — a shared middleware in `shared/audit` (on top of the same table used by the Operator API OTel exporter) — a separate security task before release. The audit pipeline does not introduce its **own** list of masked fields — it takes it from the shared registry in `shared/`.
...
- **Consequences.**
  ...
  - **New block `audit:` in `keeper.yml`** — [config.md → audit](../keeper/config.md#audit). Three fields, all reloadable, enforced per the (i) amendment. The per-block reload-policy summary table in [config.md → Hot-reload](../keeper/config.md#hot-reload) is extended with a row.
  - **Two event types `audit.disabled` / `audit.enabled`** — [naming-rules.md → Audit-events](../naming-rules.md#audit-events). `source: keeper_internal`, payload `{kid}`: the toggle lives in a per-host file, so a flip is a fact about one instance.
