...
`audit.kafka.*` is read only when `sink: kafka`. Kafka connection secrets are **vault-ref** (the [ADR-052(e)](0052-herald-notifications.md#adr-052-herald--tiding--уведомления-о-событиях-прогонов) `secret_ref` / [ADR-025](0025-augur.md#adr-025-augur--keeper-side-брокер-внешнего-доступа-soul) `auth_ref` pattern), not PG/disk cleartext.
...
1. **Transactional outbox** — the audit event is written to a PG outbox table in **the same transaction** as the business change; a separate relay worker (claim-queue parity with [ADR-027(d)](0027-apply-work-queue.md#adr-027-модель-исполнения-apply--work-queue--claim-acolyte-пул-ward-claim)/[Herald delivery](0052-herald-notifications.md#adr-052-herald--tiding--уведомления-о-событиях-прогонов)) moves it to Kafka and marks it delivered. The strongest guarantee (atomicity with the business tx), but it **does not remove the load from PG INSERT** (the outbox INSERT is of the same order as the `audit_log` INSERT) — contradicting the motive of "removing PG writes". The outbox table lives shorter (truncated after relay), but the write rate is the same.
...
- **[known-limitations.md](../known-limitations.md#audit-scaling--рассчитан-на-малую-бету)** — clarification: the Kafka sink is designed (this ADR), not implemented in the beta; it supersedes the Redis Stream option, batched INSERT remains (i).
- **[ADR-053](0053-dependency-tiers.md#adr-053-tier-ы-инфраструктурных-зависимостей)** — a row in the OPTIONAL table (Kafka sink, fail-closed).
...
- **[ADR-053](0053-dependency-tiers.md#adr-053-tier-ы-инфраструктурных-зависимостей)** — Kafka sink = OPTIONAL-with-degradation (fail-closed); PG+Redis+Vault remain the only required core; the rule "a new required — only through a user decision" is honored.
- **[ADR-052](0052-herald-notifications.md#adr-052-herald--tiding--уведомления-о-событиях-прогонов)** — Herald tap over the sink + the `changed_tasks` dependency on audit-in-PG (open question (n)).
- **[ADR-006](0006-cache-redis.md#adr-006-кэш-и-координация--redis)** — Redis hot layer ≠ a long-term audit buffer (why Kafka, not Redis Stream).
- **[ADR-011](0011-go-layout.md#adr-011-раскладка-go-кода-gowork-с-модулями-по-сторонам)** — `shared` pgx-free + backend-agnostic; both sink implementations in `keeper/internal` (h).
- **[ADR-027](0027-apply-work-queue.md#adr-027-модель-исполнения-apply--work-queue--claim-acolyte-пул-ward-claim)** — the claim-queue pattern for the outbox-relay candidate (e).
