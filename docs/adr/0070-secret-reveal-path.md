...
**Контекст.** Оператор видит в State-вьюхе, что у инкарнации есть, скажем, redis-пользователи, но не может посмотреть их пароли: `state`/`spec` в GET-ответах маскируются ([ADR-064](0064-secret-write-path.md), defense-in-depth в [operator-api.md](../keeper/operator-api.md)), а сами значения живут в Vault по ref. Для day-2 (передать пароль пользователю, проверить подключение руками) оператору нужен **санкционированный** способ раскрыть конкретное значение — не снимая маскинг глобально и не давая прямой доступ в Vault. Механика должна быть **generic** (свойство любого сервиса с секретами в state), а не redis-хардкод в Keeper-е.
