...
**Amendment 2026-07-01: проверка пустоты реестра игнорирует системных операторов.**

С миграции **086** ([ADR-058(d)](0058-operator-auth-ldap-oidc.md#adr-058-федеративная-аутентификация-операторов-archon--ldap--oauth2oidc)) системный `archon-system` (`created_via='system'`) посеян в `operators` на любой инициализированной БД — он FK-якорь для system-инициированных вставок, не реальный Архонт. Из-за этого голый `SELECT count(*) FROM operators` на **чистой** БД возвращал 1, и обе проверки ломались: (e) `keeper init` падал с `ErrAlreadyInitialized` на пустом кластере (регрессия), а (d) restart-guard видел «реестр непуст» и молча стартовал keeper «как инициализированный» без единого Архонта. Уточнение инварианта: **«реестр пуст» = нет ни одного оператора с `created_via != 'system'`**. Обе проверки — (e) под advisory lock в `keeper init` и (d) на старте `keeper run` — считают только НЕ-системных операторов (`operator.CountNonSystem`), симметрично. Смысл инварианта (защита от re-bootstrap, HA-race, self-lockout) не меняется; корректируется только то, что системный якорь не трактуется как «настоящий» оператор.
