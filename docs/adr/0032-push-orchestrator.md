...
- **Teleport by-name transport (ADR-063 amendment) — НЕ через `push.host_ca_refs[]`.** Bootstrap-доставка токена в режиме `push.transport: teleport` (keeper-side `core.bootstrap.delivered`) делает host-verify через Teleport identity-file (Teleport host-CA из `known_hosts` identity), а НЕ через `push.host_ca_refs[]` Vault-host-CA. `host_ca_refs[]` остаётся обязательным только для generic-режимов (push-прогон Destiny + `transport: direct` bootstrap-доставки). См. [ADR-063 §Amendment](0063-bootstrap-token-delivery.md).
