...
- **Выбор версии при create — нет.** Анти-version-craft инвариант сохраняется: create/операционные формы всегда на `inc.ServiceVersion`. Единственное легальное место смены версии — **действие upgrade** (и его discovery — §6), а не создание.
...
Файлы апгрейд-сценариев — в **отдельном top-level каталоге сервис-репо** `upgrade/<slug>/main.yml` в дереве **НОВОЙ** версии, НЕ в общем `scenario/`. Причины: (а) в списках операционных сценариев они пугали бы оператора («что это за операция, можно ли её запускать руками?»); (б) отдельная папка даёт естественное разделение и удобный ретрай.
...
- **upgrade/ не показывается в обычных списках сценариев** (`GET /v1/services/{name}/scenarios`, [`api/handlers/service.go:481`](../../keeper/internal/api/handlers/service.go)) — они видны только через upgrade-контур (§6).
...
`spec.input` = **write-once рецепт создания** (используется `rerun-last` по create-ветке + аудит), истина желаемого состояния = `incarnation.state` (amendment [ADR-057](0057-state-changes-crud-verbs.md), операционный сценарий читает `state`, не `input`/`essence`). **При апгрейде `input` НЕ мигрируется** и НЕ является источником для операционных сценариев: он — pre-state seed момента create, а не желаемое состояние. Upgrade-сценарий работает с `incarnation.state` (читает развёрнутый факт, пишет через `state_changes`) и `essence` целевой версии — как любой операционный сценарий. Явная граница: апгрейд может изменить форму `state` (через [ADR-019](0019-state-migration-dsl.md)-миграции) и содержимое (через `state_changes` upgrade-сценария), но `spec.input` остаётся историческим слепком create и не переписывается.
...
4. **Анти-version-craft инвариант** ([`api/huma_incarnation_formprefill.go:23`](../../keeper/internal/api/huma_incarnation_formprefill.go)) upgrade-paths НЕ нарушает (инвариант про create/операционные формы, всегда на `inc.ServiceVersion`), но ADR должен явно зафиксировать: upgrade-paths + действие upgrade — **единственный** легальный контур смены версии; выбор версии на create по-прежнему запрещён.
...
- **upgrade-сценарии в общем `scenario/`** с дискриминатором — пугают оператора в списках операционных сценариев, ломают удобство ретрая/разделения (§3).
