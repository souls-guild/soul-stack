...
- **Без `?to=` — дёшево**: перечисление тегов реестра сервиса (реюз `ls-remote`, тот же источник, что `GET /v1/services/{name}/refs` → `ListRefsTyped`, [`api/handlers/service.go:430`](../../keeper/internal/api/handlers/service.go)), отфильтрованное/аннотированное против текущего пина этой инкарнации: forward / downgrade / no-op.
- **С `?to=<ref>` — on-demand анализ конкретной цели**: есть ли upgrade-сценарий для `inc.ServiceVersion→to` (ветвь found vs legacy §5) и какие state-миграции [ADR-019](0019-state-migration-dsl.md) применятся (реюз `LoadMigrationChain` из `PrepareUpgrade`). Дорогой per-target анализ не гоняется на весь список — только по запросу.
- UI рисует выпадашку «на что и как могу обновиться» (метка found/legacy у каждой цели).
