...
- **Context.** Currently the RBAC model is split between two stores:
  - **Archons** live in Postgres (the `operators` registry, [ADR-014](0014-operator-identity.md#adr-014-operator-identity-model-archon)).
...
- **Decision.**
  ...
  **(a) Storage — three PG tables with the `rbac_*` prefix.**
  ...
  | **`rbac_role_permissions`** | Role permissions. `role_name` FK→`rbac_roles(name)` `ON DELETE CASCADE`, `permission TEXT` (stored as a **RAW string**, parsed by the existing `ParsePermission` — [rbac.md → Permissions format](../keeper/rbac.md#format-permissions)), PK `(role_name, permission)`. |
  ...
  **(e) API/MCP permissions — four role operations + a membership pair.** RBAC becomes manageable through OpenAPI/MCP (like Archons in [ADR-014(d)](0014-operator-identity.md#adr-014-operator-identity-model-archon)), not by editing YAML:
  ...
  The names are fixed by the user (propose-and-wait passed), added to [naming-rules.md](../naming-rules.md) and the catalog [rbac.md → Permissions catalog](../keeper/rbac.md#permissions-directory).

  **(f) Self-lockout invariant extended to the new paths.** The invariant [ADR-013(c)](0013-bootstrap-archon.md#adr-013-bootstrap-the-first-archon) / [rbac.md](../keeper/rbac.md) "the cluster cannot be left without an active Archon holding an effective `*`" is now checked on three new paths:
...
- **Amendment (2026-06-09, Synod — a group of Archons [ADR-049](0049-synod.md#adr-049-synod--a-group-of-archons)).** Between "operator" and "role" an intermediate level is introduced — the **[Synod](0049-synod.md#adr-049-synod--a-group-of-archons)** (a group of Archons bundling roles). RBAC-storage is extended with **three PG tables** by the same `rbac_*` pattern:
...
- **Amendment (2026-07-24, derived roles [ADR-078](0078-rbac-derived-roles.md)).** `rbac_roles` gains **`parent_role`** (TEXT NULL, self-FK → `rbac_roles(name)`, `ON DELETE RESTRICT`, [migration 102](../../keeper/migrations/102_rbac_roles_parent_role.up.sql)): the role a **derived** role inherits from. NULL = a plain role — every existing row, semantics unchanged. Deleting a role that is still someone's parent is **refused** (`ErrRoleHasChildren`), rather than orphaning the children: clearing the parent would drop the parent's narrowing from the child and thereby **widen** it. Cycles, self-parent and chains deeper than 4 roles are refused by the schema (a CHECK + the `rbac_roles_parent_chain_guard` trigger) and re-checked when the snapshot is built — a broken graph builds **no** enforcer, so (d)'s degradation path (keep the previous snapshot, warn) applies to it exactly as it does to an unparseable permission. The snapshot build of (d) additionally carries `parent_role` into each `Role`; resolving a chain into effective rights is NIM-180.
