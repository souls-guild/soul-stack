...
3. **Мультиинкарнационность (locked).** Один хост (один SID) может входить в **несколько инкарнаций одновременно** — это допустимо и уже поддержано (`souls.coven[]` — массив; хост A может нести и `service-haproxy`, и `service-redis`, обе инкарнации содержат A). Voice привязан к **тройке `(incarnation_name, choir_name, sid)`**, поэтому один SID легально является Voice в Choir-ах **разных** инкарнаций одновременно. **Инвариант:** Voice создаётся только для SID, который **уже член этой инкарнации** (его `souls.coven[]` содержит `incarnation.name`) — Voice не подменяет membership, а уточняет позицию внутри него.
