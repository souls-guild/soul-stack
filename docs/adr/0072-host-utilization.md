...
- **Amendment 2026-07-28 (NIM-248, revised 2026-08-05 by NIM-281) — the config a host is owed follows MEMBERSHIP; its coven overlays are the INCARNATION's own labels.** Delivery (`grpc.telemetrySource.ResolveForSID`) resolves the host's incarnation through **`incarnation_membership`**, then builds that incarnation's essence — whose coven overlay stage selects on `incarnation.covens`, the labels of the incarnation object itself ([ADR-0082 §service vars](0082-service-vars.md)). Two questions, two sources, on purpose. Neither one reads the host's `souls.coven[]`: the original wording said the overlays followed the host's *effective* labels, which was true only while [ADR-080](0080-label-inheritance-union.md) made those two sets the same thing. [NIM-281](0008-coven-stable-tags.md#amendment-2026-08-05-nim-281-a-label-is-never-inherited) removed inheritance, and the axis stayed where it always physically was — on the incarnation's own row.
  ...
  **Membership must not be answered from a host's tags**, here as anywhere: `incName ∈ souls.coven` is true for a host merely tagged with a string spelling the incarnation's name, and serving that host the incarnation's service config is not what the operator bound. Since NIM-281 the substitution fails in the other direction too — a bind attaches no tag, so a genuine member nobody tagged would be served nothing. The relation is the only place this answer lives. The overlay half is a different question entirely: `incarnation.covens` selects which layers of **the incarnation's own** service config are stacked, and it never looked at a host.
