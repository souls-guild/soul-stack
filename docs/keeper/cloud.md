...
> **Membership binding of a new host is a separate scenario step.** `core.cloud.provisioned` creates VMs and registers them in `souls`, but **does not bind them to the incarnation by itself**: setting incarnation **membership** (and any real stable coven tags) is a separate scenario step through the core module `core.soul.registered` ([`docs/keeper/modules.md`](modules.md)), which sets membership implicitly from the run's incarnation. The separation is deliberate: cloud-create and the membership bind are different registry operations, each with its own guard-rails, and they are assembled independently in the script.

The next steps of the script target the members of this incarnation (the omitted `on:` = all member hosts) and set the destiny to the newly created hosts.
