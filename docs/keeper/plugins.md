...
| `modules[].side` | `enum{soul,keeper}` | `soul` | Which half of the platform executes this module (NIM-747). **A property of the module, not of the task**: `core.state` can only be applied against the incarnation row and `core.pkg` only on a host, so the module declares the side once and every task addressing it inherits it — which is what let `on:` in a scenario go back to its single meaning, "which covens" ([orchestration.md §3](../scenario/orchestration.md#the-side-is-the-modules-not-the-tasks)). Omitted means `soul`: a module that declares nothing runs where modules have always run, so no artifact written before this key changes behaviour. An unrecognised value is `module_side_invalid` rather than the default — `side: Keeper` quietly meaning "runs on every host" is the failure the check exists for. **A keeper-side plugin is not executable yet** (NIM-688), so `keeper` here is a recorded intent the engine does not route by; such a scenario still writes `on: keeper` on the task, which stays legal on a plugin address for exactly that reason. |
...
| `modules[].side` | `enum{keeper,soul}` | `soul` | **[ADR-0087](../adr/0087-task-side-derived-from-module-address.md) — accepted, NOT implemented. ⛔ Do not write this key yet: a document carrying `side:` today FAILS TO PARSE.** Decoding is strict (`dec.DisallowUnknownFields()`, [`sdk/schema/canonical.go:62`](../../sdk/schema/canonical.go)), and `soul` reads the document at install time, so stamping `side:` into an artifact now makes it unloadable rather than merely unrecognised. It becomes writable only after the NIM-749 / NIM-750 rollout, and in that order — **souls first, then keeper, then re-stamp** — because an older `soul` is exactly the reader that refuses it. Once live: where this module runs. **The absent key and `side: soul` are indistinguishable by design, permanently**: documents are signed and stored, so requiring the key — or later stamping the default into stored documents — would change every artifact's sha256 and invalidate every approval at once. ⚠ **Accepted and inert for plugins** until NIM-688 lands a keeper-side plugin executor: a `side: keeper` plugin step still fails `unknown keeper-side module`, **loudly, never as a silent skip**. Not to be confused with its neighbour — **`side` is where the module runs; `side_effects` is what it touches.** |
