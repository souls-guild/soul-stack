...
| Grant lookup | By registration alias, on the SPAWN's context. Two failures, both fail-closed and deliberately distinct: no grant for the alias → `no_sigil`, whose hint tells the operator to issue one; the registry could not be READ (the Keeper's Postgres is down, the run was cancelled) → `lookup_unavailable`, which says the answer is UNKNOWN and must not send anyone to issue an approval over an outage. The underlying error stays reachable through the returned error, so a cancelled run is still recognisable as one. |
...
| Re-exec from cache | The sealed sidecar is compared against the digest computed for step (2) of this same spawn — one pass over the artifact, not two. Defense-in-depth for the shared cache; the control that decides whether the binary runs is step (2) against the grant, and the `exec` follows both, so a second read of the file could only ever observe bytes that are about to run anyway. Discrepancy → failure, the binary does not start. |
...
2. shallow `clone` (`Depth=1`) working clone in `<work_root>/<name>/` (STRICTLY outside `cache_root`), or `fetch` if there is already a clone. Transport - **go-git** (pure-Go, without system fork `git`); auth - a matching `keeper.yml::git.credentials[]` entry if the source's host has one ([config.md → `git`](config.md#git), NIM-898), else the SSH agent for ssh/scp forms and nothing for `https://`.
