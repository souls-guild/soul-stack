...
| `incarnation` | Runtime-инстансы сервисов: `spec`, `state`, `status`, `service_version`, `state_schema_version`. **Applying-epoch** колонки (миграция 082, [ADR-027(m)](../adr/0027-apply-work-queue.md#adr-027-модель-исполнения-apply--work-queue--claim-acolyte-пул-ward-claim)) — все NULLABLE, превращают голый `status='applying'`-флаг в inline-epoch владельца прогона, по которому Reaper-правило `reconcile_orphan_applying` отличает живой прогон от осиротевшего lock-а: `applying_apply_id` (`apply_id` прогона, держащего lock), `applying_attempt` (fencing-epoch прогона, parity `apply_runs.attempt`), `applying_by_kid` (KID Keeper-владельца — presence-чек этого KID в Conclave через `InstanceAlive` различает «прогон идёт» / «владелец мёртв»), `applying_since` (момент взятия lock — правило ищет stale-кандидатов по age `applying_since < NOW()-stale_after`). Все четыре пишутся ОДНИМ UPDATE атомарно со `status='applying'` в `lockRun` (окна applying-без-epoch нет); очищаются на терминале прогона. NULL-epoch (legacy/pre-082 строки) правило НЕ реклеймит (fail-safe). Индекс `incarnation_applying_scan_idx` (partial, `ON (status, applying_since) WHERE status='applying'`) держит Reaper-scan stale-кандидатов узким. Управляются через API/MCP. | [architecture.md → Incarnation](../architecture.md#incarnation--runtime-инстанс-сервиса), [reaper.md → `reconcile_orphan_applying`](reaper.md#правила). |
