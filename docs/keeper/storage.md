...
**Every registry row carries a display caption** ([ADR-0085](../adr/0085-entity-id-and-label.md), migration `117`): a nullable, non-unique `label TEXT` with **no CHECK on its form**, on all ten entities of the eight registries that share a name grammar — `incarnation`, `service_registry`, `providers`, `profiles`, `push_providers`, `omens`, `heralds`, `tidings`, `vigils`, `decrees`. It is free text with capitals and spaces, set at registration and changed afterwards through `PUT /v1/<collection>/{name}/label`; NULL means the consumer shows the identifier instead.
