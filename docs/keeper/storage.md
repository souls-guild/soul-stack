...
| `service_registry` | Registry of Services (service type → git coordinates): `id` (PK, kebab-case, immutable — [ADR-0085](../adr/0085-entity-id-and-label.md), migration `118`), `git`, `ref` ([ADR-007](../adr/0007-versioning-git-ref.md): version = git ref), `refresh` (optional auto-fetch duration), operator audit (`created_by_aid` / `updated_by_aid` FK to `operators(aid)`). Moved out of `keeper.yml → services:` ([ADR-029](../adr/0029-service-registry.md)); managed via the `service.*` API/MCP ([operator-api.md](operator-api.md)). The runtime reads an in-memory snapshot (`serviceregistry.Holder`, TTL-poll + pub/sub invalidation `service:invalidate`). | [architecture.md → ADR-029](../adr/0029-service-registry.md), [config.md](config.md). |
...
**Every registry row carries a display caption** ([ADR-0085](../adr/0085-entity-id-and-label.md), migration `117`): a nullable, non-unique `label TEXT` with **no CHECK on its form**, on all ten entities of the eight registries that share a name grammar — `incarnation`, `service_registry`, `providers`, `profiles`, `push_providers`, `omens`, `heralds`, `tidings`, `vigils`, `decrees`. It is free text with capitals and spaces, set at registration and changed afterwards through `PUT /v1/<collection>/{id}/label`; NULL means the consumer shows the identifier instead. **The identifier column itself is `id`, not `name`** ([ADR-0085](../adr/0085-entity-id-and-label.md), migration `118`) — the rename lands registry by registry, so read migration `118` for which of the ten have converted; the MCP argument and the `<resource>.label_changed` audit key moved for all ten at once, because both are stated in one shared place.

The column deliberately takes none of the identifier's constraints, and **carries no index**, because an index would be evidence that something looks a row up by it and nothing may: `label` participates in nothing derived — not a Vault path segment, not an RBAC scope value, not a snapshot directory, not a CEL root. That is the whole reason the field is split off the identifier, which keeps `TEXT PRIMARY KEY`, its kebab CHECK, and every derivation.
...
**The cancel semantics** are identical to a local Cancel: the incarnation goes into `error_locked` (`status_details.reason = dispatch_failed`), the state does not change — the operator removes the lock via `POST /v1/incarnations/{id}/unlock` after analyzing the consequences ([operator-api/incarnations.md → unlock](operator-api/incarnations.md)). To Souls already sent to, Keeper in the pilot does not send `CancelApply` (best-effort cancel — a separate layer via `Outbound.SendCancel`).
