...
|---|---|---|
| `type` (URI suffix under `https://soul-stack.com/errors/`) | `code` | Take suffix URN: `https://soul-stack.com/errors/incarnation-locked` → `code: "incarnation-locked"`. |
...
- **Subscription RBAC** (`authorizeSSE`, [`keeper/internal/mcp/sse.go`](https://github.com/souls-guild/soul-stack/blob/main/keeper/internal/mcp/sse.go)): a **non-revoked** Archon who is either the run's initiator (`apply_runs.started_by_aid == JWT.sub`) OR holds `incarnation.get` on the run's incarnation. Revocation is asked first and applies to the initiator too — the initiator branch admits a caller holding no permission at all, so it is the only place on this listener that would ever consult the revoked projection. A nonexistent or foreign `apply_id` answers the same **403** as a denial (anti-enum: ULIDs are guessable). The M0.7.c note that "any authorized Archon can subscribe" was superseded when this check landed.
- **The decision is re-asked every 15 s, not taken once (NIM-858).** A stream runs for up to `sseMaxLifetime` (30 min), and this listener carries no `RejectRevoked` link at all (NIM-551) — so a revoked Archon's open stream had nothing anywhere that would ever refuse it. `buildSSEHandler` re-evaluates the same rule on its own ticker ([`middleware.ReauthTicker`](https://github.com/souls-guild/soul-stack/blob/main/keeper/internal/api/middleware/longlived.go), shared with the console socket and the `/v1` run-events stream) and ends the body when the answer changes: revocation first — the token is no longer trusted, so being the initiator stops earning anything — then `incarnation.get`, which is also what catches a *narrowing* short of revocation. What cannot change is not re-read: the run's incarnation and its initiator are fixed once the row exists, so the re-check costs no database round trip. The stream ends **without a closing frame**: the event kinds here are the bus's, and the client learns the way it does after any disconnect — it re-opens and is answered 403.
