...
Output `created` (`register.<name>.*`): `hosts[]` (`sid` / `vm_id` / `primary_ip` / `attributes`; in B-flat additionally `bootstrap_token` - plain, the only point of visibility, masked by `audit.MaskSecrets`), `count`, `vm_ids`, `action`, `reused`. With `self_onboard: true` - plus the sign `self_onboard: true` and **without** `bootstrap_token`.

**Re-run (idempotency, [ADR-017 amendment 2026-07-24](../adr/0017-keeper-side-core.md)).** Registering a VM in `souls` tolerates a repeated `create` over the same incarnation: a row left by an earlier attempt (`pending` / `destroyed`, belonging to this incarnation or to none yet) is re-armed for onboarding instead of colliding with the PK, and its still-active bootstrap token is replaced. `reused` counts the taken-over records. A row that carries a LIVE registration (`connected` / `disconnected` / `revoked` / `expired`), or one that belongs only to other incarnations, is **refused** - the step fails with the status and the owning incarnations rather than taking the host over. Full table - [cloud.md → Re-running create](cloud.md#re-running-create-provision-idempotency-nim-170). Params `destroyed` (`provider` / `vm_ids` / `sids` + cascade semantics) - [per-module README](../module/core/cloud/README.md) and [cloud.md](cloud.md).
