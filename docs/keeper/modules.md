...
| **omitted** (keeper-side module address) | on the keeper itself | Keeper-side core (`core.soul.registered`, `core.bootstrap.issued` - tokens for ready-made VMs, ...) |

**The side is the module's, and a task does not restate it** (NIM-747). The seven keeper-side core base addresses - `core.bootstrap` / `core.cert` / `core.choir` / `core.soul` / `core.ssh` / `core.state` / `core.vault` - are disjoint from the twenty-one Soul-side ones, so the address alone routes the step. Read the list from `coremanifest.KeeperSideAddrs()` rather than from here: an address the catalog does not know does not error, it routes Soul-side (NIM-863). `on: keeper` on one of them is a validation error (`on_keeper_redundant`), and a Soul-side address is a host task however it is written. The catalog is one list, `shared/coremanifest`, read by both `soul-lint` and the render pipeline; a plugin declares its own side in its schema document (`side: keeper | soul`, default `soul`), and since NIM-758 the Keeper **routes by it** - see [Keeper-side plugin modules](#keeper-side-plugin-modules) below. `on: keeper` on a PLUGIN address stays legal regardless: a scenario cannot read the artifact's document, so unlike a core address it has nothing to derive the side from.
...
Keeper-side core modules are registered in the keeper-side Registry (`keeper/internal/coremod/registry.go`) by **base name** - `<namespace>.<module>` without state suffix: `core.soul`, `core.bootstrap`, `core.cert`, `core.choir`, `core.ssh`, `core.state`, `core.vault`. State comes from the last segment of the task address.
...
| `core.bootstrap.issued` | `core.bootstrap` | `issued` |
...
- ⚠ **a secret carried by a register the seal did not mark.** A `vars:`/`compute:` hop IS covered since NIM-811 — `params: { pw: "${ vars.db_pw }" }` with `vars.db_pw` = `${ vault(…) }` is sealed, and so is a destiny's own `${ input.<secret> }` (NIM-812) — but `${ register.<n>.<f> }` is sealed only when `<n>` was itself marked: a `vault:` payload resolved at the [ADR-0083](../adr/0083-declared-secret-state-fields.md) §6 boundary, or a module output declared `secret: true` (§8). A credential a `core.exec.run` scraped into an ordinary register is indistinguishable from its other stdout;
- a **plaintext literal the author typed**, or an operator-set `soulprint.self.traits` value. The render never saw it as a secret and it is not distinguishable from any other text — the same bound as the first bullet, restated because a trait *looks* like structured data the platform understands;
- ⚠ **`${ incarnation.state.<field> }` — open, NIM-826.** The two state markers are not the same ([ADR-0083](../adr/0083-declared-secret-state-fields.md) §1): `type: secret` keeps the value in Vault so a read takes home a `vault:` ref the vault-origin layer masks, but `secret: true` on a state property means the value *lives in state* and is masked only on the way out — so a cell reading it renders plaintext and the seal does not mark it. Unlike the hops NIM-811/812/822/823 closed, this one is not a binding the scenario created;
...
Input `sids` is a required, non-empty, unique list of canonical SID/FQDN values. The whole batch is a single Postgres transaction. A free SID becomes a `pending`, `transport=agent` Soul; an existing `pending`/`expired` agent Soul is re-armed and gets a fresh token. `revoked`, `destroyed`, and `transport=ssh` are refused fail-closed. A failure identifies the SID and rolls back the complete batch.

Input `reissue` (bool, optional, default **`false`**) decides what happens to a host that already holds an **active** token — `used_at IS NULL AND expires_at > NOW()`, the predicate `Burn` authorizes on (NIM-900). `true` invalidates it and issues a new one, which is what the step always did. `false` leaves it — no token write, no `requested_at` refresh, no recovery re-mark — and reports the host as `{sid, token_held: true}`. **The default is off because the destructive direction is unrecoverable:** the plaintext dies the moment it is issued (Postgres keeps only the SHA-256), so a step that reissues unasked destroys a capability that may already be on the machine, and a further run cannot return the old one. An *expired* unused token is deliberately not "active" — nothing can redeem it, so issuance proceeds under either value. `reissue` does **not** reach the identity arm: a host with an active seed is converged over or refused exactly as before under either value, which is why the parameter is not named `force` like [`?force=true` on `POST /v1/souls/{sid}/issue-token`](operator-api/souls.md#post-v1soulssidissue-token--reissue-bootstrap-token) — the same operation, but that endpoint has no second guard to over-promise about. Re-registering a host that holds an identity is `soul.forget`, not a flag here.
...
Output `register.<name>.hosts[] = {sid, bootstrap_token, expires_at, created, reissued}` plus `count` / `created` / `reissued` / `skipped` / `held` / `action: issued`. ★ **An entry has THREE possible shapes, and two carry no `bootstrap_token` key at all:** the issued one above, `{sid, onboarded: true}` for a host that already holds an identity (NIM-780), and `{sid, token_held: true}` for a host whose active token `reissue: false` left alone (NIM-900). Every requested host keeps its slot, in order — the list answers for all of them and a flag carries WHY one has no token; `skipped` counts the converged, `held` the kept. **Anything downstream that re-maps `hosts` must branch on BOTH flags before reaching for the token** — on either tokenless entry the `bootstrap_token` key is ABSENT, not empty, and reading an absent key in CEL is an error, so an expression written for the happy path fails the step on exactly the repeat run. A `token_held` entry reaching [`core.ssh.run`](#coresshrun) is refused there by name, before the connect, and that is the honest outcome: there is no plaintext to deliver, and skipping the host would report success over a machine that never onboards — a repeat path that must deliver a token wants `reissue: true`. **That guarantee is `core.ssh.run`'s, not the flag's**: a site's own keeper-side install module gets no such refusal for free and must branch on `token_held` itself. A repeat over an SID whose previous unused token is EXPIRED returns new plaintext under either value. The plaintext exists only in the current run register for per-host delivery; Postgres stores only SHA-256. The `bootstrap_token` key is masked on audit/OTel/SSE/log surfaces and must not be projected to `incarnation.state`. Audit `bootstrap.issued` carries only `{action,count,created,reissued,skipped,held,sids}`, converged and held hosts included in `sids`.

`changed` is true exactly when the step issued or reissued at least one token. It was pinned to the constant `true` until NIM-900, so a repeat over a fleet that was entirely up — nothing issued, every host passed through — reported itself as having changed something and fired every `onchanges:` behind it.

Ready-made VM chain: `core.bootstrap.issued` (`reissue: true` when the install step must get a token on every run) → [`core.ssh.run`](#coresshrun) (the site's own install commands) → `core.soul.registered` (`await_online: true`, normally `refresh_soulprint: true`). The middle step carries the transport and not the policy: what gets installed is the service author's shell, and a run whose commands never redeem the token still blocks at the barrier until the run timeout.
...
**`stdin_from:` names a FIELD of the host entry, and `${ host.* }` does not exist.** A keeper-side task renders ONCE for the whole run, not once per host — `keeperVars` binds no roster — and `loop:` is not supported on a keeper-side task, so a per-host value has no expression that could reach a params cell. Naming the field instead keeps the per-host secret out of the task's params entirely. `stdin:` is the other source, for a value that is the same on every host (`${ vault(…) }`, `${ vars.* }`); declaring both on one step is a refusal, not a precedence rule.
...
A host entry flagged **`onboarded: true` is skipped, not dialed** — it carries no per-host material at all (NIM-189, NIM-780), and the `primary_ip` requirement of the direct transport is settled AFTER the flag. The skip lives inside the module on purpose: `when:` on a keeper-side task is only half-evaluated (a static predicate works, one reading `register.*` is silently ignored), so a scenario cannot do it. **`onboarded` is the only such exemption**, and deliberately: a `{sid, token_held: true}` entry from `core.bootstrap.issued` (NIM-900) also carries no `bootstrap_token`, and the step **refuses it by name before the connect** rather than skipping the host — that host still needs installing and the plaintext it holds cannot be re-read, so silence would report success over a machine that never onboards. The refusal is explicit because what used to happen was decided by two things and **neither was the transport**: whether a dial address was available (`direct` needs `primary_ip`, `teleport` never does) and whether any step read the token through `stdin_from`. Without an address it was refused during param parsing — the one clean stop, reachable only on `direct`. With an address and a `stdin_from` step it **dialed the host and executed every step before that one** (the dial is outside the step loop, the stdin source resolved inside it; in the wb-redis list `stdin_from` is step 6 of 8). With an address and no `stdin_from` step nothing stopped it at all: everything ran and the step reported **success**, on either transport. So the old behaviour was a partial install and a false success, not a late failure. The mint is where the remedy lives, with `reissue: true`.

Parameters: `hosts` (required), `steps` (required), `ssh_provider` (required), `ssh_user` (default `root`), `ssh_port` (default `22`), `join_wait_timeout` (default `15m` — the bounded wait for a host to become reachable, on **both** transports since NIM-872: a fresh VM joins Teleport minutes after it is created, and on `direct` its sshd starts listening well after the DHCP lease the cloud calls readiness. `teleport` retries every dial error; `direct` retries only a failure to establish the TCP connection — a rejected handshake, an `Authorize` deny and a non-zero exit are not waits and fail at once). The **transport** is `keeper.yml::push.transport`, not a param: `direct` dials `primary_ip` with `Authorize` → ephemeral ed25519 keypair + `Sign` → CA-signed host-cert verify against the Vault host CA; `teleport` dials the SID by name and takes transport, user-auth and host-verify from the identity file. An empty host-CA set is an error, never a blind connect.
...
The delivery state is **gone**. It did two unrelated things — put the Soul binary on the host and hand the host its token — and installation has no portable form: every platform installs differently (a prepared image, a cloud-init cycle, its own shell, a foreign fleet manager). Installing a host is the site's own job now.

**What it guaranteed did not go with it.** Three requirements, each paid for by a live run and **none reproduced by writing a file with the token in it**, now bind whoever installs the host — see the [ADR-063 amendment 2026-09-09](../adr/0063-bootstrap-token-delivery.md#amendment-2026-09-09--delivered-is-removed-installing-a-host-is-site-specific-nim-834) for all of them and for what else came free (fail-closed `Authorize`, an ephemeral keypair that never leaves the Keeper, CA-signed host-cert verify, no token in the step output, B1-strict, a bounded wait for the host to become reachable):

1. **Redeem the token, do not merely place it.** There is no soul-side pickup of a token file; the seed is created ONLY by `soul init`, behind the seed-cert guard `test -e /var/lib/soul-stack/seed/current/cert.pem` — a bootstrap token is single-use, so an unguarded retry fails the host on the second run.
2. **The token travels in STDIN, never in argv** — argv is visible in `ps`, `audit.log` and journald **on the host itself**. The redeem carries the literal unexpanded `$(cat <token_path>)`, expanded by the subshell on the host.
...
- **One step per alias, and the alias is what a consumer is matched on.** A task addressing `redis.sentinel.present` is a consumer of the row `redis` exactly as `redis.instance.configured` is — one slot, one binary, and which object the address names decides nothing about installing it. Several deprecated entries of one artifact (`redis.instance`, `redis.sentinel`) therefore install once, before the **earliest** consumer of the alias, which is the same step the canonical one-row form produces. `service.yml` validation requires such entries to agree on `ref` (`conflicting_module_ref`), a check that goes away with the form it guards. Before NIM-829 the match was against the whole two-level entry, so a plan calling an object the manifest had not enumerated got no install at all, or one placed after the consumer that needed it.
