...
| `traits` | `object` | optional | Operator-set key-value incarnation trait marks ([ADR-060](../../adr/0060-traits.md) R1 slice a): key → value `scalar` OR `list of scalars` (`{"owner": "alice", "owners": ["alice", "bob"]}`). Placed in `incarnation.traits`; member hosts INHERIT them at read time ([ADR-080](../../adr/0080-label-inheritance-union.md)), including hosts that join the incarnation later — nothing is written to any host row. Nested object/array-within-array → `422`. Default is `{}` (no labels). Day-2 replacement - `PUT /v1/incarnations/{name}/traits`. |
...
Integrity **replaces** operator-set trait incarnation tags (`incarnation.traits` jsonb - source of truth, [ADR-060](../../adr/0060-traits.md) R1 slice a). Trait - organizational label of the owner/product/namespace of the entire instance (`owner=alice`, `product=aboba`, `namespace=dba-ns`), **separate axis next to the flat Coven** ([ADR-008](../../adr/0008-coven-stable-tags.md)): Coven - stable targeting/RBAC tags, Trait - key-value attributes (incarnation **membership** is yet another axis, the relation `incarnation_membership`, [ADR-008 amendment 2026-07-17](../../adr/0008-coven-stable-tags.md#amendment-2026-07-17-nim-124-incarnationname-is-not-a-coven--membership-is-a-first-class-relation)). Atomicity - one PG transaction (`SELECT FOR UPDATE` → `UPDATE traits`); there is no status gate (it is safe to change labels at any status). The write ends there: member hosts (the `incarnation_membership` relation) **inherit** the new set on their next read ([ADR-080](../../adr/0080-label-inheritance-union.md)), so a removed key stops granting immediately and a label an operator attached to a host is never overwritten.

The per-host counterpart is `POST /v1/souls/traits` (first-class, see [Soul → bulk traits](souls.md)): this route labels the INSTANCE, that one labels ONE VM, and a host's effective traits are the union of both. Prefer this route — it covers hosts that join the incarnation later, with no re-stamping.
