...
- **`POST …/choirs/{choir}/voices`** (`choir.add-voice`): Adds Voice (SID Choir membership). **Membership invariant:** Voice is created only for a SID that is already a member of the incarnation (a row in `incarnation_membership`, [ADR-008 amendment 2026-07-17](../../adr/0008-coven-stable-tags.md#amendment-2026-07-17-nim-124-incarnationname-is-not-a-coven--membership-is-a-first-class-relation), no longer `souls.coven[]` contains `incarnation.name`); SIDs outside of membership → `422` (violating SIDs in `detail`). `role`/`position` - opt. declared attributes; `role` is the **declared role** of the host and, since [ADR-044 amendment 2026-07-30](../../adr/0044-choir.md#amendment-2026-07-30-nim-330-spechosts-is-removed-voice-is-the-only-source-of-a-declared-role) (NIM-330), its only home — the former `incarnation.spec.hosts[].role` and its `PATCH .../hosts` endpoint are gone. Omitted `role` is stored as SQL `NULL` and resolves to "no declared role", NOT to a default group. Response `201 Voice`.
