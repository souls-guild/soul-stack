...
| [console.md](console.md) | **Interactive console plane**: the WebSocket `/v1/console`, the session manager mapping operator sockets onto the Keeper↔Soul EventStream, backpressure, kill-on-disconnect, cross-Keeper routing and the operator-side session caps. Soul half — [soul/console.md](../soul/console.md). |
...
| [plugins.md](plugins.md) | Keeper plugin infrastructure: a `side: keeper` SoulModule and the `SshProvider` contract (`CloudDriver` was removed in NIM-761), plugin directory in `keeper.yml`. |
| [modules.md](modules.md) | Keeper-side core modules: `core.soul.registered` specification (SID binding to souls registry coven tags), the Soul-side / Keeper-side split, which is derived from the module ADDRESS since NIM-747, not written in `on:`. |
