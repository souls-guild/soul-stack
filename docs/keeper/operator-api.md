...
| `validation-failed` | 422 | Semantic validation error: incarnation input does not match scenario `input:`-scheme ([destiny/input](../input.md)); `create_scenario` in `POST /v1/incarnations` is empty if there are create scenarios (`detail` starts with `create_scenario_required:` + list of valid ones) or points to a scenario outside the create set (`create_scenario_invalid:`, see [operator-api/incarnations.md → Selecting a starting scenario](operator-api/incarnations.md)); profile `params` does not match CloudDriver `profile_schema` (cloud.md); request `issue-token` for Soul with `transport: ssh` (ssh host does not have bootstrap phase - `POST /v1/souls/{sid}/issue-token`). `detail` - path to a specific field or reason. |
