...
### Incarnation (12) — жизненный цикл runtime-инстансов, [ADR-009](../adr/0009-scenario-dsl.md#adr-009-scenario--полная-dsl-задач-destiny-граница-с-destiny--рекомендация)
...
>
> - **Incarnation** — REST «(12)» (router) vs MCP «(11)» (`manifest.go`). Разница НЕ ошибка: `PATCH /v1/incarnations/{name}/hosts` — REST-only (MCP-tool `keeper.incarnation.hosts.update` в `manifest.go` отсутствует), остальные 11 REST-роутов (включая `PUT .../traits` ↔ `keeper.incarnation.traits-set`, ADR-060) имеют MCP-парность. Заголовок mcp-tools.md «(11)» считает MCP-tools, разбор — в [mcp-tools/incarnations.md](mcp-tools/incarnations.md).
