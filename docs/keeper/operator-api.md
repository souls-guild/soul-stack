...
| `validation-failed` | 422 | Семантическая ошибка валидации: incarnation input не матчит scenario `input:`-схему ([destiny/input](../input.md)); profile `params` не матчит CloudDriver `profile_schema` ([cloud.md](cloud.md)); запрос `issue-token` для Soul с `transport: ssh` (ssh-хост не имеет bootstrap-фазы — `POST /v1/souls/{sid}/issue-token`). `detail` — путь до конкретного поля или причина. |
| `malformed-request` | 400 | Синтаксис JSON / неверные query params. |
| `incarnation-locked` | 409 | Incarnation в `error_locked` — нужен `POST /v1/incarnations/{name}/unlock` перед новым прогоном ([architecture.md → Атомарность и `error_locked`](../architecture.md#атомарность-и-error_locked)). | `POST /v1/incarnations/{name}/scenarios/{scenario}`, `DELETE /v1/incarnations/{name}`, `POST /v1/incarnations/{name}/upgrade`. |
...
### Incarnation (11) — жизненный цикл runtime-инстансов, [ADR-009](../adr/0009-scenario-dsl.md#adr-009-scenario--полная-dsl-задач-destiny-граница-с-destiny--рекомендация)

| Method | Path | Permission | MCP-tool |
|---|---|---|---|
| `POST` | `/v1/incarnations` | `incarnation.create` | `keeper.incarnation.create` |
| `POST` | `/v1/incarnations/{name}/rerun-create` | `incarnation.create-rerun` | `keeper.incarnation.rerun-create` |
...
>
> - **Incarnation** — REST «(11)» (router) vs MCP «(10)» (`manifest.go`). Разница НЕ ошибка: `PATCH /v1/incarnations/{name}/hosts` — REST-only (MCP-tool `keeper.incarnation.hosts.update` в `manifest.go` отсутствует), остальные 10 REST-роутов имеют MCP-парность. Заголовок mcp-tools.md «(10)» считает MCP-tools (вкл. `rerun-create`), разбор — в [mcp-tools/incarnations.md](mcp-tools/incarnations.md).
...
Вынесены в доменный файл — [operator-api/incarnations.md → Endpoint-секции](operator-api/incarnations.md#endpoint-секции): `POST /v1/incarnations` (создать instance), `POST …/rerun-create` (перезапуск `create` из `error_locked`), `POST …/scenarios/{scenario}` (произвольный сценарий), `GET …/{name}` (spec+state+status), `GET /v1/incarnations` (список), `GET …/history` (журнал state), `POST …/unlock`, `POST …/upgrade`, `POST …/check-drift` (Scry), `DELETE …/{name}` (destroy), плюс два superseded-by-Voyage Tide-раздела (историческая запись). MCP-сторона — [mcp-tools/incarnations.md](mcp-tools/incarnations.md).
