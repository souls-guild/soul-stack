...
> **Selector reality check.** Exactly three routes are wrapped in `RequirePermission(..., SoulSIDSelector)`: `soul.forget`, `soul.issue-token`, `soul.ssh-target-update`. That selector puts **only** `host` into the RBAC context, and a missing dimension fails closed ([rbac.md](rbac.md)) - so narrowing any of the three by `coven=<label>` denies **every** call rather than restricting it to that coven. Narrow them by `host=`. This holds for **both** ways a coven attaches: the suffix (`soul.forget on coven=web`) and a **bare** `soul.forget` sitting in a role whose `default_scope` is `coven=web` — a role's `default_scope` is inherited by its bare permissions, so that one arrives coven-narrowed too and denies just the same. `soul.console` is not in this set: it is mounted behind `RequireAction` with the scope applied inside the handler, so a `coven=` there narrows as written. Making `coven=` narrow rather than deny on the three is NIM-588.
