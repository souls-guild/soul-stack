...
### Incarnation (17) — жизненный цикл runtime-инстансов, [ADR-009](../adr/0009-scenario-dsl.md#adr-009-scenario--полная-dsl-задач-destiny-граница-с-destiny--рекомендация)
...
`POST /v1/incarnations/{name}/secrets/reveal` + discovery `GET …/secrets/revealable` — раскрытие оператору plaintext-значения секрета инкарнации, объявленного `revealable_secrets` сервиса ([ADR-070](../adr/0070-secret-reveal-path.md), READ-двойник [ADR-064](../adr/0064-secret-write-path.md) write-path). `reveal` `{secret_id, key}` → `{value}`: резолвит `vault_ref` литеральной подстановкой `{service}`/`{incarnation}`/`{key}` (**не CEL**; `{service}`+`{incarnation}` обязательны в манифесте; `key` обязан ∈ enumerate-массива текущего `state`; `vault.ParseRef` — traversal-guard) и читает значение из Vault **только** если резолвнутый путь лёг под `secret/<service>/<incarnation>/` (позитивный prefix-allowlist — **главный guard**; system-floor `secret/keeper/`/`secret/internal/` — backstop), иначе `404` + audit `out_of_service_scope`. `revealable` → `{items:[{secret_id, label, state_path, keys}]}` — discovery для UI State-вьюхи (пустой список валиден). Permission `incarnation.view-secrets` (scope incarnation/coven/service по path-`name`, **строго привилегированнее `incarnation.get`**; вне scope → **404** fail-closed, parity Get). `reveal` пишет self-audit `incarnation.secret_revealed` — успех (`result:"ok"`) И denied-ветки (`result:"denied"`+`reason`), payload `{name, secret_id, key, path, result, reason}` **БЕЗ значения** (leak-guard-тесты); `revealable` — read, без audit. Санкционированное раскрытие → 200-DTO **мимо `MaskSecrets`**. MCP-tool-ов **нет** (REST-only, как form-prefill). Полная фиксация — [ADR-070](../adr/0070-secret-reveal-path.md).
...
`POST /v1/incarnations/{name}/scenarios/{scenario}/form-prefill` — pre-fill UI-формы сценария из `incarnation.state` (только объявленные в схеме `prefill_from_state`-пути, secret-исключение внутри; [input.md → prefill_from_state](../input.md)). Permission `incarnation.get` (reuse: кто читает инкарнацию, тот получает prefill её формы), per-`{name}` scope — in-handler, как у Get/History. Read-резолв, без audit, **REST-only**.
