...
**Only the response is masked - in Postgres `state`/`spec` are stored unchanged** (last known-good for apply / migrations / unlock should not depend on masking on read). This is the second layer of protection: the first is that a declared secret does not physically fall into `incarnation.state` - the state field holds a `vault:` ref and the value lives in Vault ([ADR-0083](../adr/0083-declared-secret-state-fields.md) §1, §6; the older mechanism, dropping a `no_log: true` probe's register from the state graph, went with the key - [scenario/orchestration.md §7.1](../scenario/orchestration.md)).
...
| `operator-revoked-token` | 401 | The token is well-formed and unexpired, but the Archon it names has been revoked (`operators.revoked_at`). Distinct from `unauthenticated` (nothing is wrong with the token) and from `forbidden` (no grant can fix it — the identity is gone; a client should log the operator out, not ask for permissions). **Since 2026-08-07 ([ADR-014 amendment, NIM-421](../adr/0014-operator-identity.md)) it is answered by `RejectRevoked`, one middleware link right after `RequireJWT`, so it reaches every authenticated route by construction** — previously only routes gated by `RequirePermission` could return it, existence-gate routes said `403 forbidden` and four ungated read routes kept answering 200. | Any authenticated endpoint: all of `/v1/*`, the two spec routes `/openapi.yaml` / `/openapi.json`, and `/auth/ldap/login` / `/auth/oidc/callback`. |
...
| `validation-failed` | 422 | Semantic validation error: incarnation input does not match scenario `input:`-scheme ([destiny/input](../input.md)); `create_scenario` in `POST /v1/incarnations` is empty if there are create scenarios (`detail` starts with `create_scenario_required:` + list of valid ones) or points to a scenario outside the create set (`create_scenario_invalid:`, see [operator-api/incarnations.md → Selecting a starting scenario](operator-api/incarnations.md)); ⛔ profile `params` does not match CloudDriver `profile_schema` — gone with the contract and the Profile registry in NIM-761 (`cloud.md` no longer exists); request `issue-token` for Soul with `transport: ssh` (ssh host does not have bootstrap phase - `POST /v1/souls/{sid}/issue-token`). `detail` - path to a specific field or reason. |
...
`PUT /v1/souls/{sid}/ssh-target` (`soul.ssh-target-update`, selector `host=<sid>` plus the host's covens - see the note below) updates per-host SSH push-flow details (`souls.ssh_target` jsonb: `ssh_port`/`ssh_user`/`soul_path`, [ADR-032](../adr/0032-push-orchestrator.md) amendment S7-1). 3-segment MCP-tool `keeper.soul.ssh-target.update` ↔ 2-segment permission `soul.ssh-target-update` (permission grammar is exactly `<resource>.<action>`). Audited (`soul.ssh_target_updated`).

`DELETE /v1/souls/{sid}` (`soul.forget`, selector `host=<sid>` plus the host's covens - see the note below) erases the host from the registry and releases what it held; legal in every status, including `connected` ([operator-api/souls.md § Forget](operator-api/souls.md), NIM-386). The only destructive call in this domain, and the only one that removes rows the operator did not name: `incarnation_membership` and `incarnation_choir_voices` go with the host through `ON DELETE CASCADE`, so the 200 body carries all four counts rather than a bare 204. If the cluster-wide teardown notice cannot be published the call answers `503` `teardown-unavailable` having deleted **nothing** - retry it. The other failure shape is not a retry: a `200` whose `warnings[]` is non-empty means the host **is** forgotten and something it held was not released, so a second `DELETE` answers `404` and re-releases nothing - the named resource has to be dealt with by hand. An empty `warnings[]` is the only shape that means fully released ([souls.md § Forget](operator-api/souls.md)). Audited (`soul.forgotten`) - and that event is the only durable record, since every row it describes is already gone. Unlike the read routes, the gate is a scope-aware `RequirePermissionMulti`, not an existence-gate.

> **Selector reality check.** Exactly three routes are wrapped in `RequirePermissionMulti(..., SoulSIDScopeSelector)`: `soul.forget`, `soul.issue-token`, `soul.ssh-target-update`. The selector reads the host's `souls.coven` list and offers the enforcer one context per coven plus the host itself, and the gate admits if any of them passes - so narrowing any of the three by `coven=<coven-tag>` restricts it to hosts carrying that label, in both forms a coven attaches: the suffix (`soul.forget on coven=web`) and a **bare** `soul.forget` in a role whose `default_scope` is `coven=web` (a role's `default_scope` is inherited by its bare permissions, and both forms meet at the same scope expression before the check). Before NIM-588 the context held **only** `host`, and a missing dimension fails closed ([rbac.md](rbac.md)), so those grants denied **every** call including one aimed at a host that really was in the coven. Two things have not moved: an `on host=` grant behaves exactly as before, and if the coven cannot be read - unknown host, database down - the context keeps the host alone, so coven-scoped grants fail closed and nothing widens. `errand.run` (`POST /v1/souls/{sid}/exec`) and the live `soul.console` are **not** in this set: both still build a host-only context, so a `coven=` on either still denies.
