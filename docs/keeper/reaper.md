...
- **`oracle_circuit`** (the Oracle circuit-breaker, [ADR-030(a)](../adr/0030-vigil-oracle.md)) — a per-decree fixed-window trip counter. There is **no** separate Reaper rule: the table self-cleans via the FK `decree → decrees(id) ON DELETE CASCADE` — a row lives exactly as long as the Decree lives, and goes away with it.
  - **Known-op: re-enable a failed (tripped) Decree (MVP)** = **delete + recreate**. The circuit-breaker auto-`disable`s a rule that spun into a loop (`decrees.enabled=false`); to bring it back into service with a **clean window**, the operator deletes the Decree (`DELETE /v1/decrees/{id}`) and creates it anew (`POST /v1/decrees`). The cascade on delete carries away the `oracle_circuit` row, so the recreated Decree starts with a `fire_count` from zero (rather than immediately at the threshold). A toggle endpoint (`enabled` without recreation) — a separate effort.
...
| `purge_old_console_recordings` | `console_recordings` (migration 104, [ADR-0074(g)](../adr/0074-interactive-console-pty.md)) | `delete` | `max_age` (+ `enabled` opt.) | `DELETE FROM console_recordings WHERE ttl_at < NOW()`. **Retention of the mandatory console session recordings.** Default TTL **90d** (`console_recordings.ttl_at = started_at + console.recording.retention`), baked into the row on INSERT by `consolepg.Store`; the index `console_recordings_ttl_idx` (migration 104) makes the condition cheap-scanable. Same shape as `purge_old_errands`: the rule's `max_age` is **not part** of the SQL predicate, because the TTL a recording was taken under is a property of that recording — a retention change must never be retroactive (lowering it must not silently delete recordings kept under the old policy). **Cascade:** `console_recording_parts` (the cast body) goes with it through `ON DELETE CASCADE`. **Why it matters more than a usual retention rule:** console recording is **mandatory** ([ADR-0074(g)](../adr/0074-interactive-console-pty.md)) — the table grows with every shell anybody opens and there is no operator action that stops it, so leaving this rule disabled is unbounded growth, not a tuning choice. |
