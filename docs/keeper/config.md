...
Подключение к Redis — горячему слою и шине координации ([ADR-006](../adr/0006-cache-redis.md#adr-006-кэш-и-координация--redis), [storage.md](storage.md)).

| Поле | Тип | Default | Смысл |
|---|---|---|---|
| `redis.addr` | `string(host:port)` | — | Адрес инстанса/кластера. Кластерная топология — на стороне Redis. |
| `redis.password_ref` | `vault-ref` | — | Vault-reference на пароль. |
...
| `vault.kv_mount` | `string` | `secret` | Mount point KV v2 (без trailing slash). Используется client-ом для чтения signing-key JWT и других secrets. |
...
Аутентификация операторов (Archon) для OpenAPI / MCP. Подблоки:

- `jwt` — внутренний JWT-issuer ([ADR-014](../adr/0014-operator-identity.md#adr-014-identity-модель-оператора-archon)), подпись и формат токенов (действующая часть; ниже).
- `ldap` — федеративная LDAP-аутентификация ([ADR-058](../adr/0058-operator-auth-ldap-oidc.md), стадия 1).
- `oidc` — федеративная OAuth2/OIDC-аутентификация ([ADR-058](../adr/0058-operator-auth-ldap-oidc.md), стадия 2, требует Redis).
- `rate_limit` — anti-bruteforce login-эндпоинтов ([ADR-058(g)](../adr/0058-operator-auth-ldap-oidc.md)).

Федеративные методы (`ldap`/`oidc`) валидируют внешний identity на Keeper-е, маппят его на строку `operators(aid)` + RBAC-роли и выпускают **внутренний** JWT — RBAC/MCP/middleware работают без изменений. Все три подблока опциональны: не заданы → способ логина недоступен, Keeper стартует. Реестр операторов и RBAC-каталог — в Postgres (`operators` и `rbac_*` соответственно, ADR-028), управление ролями — через `role.*` API/MCP ([rbac.md](rbac.md)). Секрет-поля федеративных блоков — через `*_ref` (`vault:<mount>/<path>[#field]`, резолв load-time как `redis.password_ref`).
...
### `auth.ldap` — федеративная LDAP-аутентификация ([ADR-058](../adr/0058-operator-auth-ldap-oidc.md))

Опциональный блок. При наличии монтируется `POST /auth/ldap/login` (вне `/v1`): Keeper валидирует логин/пароль против LDAP, маппит внешние группы на RBAC-роли (`group_role_map`), при первом успешном логине auto-provision-ит строку `operators` (`created_via='ldap'`, `auth_method='ldap'`) и выпускает **внутренний** JWT в HttpOnly+Secure+`SameSite=Strict` cookie `soul_session` (дальше RBAC/MCP/middleware работают без изменений). Не задан → способ логина недоступен, Keeper стартует ([ADR-053](../adr/0053-dependency-tiers.md) OPTIONAL-tier).

Реализован только **search-bind** (`bind_mode: search`, дефолт): service-account (`bind_dn` + `bind_password_ref`) ищет пользователя по `user_filter`, затем bind проверяемыми credentials. `bind_mode: direct` (поле `user_dn_template`) отложен (стадия 1) — любое значение `bind_mode` вне `{"", search}` отвергается на старте (`ldap_bind_mode_unsupported`). **TLS обязателен**: либо `ldaps://`, либо `ldap://` + `start_tls: true` (взаимоисключимы); plaintext-LDAP отвергается на старте.

```yaml
auth:
  ldap:
    url: ldaps://ldap.corp.internal:636   # ldaps:// | ldap:// (последний требует start_tls)
    # start_tls: true                       # StartTLS поверх ldap://; взаимоисключим с ldaps://
    bind_mode: search                       # только search (дефолт); direct отложен
    bind_dn: "cn=keeper,ou=svc,dc=corp,dc=internal"
    bind_password_ref: vault:secret/keeper/ldap-bind   # поле в KV — password
    base_dn: "ou=people,dc=corp,dc=internal"
    user_filter: "(uid=%s)"
    group_filter: "(member=%s)"
    group_attr: cn
    aid_attr: uid                           # атрибут пользователя → AID
    timeout: 5s
    tls:
      ca_ref: vault:secret/keeper/ldap-ca   # опц. CA-bundle для LDAPS (поле в KV — ca)
      # insecure_skip_verify: true          # dev-only, на старте WARN
    group_role_map:
...
| Поле | Тип | Default | Смысл |
|---|---|---|---|
| `auth.ldap.url` | `string` (URL) | — | `ldaps://host:636` либо `ldap://host:389`. plaintext (`ldap://` без `start_tls`) отвергается на старте. |
| `auth.ldap.start_tls` | `bool` | `false` | StartTLS поверх `ldap://`. Взаимоисключим с `ldaps://` (`ldaps://` уже шифрует канал) — конфликт отвергается на старте. |
| `auth.ldap.bind_mode` | `enum{search}` | `search` | Только `search` (или опущено = `search`). `direct` отложен (стадия 1): значение вне `{"", search}` → diag `ldap_bind_mode_unsupported`. |
| `auth.ldap.bind_dn` | `string` (DN) | — | DN service-account для search-bind. Обязателен при `bind_mode: search` (иначе `ldap_search_requires_bind_dn`). |
| `auth.ldap.bind_password_ref` | `vault-ref` | — | Vault-reference на пароль service-account (поле в KV — `password`). Обязателен при `bind_mode: search` (`ldap_search_requires_bind_password`). Plaintext в конфиге не предусмотрен. |
| `auth.ldap.base_dn` | `string` (DN) | — | База поиска пользователей. |
| `auth.ldap.user_filter` | `string` | — | LDAP-фильтр поиска пользователя, `%s` подставляется логином (например `(uid=%s)`). |
| `auth.ldap.user_dn_template` | `string` | — | Шаблон DN для `bind_mode: direct` (например `uid=%s,ou=people,...`). Поле зарезервировано (direct отложен), `bind_mode`-ом не активируется. |
| `auth.ldap.group_filter` | `string` | — | Фильтр поиска групп пользователя (`%s` = user-DN), например `(member=%s)`. |
| `auth.ldap.group_attr` | `string` | — | Атрибут найденной группы, значение которого ключуется в `group_role_map` (например `cn`). |
| `auth.ldap.aid_attr` | `string` | `uid` | Атрибут пользователя, чьё значение становится AID оператора. |
| `auth.ldap.timeout` | `duration` | — | Таймаут LDAP-операций. |
| `auth.ldap.tls.ca_ref` | `vault-ref` | — | Опц. Vault-reference на CA-bundle для верификации LDAPS-сертификата (поле в KV — `ca`). |
| `auth.ldap.tls.insecure_skip_verify` | `bool` | `false` | Отключает проверку TLS-сертификата. **dev-only**: `true` → WARN на старте (`ldap_insecure_skip_verify`). |
| `auth.ldap.group_role_map` | `map<string, list<string>>` | — | Внешняя группа (значение `group_attr`) → список RBAC-ролей, которые получит оператор. Источник ролей при auto-provision и при реконсиляции на каждом логине. |

### `auth.oidc` — федеративная OAuth2/OIDC-аутентификация ([ADR-058](../adr/0058-operator-auth-ldap-oidc.md))

Опциональный блок. При наличии (**и работающем Redis**) монтируются `GET /auth/oidc/login` и `GET /auth/oidc/callback` (вне `/v1`): authorization-code flow с discovery (`go-oidc`), JWKS-валидацией `id_token`, маппингом `groups_claim`→роли и auto-provision (`created_via='oidc'`, `auth_method='oidc'`); итоговый внутренний JWT — в cookie `soul_session` (`SameSite=Lax` — cross-site redirect от IdP). **PKCE (S256) всегда включён** реализацией — оператору на выбор не оставлен, config-флага `use_pkce` нет. Flow-state (state→nonce/verifier) хранится в Redis (single-use, TTL 5m), поэтому без Redis эндпоинты не монтируются ([ADR-053](../adr/0053-dependency-tiers.md) OPTIONAL-tier). `issuer` — только HTTPS.

```yaml
auth:
  oidc:
    issuer: https://idp.corp.internal/realms/soul-stack    # discovery base, только https
    client_id: soul-stack-keeper
    client_secret_ref: vault:secret/keeper/oidc-client      # опц. (public-client может не иметь); поле в KV — client_secret
    redirect_url: https://keeper.corp.internal/auth/oidc/callback
    scopes: [openid, email, profile, groups]
    aid_claim: sub                # sub | email | preferred_username (дефолт sub)
    groups_claim: groups
    tls:
      ca_ref: vault:secret/keeper/oidc-ca    # опц. кастомный CA IdP (поле в KV — ca)
    group_role_map:
      ops:    [operator]
...
| Поле | Тип | Default | Смысл |
|---|---|---|---|
| `auth.oidc.issuer` | `string` (https URL) | — | Базовый URL OIDC-провайдера для discovery (`/.well-known/openid-configuration`). Только HTTPS (иначе diag на старте). Обязателен. |
| `auth.oidc.client_id` | `string` | — | OAuth2 client-id, зарегистрированный у IdP. Обязателен (`oidc_client_id_required`). |
| `auth.oidc.client_secret_ref` | `vault-ref` | — | Опц. Vault-reference на client-secret (поле в KV — `client_secret`); public-client без секрета его опускает. Plaintext не предусмотрен — не-vault-ref отвергается на старте. |
| `auth.oidc.redirect_url` | `string` (URL) | — | Callback-URL, регистрируемый у IdP; должен указывать на `…/auth/oidc/callback` этого Keeper-а. Обязателен (`oidc_redirect_url_required`). |
| `auth.oidc.scopes` | `list<string>` | — | OAuth2 scopes запроса (типично `openid`, `email`, `profile`, `groups`). |
| `auth.oidc.aid_claim` | `string` | `sub` | Claim `id_token`, чьё значение становится AID. Дефолт `sub` (иммутабельный subject). Mutable-claim (`email`/`preferred_username`) → WARN на старте (`oidc_aid_claim_mutable`, identity-spoofing-риск при переназначении). |
| `auth.oidc.groups_claim` | `string` | `groups` | Claim `id_token` со списком групп, ключуемых в `group_role_map`. |
| `auth.oidc.tls.ca_ref` | `vault-ref` | — | Опц. Vault-reference на кастомный CA IdP (поле в KV — `ca`). |
| `auth.oidc.group_role_map` | `map<string, list<string>>` | — | Внешняя группа (значение из `groups_claim`) → список RBAC-ролей. Источник ролей при auto-provision и при реконсиляции на логине. |

### `auth.rate_limit` — anti-bruteforce login-эндпоинтов ([ADR-058(g)](../adr/0058-operator-auth-ldap-oidc.md))

Опциональный блок. Защита публичных login-путей (`/auth/ldap/login`, `/auth/oidc/login`) от перебора: per-IP + per-username троттл частоты попыток (token-bucket) **плюс** lockout после серии неудач. **Default-ON** (nil/опущено → включён, footgun-guard как `tempo`/`toll`); явный `enabled: false` — opt-out. Фактически поднимается только при работающем Redis (limiter cluster-shared — авторитет в Redis, а не ×N по stateless-инстансам); без Redis login-пути деградируют без throttle. Lockout-проверка **fail-closed** при Redis-ошибке (login — security-периметр, недоступность Redis не должна открывать брутфорс). Превышение → `429` + `Retry-After` + `application/problem+json` с типом [`auth-throttled`](../naming-rules.md#error-codes) (detail anti-oracle — без указания scope/причины). Hot-reloadable. Примитив — **LoginGuard** (`keeper/internal/redis/loginguard.go`).

```yaml
auth:
  rate_limit:
    enabled: true              # nil/опущено → true (default-ON); false → opt-out
    rate: 0.5                  # попыток/сек на принципал (token-bucket refill)
    burst: 10                  # глубина бакета попыток
    lockout_threshold: 5       # неудач в окне до блокировки принципала
    lockout_window: 15m        # окно подсчёта неудач
    lockout_backoff: 15m       # длительность блокировки после порога
```

| Поле | Тип | Default | Смысл |
|---|---|---|---|
| `auth.rate_limit.enabled` | `bool` (tri-state) | `nil` → `true` | Включение. Опущено/`null` → default-ON; явный `false` → opt-out (dev). Фактический подъём требует Redis. |
| `auth.rate_limit.rate` | `float` (попыток/сек) | `0.5` | Refill-скорость token-bucket-а попыток на один принципал (IP **или** username). `0`/опущено → дефолт. |
| `auth.rate_limit.burst` | `int` | `10` | Глубина бакета попыток (одномоментная пачка). `0`/опущено → дефолт. |
| `auth.rate_limit.lockout_threshold` | `int` | `5` | Число неудачных логинов в окне, после которого принципал блокируется. `0`/опущено → дефолт. |
| `auth.rate_limit.lockout_window` | `duration` | `15m` | Скользящее окно подсчёта неудач. Пусто → дефолт. |
| `auth.rate_limit.lockout_backoff` | `duration` | `15m` | Длительность блокировки после достижения порога. Пусто → дефолт. |

### Политика провижининга операторов — `provisioning_allowed_methods` ([ADR-058(i)](../adr/0058-operator-auth-ldap-oidc.md))

Способ **СОЗДАНИЯ** оператора (`created_via`) можно ограничить политикой `provisioning_allowed_methods` — это **well-known ключ `keeper_settings` в Postgres**, а не поле `keeper.yml` (управление через API/MCP, не правкой файла, симметрично реестру Service-ов). CSV из домена `{user, ldap, oidc}`: гейтит только ветку создания (`POST /v1/operators` → `user`; federated auto-provision → `ldap`/`oidc`). Существующие операторы логинятся независимо от политики; `bootstrap`/`system` **не гейтятся никогда** (в домен не входят).

- **Ключ отсутствует** → все способы разрешены (back-compat).
- **Ключ задан, но пуст** → config-error (anti-lockout: запретить ВСЕ способы и залочить заведение операторов нельзя).
- Метод вне `{user,ldap,oidc}` → отказ.

Runtime-управление — эндпоинты `GET`/`PUT /v1/provisioning-policy` (permissions `provisioning.read` / `provisioning.update`, [rbac.md → Provisioning](rbac.md#provisioning-2--adr-058)). `GET` без audit (`policy_set=false` → ключ не задан = дефолт «всё разрешено»); `PUT` — replace-семантика, пишет audit `provisioning.policy_changed`, пустой список → `422` anti-lockout, запрещённый политикой способ создания → `422` [`provisioning-method-disabled`](../naming-rules.md#error-codes). См. [storage.md](storage.md) (`keeper_settings`), [naming-rules.md → `provisioning_allowed_methods`](../naming-rules.md).
