...
**Bootstrap listener resource limits** (NIM-839) — not config keys, the way `max_apply_size_mb` is; they are invariants of the pre-auth listener and are stated here because an operator debugging a refused onboarding needs to know they exist. Concurrent connections are capped at **256**, connection setup (TLS handshake plus HTTP/2 preface) at **10s**, a connection with no RPC on it at **2 min** idle, and any connection at **5 min** of age with a 20s grace. On top of those the handler admits **16** Bootstrap RPCs past the free checks at a time; a 17th **waits for a slot** and only then gets `ResourceExhausted`. It waits holding a goroutine, a stream and its request (capped at 16 KiB), never a pooled connection — which is the whole point, and why a burst is served rather than refused: `soul bootstrap` is one-shot and does not retry, so an instant refusal would fail the hosts that lost the race with nothing to pick them back up. A burst drains at the rate Vault signs; a flood does not drain, and that is what runs the wait out.

**Queueing never takes more than half the caller's remaining time.** The wait is `min(5s, remaining ÷ 2)` — a *share*, not a fixed subtraction. The Soul wraps dial, handshake and RPC in **one** deadline (`keeper.retry.handshake_timeout` in [soul/config.md](../soul/config.md), default 10s) and writes its private key only after a successful reply, while Keeper burns the token inside the transaction — so time spent in this queue is time the work does not get, and the bound says how much of it the queue may take.

A share rather than "remaining − 5s" because that timeout is **an operator knob in a different file, which this listener cannot read** and which [connection.md](../soul/connection.md) advises lowering for faster failover. Any fixed subtraction turns into "no wait at all" once the timeout drops to it, restoring the very onboarding outage the wait exists to prevent — silently, since it only shows under contention. A ratio has no such cliff: at the 10s default the listener sees ~9.8s left and waits up to ~4.9s; at a 3s timeout it sees ~2.6s and waits up to ~1.3s (both after dial and TLS, which come out of the same deadline).

⚠️ **What this does not promise: that an admitted caller has time to finish.** Keeper cannot know — the cost is dominated by a Vault round trip of unpredictable latency, and an *uncontended* caller with the same short timeout runs exactly the same risk without queueing at all. An absolute floor would be the same cliff at another value: it would refuse callers whose deadline is short but whose Vault is fast, which is a working deployment today. If a commit lands after the reply can be delivered, the token is burned for a host that never learns it onboarded — that is **NIM-865**, and it belongs to the burn, not to how long anyone queued. Set `handshake_timeout` against your Vault's real signing latency, not against this number.

When the budget is spent Keeper warns at most once per 30s, reporting the wait it **measured** alongside the configured one: `keeper_grpc_bootstrap_total` has only `ok`/`failed` and cannot tell "at capacity" from a bad token, and if the two durations differ the answer is `handshake_timeout`, not more capacity. A caller that hangs up or times out mid-wait gets `Canceled`/`DeadlineExceeded` back and is **not** counted as a capacity refusal — nor is one whose deadline had already passed before Keeper read the request.

What each limit closes: the per-connection stream ceiling bounded nothing while the number of connections was the caller's to choose, and the connection cap on its own would have been held open by peers that never finish a handshake.

⚠️ **The 256-connection cap is a deliberate trade, and it is not free.** There is no per-source-IP bound anywhere on this listener — Keeper has no trusted-proxy configuration, so a per-IP cap would collapse every client behind a NAT or an L7 proxy onto one bucket and break legitimate mass onboarding. One host can therefore hold all 256 slots and stop onboarding **on that instance**, and clients that cannot get in see no error, only their own deadline. That is the intended direction: before this change the same attacker starved `/v1`, EventStream, the Reaper and the audit writer instead. Onboarding availability was traded for control-plane availability, on purpose.
...
| `postgres.pool.max` | `int` (≥`min`) | `20` | Maximum pool size. Total flow to PG = `max × number_of_keeper_instances`, **plus the bootstrap pool below**. |

**A second, derived pool for the Bootstrap listener** (NIM-839). `listen.grpc.bootstrap` is the one listener an unauthenticated caller can reach — a host that has not been onboarded has to be able to reach it — and the first thing it does is read `bootstrap_tokens`, to check the token it was handed. On the main pool that put unauthenticated traffic in the same `Acquire` queue as `/v1`, EventStream, the Reaper and the audit writer, so enough of it starved the control plane without presenting a credential.

The listener therefore gets its own pool over the same DSN, sized at **a quarter of `postgres.pool.max`, never below 2**, with `min: 0` (and `pool_min_idle_conns` zeroed too, so a DSN that sets it does not make the second pool eager). There is no key for it: `postgres.pool.max` stays the single number sized against the server's `max_connections`, and a quarter of it is a cost derivable from the number already set. The ceiling per instance is `pool.max + max(2, pool.max/4)`; at idle the second pool holds nothing, so the steady state is unchanged.

**The boundary is pre-auth, not the whole listener.** The token pre-check and the onboarding transaction move to the dedicated pool — that is everything an unauthenticated caller can reach. The two `soul.bootstrapped` / `soul.seed-issued` audit writes that follow still go through the shared audit writer, which is on the main pool; they run only after the token is burned, so no unauthenticated request reaches them.
...
| `vault.kv_mount` | `string` | `secret` | Mount point KV — **one path segment**, no slashes (letters, digits, `_`, `-`); anything else → diag `vault_kv_mount_invalid`. A multi-segment mount would push the service segment out of the position the [ADR-0083](../adr/0083-declared-secret-state-fields.md) §7 fence compares, silently disabling it. Used by the client to read the JWT signing-key and other secrets. Mount can be KV **v1 or v2** - the version is determined automatically (probe `sys/internal/ui/mounts/<mount>` on the first reading), there is no need to specify it in `kv_mount`. The reading contract is the same for both versions: the module/resolver receives a flat payload (for v2, the `data.data` wrapper is unpacked by the client). |
...
| `plugin_runtime.allowed_capabilities` | `list<enum>` | all 6 capabilities (see YAML-block above) | Closed enum (full catalog - [plugins.md → required_capabilities-table](plugins.md), [ADR-020(f)](../adr/0020-plugin-infrastructure.md#amendment-2026-08-06-nim-377-the-schema-is-generated-from-go-the-artifact-carries-no-name)). A whitelist checked by **the host at spawn** (`pluginhost.CheckCapabilities`), which refuses to exec on a mismatch. ⚠ **`soul-lint` does NOT check it** — it never reads capabilities; the older claim of a destiny rejection *before launch* was wrong. **Unset = no filter** (everything allowed), which is the shipped default; the operator narrows it by security policy. Note what this gates: the plugin's **declaration**, not its behaviour — there is no sandbox ([plugins.md → Capabilities and side_effects are disclosure](plugins.md#capabilities-and-side_effects-are-disclosure)). The parser rejects values outside the closed enum with `unknown_capability`. |
| `plugin_runtime.conflict_policy` | `enum{warn,fail}` | `warn` | ⚠ **Parsed and enum-validated, then read by nothing.** It is documented as the policy for two plugins in one run claiming the same `side_effects` resource, but **no conflict is ever detected** — `side_effects` values are read only by the grammar validator, and no production code emits `policy_violation` ([ADR-020(g)](../adr/0020-plugin-infrastructure.md#amendment-2026-08-06-nim-377-the-schema-is-generated-from-go-the-artifact-carries-no-name) as corrected 2026-08-06, [plugins.md → Capabilities and side_effects are disclosure](plugins.md#capabilities-and-side_effects-are-disclosure)). |
...
| Field | Type | Default | Meaning |
|---|---|---|---|
| `audit.enabled` | `bool` | `true` | Global switch of the audit-pipeline. With `false`, none of the write-path initiators (see [ADR-022(g)](../adr/0022-audit-pipeline.md#adr-022-audit-pipeline-storage-schema-retention)) writes to Postgres `audit_log`; OTel dual-write (see `otel_export` below) and the herald notification tap stop with it. Enforced by one gate over the shared `audit.Writer` (`keeper/internal/auditgate`), so a new initiator cannot forget to honor it. **Four events are written anyway** — `config.reload_succeeded` / `config.reload_failed` / `audit.disabled` / `audit.enabled`: switching the audit off must itself leave a record, otherwise the flag would buy the ability to work without traces. Omitting the field, or the whole block, means `true`. Use only for development / incident investigation — a production installation must keep `true` for compliance invariants. |
| `audit.otel_export` | `bool` | `true` | Duplicate the audit-event into an OTel span (transient debugging aid, Postgres — source of truth). With `false` audit is written only to `audit_log`; Keeper's own OTel-spans continue to go through [`otel:`](#otel) as usual ([ADR-022(f)](../adr/0022-audit-pipeline.md#adr-022-audit-pipeline-storage-schema-retention)). **Precondition:** the secondary writer exists only when [`otel.enabled`](#otel) is `true` — with no OTel stack configured there is nowhere to export to, and this flag has nothing to switch. |
| `audit.retention_days` | `int` (≥1) | `365` | Record retention period of `audit_log` (days). **Alias** for `reaper.rules.purge_audit_old.max_age` ([ADR-022(d)/(i)](../adr/0022-audit-pipeline.md#adr-022-audit-pipeline-storage-schema-retention)) — one source of truth for retention, a convenient form in days here and a `duration` in [`reaper:`](#reaper) ([reaper.md → Rules](reaper.md)). If the values in the two blocks diverge, the `keeper.yml` parser rejects the config with the error `audit_retention_mismatch`. If `reaper:` is configured but declares no `purge_audit_old` rule, the alias materializes one (`enabled: true`, `action: delete`, `max_age: <retention_days>d`) — the value is applied rather than merely declared. Without a `reaper:` block nothing is fabricated: there is no Reaper to run the rule. |
...
All three fields are **reload-able without restart** of the `keeper` process: two are read from the live `config.Store` snapshot on every write to `audit_log`, the third is a Reaper rule parameter (read at the next iteration of the Reaper, [reaper.md](reaper.md)).

| Field | Reload without restarting the `keeper` process | Rationale |
|---|---|---|
| `enabled` | yes | Read per event by the write-path gate; a record already in flight completes under the value it started with. |
| `otel_export` | yes | The same, per event, on the secondary writer. The dual-write chain itself is assembled once at startup (from [`otel.enabled`](#otel)), so this flag switches the export, not the wiring. |
| `retention_days` | yes | Applied by the Reaper at the next iteration (via the alias to `reaper.rules.purge_audit_old.max_age`). |

**A flip of `enabled` is journaled.** The instance writes `audit.disabled` when the toggle goes off and `audit.enabled` when it comes back on (`source: keeper_internal`, payload `{kid}` — the toggle is per instance, the file is per host). The `audit.disabled` marker is written in-line on the write path, ahead of the first event it suppresses, so the trail never holds an unexplained gap. Both names are in [naming-rules.md → Audit-events](../naming-rules.md#audit-events).

**`keeper init` is not gated.** Bootstrap of the first Archon writes its `operator.created` through a separate writer built before the daemon exists ([ADR-013](../adr/0013-bootstrap-archon.md) requires that record with `bootstrap_initial: true`). A one-shot administrative command that creates cluster-admin credentials leaving no trace would be a worse outcome than ignoring the flag on it.
...
| `toll.threshold` / `window_size` / `degraded_ttl` / `clear_grace` / `per_coven_thresholds` / `webhook.*` | yes (all) | — | Reinjected into the live leader by `applyTollReload` from the `Store.OnReload` callback; the webhook notifier is recycled only when its own config changed ([§ toll](#toll), ADR-038). `toll.threshold` is additionally cluster-overridable through `cfg_toll_threshold` ([ADR-0073](../adr/0073-keeper-runtime-config-pg.md)). |
| `toll.enabled` / `lease_ttl` / `warmup_delay` | — | yes | Read once at startup `setupToll` (raise/quench the detector, lease TTL is set on acquire, the warmup window is anchored to process start); hot-toggle of the subsystem is a separate slice (symmetrically `tempo.enabled`). |
| `tempo.voyage_create.rate` / `tempo.voyage_create.burst` | yes (both) | — | Stateless limiter: rate/burst are read from a fresh Store snapshot on each request ([§ tempo](#tempo), ADR-050). Cluster-overridable through `cfg_tempo_voyage_create_rate` / `cfg_tempo_voyage_create_burst` ([ADR-0073](../adr/0073-keeper-runtime-config-pg.md)). |
...
| `audit.enabled` / `audit.otel_export` / `audit.retention_days` | yes (all three) | — | Write-path parameters (`enabled` / `otel_export`) — read per event from the live snapshot; `retention_days` — alias to `reaper.rules.purge_audit_old.max_age`, read by the Reaper at the next iteration ([§ Hot-reload block `audit:`](#hot-reload-block-audit)). File-only by [ADR-0073(j.2)](../adr/0073-keeper-runtime-config-pg.md) — see the admission note below. |
