...
> **`coven=` on the three `host=<sid>` mutations denies instead of narrowing.** `soul.issue-token`, `soul.forget` and `soul.ssh-target-update` are wrapped in `RequirePermission(..., SoulSIDSelector)`, and that extractor puts **only** `host` into the RBAC context. A dimension absent from the context fails closed (§ Two-layer authorization of read endpoints), so `soul.forget on coven=web` is not "forget hosts in `web`" - it refuses **every** call, including one aimed at a host that really is in `web`, and the 403 names a permission the operator demonstrably holds. Narrow these three by `host=`. `soul.console` is **not** affected: it is mounted behind `RequireAction` with the scope applied inside the handler, so `coven=` narrows there as the row above says.
>
> **A coven reaches these three by two routes, and both deny.** The suffix form (`soul.forget on coven=web`) is the visible one. The other is a role whose `default_scope` is `coven=web` holding a **bare** `soul.forget`: `default_scope` is inherited by every bare permission of the role (§ A cluster-level right does not belong on a scoped role), so the permission arrives at the gate coven-narrowed and fails closed identically. The two are written by different people — the suffix by whoever composes the permission string, the `default_scope` by whoever creates the role — so an operator who has read the warning about the suffix can still walk into this by scoping the role. Both forms are pinned: `TestSoulForgetRoute_CovenNarrowedGrantDeniesEverything` and `TestSoulForgetRoute_CovenRoleDefaultScopeDeniesEverything`. A role that needs a coven scope **and** one of these three splits into two roles, or writes the permission with its own overriding `on host=<sid>`. Making `coven=` narrow rather than deny is NIM-588, and it has to answer for both forms.
