...
| `soul.console` | Opening an interactive console (PTY) on a host over the WebSocket `/v1/console` ([ADR-0074](../adr/0074-interactive-console-pty.md), [console.md](console.md)), running a one-shot command line on a host through the MCP tool `keeper.soul.run-command` ([ADR-0074 amendment](../adr/0074-interactive-console-pty.md), NIM-147), **and** reading a recorded session back over `GET /v1/console/recordings…` (NIM-148) - a recording is the session's content moved in time, so no lighter right is minted for it; withhold playback by narrowing the SCOPE, not by a weaker right. Selectors: `host=<sid>` / `coven=<label>`; bare - unrestricted. **`coven=` narrows playback but denies live sessions** - the recording routes resolve scope through `ResolvePurview`, the `open` frame and `keeper.soul.run-command` build a host-only context (see the note under this table). **The most privileged right in the catalog on the execution axis** - see § Console below. Audit `console.opened` / `console.closed` / `console.command` / `console.recording-read`. |
...
>
> **Two rights are still host-only, and `coven=` on them still denies:** `errand.run` on `POST /v1/souls/{sid}/exec` (`ErrandSIDSelector`), and `soul.console` on its per-`open`-frame check and on the MCP `keeper.soul.run-command` - both build `{host: sid}` by hand. Narrow those two by `host=`. Console **recordings** are not in that set: they resolve through `ResolvePurview`, which carries the coven dimension, so a `coven=`-scoped operator sees exactly the recordings of hosts in its covens. Moving the remaining host-only contexts to this shape is tracked separately.
...
**No new selector keys.** Per-host consoles reuse `host=<sid>`, per-environment consoles reuse `coven=<label>` — grammatically; the live half does not yet honour the second one, because both per-host checks above pass a host-only context and a `coven=`-scoped `soul.console` therefore fails closed on every `open` frame (§ the note under the permission table; the three Soul mutations were moved off that shape by NIM-588, these two were not). The RBAC selector keys stay `{service, coven, incarnation, host}` and any narrowing beyond them goes through the existing Purview dimensions ([ADR-047 §S4](../adr/0047-purview.md)).
