...
| `trait.<key>=v` | Restricts to incarnations whose operator-set trait label `<key>` equals `v` (exact scalar-equality, **not** CEL). [ADR-060](../adr/0060-traits.md) p. 7, realised by NIM-128 as a boolean-builder dimension. | incarnation trait pair (`incarnation.traits` jsonb) - fed to the resolver `incarnation.list`/`get` (`inc.Traits[key] == v`). |
...
**`trait.<key>=v`.** Dot-notation (`trait.owner=dba`), exactly one pair per condition; `<key>` is lower-case `[a-z][a-z0-9_.-]*`, the value is the ordinary exact-value character class. Semantics = exact scalar-equality against `incarnation.traits[key]` (like `coven=`, not a CEL predicate). Multiple owners are an OR of two conditions (`trait.owner=dba OR trait.owner=platform`); `trait.<key> in (a,b)` and AND-narrowing over several keys are a follow-up. This is the incarnation-dimension trait scope; the per-soul bulk-write `soul.traits-assign` remains deprecated and is not a scope dimension ([ADR-060](../adr/0060-traits.md) R1 slice a, § Soul).
...
| `soul.console` | Opening an interactive console (PTY) on a host over the WebSocket `/v1/console` ([ADR-0074](../adr/0074-interactive-console-pty.md), `docs/keeper/console.md`), **and** running a one-shot command line on a host through the MCP tool `keeper.soul.run-command` ([ADR-0074 amendment](../adr/0074-interactive-console-pty.md), NIM-147). Selectors: `host=<sid>` / `coven=<label>`; bare - unrestricted. **The most privileged right in the catalog on the execution axis** - see § Console below. Audit `console.opened` / `console.closed` / `console.command`. |
