...
| `coven=v` / `coven in (a,b)` | Restricts to the specified Coven tags. | **incarnation operations** (run / destroy / upgrade / get / history): declared `incarnation.covens` — the incarnation's **real stable** env tags **only** ([ADR-008 amendment 2026-07-17](../adr/0008-coven-stable-tags.md#amendment-2026-07-17-nim-124-incarnationname-is-not-a-coven--membership-is-a-first-class-relation): `incarnation.id` is **no longer** added as a root coven tag). On create - declared `covens` from the request body. **soul-operations** (`soul.coven-assign` / `soul.list`): host label from body/query (`souls.coven[]`). **Per-host mutations** (`soul.issue-token` / `soul.ssh-target-update` / `soul.forget`, NIM-588): the labels on the `souls` row named by the path SID, read before the handler; a host holds several, so the gate is offered one context per label and admits on any (§ the note under the Soul table). To scope by the incarnation's own name, use `incarnation=<name>`, **not** `coven=<name>`. |
| `incarnation=v` / `incarnation in (a,b)` | Restricts to a specific instance. | `incarnation.id` of the target instance. **This is the scope for the incarnation's own name** — a role that previously used `coven=<incarnation-name>` migrates here ([ADR-008 amendment 2026-07-17](../adr/0008-coven-stable-tags.md#amendment-2026-07-17-nim-124-incarnationname-is-not-a-coven--membership-is-a-first-class-relation)). |
| `host=v` / `host in (a,b)` | Restricts to a specific host (exact SID). **Context source is path/body mutation** (`soul.issue-token` / `soul.ssh-target-update` / `soul.forget` / `errand.run`), where the SID is known before the handler. `soul.console` is the same class with a later context point: the SID arrives in the WebSocket `open` frame, so the check is in-handler per session rather than in middleware (§ Console). On the read-visibility of souls (`soul.list` and the get/soulprint/history covered by it) the `host=` condition at the gate stage is NOT applied - read is gated by existence-`RequireAction`, narrowing by scope is done by handler (ADR-047 §g G1, § Two-layer authorization read-endpoints). | Host SID ([identity.md](../soul/identity.md)) from path/body mutation, or from the console `open` frame. |
...
| `incarnation matches <glob>` | Restricts to incarnations whose **name** matches the **glob** (`*`, `?`; full anchored) — NIM-128 extends the glob form to the incarnation identity dimension (`incarnation matches "prod-*"`). Same anchored-glob → `LIKE` pushdown over `incarnation.id`. | `incarnation.id` of the target instance. |
...
- **Updating the snapshot - B2 (implemented).** The snapshot is invalidated via Redis pub/sub: each mutation of the role / permissions / membership publishes a signal to the topic **`rbac:invalidate`** (envelope `{origin_kid, at}`), and all nodes re-read the snapshot from the database. **Self-filter by KID**: the node ignores its own signal on the wire (pattern `applybus`) — but it does **not** get its own update for free. Until NIM-421 the parenthetical here claimed the publishing node "has already updated the snapshot in the same mutation transaction"; it had not, so the revoking node was the one node that learned nothing and fell back to the TTL poll — measured at 9.82–10.19s against 0.04–0.06s on a subscriber. The invalidator now refreshes the local snapshot itself, right after the commit and before publishing ([ADR-014 amendment 2026-08-07](../adr/0014-operator-identity.md)); the wire-level self-filter stays as it is, because a node must not re-handle its own message. The local refresh is wired unconditionally, so a Redis-less single-node stand gets it too; if it fails it is logged and the TTL poll takes over. **B1 TTL-poll remains fallback**: background goroutine (`rbac.Holder`) rereads the snapshot from the database at a fixed interval (`DefaultRefreshInterval` = 10s) - best-effort insurance in case of Redis being unavailable/lost signal; If the reread fails, the previous snapshot remains + warn. The aging window (seconds until the next TTL reread when the signal is lost) is acceptable: role/membership mutations are rare.
...
- **RBAC outside the hot-reload-config-path.** The snapshot is rebuilt from the database (Redis pub/sub-invalidation via topic `rbac:invalidate` + TTL-poll fallback), **not** by `SIGHUP` / config-swap ([ADR-021](../adr/0021-hot-reload-config.md), clarified [ADR-028](../adr/0028-rbac-storage.md#adr-028-rbac-storage--postgres)). Revocation of role/membership (`role.revoke-operator` / `role.delete`) - `DELETE` in the database + re-reads the snapshot: **immediate on the node that performed the mutation** (NIM-421), milliseconds on the others via pub/sub, and no later than the TTL window if the signal is lost. Operator revocation (`operator.revoke`) rides the same path and, since 2026-08-07, is answered as `401 operator-revoked-token` on **every** authenticated HTTP route by the `RejectRevoked` link rather than depending on which gate a route happens to carry — see [ADR-014 amendment 2026-08-07](../adr/0014-operator-identity.md), which also supersedes the older "active JWT works until `exp`" reading of ADR-014(d). On **MCP** the revoked operator is refused every tool by the same `Check`, but the refusal still comes back as `forbidden` rather than a distinct revoked code, and `initialize` / `tools/list` plus the SSE stream of an apply the operator started herself are not gated at all (NIM-551) — MCP is a separate listener and `RejectRevoked` does not run there.
...
| `incarnation.history` | Reading `state_history` instance (snapshot per-change). Also gates the run-history reads (`GET /v1/voyages[…]`, `GET /v1/push-runs`, `GET /v1/cadences/{id}/runs`); those return host identity, so they are additionally narrowed by the caller's `soul.list` purview (NIM-842, § Voyage). |
...
| `incarnation.unbind-member` | Remove a host from the incarnation's roster (`DELETE /v1/incarnations/{id}/members/{sid}`, MCP `keeper.incarnation.unbind-member`). Split from `bind-member` (the `choir.add-voice`/`choir.remove-voice` pattern) because it is the destructive half: the host stops being a target of every FUTURE run. Same selectors and the same second gate as `bind-member`. Idempotent (non-member → `removed:false`). Audit `incarnation.member_unbound`. **Reading the roster** (`GET .../members`, MCP `keeper.incarnation.members`) has no right of its own - it rides on `incarnation.get` and is narrowed to the caller's soul visibility. |
...
**Gate (b) judges `souls.coven`, the same basis as every other host read.** Visibility resolves over the host's own tags ([NIM-281](../adr/0008-coven-stable-tags.md#amendment-2026-08-05-nim-281-a-label-is-never-inherited)) — the same predicate as the souls read, the roster resolver and the SQL pushdown, so a `where:` and a scope check can never disagree about a host. The concern that once made this paragraph necessary is gone with inheritance: a bind confers no label, so it cannot mint the label that authorizes it. The caller must reach the host by an ordinary route — its own coven, a `host=` selector, or `incarnation=` on an incarnation the host is already in.
...
CRUD named host topology inside incarnation (Choir / Voice, tables `incarnation_choirs` / `incarnation_choir_voices`). **REST-only** (`/v1/incarnations/{id}/choirs*`, no MCP tools; bodies and semantics - [operator-api/choirs.md](operator-api/choirs.md)); routes are connected only when the ChoirDB pool is configured. Choir belongs to the incarnation, so the selector is the same as `incarnation.*`: `incarnation=` / `service=` / `coven=` (landing on path-`{name}`); bare - unrestricted. Those who mutate write audit, read-only `choir.list` - no.

| Permission | Semantics | Audit-event |
|---|---|---|
| `choir.create` | Creating a Choir within an incarnation (`POST /v1/incarnations/{id}/choirs`). | `choir.created` |
| `choir.delete` | Delete Choir (`DELETE /v1/incarnations/{id}/choirs/{choir}`). | `choir.deleted` |
...
| `soul.traits-assign` | Bulk assignment of operator-set key-value trait tags **to hosts** by selector (`POST /v1/souls/traits`, jsonb column `souls.traits`). Modes `merge` / `replace` / `remove`. **First-class since [ADR-080](../adr/0080-label-inheritance-union.md)** (NIM-121, deprecation lifted): a host label is stored on the host and nothing projects over it. Since [NIM-281](../adr/0008-coven-stable-tags.md#amendment-2026-08-05-nim-281-a-label-is-never-inherited) it is also the **only** way a host acquires one - the per-incarnation counterpart `incarnation.traits-set` labels the incarnation and never reaches its members. Mirrors the coven-write-path (`soul.coven-assign` / `soul.coven-changed` / `POST /v1/souls/coven`) in **both** gates: existence-gate `RequireAction(soul, traits-assign)` + service-layer intersection of target hosts with the operator's coven-scope (gate a - `souls.coven && scope`, the same `BulkScope`), **plus gate (b)** on merge/replace - every pair attached must lie inside the operator's own trait-scope ([Enforcer.TraitScope](../../keeper/internal/rbac/enforcer.go), pure-trait disjuncts only). Gate (b) is required because a host-attached pair GRANTS visibility permanently and `trait.<key>=v` is a scope dimension; `remove` is ungated on the pair, exactly as with coven. Since NIM-587 gate (b) is not a per-host peculiarity: `incarnation.traits-set` applies the identical check through the same function ([handlers.ScreenTraitPairsInScope](../../keeper/internal/api/handlers/trait_scope_gate.go)), because an incarnation-attached pair grants exactly as much. More details - § Two-layer authorization of read endpoints. |
| `soul.ssh-target-update` | Update per-host SSH push-flow details (`PUT /v1/souls/{sid}/ssh-target`, ADR-032 amendment 2026-05-26, S7-1). Body: `{ssh_port, ssh_user, soul_path}`. Action - hyphenated (`ssh-target-update`), because permission grammar - exactly `<resource>.<action>` (pattern `sigil.key-introduce`); MCP-tool - 3-segment `keeper.soul.ssh-target.update`. **Mutation** - scope-aware gate `RequirePermissionMulti`, selector `host=<sid>` from the path **plus every `coven=` the host carries** (NIM-588, see the note under the table). Audit `soul.ssh-target.updated`. |
| `soul.forget` | Erasing a host from the registry (`DELETE /v1/souls/{sid}`, MCP `keeper.soul.forget`, NIM-386). **Irreversible and wider than the row it names:** every FK on `souls(sid)` is `ON DELETE CASCADE`, so the same call revokes the host's SoulSeeds, burns its bootstrap tokens, severs its incarnation memberships and removes its Choir Voices - rows the operator never named in the request. The response and the `soul.forgotten` audit event carry each of those counts, because after the call there is nothing left to read them from. **Not gated on status:** any host can be forgotten, including one that is `connected` (the call tears its stream down) or that the cluster never met; the reconnect guarantee is structural rather than procedural - seed authentication is an allow-list over `soul_seeds.fingerprint`, so a host whose rows are gone fails closed at the next handshake with no revocation list to consult. **There is deliberately no `force` flag**: a single verb must not be able to degrade from "release the host" to "drop its row". If the cluster-wide teardown notice cannot be sent the call fails `503` with **nothing deleted**; if a resource could not be released after the delete, the reply's `warnings[]` says which one. **Mutation** - scope-aware gate `RequirePermissionMulti`, selector `host=<sid>` from the path **plus every `coven=` the host carries** (NIM-588, see the note under the table). Audit `soul.forgotten`. |
| `soul.console` | Opening an interactive console (PTY) on a host over the WebSocket `/v1/console` ([ADR-0074](../adr/0074-interactive-console-pty.md), [console.md](console.md)), running a one-shot command line on a host through the MCP tool `keeper.soul.run-command` ([ADR-0074 amendment](../adr/0074-interactive-console-pty.md), NIM-147), **and** reading a recorded session back over `GET /v1/console/recordings…` (NIM-148) - a recording is the session's content moved in time, so no lighter right is minted for it; withhold playback by narrowing the SCOPE, not by a weaker right. Selectors: `host=<sid>` / `coven=<coven-tag>`; bare - unrestricted. **`coven=` narrows every surface of the right** (NIM-650) - playback resolves scope through `ResolvePurview`, and the `open` frame, `keeper.soul.run-command` and the verb-shell half of the Errand path all resolve `host=<sid>` **plus every `coven=` the host carries** (see the note under this table). Until NIM-650 the live surfaces built a host-only context and a `coven=`-scoped grant denied every session it named. **The most privileged right in the catalog on the execution axis** - see § Console below. Audit `console.opened` / `console.closed` / `console.command` / `console.recording-read`. |

> **`coven=` on the three `host=<sid>` mutations narrows the target set** (NIM-588). `soul.issue-token`, `soul.forget` and `soul.ssh-target-update` are wrapped in `RequirePermissionMulti(..., SoulSIDScopeSelector)`. That extractor reads `souls.coven` for the host named in the path and hands the enforcer **one context per coven** - `{host, coven}` for each label the host carries, or a bare `{host}` when it carries none - and the gate admits if **any** of them passes. The fan-out is not a convenience: a host holds a *list* of covens ([ADR-008](../adr/0008-coven-stable-tags.md)), so a single context could only have asked about one of them. `soul.forget on coven=web` therefore means what it reads as - it reaches a host whose coven list contains `web` and refuses one whose list does not.
>
> Before NIM-588 the selector put **only** `host` into the context, and a dimension absent from the context fails closed (§ Two-layer authorization of read endpoints): all three answered 403 to **every** call, including one aimed at a host that really was in `web`, while naming a permission the operator demonstrably held. Grants written `on host=` are unaffected by the change - the host dimension is still asserted in every context, whatever the coven read returns.
>
> **A coven reaches these three by two routes, and one fix closes both.** The suffix form (`soul.forget on coven=web`) is the visible one; the other is a **bare** `soul.forget` inside a role whose `default_scope` is `coven=web`, inherited by every bare permission of the role (§ A cluster-level right does not belong on a scoped role). They are written by different people - the suffix by whoever composes the permission string, the `default_scope` by whoever creates the role - but they converge before the check: [`effectiveScope`](../../keeper/internal/rbac/permission.go) takes the per-permission scope when there is one and the role's `default_scope` otherwise, so `evalScope` receives the same expression either way and the two forms cannot diverge in behaviour. They are pinned separately regardless, so a partial fix goes red: `TestSoulForgetRoute_CovenGrantAdmitsHostInThatCoven` / `…_CovenGrantRefusesHostInAnotherCoven` for the suffix, `TestSoulForgetRoute_CovenRoleDefaultScopeAdmitsHostInThatCoven` / `…_CovenRoleDefaultScopeRefusesHostInAnotherCoven` for the role, `TestSoulForgetRoute_CovenGrantAdmitsHostInAnyOfItsCovens` for a host in more than one, and `TestSoulHostRoutes_*` for the other two mounts - which are wired one by one in `router.go` and can each be left behind on its own.
>
> **An unreadable row falls back to the host dimension alone.** If the coven read fails - the host is not in the registry, or Postgres is unreachable - the selector still asserts `{host}`. Coven-scoped grants then fail closed, `on host=` grants keep working, and a database outage widens nothing (`TestSoulForgetRoute_CovenUnreadableKeepsHostDimension`). A host that is simply absent still reaches the handler and gets its `404`; a missing row is not turned into a 403.
>
> **MCP narrows on the same terms.** OpenAPI and MCP are both primary operator interfaces ([ADR-004](../adr/0004-binaries.md)), so `keeper.soul.forget`, `keeper.soul.issue-token` and `keeper.soul.ssh-target.update` resolve the identical contexts through `checkSoulHostScope` before the tool body runs - narrowing over REST while denying over MCP would leave the same operator with the same 403, one surface over. Pinned by `TestSoulForget_CovenGrantNarrowsHere_Too` and `TestSoulIssueToken_RBACNarrowsByCoven`.
>
> **`errand.run` and a live `soul.console` resolve the same contexts** (NIM-650). `POST /v1/souls/{sid}/exec` shares `SoulSIDScopeSelector` with the three mutations above; the console `open` frame, `keeper.soul.run-command`, MCP `keeper.soul.errand.run`, the `kind=command` Voyage and the background Cadence spawn build the identical set in-handler through [`soul.HostContextsBySID`](../../keeper/internal/soul/rbac_scope.go) / `HostContextsBySIDs` and OR it with `AllowAnyContext` - one primitive, so no surface can drift into a different answer about the same host. Until NIM-650 all of them built `{host: sid}` by hand, which made `errand.run on coven=web` and `soul.console on coven=web` deny **every** call, including one aimed at a host that really was in `web`; on the Errand path that also contradicted the resolve directly in front of it, since `ResolvePurview` had already admitted the target by its coven. Console **recordings** were never in that set - they resolve through `ResolvePurview` and narrowed correctly all along.
>
> **The batch surfaces read the whole target in one round-trip** and stay all-or-nothing: a `kind=command` Voyage and a Cadence spawn require the right on **every** resolved host (`CovenBySIDs`, one `= ANY($1)`), because silently dropping hosts from a batch answers a different question than the one that was asked. The read is taken **inside** the console-gate probe, so an ordinary Errand - the overwhelming majority - never pays for it. The Cadence spawn reads through the pool rather than the tick's own transaction: a failed statement poisons its transaction, and this read sits between the `FOR UPDATE` select and `AdvanceSchedule`, so reading it on the tx would turn a transient error into a rolled-back tick that stalls every other due schedule.
...
| `soul.console` + `host=<sid>` **+ the host's covens** | in-handler, per `open` frame | May they open one on **this** host? | A session-scoped `error{code: "forbidden"}` frame; the socket and its other panes stay live. |
| `soul.console` + `host=<sid>` **+ the host's covens** | in-tool, MCP `keeper.soul.run-command` | May they run a command on **this** host? | MCP `forbidden`. |
...
**No new selector keys.** Per-host consoles reuse `host=<sid>`, per-environment consoles reuse `coven=<coven-tag>`, and since NIM-650 the live half honours the second one: both per-host checks above resolve the host's Coven labels and OR one context per label (§ the note under the permission table, where the same primitive serves the three Soul mutations of NIM-588). The RBAC selector keys stay `{service, coven, incarnation, host}` and any narrowing beyond them goes through the existing Purview dimensions ([ADR-047 §S4](../adr/0047-purview.md)).
...
`soul.traits-assign` carries **two** gates, like coven-assign: the permission itself is authorized by the existence-gate (see the directory line above) and the target hosts are narrowed by the operator's coven-scope (gate a, a single `BulkScope`); on `merge`/`replace` every pair being attached must additionally lie inside the operator's own trait-scope (gate b). The earlier reasoning for having no key gate - "a trait key is not a scope dimension" - stopped holding when NIM-128 made `trait.<key>=v` a scope dimension and NIM-121 removed the projection that used to wipe a host-attached pair on the next sync: the pair is now permanent, so without gate (b) any holder of this permission could hand a host to a foreign role by stamping it. [NIM-281](../adr/0008-coven-stable-tags.md#amendment-2026-08-05-nim-281-a-label-is-never-inherited) reinforces this rather than relaxing it — this endpoint is the ONLY way a host acquires a trait at all, so gate (b) is the only place that decision is judged. AND-narrowing on several trait pairs remains a follow-up.

**Gate (b) belongs to the pair, not to the host** (NIM-587). Until NIM-587 the check lived only on the two soul surfaces (REST `POST /v1/souls/traits` and `keeper.soul.traits-assign`) as two copies of one function, and the incarnation write path carried no such gate at all - the reasoning written into its header was that the operator "already holds the incarnation by scope", i.e. gate (a) implies gate (b). It does not. `trait.<key>` is a live read-side dimension for incarnations too (`incScopeColumns.Traits`), so `incarnation.traits-set` stamping `tier=gold` hands every `trait.tier=gold` role sight of that incarnation - visibility the caller need not hold itself - and through a `trait` Rite ([NIM-280](../adr/0008-coven-stable-tags.md#amendment-2026-08-05-nim-280-a-rules-subject-reads-both-levels--targeting-only)) reaches its members as well. All write surfaces now call ONE function, [handlers.ScreenTraitPairsInScope](../../keeper/internal/api/handlers/trait_scope_gate.go), and an agreement guard drives them against one expectation so a forgotten copy fails rather than drifts. Two consequences for role authors:

- A role whose `incarnation.traits-set` (or `soul.traits-assign`) is scoped on any dimension **other** than `trait.` - `coven=`, `service=`, `incarnation=` - has an EMPTY trait-scope and is refused every pair (`422 trait <k>=<v> is outside operator trait-scope`); an empty payload, which clears the labels, still passes. A **bare** permission with no `default_scope` is `Unrestricted` and unaffected (exception #2 above).
- Restoring the ability means a separate permission constraining trait ALONE (`incarnation.traits-set on trait.tier="gold"`) - a disjunct mixing trait with another dimension contributes nothing, by the same fail-closed rule that governs `coven`. Note that such a grant also **widens gate (a)**: the model has one scope per (resource, action), so "may stamp this label" and "may write to objects carrying it" cannot presently be separated.

`incarnation.create` is deliberately **not** covered: a create still accepts any well-formed trait, so an operator refused a label on `traits-set` can still carry it at birth. Whether create refuses or the creator is taken to hold what it stamped is an open permissions decision, tracked in NIM-622 (the NIM-371 create-roster precedent - a create that writes something with its own standalone permission must clear that permission, or create becomes the way around it - argues for refusal); both create surfaces move together when it lands.
...
| `service.update` | Editing a registry entry (`PATCH /v1/services/{id}`; MCP `keeper.service.update`). |
| `service.list` | Enumeration (`GET /v1/services`) + single-get (`GET /v1/services/{id}`) + four git projections (`/refs` / `/scenarios` / `/state-schema` / `/dependencies`) - one-permission-on-read, no separate `service.get`. MCP `keeper.service.list`. |
| `service.deregister` | Removing Service from the registry (`DELETE /v1/services/{id}`; MCP `keeper.service.deregister`). |
| `service.label-set` | Replace the display caption of a registry entry (`PUT /v1/services/{id}/label`; MCP `keeper.service.label-set`). The caption is free text - capitals and spaces allowed, nothing validates its form; `null` clears it and consumers fall back to showing the identifier. It participates in **nothing derived** - no Vault path, no RBAC scope, no snapshot directory, no CEL root ([ADR-0085](../adr/0085-entity-id-and-label.md)) - so this is the narrowest write in the catalog: it can move a screen and nothing else. Narrower than `service.update`, which re-points `git`/`ref` and invalidates every artifact cache; in particular the caption is NOT segment 2 of the derived secret path `<mount>/<service>/<incarnation>/<state-field>` and NOT the artifact cache directory, so changing it orphans no secret and re-clones nothing. Audit event `service.label_changed`. |
...
| `push.read` | Read push run status (`GET /v1/push/{apply_id}`, Variant C orchestrator). The reply carries the run's whole SSH `inventory_sids`, so the read is additionally narrowed by the caller's `soul.list` purview and is shown only when every host in that inventory is inside it (NIM-842, § Voyage). Out of the boundary answers as the same 404 an unknown apply_id gets. |
...
| `push-provider.update` | Replace params of an existing record (`PUT /v1/push-providers/{id}`, replace semantics). |
| `push-provider.delete` | Delete entry (`DELETE /v1/push-providers/{id}`). |
| `push-provider.list` | List records (`GET /v1/push-providers`). |
| `push-provider.read` | Read one entry (`GET /v1/push-providers/{id}`). |
| `push-provider.label-set` | Replace the display caption (`PUT /v1/push-providers/{id}/label`). The caption is free text - capitals and spaces allowed, nothing validates its form; `null` clears it and consumers fall back to showing the identifier. It participates in **nothing derived** - no Vault path, no RBAC scope, no snapshot directory, no CEL root ([ADR-0085](../adr/0085-entity-id-and-label.md)) - so this is the narrowest write in the catalog: it can move a screen and nothing else. Unlike `push-provider.update` it publishes **no** `push-providers:changed` invalidation: the dispatcher snapshot carries params, and a caption is not one of them. The caption is also not the `SOUL_SSH_<UPPER_SNAKE(id)>_PARAMS` env-var name, which is why `id` keeps the letter-first rule and the caption needs no rule at all. Audit event `push-provider.label_changed`. |

### Errand (3) — [ADR-033](../adr/0033-errand.md)

| Permission | Semantics |
|---|---|
| `errand.run` | Running Errand on Soul via `POST /v1/souls/{sid}/exec` ([ADR-033](../adr/0033-errand.md)). Selectors: `host=<sid>` / `coven=<coven-tag>`; bare - unrestricted. **Mutation** - scope-aware gate `RequirePermissionMulti`, selector `host=<sid>` from the path **plus every `coven=` the host carries** (NIM-650, the same `SoulSIDScopeSelector` the three Soul mutations use). |
| `errand.cancel` | Cancel in-flight Errand via `DELETE /v1/errands/{errand_id}` (ADR-033 slice E5). Selectors: `host=<sid>` / `coven=<coven-tag>`; bare - unrestricted. The route gate is an **existence gate** and the scope is resolved in the handler, because the SID is known only after the errand row is read (NIM-841). An errand on a host outside the boundary answers with the same 404 an unknown id gets. |
| `errand.list` | Reading registry Errand (`GET /v1/errands` + `GET /v1/errands/{errand_id}`). Read-only. Selectors: `host=<sid>` / `coven=<coven-tag>` / `trait.<k>=<v>`; bare - unrestricted. |

**The two read routes are scoped, and that took an existence gate** ([NIM-841](../adr/0033-errand.md)). They used to be `RequirePermission(errand, list, NoSelector)`, which made the right all-or-nothing in both directions: a scope-aware gate cannot be asked with an absent host dimension — neither route carries a SID in its path — so `errand.list on coven=dev` was *denied outright*, and the only usable spelling was the bare one, which returned the stdout and stderr of every command run anywhere in the fleet. The gate now asks only whether the right is held; the boundary is resolved where a SID exists. `GET /v1/errands` pushes it into SQL, so `total` and pagination narrow with the rows rather than counting what they hide; `GET /v1/errands/{id}` folds out-of-scope into the same 404 an absent id gets, so the route is not an oracle over which hosts have been commanded. The MCP twins `keeper.errand.list` / `.get` / `.cancel` do the same — closing only REST would have left the same disclosure one JSON-RPC call away. This is the split `soul.console` already uses for `/v1/console/recordings` (§ Console), and for the same reason (ADR-047 §g G1).
...
| Cadence spawn (background) | — | `host=<sid>` + that host's covens, on every resolved host, against the recipe's `created_by_aid` |

`DELETE /v1/voyages/{id}` carries no chi gate, because the required right depends on the Voyage's `kind` and only the loaded row shows it. (It is not alone: `POST /v1/voyages` and `POST /v1/voyages/preview` are mounted with only the Tempo rate-limit for the sibling reason — they pick the permission from the body's `kind` inside `Create`/`Preview`. The three are the whole set of `/v1` routes without a permission middleware.) **The order is the control** ([NIM-846](../adr/0043-voyage.md)): the handler first checks that the caller holds `incarnation.run` OR `errand.run` — the disjunction of what the by-kind check can ask for, so it never refuses anyone the second gate would admit — and answers **403 before the row is read**. Only then is the Voyage loaded, and a by-kind refusal is folded into the **same 404** an absent id gets. Reading first made the pair an oracle: an auditor holding neither right learned from 403-vs-404 whether any ULID they could guess or observe was a live run. An entitled caller still gets a true 404 on a mistyped id, which is why the fold goes this way round rather than turning every not-found into a 403.

**The run-history reads are narrowed by `soul.list`** ([NIM-842](../adr/0043-voyage.md)). `GET /v1/voyages`, `/{id}`, `/{id}/targets`, `GET /v1/push-runs` and `GET /v1/push/{apply_id}` are gated by `incarnation.history` / `push.read` — rights about *runs*, which say nothing about *hosts* — and every one of them returns host identity: a `kind=command` Voyage's `target_origin` names the covens and SIDs it was aimed at, `/targets` returns the resolved SIDs as `target_id`, and a push run's `inventory_sids` is a literal list of machines. A bare `incarnation.history` with no soul right at all therefore enumerated every host any command run had ever touched, covens included whose existence `GET /v1/souls` would have hidden.

> ⚠️ **Grant `soul.list` to your run-history roles before upgrading.** This narrowing is silent and it is not a 403: a role holding bare `incarnation.history` or `push.read` and **no soul right at all** now gets an empty page with `total: 0` from `GET /v1/voyages?kind=command`, `GET /v1/cadences/{id}/runs` and `GET /v1/push-runs`, and a **404** from `GET /v1/push/{apply_id}` — indistinguishable, by design, from "there were no runs". A read-only audit role is the usual shape that breaks. The fix is to add `soul.list` (bare, or scoped to what that role should see) to every role that reads run history. The same applies to `errand.list`: a role carrying a `default_scope` plus a bare `errand.list` inherits that scope now, where before the bare grant returned the whole fleet's errands.

The boundary is the caller's **`soul.list`** purview — the same one `GET /v1/souls` draws — and it is pushed into the query, so `total` and pagination narrow with the rows. Visibility is **all-or-nothing per run**: a `kind=command` Voyage or a push run is shown only if *every* host it names is inside the purview, because showing it means showing its whole target list and the identities on the far side are the disclosure. A `kind=scenario` Voyage names incarnations rather than hosts and is **not** narrowed — `incarnation.history` is the right that governs it. A host that has left `souls` reads as out-of-scope for a coven- or trait-scoped operator (its labels are gone) while still matching a `host=`-scoped one, which resolves over the SID the run recorded.

Note what did **not** change: these routes keep `RequirePermission(…, NoSelector)` on their own gate, unlike the Errand reads above. That is deliberate and not an oversight. `errand.list` had to become an existence gate because it IS the host right for that surface — scoping it was the fix. `incarnation.history` and `push.read` are not host rights at all, so `incarnation.history on coven=dev` stays meaningless and is still refused at the gate; the host dimension is answered by the separate `soul.list` purview resolved in the handler.
...
| `herald.read` | Read one Herald channel (`GET /v1/heralds/{id}`). |
| `herald.list` | List Herald channels (`GET /v1/heralds`). |
| `herald.update` | Replace mutable channel fields (`PUT /v1/heralds/{id}`, replace semantics as Push-Provider). |
| `herald.delete` | Delete channel(`DELETE /v1/heralds/{id}`); cascade demolishes related Tiding subscriptions. |
...
| Permission | Semantics | Audit-event |
|---|---|---|
| `plugin.allow` | Allowance keyed on `(source, ref)` in the allow-list `plugin_sigils`, carrying the registration `alias` as a separate column (NIM-377 / NIM-438) — Keeper reads the active cache slot via the `current` symlink, takes the release's artifacts (`{os, arch, path, sha256}`, one row per platform since NIM-793), signs the whole list and inserts the record. The slot is resolved by the provider for the entry's `kind`: `git` (go-git → `commit_sha` slot, [ADR-026(g)](../adr/0026-sigil.md)) or `artifact` (https + per-file digest check), and for the artifact kind the asserted `source` must equal the address the release was fetched from. **One approval covers a release, not a hash.** Human-verified supply-chain control operation. | `plugin.allowed` |
