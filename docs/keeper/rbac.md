...
`soul.traits-assign` стоит особняком: trait-**ключ** **не является scope-измерением RBAC** (в отличие от Coven-метки), поэтому селектор по traits в грамматике отсутствует, а сам permission авторизуется existence-gate-ом (см. строку каталога выше). RBAC-scope по traits пока не реализован — это отдельный slice; в текущем write-path traits меняются только в пределах coven-scope оператора (единый `BulkScope`).
