...
| `soulprint.self.traits.<key>` | Operator-set key-value of the host label ([ADR-060](adr/0060-traits.md)). **Registry-projection** (not Soul-reported), as `covens`/`choirs`; the keys are dynamic. The **effective** set — the host's own `souls.traits` unioned with the `incarnation.traits` of every incarnation it belongs to ([ADR-080](adr/0080-label-inheritance-union.md)); a key set at both levels holds BOTH values, so prefer `in` over `==` for such keys. Value is a scalar or list: comparison scalar `traits.<key> == <v>`, membership `<v> in traits.<list-key>`. | `soulprint.self.traits.namespace == 'dba-ns'`; `'alice' in soulprint.self.traits.owners` |
