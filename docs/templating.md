...
Standard template engine specification for all YAML expressions (destiny, scenario, service vars, keeper.yml, migration) and for file templates on the host. The solution is recorded in [architecture.md → ADR-010](adr/0010-templating.md).
...
| Interpolation in string contexts (`params:`, `apply: input:`, `on:` literals, `vars:`, `vars/_stack.yaml` expressions, `set:` migrations) | CEL | `${ … }` |
...
> - `failed_when:` → override `failed`: `true` with OK module → FAILED (artificial failure due to business condition, e.g. `failed_when: contains(register.self.stdout, "DEGRADED")` — note that a bare `register.self.exit_code != 0` adds nothing over a verb-shell module, whose accepted codes already default to `[0]`, [destiny/tasks.md](destiny/tasks.md)); **`failed_when: false` with a failed module = ignore_errors** - the status is NOT FAILED (OK/CHANGED), the run is NOT stopped (fail-stop does not work), and the original module error is SAVED as informational (`register.<name>.ignored_error` + `TaskEvent.error` without FAILED status). `failed` takes precedence over `changed` (FAILED overrides CHANGED) — so a task failed by its exit code and then waived by `failed_when: false` ends **OK, not CHANGED**, and its `onchanges:` dependents do not fire.
...
In any YAML string fields (`params:`, `vars:`, `apply: input:`, `on:` literals, `vars/_stack.yaml` expressions, migration `set:`) the CEL expression is wrapped in `${ … }`.
...
| `s.matches(regex) -> bool` | Regex string matching (RE2, stdlib CEL). Member form. | `input.sid.matches("^db-[0-9]+$")` |
| `s.glob(pattern) -> bool` | Shell-glob matching (`*`/`?`/`[abc]`/`[a-z]` via [filepath.Match](https://pkg.go.dev/path/filepath#Match)). Member form. Broken pattern → `false` without error (per-host predicate `target.where` should not fail on a separate host; the syntax validates `soul-lint`). Not available in migration-CEL ([ADR-019], sandbox). | `input.sid.glob("prod-*")` |
...
| `vault(path) -> map / value` | **Keeper-side** reading the Vault KV secret in the CEL-render phase (phase 3, see [§4](#4-yaml-processing-phase-relationship)). Without `#field`, returns the entire map of the secret - the field is taken by CEL access `.field`; with `#field` in the path - one value directly. The real value is substituted into params and goes to Soul; masked only at the output (logs/OTel/UI). `path` is a string literal OR CEL expression from a **trusted** context (`incarnation`/`vars`, not operator-`input`); Resolved by CEL before reading Vault, there is no injection into the Vault query. See [ADR-017](adr/0017-keeper-side-core.md). **The service's own namespace is fenced** ([ADR-0083](adr/0083-declared-secret-state-fields.md) §7): a path under `<mount>/<service>/` is refused - at load when it is written out, on the evaluated path when it is assembled from variables, and post-render for the two `core.vault.*` module params. That prefix is derived from `state_schema` (`type: secret`), not authored. Everything OUTSIDE it is unchanged, which is where the real cross-namespace read lives - a shared TLS CA, another service's credential. | `${ vault('secret/shared/tls-ca').cert }` or `${ vault('secret/shared/tls-ca#cert') }` |
| `generate_secret(policy_map) -> SecretRequest` | **Keeper-side**, render phase. Returns an opaque **`SecretRequest`** marker, **not a value** ([ADR-0083](adr/0083-declared-secret-state-fields.md) §3): no plaintext exists at render time, so none can leak through a register, a log line or a diff. Only a [`core.state.<verb>`](keeper/modules.md#corestateverb) capture step resolves the marker and writes the minted value to its derived path ([ADR-0084](adr/0084-explicit-state-capture.md)). The argument is a **map** because CEL has no keyword arguments and no `=` token - `generate_secret(length=32)` does not lex. Grammar: `length` (8..1024, default 32) + `charset` (`alphanumeric`/`hex`/`base64url`/`ascii-printable-safe`) OR `allowed_chars`; a key outside it is **rejected**, not ignored. The type is opaque - no readable fields, no string form - so `generate_secret({}).length` and `"pw-${ generate_secret({}) }"` are errors rather than something that renders. Ordinary scenario/destiny render pass **only**: refused in migration-CEL, flow-control and service-vars, where nothing would resolve it. Plain function, **not** a macro - `vault()` needs one only to inject its hidden resolver argument. | `${ generate_secret({'length': 32, 'charset': 'alphanumeric'}) }` |
| `merge(m, m...) -> map` ∪ `merge(list(map)) -> map` | Merging maps from left to right by **top-level** key - **SHALLOW** (nested map is NOT merged deeply: the right one completely replaces the matching top key), **last-wins** (the right one overlaps the left one). Pure (no I/O/secrets/crypto). Two forms: **varargs** `merge(m, m...)` (≥1 map argument) and **`merge(list(map))`** (ONE argument-list of maps, flatten from left to right - for a collection of `.map(...)`-comprehension, which must be given to the template as a map for the sake of order determinism, see [§6](#6-data-transfer-between-engines-pipeline)). Non-map argument/element → error; empty list → empty map. Slot for translation "simple typed `input` → detailed config": merge the author's preset with passthrough-`input`-map. Not available in migration-CEL ([ADR-019], sandbox). See [ADR-010 Amendment 2026-06-22](adr/0010-templating.md). | `${ merge(vars.redis_defaults, input.redis_settings) }`; `${ merge(input.users.map(n, {n: input.users[n]})) }` |
| `default(x, y) -> dyn` | Value-or-default: `x` available/present → `x`, otherwise `y`. **Compile-time macro** (as `vault()`): `x` is expanded to `has(x) ? x : y` BEFORE eval, so CEL greed is bypassed - a missing key does not crash the render. Abbreviates [canonical has()-guard](input.md#reading-optional-without-default-input-canonical-has-guard). **Limitation:** `x` - select-chain or identifier (`vars.tls_enable`, `input.shards`, `a.b.c`); argument-expression (`default(size(x), 0)`, `default(a+b, 0)`) → clear compile error (for calculations - ternary). Pure. Not available in migration-CEL ([ADR-019], sandbox). See [ADR-010 Amendment 2026-06-23](adr/0010-templating.md). | `${ default(vars.tls_enable, false) }`; `${ default(input.redis_settings, {}) }`; `int(default(vars.tls_port, 7379))` |
| `soulprint.self.<path>` | Soulprint fields of the current host. | `soulprint.self.os.family == "debian"` |
| `soulprint.self.traits.<key>` | Operator-set key-value of the host label ([ADR-060](adr/0060-traits.md)). **Registry-projection** (not Soul-reported), as `covens`/`choirs`; the keys are dynamic. The host's OWN `souls.traits` and nothing else — belonging to an incarnation attaches no trait ([NIM-281](adr/0008-coven-stable-tags.md#amendment-2026-08-05-nim-281-a-label-is-never-inherited)); `incarnation.traits` labels the incarnation object and is invisible here. Value is a scalar or list: comparison scalar `traits.<key> == <v>`, membership `<v> in traits.<list-key>`. | `soulprint.self.traits.namespace == 'dba-ns'`; `'alice' in soulprint.self.traits.owners` |
...
The `.tmpl` rendering context is **isolated**, not global. text/template does not see `register.*`, `soulprint.*`, `input.*` directly - only the values explicitly raised by the author (`params.vars`) plus a fixed set of system fields.

**Context root** is `{ vars, self, role }` plus **conditional** `input` (referring to something else in the root → strict-mode error). This root collects **Keeper-side, per-host** and delivers next to `template_content` (see [§4](#4-yaml-processing-phase-relationship) and [ADR-012(d)](adr/0012-keeper-soul-grpc.md)); Soul passes it to the engine **root**, so the template accesses `.vars.<name>`, `.self.<path>`, `.role` (and `.input.<name>` when the template reads it).
...
| `role` | declared role from the host's Choir Voice (`incarnation_choir_voices.role`; the former `incarnation.spec.hosts[].role` is removed, [ADR-044 amendment 2026-07-30](adr/0044-choir.md#amendment-2026-07-30-nim-330-spechosts-is-removed-voice-is-the-only-source-of-a-declared-role)). **Bootstrap-create only**: probe is not yet possible; the declared role is NOT used in runtime operations, where the current role is taken by probe + `register.*` ([ADR-008](adr/0008-coven-stable-tags.md)). It may be empty (no Voice, or a Voice with no role). |
...
All YAML sources (scenario, destiny, service vars, keeper.yml, migrations) go through the same processing pipeline. The order is fixed, the phases do not mix.
...
3. **CEL-render** (Keeper). Top-level expression key `where:` (target resolution) and all interpolations `${ … }` (params/vars/on) are calculated. Non-string results are substituted according to the rule [§5](#5-non-string-cel-result-in-yaml). Here the **CEL function `vault(path)`** ([§2.3](#23-registered-cel-functions-starting-minimum)) is resolved - keeper-side reading of Vault KV: the real value of the secret is substituted in params and goes to Soul (it is masked only at the output - logs/OTel/UI). Unlike `vault:`-ref (phase 1, static string), `vault(...)` accepts the path as a CEL expression from the trusted context and is accessible in any `${ … }` cell. External access (Vault) - entirely Keeper-side. Flow-control keys `when:`/`changed_when:`/`failed_when:` are **NOT calculated here** - Keeper pulls them into `RenderedTask` as CEL strings (eval - phase 4a, Soul); Keeper also collects `flow_context` (see below).
   - **Template delivery + context root assembly** (Keeper, between phases 3 and 4). For step `core.file.rendered` Keeper: (1) reads the literal content of `templates/<path>.tmpl` from the snapshot (two-level resolve scenario-local→service-level, [ADR-009](adr/0009-scenario-dsl.md)) and puts it in `params.template_content`; (2) collects the **per-host** root of the text/template context `{ vars, self, role }` ([§3.2](#32-render-context)) and puts it in `params.render_context`. The path-key `template` and the flat `vars`-key from params are **removed** (Soul only reads the root from `render_context`). text/template here **not executed** - Keeper delivers the template as-is. A1-option: both `template_content` and `render_context` ride inside `RenderedTask.params`, without proto changes. `render_context` host-variant (`self` per-host) - it is excluded from the per-host params host-invariance check.
   - **Build `flow_context`** (Keeper, for each task). A literal per-host snapshot of the non-register part of the CEL context of the flow-control predicates `{ input, vars, incarnation, self }` (minus `soulprint.hosts` and loop) is placed in `RenderedTask.flow_context`. `register.*` is NOT included in it - Soul builds it itself in phase 4a. Host-variant (`self` per-host), excluded from per-host-verification of host-invariance params.
     - **Scoped to what this task's predicates read** (NIM-813). The three operator-supplied sections `input`/`vars`/`incarnation` are narrowed, per section, to the fields the task's OWN four predicates (`when:`/`changed_when:`/`failed_when:`/`retry.until`) name - so **a task with no predicate carries none of them at all**, since Soul never opens the snapshot without one. It used to be the whole context, which put every `secret: true` field of the run input on the wire to every targeted host on every task, and made one compromised host worth the whole run's secrets rather than the ones its own tasks used. The set comes from the predicates' **raw text** (AST, `cel.Engine.PredicateReads`), never from resolved values: a set derived from what a value turned out to be would differ between hosts, and a host-variant `flow_context` is what the two fail-closed circuits below exist to reject. A read the AST holds no field name for - bare `input`, `input['k']` - widens back to the whole section rather than dropping it, the same rule targeted file-vars injection follows for `.vars` read as a map (§3.2). `self` is NOT narrowed: it is the receiving host's own facts, and both circuits are defined against its presence.
...
4a. **flow-control CEL** (Soul, gating BEFORE Apply + override AFTER Apply). Before `module.Apply`, Soul evaluates the predicate `when:` with a stripped-down cel-go sandbox (`shared/cel.NewFlowControl`, [§7.1](#71-cel--sandbox-by-design)). Activation: `register.*` (results of previous tasks of running by register name, Soul builds itself) + `flow_context` from Keeper (`input`/`vars`/`incarnation` top-level, `soulprint.self` ← `flow_context.self`). `when:false` → task SKIPPED (Apply is not called); connection with `onchanges:` - AND (executed only with `when && onchanges-satisfied`). **Local compute without I/O**: `vault()`/`now()`/`soulprint.hosts`/`soulprint.where` in the sandbox are not structurally available.
...
>
> 1. **Direct reference to `soulprint.self` in the predicate text** - regex-guard is cut off by the text `when:`/`changed_when:`/`failed_when:`. A link to `soulprint.self` is valid **only** for a single-host target; on multi-host the render fails-closed. Host-invariant links (`register.*`/`input.*`/`vars.*`/`incarnation.*`) always work. Symmetrical to constraint `loop.when` (`reLoopWhenSoulprint`).
> 2. **Derived host-variable `vars`** - first circuit bypass: the value `vars`, derived from `soulprint.self` (for example `vars: { is_debian: "${ soulprint.self.os.family == 'debian' }" }` + `when: vars.is_debian`), flows into `flow_context.vars`, and the text of the soulprint predicate does not contain - regex-guard does not catch it. It is caught by checking the collected `flow_context`-**minus-`self`** between targeted hosts: `input`/`incarnation` host-invariant by construction, `self` host-variant by nature (it is covered by circuit 1), `vars` remains - if it differs between hosts, the render fails-closed. Reconciliation is active only if there is a non-empty flow-control predicate (without it, Soul `flow_context` does not read; host-variant `vars`-in-params without `when` crashes on a separate params host-invariance check). **All FOUR predicates count for that gate**, `retry.until` included (NIM-813): Soul evaluates `until:` against the same snapshot through the same engine, and while the gate counted three, a task whose only predicate was `until: vars.is_debian` evaluated every host against the FIRST host's value with neither circuit objecting - circuit 1 misses it too, since the predicate text carries no `soulprint`. Circuit 1's regex still reads only the three: a host-variant `until:` naming `soulprint.self` DIRECTLY on a multi-host target is not yet refused.

> **Caveat (path-injection into `vault()`).** The function path `vault(path)` must come from a **trusted** context (literal, `incarnation`, `vars`). A path concatenated from operator-`input` - for example `vault('secret/' + input.tenant + '/db')` - allows the operator to point `vault()` to an **arbitrary** KV secret, which according to the contract he should not have access to. This is a **contractual assumption, not a bug**: according to the accepted option (a), the responsibility for ensuring that the path is not derived from `input` lies with the scenario/destiny author, plus RBAC on the secret path in the Vault itself (narrow Keeper token policies). Statically prohibiting input-derived paths in `vault()` is a separate task (if security deems it necessary to strengthen it). There is no text injection into the Vault request: the path is a CEL value calculated before `ReadKV`, and not a string concatenation into the Vault protocol. **Partially closed since [ADR-0083](adr/0083-declared-secret-state-fields.md) §7**, and it is worth being exact about how much: the own-namespace fence refuses any path under `<mount>/<service>/` however it was assembled, because the evaluated-path guard runs on the value CEL produced, not on the text the author wrote. That covers the case where the operator steers `vault()` at *this* service's derived secrets. It does **not** cover steering it anywhere else - `vault('secret/' + input.tenant + '/db')` reaching another service's namespace is still the author's responsibility, plus Vault-side RBAC, exactly as this caveat says.
...
redis_config: "${ merge(vars.redis_defaults, input.redis_settings) }"
...
1. **CEL in YAML (Keeper)** calculates the `params.vars` values of the `core.file.rendered` step. CEL has a full scenario context (`input`, `register`, `soulprint`, `vars`). Keeper delivers the literal `.tmpl` to `params.template_content`.
2. **text/template in `.tmpl` (Soul)** gets the root `{ vars, self, role }` + **conditional** `input` ([§3.2](#32-render-context)): `vars` - CEL-rendered `params.vars` (derived values), `input` - resolved operator-input of the pass (Option B: the template reads `.input.<name>` directly; the key is placed by the Keeper **only when the template accesses it**, detection by parse-AST), `self`/`role` - system fields collected by the Keeper per-host (`render_context`). The text/template context **does not** contain direct access to `register`/`soulprint.hosts` - only operator-`input`, explicitly raised by the author of `vars` and a narrow system set. Soul renders `template_content` locally, without accessing external systems.
...
3. **Isolated render context** ([§3.2](#32-render-context)) - no global access to `register`/`soulprint.hosts` and no call to external systems; Exactly the fields of the root `{ vars, self, role }` are available (+ conditional `input` of the pass when the template accesses it) + explicitly raised `vars` + narrow system set collected by Keeper per-host.
...
CEL processes values with `secret: true` (from `input:`, [docs/input.md](input.md)) **as usual** - without special processing inside the engine. This is critical: a legitimate "pass secret to `params: { password: \"${ input.password }\" }`" case should work. CEL has no right to refuse.
...
1. **schema** (declarative, primary) - The field is masked if its path is declared `secret: true` in the active source schema (`input_schema` / `state_schema` / manifest `InputParamDef.Secret`). Does not depend on guessing by key name. On the read-path (`GET /v1/incarnations/...` spec/state/history) Keeper materializes the service secret schema (`state_schema` + scenario input schema `create`) and masks along declared paths (`audit.MaskSecretsWithSchema`). The walk enters a `state_schema` at its **root field map** — a `state_schema` carries no root `properties` wrapper, being written in the input DSL like an `input:` block ([ADR-0086](adr/0086-one-schema-dialect.md)) — and from there descends recursively through the `properties` and `items` of nested nodes and through **nested** `properties` under `additional_properties`. ⚠ **Not implemented:** the walk today still expects a root `properties` and the camelCase spelling `additionalProperties`; realigning it to the dialect above is part of **NIM-742**. **Limitation (TODO with a separate slice):** `secret: true` on the `additional_properties` node ITSELF (the value of an arbitrary map secret key) read-path schema layer **NOT covered** - `SecretPathSet.IsSecret` does not substitute the `.*` segment. Such a leaf degrades to the vault-origin + regex-last-resort layers (below).
...
3. **seal / sealed-paths** (render-time provenance/taint, central mechanics) - Keeper in the render phase marks the path of the cell `params` **sealed** when its CEL expression reads the secret source:
   ...
   - `vault(...)`;
   - transitive - `vars.<x>`/`compute.<x>`, whose value is itself sealed. The taint is derived from the **RAW** `vars:`/`compute:` text before params are walked, never from the resolved value: `compute:` resolves once per run, but a task's `vars:` resolve per task **per host**, while the params walk runs once per task (a cell's provenance is host-invariant). Two of the three layers of the flat `vars.*` namespace ([ADR-0082](adr/0082-service-vars.md)) are walked - a destiny's `vars.yml` locals and a task's own `vars:`. The **service** layer below them is exempt structurally, not by omission: its engine (`cel.NewServiceVars`) declares neither `input` nor `vault()`, so a service var has no secret source to read. The lower layer is carried down whole, **shadowed names included**, for the sake of the ones it does not shadow - a `vars.yml` local no task var redeclares is still `vars.<x>` in that task's params, and its mark exists nowhere else. For a shadowed name that over-seals, and it is left rather than subtracted: a shadow cannot carry the lower value today (a task's `vars:` layer sits on the SERVICE layer alone, so `pw: "${ vars.pw }-suffix"` derives from a layer with no secret source, or is `var_cycle` when `pw` is only a file var), and the seal should not be what has to notice if that relaxes;
   - a **`loop:` bind**, tainted WHOLE (NIM-822 / NIM-823). A loop variable is bound at the top level of the activation under whatever name `as:` gave it (default `item`), so it can never be a fixed root; the taint is decided from the RAW `items:` expression, and if that reads a secret then every `${ <as>.<field> }`, and a bare `${ <as> }` too, is sealed for every iteration. Deciding it from `items:` rather than from the resolved element keeps it iteration-invariant, matching a collection that runs once per task - at the cost of tainting all iterations when only some elements came from a secret, which is the same safe direction taken elsewhere here. **`index_as:` is never tainted**: it binds a position, not a value out of `items`;
   - **`incarnation.state.<field>`** declared secret in the manifest's `state_schema` (NIM-826). ⚠ The two state markers are NOT the same and neither replaces the other ([ADR-0083](adr/0083-declared-secret-state-fields.md) §1): `secret: true` on a state property means the value **lives in state**, so `${ incarnation.state.<field> }` renders plaintext and only this layer marks it; `type: secret` keeps the value in Vault and the state merge (`config.StripDeclaredSecrets`, called from `stateop.Merge`) strips it from the record, so it is normally not there to read. The address is per **field**, not the whole `state` map: `selectBaseField` flattens the two levels the way it already does for `register.hosts.<name>`, because sealing the map would seal every cell reading any state field there is, and a mask covering everything says nothing. A secret nested deeper (`tls.key`) is addressable only at its top segment (`tls`) and seals that subtree whole - the same subtree rule a resolved `vault:` map follows. ★ **The producer therefore decides per ADDRESS, not per marker** (`incarnation.StateSchemaSecretFields`): a declaration is taken when the address the seal can spell is no wider than the declaration. `secret: true` always - the value really is in the record. `type: secret` only where the fold is the identity: a top-level scalar is exact and costs nothing, while the array-items shape (`redis_users[].password`) folds to the whole `redis_users` collection - public, read by every day-2 scenario, and taking it would blank the ACL inventory out of every run plan the service writes. The schema-layer mask above can afford every path because it addresses each one exactly; the seal cannot, and the difference is the rule. The name list is the manifest's, not the snapshot's: a field the state does not carry yet must still seal the cell that reads it after a capture. Both render paths are covered - a keeper task's own incarnation root (`keeperVars`) is built from the same snapshot as a host task's;
...
★ **The source list above is a set of ADDRESSES, not a switch over root names** (NIM-822). Detection asks whether a binding's address is tainted - `<root>.<field>`, or the bare name when the whole binding is - and names no root of its own. The previous shape had one field per fixed root and a `switch` that answered "not a secret" for anything it did not name, which is how three hops (`vars:`, `compute:`, the `loop:` bind) stayed open at once: a new kind of binding was **silently unsealed by default**. What cannot be derived away is somebody computing that a given binding holds a secret; that obligation now sits in one classification table covering every name the CEL activation can put in scope, and a test compares it against the real activation, so an unclassified binding **fails the build** instead of leaking quietly.

⚠ **Two stated bounds, both open, both pinned by test so they are visible rather than assumed.** Each is a property of every per-field address base, not of one root, which is why neither was closed for `incarnation.state` alone:

- **a whole-map read is NOT sealed** - `${ input }`, `${ vars }`, `${ register }`, `${ incarnation }`, `${ incarnation.state }`. Such a read reaches no `<root>.<field>` pair, so no address matches, and under the non-string rule the cell receives the entire map with whatever secrets are in it. `${ incarnation }` is the widest of them: the root carries `state` whole. Pinned for `input` (`TestDetectSealed_BareFixedRootIsNotWholeTainted`) and for both incarnation spellings (`TestDetectSealed_IncarnationStateWholeMapIsItsOwnQuestion`); `${ vars }` and `${ register }` are the same node kind and are not separately pinned. Open as **NIM-827**;
- **an INDEXED field read is NOT sealed** - `${ input["password"] }`, `${ incarnation.state["db_password"] }`. CEL treats `x.y` and `x["y"]` as the same read, but the second is a `Call` node and the detector only reaches `Select`. The index form IS closed in the OPERAND position (`${ incarnation["state"].db_password }` seals), which is what makes the field position easy to assume covered. Pinned by `TestDetectSealed_IndexedFieldReadIsItsOwnQuestion`; open as **NIM-830**.

Both are widenings rather than bug fixes - closing the first seals a bare read for every scenario declaring one secret anywhere under the root, and closing the second has to decide what a non-literal index (`${ input[key] }`) does.
...
Detection - **AST traversal** expressions (not single-ident match): ternary `has(input.tls_cert) ? input.tls_cert : ''` traverses all branches - any one reads the secret → cell sealed (whole-cell taint); gluing `literal + ${ secret }` → the entire result is sealed (whole-value taint, safe). A set of sealed paths (in-memory, current run) is attached to the result of the render pass and brought to the write points of the run (`status_details` / `error_summary` / logs), where masking occurs according to provenance (`audit.MaskSecretsSealed`). The metaphor is "sealed paths" (see [naming-rules.md → seal/sealed-paths](naming-rules.md#domain-entities)).
   ...
   > **Destiny pass (S-1) - CLOSED 2026-09-05 (NIM-812).** The S-1 mechanism takes secret names from the **active pass** input schema, and a destiny pass now has one: `renderApplyDestiny` carries the destiny's own `input:` onto the synthetic manifest it builds, so `secretInputNames` answers about the destiny's contract rather than about nothing. A destiny template reading `.input.<secret>` is therefore sealed by S-1, and so is a destiny task cell reading `${ input.<secret> }` by ordinary AST provenance. The schema is the destiny's OWN, never the caller's - the caller's names do not exist inside a destiny (destiny isolation, [ADR-009](adr/0009-scenario-dsl.md) V2), and `apply: input:` is the only channel across.
   >
   > Previously the destiny-input secret flag did not cross that boundary, leaving those fields to vault-origin (only when the value was a `vault:` ref) and regex-last-resort (only when the key name matched `sensitiveKeyRe`) - and since `apply: input:` is the one way to hand a destiny a credential, that was the shape the DSL steered an author toward.
...
- **`core.http.probe` / `core.http.request` → `headers`** (`map[string]string`). Same category and same reason as `core.url`: request headers carry `Authorization`/`Cookie`/API tokens, notably Consul's `X-Consul-Token`. The output contains only the sorted list of effective header keys (`headers_keys`; for `request`, this also includes `Content-Type` supplied through `content_type`); values and the `headers` block itself are excluded by construction from output/log/audit. The response `body` is **not** sensitive in its entirety — it goes through the usual vault-ref masking. The mutating request body param is likewise never echoed; `output.body` is always the response body.
