...
### [ADR-060. Trait — operator-set key-value метки на Soul](adr/0060-traits.md)

Вынесен в [`docs/adr/0060-traits.md`](adr/0060-traits.md). **Trait** — operator-set key-value метки на Soul (значение scalar\|list, напр. `namespace: dba-ns` / `owners: [alice, bob]` / `product: aboba`), **отдельная ось рядом с плоским Coven** (Вариант B; расширение `souls.coven` до key-value отвергнуто — ломало бы [ADR-008](#adr-008-coven--только-стабильные-логические-теги)-семантику плоской метки, RBAC scope-pushdown `$1 = ANY(coven)` ([ADR-047](#adr-047-purview--scoped-rbac-видимость-узлов-role-default_scope--расширенный-селектор)) и предикаты `'x' in soulprint.self.covens`). Хранение — `souls.traits jsonb NOT NULL DEFAULT '{}'` + GIN-индекс; `souls.coven TEXT[]` не трогается. Проекция в `soulprint.self.traits` — registry-проекция (как `covens`/`choirs`, [ADR-018](#adr-018-soulprint-typed-схема-mvp)); `SoulprintFacts` proto и registration-контракт Soul→Keeper **не меняются** (Trait — operator-данность, не Soul-reported). Таргетинг — `where: soulprint.self.traits.<key>` (`soulprint` объявлен CEL `DynType`, [ADR-010](#adr-010-шаблонизатор-cel-для-yaml-выражений-go-texttemplate-для-файлов) — ключ резолвится динамически, без AST-rewrite): `traits.namespace == 'dba-ns'` / `'alice' in traits.owners`. Источник — оператор (Soul-reported отвергнут: противоречит [ADR-008](#adr-008-coven--только-стабильные-логические-теги)/[ADR-018](#adr-018-soulprint-typed-схема-mvp), Trait = орг-метка владельца, не факт хоста). Мутабельность как у Coven. **RBAC-scope по traits — ОТЛОЖЕН** (пилот: только таргетинг + метаданные). **Amends [ADR-008](#adr-008-coven--только-стабильные-логические-теги) / [ADR-018](#adr-018-soulprint-typed-схема-mvp).**
