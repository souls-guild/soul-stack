...
- `keeper.incarnation.rerun-create name=X reason="..."` (REST: `POST /v1/incarnations/{name}/rerun-create`, permission `incarnation.create-rerun`) — атомарно снимает `error_locked` (state не трогается, last-known-good, snapshot в `state_history`) и тем же действием перезапускает **сценарий первичного создания инкарнации** — `created_scenario`, зафиксированный при её создании (это `create` для bootstrap-инкарнаций, но может быть и другой create-вариант, например `create_from_souls`). Для bare-инкарнации без `created_scenario` rerun-create отклоняется `409`. Под одним `FOR UPDATE`: переход `error_locked → applying` минуя `ready` — исключает окно, в котором конкурентный прогон проскочил бы в освободившийся `ready`. Перезапуск стартует в режиме «по зарезервированному `applying`» (`RunSpec.FromLocked`): runner НЕ транзитит статус повторно, а верифицирует, что строка именно в `applying` — fail-closed, иной статус отклоняет старт, не применяя create-сценарий поверх перехваченной recovery строки. Инвариант «оператор явно подтвердил» сохраняется обязательным `reason` + confirm в UI; audit-событие `incarnation.create_rerun` (НЕ переиспользует `incarnation.unlocked`). Scope ЖЁСТКО ограничен `created_scenario` инкарнации (rerun первичного создания); для прочих случаев — обычный `unlock` + ручной повторный run.
