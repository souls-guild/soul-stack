...
Вынесен в [`docs/adr/0059-audit-sink-pluggable.md`](adr/0059-audit-sink-pluggable.md). **Статус proposed / deferred — design-only, кода в бете нет** (план пост-беты; синхронная PG-запись audit на 100k VM не масштабируется — решение пользователя выгружать в Kafka под тумблером, без in-app предпросмотра). Backend audit-выгрузки — выбор **реализации** существующей абстракции `shared/audit.Writer` (новый интерфейс не вводится; `MultiWriter`-tap [Herald](adr/0052-herald-notifications.md#adr-052-herald--tiding--уведомления-о-событиях-прогонов) сохраняется), врезка — `setupAudit` в [`keeper/cmd/keeper/daemon.go`](../keeper/cmd/keeper/daemon.go). Конфиг `keeper.yml → audit.sink: pg | kafka | off` (default **`pg`** = текущий PG-`audit_log` [ADR-022](adr/0022-audit-pipeline.md#adr-022-audit-pipeline-storage-schema-retention), обязательный контур [ADR-053](adr/0053-dependency-tiers.md#adr-053-tier-ы-инфраструктурных-зависимостей) цел; `kafka` — новый sink с `brokers`/`topic`/`acks=all`, секреты vault-ref; `off` — осознанный no-выгрузки, внятный лог). Обе реализации в `keeper/internal` — `shared` остаётся **pgx-free и Kafka-free** ([ADR-011](adr/0011-go-layout.md#adr-011-раскладка-go-кода-gowork-с-модулями-по-сторонам)). Гарантии: at-least-once `acks=all`, деградация при недоступности Kafka — **fail-closed** (audit compliance-критичен, терять события нельзя), дедуп downstream по `audit_id` (ULID PK). Гарантия записи — два кандидата (открыто): transactional outbox (сильнее, но PG-write остаётся) vs direct-producer + durable-fallback. Tier: Kafka строго **OPTIONAL-with-degradation**, НЕ 4-й required ([ADR-053](adr/0053-dependency-tiers.md#adr-053-tier-ы-инфраструктурных-зависимостей)). Переключение sink — **restart-required** (паттерн `web_ui_enabled` [ADR-055](adr/0055-embed-ui-bundle.md)). Вытесняет Redis-Stream-вариант backlog-а audit-scaling; batched-INSERT остаётся (более дешёвая альтернатива). **Hard-зависимость ДО реализации (open question):** `changed_tasks`/`incarnation.run_completed` ([ADR-052 §k](adr/0052-herald-notifications.md#k-терминальное-событие-прогона-incarnationrun_completed-несёт-changed_tasks)) и `GET /v1/audit` сегодня деривят данные SQL-запросом по `audit_log` в PG — при `sink: kafka` без PG они молча ломаются (Herald-уведомления + audit-чтение); нужен альтернативный источник событий (in-memory-агрегат прогона / гибрид sink / Kafka-проекция). Рабочее имя `audit sink`; тематическое **Chronicle** — кандидат-альтернатива, в [naming-rules.md](naming-rules.md) НЕ зафиксирован. **Amends [ADR-022](adr/0022-audit-pipeline.md#adr-022-audit-pipeline-storage-schema-retention)** (указатель: PG остаётся default + source-of-truth ПОКА) **/ [ADR-053](adr/0053-dependency-tiers.md#adr-053-tier-ы-инфраструктурных-зависимостей)** (OPTIONAL-строка).

### [ADR-060. Trait — operator-set key-value метки на инкарнации (релоцировано с Soul)](adr/0060-traits.md)

Вынесен в [`docs/adr/0060-traits.md`](adr/0060-traits.md). **Trait** — operator-set key-value метки (значение scalar\|list, напр. `namespace: dba-ns` / `owners: [alice, bob]` / `product: aboba`), **отдельная ось рядом с плоским Coven** (Вариант B; расширение `souls.coven` до key-value отвергнуто — ломало бы [ADR-008](#adr-008-coven--только-стабильные-логические-теги)-семантику плоской метки, RBAC scope-pushdown `$1 = ANY(coven)` ([ADR-047](#adr-047-purview--scoped-rbac-видимость-узлов-role-default_scope--расширенный-селектор)) и предикаты `'x' in soulprint.self.covens`). **R1 (2026-06-25): Trait РЕЛОЦИРОВАН per-soul → per-incarnation.** Источник истины — `incarnation.traits jsonb NOT NULL DEFAULT '{}'` + GIN (миграция `088`, зеркало `incarnation.covens`/`046`), operator-set в `incarnation.spec.traits` при create. Проецируется **МАТЕРИАЛИЗОВАННО** в `souls.traits` хостов-членов через **sync-hook** (`incarnation.SyncTraitsToHosts`, переиспользует `soul.BulkReplaceTraits`; врезка — incarnation-create + bind хоста `core.soul.registered`). `souls.traits` (`087`) ОСТАЁТСЯ как **projection target**: read-слой переиспользуется без изменений — проекция `soulprint.self.traits` (registry, как `covens`/`choirs`, [ADR-018](#adr-018-soulprint-typed-схема-mvp); `SoulprintFacts` proto и registration-контракт Soul→Keeper **не меняются**), `topology.HostFacts.Traits`, soul-lint whitelist `traits`. Таргетинг — `where: soulprint.self.traits.<key>` (`soulprint` объявлен CEL `DynType`, [ADR-010](#adr-010-шаблонизатор-cel-для-yaml-выражений-go-texttemplate-для-файлов) — ключ резолвится динамически, без AST-rewrite): `traits.namespace == 'dba-ns'` / `'alice' in traits.owners`. **Переходное состояние:** per-soul bulk-write `POST /v1/souls/traits` (+ MCP `soul.traits-assign`) ещё работает на `souls.traits` напрямую, НО перетирается проекцией при следующем sync `incarnation.traits` — ожидаемо до relocate per-soul bulk → per-incarnation (+ deprecate, следующий слайс). **RBAC-scope по traits на incarnation-измерении — РАЗБЛОКИРОВАН** R1 (реализация — следующий слайс). **Amends [ADR-008](#adr-008-coven--только-стабильные-логические-теги) / [ADR-018](#adr-018-soulprint-typed-схема-mvp).**

### [ADR-061. Единый-прогон provision→онбординг→роль: onboarding-await + mid-run re-resolve roster](adr/0061-onboarding-await-and-midrun-reresolve.md)
...
### [ADR-063. core.bootstrap.delivered — keeper-side доставка bootstrap-токена по SSH](adr/0063-bootstrap-token-delivery.md)

Вынесен в [`docs/adr/0063-bootstrap-token-delivery.md`](adr/0063-bootstrap-token-delivery.md). Новый keeper-side core-модуль **`core.bootstrap.delivered`** (`on: keeper`) тонкой доставки per-VM bootstrap-токена по SSH на свежесозданные cloud-init-VM. **Закрывает BUG#2 cloud-provision**: заглушка-адрес `keeper.push.applied` keeper-side не существовала (нет такого модуля) → созданная VM ([ADR-061](#adr-061-единый-прогон-provisionонбордингроль-onboarding-await--mid-run-re-resolve-roster)) не получала токен → CSR-онбординг не стартовал → барьер `await_online` не набирал presence → `error_locked`. **Дизайн A1 «тонкая доставка»:** cloud-init (B-flat, [ADR-017(h)](#adr-017-keeper-side-core-модули-расширены-corecloudprovisioned-corevaultkv-read)) уже поставил soul-бинарь+CA+systemd-unit; модуль кладёт **ТОЛЬКО** токен (`/etc/soul/token`, umask 077, chmod 0400, **★ токен в STDIN, не в argv** — иначе утечёт в `ps`/audit на VM) + опц. `systemctl start soul`. Поток per-host (**последовательно**): `SshProvider.Authorize` (deny → fail-closed) → ephemeral ed25519-keypair + `Sign` → `push.Dial` (CA-signed host-cert verify) → write token (STDIN) → опц. start. **B1-strict** (ошибка любого хоста → шаг `failed` → state не коммитится → `error_locked`). Params: `hosts` (`${ register.<provision>.hosts }` от `core.cloud.created`), `ssh_provider`, опц. `token_path`/`ssh_user`/`ssh_port`/`start_soul`. Output `register.<name>.hosts[]={sid,delivered,started}`+`count` — **БЕЗ токена**; audit `bootstrap.delivered` — `{action, ssh_provider, count, sids}` без токенов (параллель cloud.provisioned-маскинга). **Переиспользует** [`keeper/internal/push`](keeper/push.md) (ephemeral keypair / Sign / Dial / Session — тот же путь, что `SshDispatcher.SendApply`). Регистрация в `coremod` **условна** (как `core.choir` при `ChoirStore`): при полном наборе SSH-deps (provider-карта + host-CA из Vault + dialer), иначе шаг падает «unknown keeper-side module». **Границы MVP:** один key-based SshProvider, только токен, хосты последовательно. **★ C1 — cloud-init CA-signed host-key (required-для-live, СЛЕДУЮЩИЙ слайс):** `push.Dial` доверяет только host-cert, подписанному host-CA (отказ от TOFU) — свежая VM обязана иметь CA-signed host-key, иначе handshake реджектится и доставка падает на connect-е; до C1 модуль валиден на render (L0 Trial) + unit-тестах, но live-e2e не пройдёт. **Amends [ADR-017](#adr-017-keeper-side-core-модули-расширены-corecloudprovisioned-corevaultkv-read) / [ADR-061](#adr-061-единый-прогон-provisionонбордингроль-onboarding-await--mid-run-re-resolve-roster) / [ADR-015](#adr-015-core-модули-mvp).**

### [ADR-064. Secret write-path — приём plaintext-секрета от оператора, запись в Vault keeper-side](adr/0064-secret-write-path.md)

Вынесен в [`docs/adr/0064-secret-write-path.md`](adr/0064-secret-write-path.md). Dual-mode приёма секрета в Herald- и Provider-CRUD: оператор передаёт **`secret`** (plaintext) **XOR** **`secret_ref`** (vault-путь, текущее поведение) — в UI radio «значение / путь». При plaintext Keeper **сам** пишет секрет в Vault по детерминированному пути `secret/<domain>/<entity>/<field>` (`vault.Client.WriteKV` — тот же, что `sigil.Introduce` / cert `issueMaterial`) → в Postgres кладёт только внутренний ref `vault:<path>#<field>` (как sigil/warrant); plaintext **нигде не персистится** (только Vault). При `secret_ref` — поведение как сейчас (Keeper в Vault не пишет). **Обобщение уже существующего keeper-side write-path** (`sigil.Introduce` / cert `issueMaterial` / `core.vault.kv-present`) на новый случай — приём plaintext-а **ОТ оператора** (не генерируемого системой), не новый инфра-код. **★ Security trade-off (осознанное послабление):** plaintext по проводу оператор→Keeper ломает предпосылку «секрет не покидает Vault» ([requirements.md](requirements.md)); послабление ради UX **подтверждено пользователем (2026-07-01)** ПРИ обязательных митигациях-блокерах: **(a)** TLS-транспорт обязателен; **(b)** строгий masking всех sink (логи/audit/OTel/UI) + guard-тесты на leak — поле `secret` под `shared/audit.sensitiveKeyRe`, но **huma-request-body НЕ авто-masked** → явный аудит точек логирования тела; **(c)** plaintext не-персист; **(d)** vault-policy Keeper-а с write-префиксами `secret/herald/*`/`secret/provider/*`. `update` перезаписывает по стабильному пути (idempotent). RBAC — **переиспользует** `herald.create`/`provider.create` (нового permission нет). Имя — **DevOps-поле `secret`**, без нового паттерна словаря. Scope MVP = **Herald** (webhook-secret + channel-token) + **Provider** (cloud creds); TLS-PEM оператора **отложен** (ref в essence → тянет миграцию essence→PG). Отвергнуто: имя-паттерн `Consign`/`Entrust`; `oneof`-форма контракта; permission `secret.write`; ULID-immutable path. **Статус: accepted, реализация pending** (фиксация решения документом). **Amends [ADR-052](#adr-052-herald--tiding--уведомления-о-событиях-прогонов) / [ADR-017](#adr-017-keeper-side-core-модули-расширены-corecloudprovisioned-corevaultkv-read).**

### [ADR-065. core.module.installed — доставка SoulModule-плагинов на Soul-хост (FetchModule + plugins.soul_modules)](adr/0065-core-module-installed.md)

Вынесен в [`docs/adr/0065-core-module-installed.md`](adr/0065-core-module-installed.md). Канонический канал доставки custom-модулей (`soul-mod-*`) на Soul-хост; **закрывает open Q №5** «где живёт реестр модулей в Keeper-е». **Транспорт** — третий RPC в `service Keeper`: server-streaming **`FetchModule(PluginFetchRequest) returns (stream PluginChunk)`** (тот же mTLS-listener, что EventStream; байты артефакта едут **отдельным HTTP/2-стримом** и не душат control-plane; **content-addressed** — Keeper отдаёт ТОЛЬКО байты, чей sha256 есть в активном Sigil-допуске `kind: soul_module`; авторизация — mTLS peer-cert; guard-rails: `plugins.max_artifact_size_mb` + rate-limit параллельных fetch per-SID; only-add по [ADR-012(c)](#adr-012-контракт-keepersoul-grpc-один-eventstream-с-oneof-keeper-side-рендер-forward-compat-only-add)). **Реестр байтов** — config-каталог **`plugins.soul_modules[]`** (`{name, source, ref}`, симметрия `cloud_drivers`/`ssh_providers`) + существующий plugingit-резолв в ФС-кеш + Sigil-допуск (реюз [ADR-026](#adr-026-sigil--целостность-плагинов-keeper-signed-digest-индекс) без изменений); **нового хранилища НЕТ** — PG = допуски (authority sha256), ФС = байты, git = происхождение ([ADR-007](#adr-007-версионирование-артефактов--через-git-ref-а-не-через-поле-в-манифесте)); HA — per-instance ФС-кеш, miss слота → on-demand резолв или отказ с retry; S3-artifact-store — post-GA расширение за fetch-абстракцией. **Params `core.module.installed`** (Soul-side, state `installed`): `name` (required, `<ns>.<name>`), `ref` (opt, **pin-сверка** — активный допуск обязан быть на этом ref, иначе `failed`; НЕ выбор версии); идемпотентность — sha256 существующего бинаря == sha активного Sigil → `changed=false`. **hot-register ОБЯЗАТЕЛЕН в MVP** (thread-safe Rescan без рестарта демона — иначе `community.redis.*` в ТОМ ЖЕ прогоне после install-шага не работают; рестарт = обрыв EventStream = обрыв прогона); beacon-реестр при rescan НЕ пересобирается (post-MVP). **Scenario** — amendment 2026-07-03: Keeper **синтезирует** Soul-side install-шаги из явной manifest-декларации `service.yml::modules[]` (scenario-runner, после раскрытия include ДО Stratify; вставка перед первым потребителем модуля; явный шаг `core.module.installed` с тем же литеральным `params.name` отключает синтез — оператор управляет позицией/ref/when сам); validation-hint post-MVP сохраняется в обратном направлении (модуль используется в задачах, но не задекларирован в `modules[]`). **Sigil-verify install-time:** allow-check ДО fetch (нет допуска → `module_not_allowed` до единого сетевого байта) + полный verify (подпись + `manifest_sha256`) перед atomic rename; manifest материализуется из `PluginSigil.manifest_raw` (не через fetch); реюз `shared/pluginhost`, новых trust-механизмов НЕТ. Раскладка кеша Soul — **каталожная** `<paths.modules>/<ns>-<name>/{manifest.yaml, soul-mod-<name>}`. TaskError-reasons: `module_not_allowed` / `module_fetch_failed` / `module_verify_failed`. Границы MVP: без `absent`-state (TTL-cleanup), ~~без auto-inject~~ (снят amendment-ом 2026-07-03 — авто-синтез из `modules[]`), без beacon-hot-reload. **Amends [ADR-012](#adr-012-контракт-keepersoul-grpc-один-eventstream-с-oneof-keeper-side-рендер-forward-compat-only-add) / [ADR-020](#adr-020-plugin-инфраструктура-формат-manifest-handshake-lifecycle) / [ADR-015](#adr-015-core-модули-mvp-точный-список).**

### [ADR-066. Онбординг через Teleport на платформах без cloud-init userdata — ограничения среды (WB) и рабочие профили](adr/0066-teleport-onboarding-profile.md)

Вынесен в [`docs/adr/0066-teleport-onboarding-profile.md`](adr/0066-teleport-onboarding-profile.md). **Deployment-profile-решение поверх существующих механик** ([ADR-063](adr/0063-bootstrap-token-delivery.md) Teleport-transport/full-install/init-фаза, [ADR-061](#adr-061-единый-прогон-provisionонбордингроль-onboarding-await--mid-run-re-resolve-roster) await-барьер, [ADR-017(h)](#adr-017-keeper-side-core-модули-расширены-corecloudprovisioned-corevaultkv-read) cloud-init/self-onboard) — их контракты не меняет; фиксирует ограничения внешней среды и штатные профили онбординга для неё. **Ограничения WB-среды (live-проверены 2026-07-01/02):** (1) `ci_user_data` выключен на стороне WB **на уровне namespace** (матрица: все авторизующиеся ключи × dev/stage+prod облака × user-key/service-account → `ci_user_data is not allowed for this user` ДО проверки прав) → оба userdata-пути (B-flat и self-onboard T) недоступны; (2) прямой SSH keeper→VM закрыт → `transport: direct` недоступен; (3) доступ к VM — только Teleport, причём Proxy за публичным L7-TLS-балансировщиком, VM за NAT. **Прод-профиль: full-install через `core.bootstrap.delivered` `transport: teleport`** с обязательными требованиями к среде: **bot-identity (Teleport Machine ID) без `pin_source_ip`** — identity от интерактивного `tsh login` несёт PinnedIP (OID `1.3.9999.1.9` = src-IP машины выписки; keeper с другим src-IP → access-denied) + MFA/TTL-ограничения интерактивной сессии; `alpn_upgrade: true` + приватный корень LB в системном trust процесса keeper; рабочий **external IP** на VM для Teleport-enroll (`set_external_ip: true` в profile + driver-probe ждёт активации external). История 403-диагноза: сперва атрибуцирован PinnedIP, фактическая стена кода — ALPN `NextProtos` h2-first (фикс `NextProtos=nil` в dial_teleport.go); PinnedIP остаётся объективным прод-ограничением identity. **Live-доказан прогоном-23 E2E create** (2026-07-02: VM из нуля → external IP → enroll → онбординг автоматом → деплой redis 77 задач → `ready`). **Demo/дев-профиль (bastion):** локальный keeper + enrolled-VM как bastion (`tsh ssh -R` реверс-туннель; Teleport-агент биндит `0.0.0.0` сам) + онбординг soul внешним Teleport shell-exec с машины оператора (src-IP = pin); keeper-cert обязан нести IP bastion в SAN, endpoint soul.yml — IP не FQDN; **границы: демо/дев, не прод**. **Отвергнуто:** `core.teleport.shell` (план Б: внешний бинарь мимо Sigil [ADR-026](#adr-026-sigil--целостность-плагинов-keeper-signed-digest-индекс), эпик ~15 файлов, НЕ решает PinnedIP — оставлен только как fallback); ожидание включения `ci_user_data` как блокер (профили сосуществуют — при включении userdata self-onboard T заработает без изменений кода).

### [ADR-067. Обязательный log-shipping (Vector) — лог-плоскость data-сервисов](adr/0067-vector-log-shipping.md)

Вынесен в [`docs/adr/0067-vector-log-shipping.md`](adr/0067-vector-log-shipping.md). **Vector** (`vectordotdev/vector`) — **PUSH-плоскость логов**, ставится во все data-сервисы (redis/dragonfly/…) **обязательно** («как node-exporter»), инвариантом сервиса, а не опцией. **Дополняет** node-exporter/redis_exporter (метрики, pull) — три независимых слоя наблюдаемости (метрики хоста / метрики Redis / логи), не пересекаются. **Дизайн — клон эталона node-exporter** (stateful-ветка [`production-conventions.md`](destiny/production-conventions.md)): новая standalone-destiny [`vector`](../examples/destiny/vector/destiny.yml) — `core.url.fetched` (release-tarball GitHub, **обязательный `sha256`** fail-closed) → `core.archive.extracted` → **стабильный system-аккаунт** (НЕ `DynamicUser`: persistent disk-buffer `data_dir` переживает рестарт, нужен фиксированный uid) + `data_dir` 0700 / `config_dir` 0750 → `core.file.present` бинарь + `core.file.rendered` `vector.yaml` (sources/sinks) + hardened systemd-unit + `core.service.running`/`restarted` (onchanges config/unit/binary); arch = Rust-триплет (`soulprint.self.os.arch` amd64/arm64 → x86_64/aarch64). **Нового core-модуля не вводит** — собран из существующих ([ADR-015](#adr-015-core-модули-mvp-точный-список)). **Встраивание — безусловный `apply: destiny vector` в конце `create`** (после deploy-ветки и экспортеров, без `when:`-гейта): весь контракт в **essence** (контракт A — author-context, версии/checksum/sink скрыты от Run-формы, `covenant`/`form` не трогаются); per-сервис отличие только `vector_log_sources`. **sink — Вариант A** (essence per-incarnation: `vector_sink_type` loki/elasticsearch/vector/console, дефолт `console` — без внешней инфры / `vector_sink_endpoint` / `vector_sink_auth_ref`); `sink_auth_ref` — Vault-ref/значение, резолв Soul-side, **в state НЕ оседает** (симметрия `tls.*_ref`; в unit через `Environment=VECTOR_SINK_TOKEN`, не в `vector.yaml` на диске). **B** (`keeper.yml` глобально) / **C** (гибрид) — follow-up. **state read-model** `logging.vector_*` (без `auth_ref`) + bump `state_schema` + миграция per-сервис (redis `013_to_014` v13→v14, forward-only, `has()`-guard). Имя `vector` — upstream (прецедент node-exporter), в [naming-rules](naming-rules.md). **Amends [ADR-024](#adr-024-observability-prometheus-primary--otel-bridge)** (добавляет лог-плоскость push третьим измерением рядом с метриками pull + трейсами OTel). Related (НЕ amend): эталон node-exporter (клон паттерна, своего ADR не имеет; корректирует фантомный «ADR-064 node-exporter» дизайна 2026-07-01 — реальный [ADR-064](#adr-064-secret-write-path--приём-plaintext-секрета-от-оператора-запись-в-vault-keeper-side) = secret-write-path), [ADR-015](#adr-015-core-модули-mvp-точный-список) / [ADR-010](#adr-010-шаблонизатор-cel-для-yaml-выражений-go-texttemplate-для-файлов) / [ADR-009](#adr-009-scenario--полная-dsl-задач-destiny-граница-с-destiny--рекомендация).

### [ADR-070. Secret reveal-path — раскрытие plaintext-секрета инкарнации оператору под RBAC-правом](adr/0070-secret-reveal-path.md)

Вынесен в [`docs/adr/0070-secret-reveal-path.md`](adr/0070-secret-reveal-path.md). **READ-двойник [ADR-064](adr/0064-secret-write-path.md)** (secret write-path): ADR-064 принимает plaintext-секрет **ОТ** оператора и пишет в Vault keeper-side, этот ADR — обратное направление: отдаёт plaintext **ОБРАТНО** оператору под явным правом. **Декларативный реестр `revealable_secrets[]`** в манифесте `service.yml` (generic, НЕ redis-хардкод в ядре): запись `{id, label, enumerate: state.<array>, vault_ref: "secret/…/{incarnation}/…/{key}#field"}` — сервис сам декларирует, что у его инкарнаций раскрываемо. **Ограниченные плейсхолдеры `{incarnation}`/`{key}`** — литеральная подстановка провалидированных величин, **не CEL** (меньше attack-surface, чем вычислимый язык в пути к секрету): `key` обязан ∈ enumerate-массива **текущего** state (анти-произвол), версия манифеста всегда `incarnation.ServiceVersion` (анти version-craft), результат прогоняется через `vault.ParseRef` (traversal-guard `..`). **Эндпоинты:** `POST /v1/incarnations/{name}/secrets/reveal` `{secret_id, key}` → `{value}` (self-audit `incarnation.secret_revealed` — `{name, secret_id, key, path}` **БЕЗ значения**) + discovery `GET /v1/incarnations/{name}/secrets/revealable` → `{items: [{secret_id, label, state_path, keys}]}` (READ, без audit). Санкционированное раскрытие → 200-DTO **мимо `MaskSecrets`** (единственная точка выхода значения из домена). **★ Security trade-off** (зеркало ADR-064: plaintext Keeper→оператор по проводу, TLS) ПРИ обязательных митигациях-блокерах: RBAC-гейт `incarnation.view-secrets` (scope `coven=`/`service=`/`incarnation=`, fail-closed **404** вне scope) / audit факта БЕЗ значения + leak-guard-тесты на каждый sink / no body-logging (plaintext только телом ответа) / ключ-в-state / traversal-guard / vault-policy read-префикс (`secret/data/redis/*`). Право **`incarnation.view-secrets`** — новое scoped-право ([ADR-047](adr/0047-purview.md)), **строго привилегированнее `incarnation.get`**; MCP-tool **нет** (REST-only, как `form-prefill`). Отвергнуто: redis-специфичный хардкод-эндпоинт (в пользу generic-реестра); CEL в `vault_ref` (в пользу ограниченных плейсхолдеров); переиспользование `incarnation.get`. Отложено (post-MVP): singleton-секреты без `enumerate` (admin-пароль `secret/redis/{incarnation}#password`); живой манифест `community.redis` несёт секцию — follow-up в репо модуля. Реализована **NIM-74**. **Amends [ADR-064](adr/0064-secret-write-path.md) / [ADR-047](adr/0047-purview.md).**

### [ADR-071. Host-Utilization — лёгкая телеметрия утилизации хостов через presence-канал](adr/0071-host-utilization.md)

Вынесен в [`docs/adr/0071-host-utilization.md`](adr/0071-host-utilization.md). **Живая утилизация хостов** (CPU%/load/mem/disk/uptime) для оператора при заходе в инкарнацию — «не задыхается ли инстанс **прямо сейчас**» — **без разворачивания Prometheus**. Третий дешёвый **push-слой** поверх presence-стрима Soul→Keeper, независимый от **статического** Soulprint ([ADR-018](adr/0018-soulprint-typed.md), refresh 5m, `soulprint.self.*` targeting-факты) и дорогого pull-node-exporter; прецедент независимого слоя наблюдаемости — [ADR-067](adr/0067-vector-log-shipping.md). **Транспорт — Вариант B:** новый `FromSoul.host_utilization = 10` (сообщение `HostUtilization`, файл `proto/keeper/v1/utilization.proto`); альтернатива A (reserved-поля 8–14 в `SoulprintFacts`) отвергнута — семантическое нарушение (статика→живое), медленный каденс 5m, засор `soulprint`-CEL-namespace; only-add по [ADR-012(c)](adr/0012-keeper-soul-grpc.md). **Экономный pulse** — 30s default / floor 10s (anti-DoS clamp+warn), single-writer через `handleSession`. **Хранение только Redis** (hot data, не PG — [ADR-006](adr/0006-cache-redis.md)): latest Hash `soul:<sid>:util` + TTL 3×интервал + list-ring `soul:<sid>:util:win` (`LPUSH`/`LTRIM`, N=60 спарклайны) — **не RedisTimeSeries** (переносимо на `redis:7-alpine` и DragonFly, где модуля нет). **Инварианты:** живость НЕ зависит от утилизации (авторитет — lease [ADR-006](adr/0006-cache-redis.md), graceful degrade при отсутствии данных); freshness — API отдаёт `stale` (протухшее не выдаётся за свежее, паттерн skew Soulprint `collected_at`/`received_at`); аутентичность — SID только из mTLS peer cert, НИКОГДА из payload. **API:** `GET /v1/souls/{sid}/telemetry` (latest + window + freshness) + `GET /v1/incarnations/{name}/telemetry` (агрегат по хостам `coven && ARRAY[name]`). **Откладывает:** доставку конфига агенту + essence-override + тумблеры коллекторов (**NIM-87**), web-панель HostsTab (**NIM-88**), расширяемость коллекторов (только only-add новые поля). **Amends [ADR-024](adr/0024-observability.md)** (лёгкий слой утилизации рядом с метриками pull / трейсами OTel / логами [ADR-067](adr/0067-vector-log-shipping.md) push). Имя слоя — `Host-Utilization`, proto-сообщение — `HostUtilization`, в [naming-rules](naming-rules.md).
...
- `keeper.incarnation.rerun-last name=X reason="..."` (REST: `POST /v1/incarnations/{name}/rerun-last`, permission `incarnation.rerun-last`) — атомарно снимает `error_locked` (state не трогается, last-known-good, snapshot в `state_history`) и тем же действием перезапускает **последний упавший сценарий** инкарнации: это create-вариант (`create`/`create_from_souls`) для bootstrap-фейла или любой day-2-сценарий. Для day-2 input восстанавливается из `apply_runs.recipe` упавшего прогона; при недоступном recipe (ретеншн/legacy) — `409` fail-closed «Unlock + ручной запуск». Под одним `FOR UPDATE`: переход `error_locked → applying` минуя `ready` — исключает окно, в котором конкурентный прогон проскочил бы в освободившийся `ready`. Перезапуск стартует в режиме «по зарезервированному `applying`» (`RunSpec.FromLocked`): runner НЕ транзитит статус повторно, а верифицирует, что строка именно в `applying` — fail-closed, иной статус отклоняет старт, не применяя сценарий поверх перехваченной recovery строки. Инвариант «оператор явно подтвердил» сохраняется обязательным `reason` + confirm в UI; audit-событие `incarnation.rerun_last` (НЕ переиспользует `incarnation.unlocked`). Для bare-инкарнации без зафиксированного сценария и при недоступном recipe остаётся обычный `unlock` + ручной повторный run.
...
```
keeper.incarnation.create name=X service=Y input={...}      # запуск create-сценария
keeper.incarnation.run    name=X scenario=add_user input={...}  # любой другой сценарий
...
5. **Под-вопросы модели модулей.** Сама модель закреплена. Открыто внутри:
   ...
   - ~~где живёт реестр модулей в Keeper-е — Postgres `bytea` / отдельный artifact store (S3-compatible) / файловая система keeper-а~~ **закрыт [ADR-065](adr/0065-core-module-installed.md):** нового хранилища нет — PG `plugin_sigils` = допуски (authority sha256), ФС-кеш keeper-а = байты (git-резолв каталога `plugins.soul_modules[]`), git = происхождение (ADR-007); доставка на Soul — server-streaming RPC `FetchModule` + core-модуль `core.module.installed`; S3-artifact-store — post-GA расширение за fetch-абстракцией;
...
25. **Per-host `RenderedTask` dispatch (host-вариативные flow-control-предикаты на multi-host).** **render_context.self — закрыт Вариантом A** (per-host render context, `RenderContextBySID`): per-host `.self` теперь резолвится для `templates/*.tmpl` и `core.file.rendered` в обеих ветках диспатча (inline-задачи и Acolyte-пул) — каждый хост получает свой `soulprint.self.*` в render файла. **Остаётся открытым** (полный Вариант B — per-host dispatch всей `RenderedTask`): host-вариативные `params:`, flow-control-предикаты (`when:`/`changed_when:`/`failed_when:`) и `loop:` со ссылкой на `soulprint.self` на multi-host таргете. Эти случаи в pilot держит fail-closed guard: они допустимы только при single-host таргете; на multi-host рендер падает с явной ошибкой ([`docs/templating.md §4`](templating.md#4-соотношение-фаз-обработки-yaml)) — guard ловит неподдержанный кейс явной ошибкой, не silent-wrong. **Полный Вариант B отложен** (решение 2026-06-29: Варианта A достаточно). Потребует отдельного ADR (снятие host-инвариантности `params:` + изменение dispatch-границы Keeper↔Soul). **Условие возобновления** — первый сервис, которому нужен `${ soulprint.self.* }` в module-`params:` ЛИБО `when: soulprint.self.*` на multi-host таргете БЕЗ возможности вынести self-вариативность в `.tmpl`. До этого Вариант A (per-host `.self` для render файлов + fail-closed guard-ы на остальное) достаточен.
