...
Addressing **declarative**: the third level is a *desired state*, not an *action*. `core.pkg.installed` reads "package installed" - not "install package". This fits better with the philosophy of destiny ("what should be"), gives a natural language to scenario writers and coincides with what Salt does.
