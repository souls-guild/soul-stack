...
Moved to [`docs/adr/0016-parity-license.md`](adr/0016-parity-license.md). License - **BSL 1.1** for the core (this repo) and frontend (`soul-stack-web`): fair-code, Change License Apache 2.0, Change Date 2 years (Amendment 2026-07-09); SDK/examples/plugins - Apache 2.0; enterprise features - separate commercial license. The parity strategy is a hybrid without a wrapper: core MVP is our Go rewrite, exotic is community plugins via SDK; a GPLv3 Python-runtime wrapper is prohibited. Amendments: Plugin SDK Phase 2 (10 official plugins, namespace `official`, template mechanism); fair-code/BSL (license); **2026-09-02 (NIM-765) — part (g), the reserved namespace `official`, is retired**: the artifact carries no name of its own since NIM-377, `official` is in neither [`shared/plugin/reserved.go`](../shared/plugin/reserved.go) nor the [reserved list](naming-rules.md#reserved-namespace-names), and the origin-grouping level of a plugin address is removed ([ADR-020 amendment 2026-09-02](adr/0020-plugin-infrastructure.md#amendment-2026-09-02-nim-764--nim-765-a-plugin-address-is-pluginobjectaction-and-the-origin-grouping-level-is-removed)) — origin is answered by the catalog entry's `source` plus the Sigil allow-list. The trademark protection of "official"/"certified" under (q) is unaffected.
...
Moved to [`docs/adr/0020-plugin-infrastructure.md`](adr/0020-plugin-infrastructure.md). A unified infrastructure of three kinds of plugins (`soul_module`/`cloud_driver`/`ssh_provider`): static `manifest.yaml` (offline validation `soul-lint`), JSON-handshake with a magic prefix → gRPC-over-unix-socket, `protocol_version` ↔ `proto/plugin/vN/`, one-shot lifecycle, closed-enum `required_capabilities` / `side_effects`, file-permissions instead of mTLS. Amendments: SDK Phase 2, SshProvider-set + credentials-flow. **Amendment 2026-09-01 (NIM-757): `cloud_driver` is removed, and `side: keeper` is what replaces it** ([ADR-020 amendment](adr/0020-plugin-infrastructure.md#amendment-2026-09-01-nim-757-cloud_driver-is-removed-and-side-keeper-is-what-replaces-it)). ⚠ The count in the summary line above ("three kinds of plugins") is **stale independently of this change** — `soul_beacon` was added as V5-2 by the [ADR-030 amendment 2026-05-26](adr/0030-vigil-oracle.md#amendment-2026-05-26-s5-closure) and never absorbed into ADR-020's Context, so the real set is four and the cut leaves **three**, not two. The infrastructure this ADR fixes — one handshake, one socket, one one-shot lifecycle, one stamped schema document, one Sigil gate — is untouched, which is precisely why the separate contract was redundant. ⚠ The closed `kind:` enum lives in **`sdk/schema/`**, not in proto (`manifest.proto` is already a hand-synced dead document), so this is an `sdk/` change with a proto `reserved` as a footnote. The 2026-08-06 open item "the authoring form for `cloud_driver` / `ssh_provider` / `soul_beacon`" is **half-closed**: the cloud half by removal, the other two untouched. Implemented: NIM-758, NIM-760, NIM-761 (2026-09-04) — `sdk/schema/schema.go:40-44` declares the three that remain, and `proto/plugin/v1/common.proto:13-16` carries the `reserved 2`. **Amendment 2026-09-02 (NIM-764 / NIM-765): a plugin address is `<plugin>.<object>.<action>`, and the origin-grouping level is removed** ([ADR-020 amendment](adr/0020-plugin-infrastructure.md#amendment-2026-09-02-nim-764--nim-765-a-plugin-address-is-pluginobjectaction-and-the-origin-grouping-level-is-removed)) — level 2 is the **object** the module manages and level 3 the **action**, `community.*` and `official.*` both go because both name origin rather than subject, and origin is answered by the catalog entry's `source` plus the source-keyed Sigil allow-list instead; ★ **nothing in the engine changes** (the address split is positional, `redis.user.present` already matches, level 1 is a config edit and one artifact may already serve several objects), and ⚠ no gate follows from a word in an address. ★ **Status 2026-09-03: the redis artifact serves it (NIM-766, NIM-767)** — `redis` under the alias `redis`, seven objects, `side: soul` per object, the schema document generated from `module.Def` (closing NIM-525), and the cluster operations moved out of `params.action` onto level 3. `redis.user.present`/`.absent` — this amendment's own worked example — ship with NIM-767: `ACL SETUSER`/`DELUSER` on one user, alongside rather than instead of the file-shaped `acl.reloaded`. ★ **Status 2026-09-05: mongo serves it too (NIM-769)** — `mongo` under the alias `mongo`, three objects (`command`/`instance`/`user`), `side: soul` per object, `module.ServeBundle` and a generated schema document; `params.state` moved onto level 3 as `user.present`/`user.absent`, so a definition choosing per item filters its loop where an address cannot vary. `community.*` is now in no effective address here and `docs/module/community/` is gone. Still open: NIM-768 (the downstream redis service off `redis-cli`). **Amendment 2026-09-04 (NIM-794): the catalog entry gains a source kind and an explicit artifact list** ([ADR-020 amendment](adr/0020-plugin-infrastructure.md#amendment-2026-09-04-nim-794-the-catalog-entry-gains-a-source-kind-and-an-explicit-artifact-list)) — a `plugins.soul_modules[]` entry gains `source_kind: git | artifact` (`git` is today's meaning and stays the default), and an `artifact` entry carries `base_url` plus an explicit `artifacts[]` of `{os, arch, path, sha256}`, each row naming its own `path` in full with **no template**. The key is `source_kind` because `kind` is already the closed plugin-type enum in `sdk/schema/`. ⚠ Left **open** there: N platform binaries carry N stamped schema trailers while the signed block holds one `schema_sha256` — NIM-795 must come back for a decision. **Design only — NIM-795 (keeper)**, epic NIM-793: no `source_kind` / `base_url` / `artifacts[]` key is parsed by any config yet. The **Soul-side consumer HAS shipped** (NIM-796), fed from a DTO nothing populates from the wire.
...
Moved to [`docs/adr/0026-sigil.md`](adr/0026-sigil.md). Keeper-signed allow-list of plugins (registry `plugin_sigils`): an Archon explicitly allows an artifact `(source, ref) → sha256` under a registration `alias` (re-keyed off `(namespace, name, ref)` by NIM-377/438 — the artifact declares no name), Keeper signs the block with the schema document attached, and the host verifies digest+signature BEFORE exec (replaces TOFU first-load). ★ The signature makes the declarations **non-forgeable, not enforced** — `capabilities`/`side_effects` are disclosure for the operator's approval, and the one real control is the approved digest. Git-verified `ref` (go-git F-fetch), multi-anchor signature key rotation (`sigil_signing_keys`), replace semantics snapshot/anchors. **Amendment 2026-09-04 (NIM-794): the grant carries a list of artifacts, and the bytes stop travelling through the keeper** ([ADR-026 amendment](adr/0026-sigil.md#amendment-2026-09-04-nim-794-the-grant-carries-a-list-of-artifacts-and-the-bytes-stop-travelling-through-the-keeper)) — the host fetches from the artifact source and the Keeper serves only the seal; the signed block's scalar `binary_sha256` becomes a list of `{os, arch, path, sha256}`, so the DST goes to **`soul-stack/sigil/v3`** and every existing grant is deleted, as at v1→v2. ★ Source-pull weakens nothing: `source` was always operator-asserted over an **untrusted** repository, and the gate was always the digest checked before materialization — a compromised source earns a verification refusal, not code execution. A grant *per platform* is refused by this ADR's own trust key (the partial unique index on `(source, ref)`), so the artifact list is the only admissible shape. **Design only — NIM-795 (keeper)**, epic NIM-793: the signed block is still `(source, ref, binary_sha256, schema_sha256)` and the DST is still v2. ⚠ The **Soul half HAS shipped** (NIM-796) and stopped here: `SigilRecord` carries read-side `BaseURL`/`Artifacts` that nothing fills from the proto, so a multi-platform grant installs only on the platform whose row digest equals `BinarySHA256hex` and every other row **fails closed at verify**.
...
|---|---|---|
| **alias** | The **registration alias** — the name the *operator* gave the artifact in `keeper.yml::plugins.*[].name`. Also the unit of distribution and caching (see [module-collections.md](module-collections.md)). | `core` (built into `soul`), `redis`, `acme` |
| **module** | The control object is "what the module is about." (`pkg`, `service`). | `pkg`, `file`, `service`, `user`, `exec` |
...
**`required_modules:` in destiny.yml is a declaration of only custom modules**, each written as the registration alias (`haproxy`) or the alias with the object it addresses (`haproxy.instance`): "this destiny requires that these custom modules be available on the host". All state forms inside them are available automatically. Specific state instances are called by a 3-level form in `tasks/main.yml` (see ["Task Structure in `tasks:`"](#destiny-task-structure) below).

**`service.yml → modules[]` is the other list, and it is NOT the same rule.** It INSTALLS: an entry pairs a name with a `ref`, and what a `ref` pins is an **artifact**. So an entry there is the registration alias and nothing else — `- { name: redis, ref: v1.0.0 }` — and the objects the artifact serves arrive with it. The two-level form is deprecated there (warning `module_name_two_level_deprecated`, an error from **2026-12-01**); it is not deprecated in `required_modules:`, which carries no `ref`, synthesizes nothing and loses nothing by naming the object. One grammar, two policies — [NIM-829](service/manifest.md#one-row-per-artifact-transition-window-to-2026-12-01).
...
### [ADR-063. core.bootstrap.issued — keeper-side onboarding tokens (delivery removed)](adr/0063-bootstrap-token-delivery.md)

Moved to [`docs/adr/0063-bootstrap-token-delivery.md`](adr/0063-bootstrap-token-delivery.md). The keeper-side base module now exposes two public states. **`core.bootstrap.issued`** atomically creates/checks `pending`, `transport=agent` Souls and issues one fresh token per ready-made VM SID without invoking CloudDriver; a repeat invalidates the prior unused token. An already onboarded identity gets no token either way, and whose host it is decides the rest (amendment 2026-09-04 / NIM-780): a `connected`/`disconnected` Soul **of this run** — member of the run's incarnation, or of none yet — is converged over as `{sid, onboarded: true}` and counted in `skipped`, which is what makes a `create` that failed AFTER onboarding repeatable; one held by ANOTHER incarnation is still an identity takeover and is still refused, rolling the whole batch back. **`core.bootstrap.delivered` is REMOVED** (amendment 2026-09-09 / NIM-834): it did two unrelated things — install the Soul binary and hand the host its token — and installation has no portable form, so the engine no longer has one way to do it. Installing a host is site-specific; what the module guaranteed becomes **requirements on whoever installs it**, each paid for by a live run and none reproduced by writing a file with the token: redeem with `soul init` behind the seed-cert guard (there is no soul-side pickup of a token file, and the token is single-use), token in STDIN and never argv (`ps`/audit.log/journald run on the host itself), and `daemon-reload && enable && start` so the unit survives a reboot — plus fail-closed `Authorize`, an ephemeral keypair whose private half never leaves the Keeper, CA-signed host-cert verify, no token in the step output, B1-strict, and a bounded wait for the host to become reachable. Plaintext exists only in the current run register under the sensitive key `bootstrap_token`; state/audit/OTel/SSE/log projections never contain it. Batch issuance is transactional. The audit id `bootstrap.delivered` is retired and its wire value stays reserved; `bootstrap.issued` is unchanged and carries no tokens. `soulinstall`/`cloudinit` are kept as the normative description of an installed host, with no caller. **Amends [ADR-017](#adr-017-keeper-side-core-modules-expanded-corecloudprovisioned-corevaultkv-read) / [ADR-061](#adr-061-single-run-provisiononboardingrole-onboarding-await--mid-run-re-resolve-roster) / [ADR-015](adr/0015-core-modules-mvp.md).**
...
Moved to [`docs/adr/0065-core-module-installed.md`](adr/0065-core-module-installed.md). Canonical channel for delivering custom module artifacts to the Soul host; **closes open Q No. 5** "where the registry of modules in Keeper lives." **Transport** - the third RPC in `service Keeper`: server-streaming **`FetchModule(PluginFetchRequest) returns (stream PluginChunk)`** (the same mTLS-listener as EventStream; artifact bytes travel **in a separate HTTP/2 stream** and do not choke the control-plane; **content-addressed** - Keeper gives ONLY bytes whose sha256 is in the active Sigil-admission `kind: soul_module`; authorization - mTLS peer-cert; guard-rails: `plugins.max_artifact_size_mb` + rate-limit parallel fetch per-SID; only-add by [ADR-012(c)](#adr-012-keepersoul-grpc-contract-one-eventstream-with-oneof-keeper-side-render-forward-compat-only-add)). **Byte register** - config-directory **`plugins.soul_modules[]`** (`{name, source, ref}`, symmetry `ssh_providers`/`soul_modules`) + existing plugingit-resolve into FS cache + Sigil-tolerance (review [ADR-026](#adr-026-sigil---plugin-integrity-keeper-signed-digest-index) without changes); **NO new repository** - PG = permissions (authority sha256), FS = bytes, git = origin ([ADR-007](adr/0007-versioning-git-ref.md)); HA - per-instance FS cache, miss slot → on-demand resolve or failure with retry; S3-artifact-store is a post-GA extension behind the fetch abstraction. **Params `core.module.installed`** (Soul-side, state `installed`): `name` (required, `<ns>.<name>`), `ref` (opt, **pin-reconciliation** - active tolerance must be on this ref, otherwise `failed`; NOT version selection); idempotency - sha256 of the existing binary == sha of the active Sigil → `changed=false`. **hot-register is MANDATORY in MVP** (thread-safe Rescan without restarting the daemon - otherwise `redis.*` in the SAME run after the install step does not work; restart = break EventStream = break of run); The beacon registry is NOT rebuilt during rescan (post-MVP). **Scenario** — amendment 2026-07-03: Keeper **synthesizes** Soul-side install steps from the explicit manifest declaration `service.yml::modules[]` (scenario-runner, after expanding include BEFORE Stratify; insertion before the first consumer of the module; explicit step `core.module.installed` with the same literal `params.name` disables synthesis - operator controls position/ref/when itself); validation-hint post-MVP is saved in the reverse direction (the module is used in tasks, but is not declared in `modules[]`). **Sigil-verify install-time:** allow-check BEFORE fetch (no tolerance → `module_not_allowed` to single network byte) + full verify (signature + `manifest_sha256`) before atomic rename; manifest is materialized from `PluginSigil.manifest_raw` (not via fetch); review `shared/pluginhost`, NO new trust mechanisms. Soul cache layout is **catalog** `<paths.modules>/<ns>-<name>/{manifest.yaml, <name>}`. TaskError-reasons: `module_not_allowed` / `module_fetch_failed` / `module_verify_failed`. MVP boundaries: without `absent`-state (TTL-cleanup), ~~without auto-inject~~ (removed by amendment 2026-07-03 - auto-synthesis from `modules[]`), without beacon-hot-reload. **Amends [ADR-012](#adr-012-keepersoul-grpc-contract-one-eventstream-with-oneof-keeper-side-render-forward-compat-only-add) / [ADR-020](#adr-020-plugin-infrastructure-manifest-handshake-lifecycle-format) / [ADR-015](#adr-015-core-mvp-modules-exact-list).** **Amendment 2026-08-06 (NIM-377):** the slot above is stale — it is named by the **registration alias** (`<paths.modules>/<alias>/`), holds one executable whose filename means nothing, and carries the canonical-JSON schema document in place of `manifest.yaml`; the schema rides in the artifact as a stamped trailer, so it now does travel through `FetchModule`. **Amendment 2026-08-07 (NIM-524):** `params.name` is address **level 1** — the bare alias naming the slot, never the `<alias>.<module>` of the `modules[]` entry. The synthesizer had been passing the whole two-level name into a field that forbids the dot, so every service declaring `modules:` failed at apply on every host; several entries of one artifact now collapse into ONE install, and a `ref` disagreement under one alias is the diagnostic `conflicting_module_ref`. **Amendment 2026-08-26 (NIM-543):** the takeover comparison reduces **both** halves to level 1 — it had been keyed on the manifest alias against the step's raw literal, so an explicit step spelled `name: redis` took over nothing and earned a second install of the same artifact beside it; a `params.name` that is not a registration alias is now the offline diagnostic `module_install_name_not_an_alias` instead of a failure on every host. **Amendment 2026-09-04 (NIM-794): the fetch step goes to the source, and `FetchModule` stays as the egress-free path** ([ADR-065 amendment](adr/0065-core-module-installed.md#amendment-2026-09-04-nim-794-the-fetch-step-goes-to-the-source-and-fetchmodule-stays-as-the-egress-free-path)) — of the install step's six numbered steps only **step 3's endpoint** changes; allow-check, idempotency, verify, atomic install and hot-register are transport-independent, and the order (rights before network, signature before disk) is preserved verbatim. ★ **`FetchModule` is not deprecated and not transitional** — it is the path for hosts with no egress, a permanent host class. ★ The platform axis was **recorded wrong and is corrected against the code**: the row is chosen from the host's **own Soulprint facts with the running binary as the fallback**, not `runtime.GOOS`/`runtime.GOARCH` alone — the original concern holds and is what the code answers, since `os.family` is a *distribution* family on Linux, so the four Linux families **collapse to `linux`** and any other family is already a GOOS. No matching row is a closed refusal naming what the release does cover. The transport rule is **six-way**: no rows → Keeper; a row matches → the source; no row for this platform → refusal; the source did not answer with a session live → Keeper with a warning; the source did not answer with no session → refusal; wrong bytes or an unusable address → refusal — because **Keeper can stand in for a source that is down, not for one serving something else, and not for a catalog field it cannot fix**. The final event carries `fetch_via` and `fetch_url`. The HTTP client and timeout are `core.url`'s; the **SSRF dial guard is lifted deliberately** (a `base_url` is catalog configuration, not run input) while https-only and the disk-gating verify stay; the path is **joined, never interpolated**. ⚠ "No new storage" now holds only on the condition that the **Keeper** cache carries N artifacts per slot, which it does not today; the **Soul** slot still holds exactly one executable. Verification is still in-memory, and the missing Soul-side size ceiling **shipped as a constant** (Keeper's default value, not a `soul.yml` field). Push-mode install is now possible on the transport and still fails closed at the allow-check, so it stays **explicitly not decided**. ⚠ **Half-wired:** `SigilRecord` gained `BaseURL`/`Artifacts` that nothing fills from the proto, so a multi-platform grant installs on one platform and every other row fails closed at verify. Status: **the Soul half is implemented (NIM-796); the keeper half is not (NIM-795)**, epic NIM-793. **Amendment 2026-09-05 (NIM-829):** a `modules[]` entry declares an **artifact**, not one of its objects — level 2 named a module under the pre-NIM-765 address model and is an OBJECT inside the one binary since, so the entry is now the registration alias alone (`- { name: redis, ref: v1.0.0 }`). The two-level form is accepted with `module_name_two_level_deprecated` until **2026-12-01**; ★ **`conflicting_module_ref` is retired with it** — the two-ref state it catches is only writable in the form being removed, and the general rule is that a guard over an impossible state usually means the FORM is wrong. Rider: consumers are matched on the **alias** now, so an object the manifest never enumerated no longer silently gets no install.
...
> and the masking walk), NIM-743 (`soul-lint list-secret-paths`), NIM-744 (`examples/` and the downstream
...
state_schema:
  ...
  redis_users:                        # a TYPED array since migration 006_acl_users_map_to_array — not a map
    type: array
    items:
...
**Scenario-conventions and lifecycle-set.** Lifecycle-set (`LifecycleScenarioNames`) = only `create` / `destroy` - specialized scenario-kinds of the corresponding life cycle phases. `converge` is an **operational** scenario-kind (launched by the usual `run` Apply-reconcile; its second role as a dry-run target left with NIM-446), it is derived from the lifecycle set (see [Amendment ADR-031 2026-06-10](#adr-031-scry--drift-detection-declarative-dry-run-reconcile)). The scenario directory (`GET /v1/services/{id}/scenarios`) carries the field `runnable: bool` - the sign "launched by the operator from the Run form": `create` = `true`, `destroy` = `false` (special deletion flow via `DELETE /v1/incarnations/{id}`), operational (incl. `converge`) = `true`. The UI filters the Run form by `runnable`, not by name hardcode ([ADR-042](adr/0042-backend-driven-ui.md)).
...
**Resolver contract** (invariant): the omitted `on:` = all **members** (via `incarnation_membership`); `on: ["${ incarnation.id }"]` is a **validation error** (`incarnation.id` is not a Coven — in either spelling: the retired `incarnation.name` is matched too, for the length of the [ADR-0085](adr/0085-entity-id-and-label.md) window); a list in `on:` - AND/intersection of stable covens, result **always ⊆ members**; **cross-incarnation targeting is prohibited by construction** (enforced by the membership roster); role in `on:` is not involved. Completely - [`docs/scenario/orchestration.md §3`](scenario/orchestration.md).
...
- `incarnation.id` - instance identifier (`incarnation.name` is the retired spelling, read for the [ADR-0085](adr/0085-entity-id-and-label.md) compatibility window).
...
The state-schema version is not stored anywhere — it is the top of the ladder. An empty `migrations/` means version 1. `service.yml` carries no `state_schema_version:` key; writing one is an `unknown_key` error (NIM-735, [ADR-019](#adr-019-state_schema-migration-dsl)). `soul-lint validate-service` also reads the ladder itself: a gap in the numbers, a duplicate, or a directory it cannot recognise as a step is an error with the offending path, because under a derived version each of those silently moves or truncates the version.

`schema.lock` is generated next to the ladder: `version` (the top of the ladder at stamp time) and `fingerprint` (a hash of the **parsed and canonicalized** `state_schema`, never of the file text — a comment would break a text hash). Written by `soul-lint schema-stamp <service dir>` and read by `soul-lint` on every `validate-service`; a service repository wraps the two as `make schema-stamp` and `make validate`, and both targets live there, not in this core repo. A lock that is present is always checked; an absent one is an error once the ladder holds a step. Layout and lock are specified in [`docs/migrations.md`](migrations.md).
...
> ★ **This section is history — epic NIM-757, decided 2026-09-01, removed in NIM-761 (2026-09-04).**
...
> keeper-side plugin → **NIM-760** the cloud driver moves and is verified live → **NIM-761** removal →
...
> **Everything below describes an engine that no longer exists**, kept for the decision trail.

Dynamic VM creation is a **scenario step addressing a `side: keeper` plugin**. Service does not know the specifics of clouds - it knows "the step of creating a VM with parameters is needed," Keeper selects a driver and executes it.

Git/DB boundary: a plugin's credentials are a Vault reference and its VM spec is ordinary service data in `vars/`. A service's default parameters live in its git repo's `vars/` and are not overridable from outside — a fleet that needs different ones forks the service repo ([ADR-0082](adr/0082-service-vars.md)). The profile parameters are validated against `profile_schema`, which CloudDriver publishes via RPC `Schema()`.
...
The plugin contracts and the plugin directory are in [`docs/keeper/plugins.md`](keeper/plugins.md); the keeper-side dispatch a `side: keeper` plugin goes through is in [`docs/keeper/modules.md`](keeper/modules.md).
