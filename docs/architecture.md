...
### [ADR-080. Coven and Trait — one label world: inheritance by membership, union at read](adr/0080-label-inheritance-union.md)

Moved to [`docs/adr/0080-label-inheritance-union.md`](adr/0080-label-inheritance-union.md). An operator label — a **Coven** tag or a **Trait** pair — lives **only where it was attached**, on a host (`souls.coven` / `souls.traits`) or on an incarnation (`incarnation.covens` / `incarnation.traits`), and is **never copied** between the two. What consumers see is the **union, computed at read time**: a host's effective labels are its own plus those of every incarnation it belongs to (`incarnation_membership`, M:N since [NIM-124](adr/0008-coven-stable-tags.md)), the incarnation's **name** included on the coven axis — the incarnation-side resolver already scopes on `covens && $x OR name = ANY($x)`, so host visibility has to agree or `coven=<incarnation>` would show the incarnation while hiding everything inside it (NIM-124 is not reverted: the name is never written into `souls.coven`). **A key held on both sides is a UNION, not a contest** — `owner=dba` on the incarnation and `owner=bobik` on the host give `owner=[dba, bobik]`, and EITHER value grants; a precedence rule ("host wins" / "incarnation wins") was rejected because whichever way it points it silently revokes access somebody was deliberately granted. Fixes the two defects the old shape produced: an incarnation-level label **granted nothing on its own hosts** (visibility resolved over the `souls` table alone), and the materialized Trait projection of [ADR-060](adr/0060-traits.md) R1 (`SyncTraitsToHosts`, wired into incarnation-create and host-bind) made "the operator cannot label a host" **structural** — a full `BulkReplaceTraits` overwrote any per-host write, and on M:N membership a sync for one incarnation erased what another had projected. That projection and both hooks are **removed**, not disabled. Both readers of the label layer resolve the **same** union: the RBAC predicate (a correlated `EXISTS` over membership on the `coven`/`trait` dimensions of [ADR-047](adr/0047-purview.md), still fully pushed down to SQL — offset/total stay exact) and targeting (`soulprint.self.covens`/`.traits`, `soulprint.hosts[].*`, the topology roster, the push inventory and the Voyage target filter) — keeping them in step is the point, since a `where:` and a scope check must never disagree about one host. **The per-soul write path is first-class again**: `POST /v1/souls/traits` / `keeper.soul.traits-assign` lose their deprecation (nothing overwrites them any more) and gain **gate (b)** — the pair must lie inside the operator's own trait-scope, the mirror of coven-assign's label gate — required because a host-attached trait now grants visibility **permanently** and `trait.<key>=v` is a scope dimension (NIM-128); `remove` stays ungated, exactly as with coven. No permission or audit event is renamed (`soul.traits-assign` / `incarnation.traits-set`, `soul.traits-changed` / `incarnation.traits_changed`, KEYS only). Migration [`106`](../keeper/migrations/106_prune_projected_soul_traits.up.sql) prunes the projection's residue out of `souls.traits` by **VALUE** equality against the host's incarnations — a pair that merely shares a KEY is genuinely host-local and stays; effective labels are unchanged by the prune, only their storage location. **Operationally: prefer labelling the incarnation** — it covers every host that joins later with no re-stamping, while a host label covers exactly one VM. **Consequence to plan for:** visibility **widens** for deployed roles scoped `coven=`/`trait.` — they now also see the hosts of matching incarnations. Rejected: keeping the projection but merging instead of replacing (it cannot tell "the host owns this key" from "I wrote it last time", so a removed key never leaves the hosts — a permanent grant inside an RBAC dimension), two columns per host with a precedence rule (still copying, plus the rejected precedence), per-host overrides declared on the incarnation (cannot label a host that belongs to zero or two incarnations). Impl — NIM-121. **Amends [ADR-008](#adr-008-coven---stable-logical-tags-only) / [ADR-060](#adr-060-trait--operator-set-key-value-labels-host-and-incarnation) / [ADR-047](adr/0047-purview.md).**
...
# What does the scenario write to incarnation.state after a successful apply.
...
The `input:` block validates the scenario input parameters before running (according to the [docs/input.md](input.md) standard). `state_changes` - **ordered list of CRUD verbs** (`set`/`add`/`modify`/`remove` + `foreach`, [ADR-057](#adr-057-state_changes---ordered-list-of-crud-verbs)): declares **what** the scenario writes to `incarnation.state` on success and **from where** the value is taken (CEL `${ … }`, rendered by the keeper after the barrier); plurality - through `match`-predicate. Regulatory - [scenario/orchestration.md §7.1](scenario/orchestration.md).
