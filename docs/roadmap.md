...
> ⚠️ **This file is a historical roadmap, frozen at 2026-05-26.** **`v0.1.0-beta.1`** has since been released (public beta, private repos `souls-guild`). Some of the statuses below are outdated relative to the actual state: for example, "Cloud parity ✓ ready" means implemented CloudDriver plugins, but **operator-flow cloud-provisioning is not operationally available in beta** (no REST/MCP/UI handles `/v1/providers` / `/v1/profiles`); "push 🟡 in progress" is actually closed as part of the Voyage epic. The current boundaries of beta and GA-readiness are NOT here, but in:
> - [known-limitations.md](known-limitations.md) - what is NOT included in public beta (source of truth for beta limits).
...
- Code on GitHub — repo `souls-guild/soul-stack` (permanent; module-path and signature finalized). Commits to `main`, CI = `make check`.
...
| drift-detection | ~~name **Scry**~~ — **WITHDRAWN 2026-08-05 (NIM-446)**: the circuit shipped, went unused and was cut whole. `Plan` pure-read and the informational `drift` status survive it |
| Shepherd (balancing) | 💤 Ideas box (possibly future releases) |
| UI | **separate artifact** (B), React+TS+Vite, custom design system, TS client from OpenAPI, Soul Stack dictionary; build when the API is almost ready |
...
- ⬜ UI (separate SPA, custom design system) - start when the API is almost ready
...
- ⛔ **Superseded by epic NIM-757 (decided 2026-09-01, removed in NIM-761 on 2026-09-04).** The "6 committed and working" claim below stays true; what changes is the shape the work lives in. The separate **CloudDriver contract is removed** — every cloud driver already *is* a plugin, so it becomes an ordinary SoulModule plugin declaring `side: keeper`, and `keeper.cloud` / `core.cloud` / the Provider and Profile registries / `sdk/clouddriver/` / the six `examples/module/soul-cloud-*` all go. Order: **NIM-758** (keeper learns to execute a keeper-side plugin) → **NIM-760** (the cloud driver moves, verified live) → **NIM-761** (removal) → **NIM-762** (web UI). See [ADR-017 amendment 2026-09-01](adr/0017-keeper-side-core.md#amendment-2026-09-01-nim-757-the-clouddriver-contract-is-removed--a-cloud-driver-is-an-ordinary-plugin).
...
- ✗ **drift-detection (Scry): REMOVED 2026-08-05 (NIM-446).** Slices A/B/C all shipped — on-demand `check-drift` (REST+MCP+CLI), the `converge` dry-run pipeline, the RBAC permission, the `incarnation.drift_checked` audit event, and the default-OFF background scan with its migration-050 columns. The background scan was never turned on and the on-demand check went unused, so at the NIM-435 acceptance the decision was to cut the feature everywhere rather than keep paying for it. What remains from the work is the **`Plan` pure-read rollout** — 12 stateful core modules with `PlanReadSafe` (pkg/file/service + group/user/cron/mount/sysctl/archive/line/repo/firewall), 2 deferred (`git`, `url` without checksum), 3 verb modules correctly default-deny — which is now the plugin contract that **Errand's** dry-run rides on, plus the only-add proto fields and the informational `drift` status. See [ADR-031 closing amendment](adr/0031-scry-drift.md#adr-031-scry--drift-detection-declarative-dry-run-reconcile).
...
- **PM 2026-05-25:** ✓ UI (Q20 resolve) · ✓ env-RBAC (ADR-008 amend (a) → IMPLEMENTED C+D) · ~~drift~~ ([ADR-031](adr/0031-scry-drift.md#adr-031-scry--drift-detection-declarative-dry-run-reconcile) — later withdrawn, NIM-446) · ✓ cloud (ADR-017 amend + Q13/Q14) · ✓ state-retention (Q19) · ✓ open Q13/14/15/19/20 updates.
...
- **Destroy guard-rail cross-feature** (⚠ when reviewing destroy-chain; written against `core.cloud.provisioned` `destroyed`, removed in NIM-761 — the concern transfers unchanged to a machine-provider plugin's destroy action): the cloud leaf = immediate TerminateInstances **without** orchestration-guard-rails. Make sure that it is called ONLY after guard-rails `incarnation.destroy` (two-phase `destroying`→`destroy_failed`, [destroy statuses](architecture.md#statuses-destroying-and-destroy_failed)) - otherwise "a typo in `count` erases the prod" (cloud-cascade ADR-017 Consequences erases souls/seeds/tokens).
- ~~**Drift Slice C**~~ — moot: the drift circuit was removed whole (NIM-446). Should drift detection ever be asked for again it starts from the open question, not from this backlog item.
