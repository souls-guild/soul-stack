...
- ⚠ **Superseded in direction by epic NIM-757 (decided 2026-09-01, NOT implemented).** The "6 committed and working" claim below stays true; what changes is the shape the work lives in. The separate **CloudDriver contract is removed** — every cloud driver already *is* a plugin, so it becomes an ordinary SoulModule plugin declaring `side: keeper`, and `keeper.cloud` / `core.cloud` / the Provider and Profile registries / `sdk/clouddriver/` / the six `examples/module/soul-cloud-*` all go. Order: **NIM-758** (keeper learns to execute a keeper-side plugin) → **NIM-760** (`soul-cloud-wb` moves, verified live) → **NIM-761** (removal) → **NIM-762** (web UI). See [ADR-017 amendment 2026-09-01](adr/0017-keeper-side-core.md#amendment-2026-09-01-nim-757-the-clouddriver-contract-is-removed--a-cloud-driver-is-an-ordinary-plugin).
