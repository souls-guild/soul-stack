...
**The second step - recording it in state.** `module: core.state.set` — with no `on:`, because the address already says keeper-side (NIM-747; writing the key is `on_keeper_redundant`) — writes `incarnation.state.greeting_file = /tmp/soul-stack-hello` into Postgres. The state suffix of the address is the verb — `set` overwrites the field whole; `present`/`add`/`append`/`modify`/`remove`/`unset` are the rest ([ADR-0084](../adr/0084-explicit-state-capture.md)). Here the value is a literal; in the general case it is a CEL expression (`${ input.* }`, `${ compute.* }`, `${ register.<keeper-task>.* }`).
