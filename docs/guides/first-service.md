...
**`input:` — параметры сценария.** Контракт того, что оператор обязан/может передать при запуске. Здесь один параметр `greeting`: строка, обязательная (`required: true`). Keeper валидирует переданный `input` против этого контракта **до** запуска — если оператор не передал `greeting`, прогон не стартует. Полный стандарт `input:` (типы, форматы, валидация, переиспользуемые именованные типы через `types:`/`$type`) — [docs/input.md](../input.md).
...
- **Эксплуатационные операции** (мониторинг, апгрейд, восстановление кластера) — раздел [Сделать](../README.md#сделать-оператор) в карте документации и [docs/operations/](../operations/README.md).
- **Готовые образцы** сервисов посложнее — [`examples/service/`](../../examples/service/) (например, [`redis/`](../../examples/service/redis/) — один сервис на все режимы развёртывания standalone/sentinel/cluster/sentinel_only с rolling-restart и reshard).
