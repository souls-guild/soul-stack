...
**SoulModule plugin** is a separate executable binary, which the `soul` daemon (or Keeper in push mode) launches as a sub-process and communicates with it via gRPC over a Unix socket. The plugin implements **the same interface** `SoulModule` (Validate / Plan / Apply) from [`sdk/module`](../../sdk/module/module.go) as the built-in core modules - the only difference is in the packaging: core modules are statically compiled into `soul`, the plugin is delivered to the host separately and cached.

**Naming.** Since NIM-851 an artifact is named after the module it serves (`redis`, `mongo`) — the `soul-mod-` / `soul-cloud-` family prefixes are gone, `soul-ssh-*` stays because SshProvider is a separate contract. The filename is a convention and nothing reads it ([ADR-020(e)](../adr/0020-plugin-infrastructure.md#amendment-2026-09-12-nim-851-the-soul-mod--and-soul-cloud--prefixes-are-dropped-soul-ssh--stays)). ⚠ `soul-lint plugin-init` still scaffolds `soul-mod-<namespace>-<name>`: that tree is a `go:embed` mirror of the companion `soul-stack-plugins` repository and cannot be changed from here.
