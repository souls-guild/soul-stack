...
If successful, SoulSeed (cert/key/ca) is expanded to `paths.seed`; the entry in `souls` stays `pending` — a signed certificate is not yet a live host, so the status moves to `connected` when the daemon below opens its stream. Start the daemon (holds EventStream to Keeper):
...
- **API returned `401` although the token "is".** The `type` field of the problem+json separates the identity case from the rest, and within the rest the `detail` string names the cause. `operator-revoked-token` — the Archon was revoked; the token is refused on every route from the next request onward, and a fresh token for the same AID will not help, because it is the identity that was withdrawn. Otherwise `type` is `unauthenticated`, and on a route behind the JWT middleware `detail` is one of five fixed strings. Read it before reissuing anything: three of the five are not repaired by a fresh token, and reissuing on those hides what is actually wrong.
  - `missing or malformed Authorization header (expect: Bearer <jwt>)` — the token never reached verification, so this says nothing about the token itself and a fresh one changes nothing. The header is absent or lacks the `Bearer ` prefix. (Two request shapes carry the token elsewhere: an SSE request — `GET` with `Accept: text/event-stream`, or any path ending in `/events` — takes it in the **`access_token`** query param, and a WebSocket upgrade takes it in a bearer subprotocol. Neither applies to an ordinary call, and outside those shapes the query param is not read at all.)
  - `token expired` / `invalid token` — issue a fresh one (bootstrap-JWT reissue - repeated `keeper init` is not available after the first Archon; for dev - `TOKEN=$(make dev-jwt)`; in production - through `POST /v1/operators/...`, see [operations/bootstrap-rbac.md](operations/bootstrap-rbac.md)). One case where that does not help: an instance whose clock runs *ahead* refuses a live token early by its drift, and the exchanged Bearer is capped by what is left of the session cookie, so a replacement can be shorter than the drift and arrive expired too. If reissuing keeps producing `token expired` in seconds, read it as skew and fix the clock. And `invalid token` is also the answer to a signature that does not verify — a token minted with another cluster's signing key never gets past it, so a fresh token repairs it only if it comes from the cluster you are calling.
  - `token issuer not trusted` — the token carries an `iss` other than the one this Keeper pins. Check the cluster's own config before concluding you are talking to a different cluster: `auth.jwt.issuer` defaults to the instance's `kid` ([keeper/config.md](keeper/config.md)), which is per-instance, so a cluster whose instances were never given one shared issuer name refuses every token minted by a *sibling* instance — on the same signing key, in the same cluster. That refusal flaps exactly like the next one, a retry landing on the minting instance succeeds, and the repair is one `auth.jwt.issuer` across the cluster, not a fresh token.
  - `token issued in the future` — **do not reissue.** The clock of the instance that minted the token is more than a minute ahead of the one refusing it, and a replacement carries the same offset. Fix the clock (NTP) on one of the two. Note that a retry may land on a third instance and succeed, which hides the fault rather than resolving it — intermittent 401s on a healthy-looking cluster are this, not an attack.
- **`soul init` has passed, but the host has not yet `connected` to `GET /v1/souls`.** The `pending → connected` transition happens when the daemon opens its stream, not when `soul init` finishes — `soul init` only obtains the certificate, and the row stays `pending` until `soul run` connects (NIM-865). The Reaper's lease-aware reconcile maintains `connected ⇄ disconnected` afterwards ([keeper/reaper.md](keeper/reaper.md)); it does not move `pending`. After `soul run` wait a few seconds and poll again. The authority of "Soul online" is a live stream (Redis-lease), the photo catches up with it with a lag - this is by-design, not freezing.
