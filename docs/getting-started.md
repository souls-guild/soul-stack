...
- **API returned `401` although the token "is".** Two different causes, and the `type` field of the problem+json tells them apart. `operator-revoked-token` — the Archon was revoked; the token is refused on every route from the next request onward, and a fresh token for the same AID will not help, because it is the identity that was withdrawn. Anything else (`unauthenticated`) — the token expired or was not accepted; issue a fresh one (bootstrap-JWT reissue - repeated `keeper init` is not available after the first Archon; for dev - `TOKEN=$(make dev-jwt)`; in production - through `POST /v1/operators/...`, see [operations/bootstrap-rbac.md](operations/bootstrap-rbac.md)).
