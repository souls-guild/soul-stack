...
Root product review - [../README.md](../README.md). The closed beta limits (what is NOT included) are [known-limitations.md](known-limitations.md).
...
| [naming-rules.md](naming-rules.md) | **Dictionary of names** Soul Stack (Keeper / Souls / Destiny / Soulprint / Essence + SoulSeed / Coven / SID / Archon-AID / Reaper) with matching SaltStack terms (master / minion / states / grains / pillars). Read before you begin to understand any configs; required before entering **any** new name. |
...
| [service/manifest.md#essence](service/manifest.md#essence) | **Essence** (analogue of Salt pillar) - hierarchical assembly of incarnation parameters (`essence/_default.yaml` + overlay by Coven / OS-family, opt. `_stack.yaml`-pipeline). Described inside manifest.md; full regulatory pipeline spec - [architecture.md → Essence](architecture.md#essence-assembly-pipeline). |
...
| [guides/plugin-author.md](guides/plugin-author.md) | **How ​​to write your own module (soul-mod-*) - index.** The author's authoritative step-by-step guide is in the companion repo `soul-stack-plugins` ([module-author-guide.md](https://github.com/co-cy/soul-stack-plugins/blob/main/docs/module-author-guide.md)); this core-side file is a short guide: when to write a plugin vs core/scenario + pointers to the author's core artifacts (SDK `sdk/module`, `proto/plugin/v1`, ADR-011 / ADR-016 / ADR-026). |
...
Full vocabulary and rules - [naming-rules.md](naming-rules.md). Basic Compliance:

| Soul Stack | SaltStack | Meaning |
|---|---|---|
| **Keeper** | master | Guardian, central server |
| **Souls** | minions | Managed Agents |
| **Destiny** | states | What is applied to the host after the run |
| **Soulprint** (Prints) | grains | Host System Facts |
| **Essence** | pillars | Parameters/values ​​collected hierarchically on incarnation |
