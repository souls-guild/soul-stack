...
- **API/MCP path** — **not provided in the MVP** for the Soul host. The Soul admin surface (a local HTTP/MCP listener) is deferred post-MVP ([open Q №8](../architecture.md)). Centralized rollout of `soul.yml` in the MVP is via CI / SSH (the operator's choice); upon receiving a new file the operator sends `SIGHUP`. The API/MCP path on the Keeper side is normalized, see [`../keeper/config.md → Hot-reload`](../keeper/config.md#hot-reload).
