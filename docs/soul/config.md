...
| `keeper.retry.max_attempts` | `int` (≥1) | `5` | Сколько раз подряд пробовать один endpoint, прежде чем считать его выбывшим из текущего захода. |
| `keeper.retry.backoff.initial` | `duration` | `1s` | Начальный интервал экспоненциального бэкоффа между попытками к одному endpoint. |
...
| `keeper.retry.*` / `keeper.failback.*` | yes | — | Параметры next retry / failback iteration. |
