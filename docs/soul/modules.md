...
- Keeper-side core (the seven of `coremanifest.KeeperSideAddrs()` — `core.soul`/`core.vault` per [ADR-017](../adr/0017-keeper-side-core.md), plus `core.choir`/`core.bootstrap`/`core.cert`/`core.ssh`/`core.state`; the list is stated once in [module/README.md](../module/README.md#catalog-status)) is added to the registry by the same mechanism (a new `<module>.yaml`).
