...
После успешного получения SoulSeed:

1. Прочитали файл с токеном в память один раз, использовали для запроса.
2. **Обнулили буфер** в памяти процесса; больше токен в RAM не живёт.
3. **`os.Remove(path)`** на файл с токеном.
4. **(best-effort)** перед `unlink` — overwrite случайными байтами + `fsync`. На classical FS даёт некоторую защиту, на SSD/NVMe с TRIM не гарантирует физическую перезапись (защищаемся коротким TTL).
5. **Никогда** не логируем содержимое токена — только `event: token_redeemed, token_id_prefix: <8 hex>`.
...
3. Оператор однократно выполняет `soul init [--config /etc/soul/soul.yml]`. Команда:
   - читает токен из stdin (приоритет) или из env `SOUL_BOOTSTRAP_TOKEN`; флаг с токеном не поддерживается, чтобы не светить в `ps`/shell-history;
   - endpoints, retry, tls берёт из конфига (`keeper.endpoints` и т.д.) — никаких аргументов командной строки;
...
- **Через `keeper.push` (целевой вариант, внутри Soul Stack).** Оператор после выписки SoulSeed-токена дёргает `keeper.push` для целевого хоста с Destiny, которая разворачивает `soul`-бинарь, кладёт токен и systemd-unit, стартует сервис. Дальше Soul делает CSR и переходит в pull. Это **тот же `keeper.push`**, который используется для агентless-доставки Destiny — просто с конкретным core-модулем `soul.installed` и токеном внутри Essence. Преимущество: единый аудит, RBAC и логи в Keeper-е, никаких сторонних тулз.
