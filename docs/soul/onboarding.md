...
1. The operator (or a script), via Keeper's OpenAPI/MCP, registers the host and obtains a bootstrap token for a specific SID:
   ...
   - **Scenario batch for ready-made VMs** — `core.bootstrap.issued` takes a unique non-empty list of FQDN/SIDs, atomically creates/checks `pending`, `transport=agent` Souls and returns one fresh token per host in the current run register. It does not call `core.cloud.created`. A repeat is the recovery operation for an interrupted/expired delivery; it invalidates every preceding unused token in the successful batch. An already onboarded identity gets no token either way, and whose host it is decides the rest (NIM-780): one of **this run** — member of the run's incarnation, or of none yet — is converged over as `{sid, onboarded: true}` and counted in `skipped`, so a `create` that failed after onboarding can be repeated to completion; one held by **another** incarnation is refused fail-closed. See [keeper/modules.md](../keeper/modules.md#corebootstrapissued).
...
- **Via the scenario steps `core.bootstrap.issued` → `core.bootstrap.delivered` (inside Soul Stack, [ADR-063](../adr/0063-bootstrap-token-delivery.md)).** `issued` supplies per-host tokens for ready-made VMs; after cloud provisioning the equivalent input already comes from `core.cloud.created`. `delivered` puts the token into a file (`token_path`, default `/etc/soul/token`; STDIN, not argv), redeems it with guarded `soul init`, and optionally activates the unit. Full-install (`install: true`) sends CA/soul.yml/unit/binary first. Teleport connects by SID and needs no `primary_ip`; direct SSH still requires one for every host it dials — a host flagged `onboarded: true` is skipped and needs none on either transport. Specifications: [issued](../keeper/modules.md#corebootstrapissued), [delivered](../keeper/modules.md#corebootstrapdelivered).
