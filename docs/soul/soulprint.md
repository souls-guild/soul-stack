...
Структура элемента `HostFacts` совпадает с `SoulprintFacts` + добавляются `covens`, `choirs`, `traits` (все три — Keeper-registry-проекции; `traits` — operator-set key-value метки, ADR-060) и `role` (Keeper-registry-проекция, доступна на любом прогоне). `role` заполняет topology-резолвер для каждого хоста roster-а с precedence **Choir Voice > spec**: источник — роль Voice-а из `incarnation_choir_voices` (ADR-044, S-T6), а `incarnation.spec.hosts[].role` — fallback для хостов без Voice (в т.ч. bootstrap-create, где Choir-членств ещё нет). Если ни Voice, ни spec роли не дают — `role` пустая.
...
- `.self.traits` — map<string, scalar|list> (registry-проекция operator-set меток, ADR-060)
...
| `souls.traits` (Postgres, Keeper-side, jsonb) | traits (operator-set key-value метки) | Оператор (registry-данность, как coven; ADR-060) |
...
**`soulprint.self.traits` в CEL** — operator-set key-value метки ([ADR-060](../adr/0060-traits.md)), отдельная ось рядом с плоским Coven. Значение Trait — скаляр (`namespace: dba-ns`, `product: aboba`) или список (`owners: [alice, bob]`); ключи задаёт оператор и они **динамичны** (произвольные имена). Источник — registry-колонка `souls.traits` (jsonb), Trait — организационная метка владельца/продукта, **НЕ** collected-факт и **НЕ** Soul-reported (как и `covens`/`choirs`/`role`). Доступна всегда; проекция **перекрывает** одноимённый reported-ключ, если бы Soul такой прислал (registry — источник истины, анти-спуфинг: скомпрометированный Soul не может подделать свою принадлежность). То же для `soulprint.hosts[].traits`. Таргетинг — `where: soulprint.self.traits.namespace == 'dba-ns'` (скаляр) или `where: 'alice' in soulprint.self.traits.owners` (список). Soul-lint статпроверяет только префикс `traits`, второй сегмент (ключ оператора) не сверяет — как у `covens`. В пилоте реализован только read/target-путь; bulk-assign API, audit-event, RBAC-scope по traits — отдельные слайсы (см. [ADR-060](../adr/0060-traits.md)).
...
- [ADR-060](../adr/0060-traits.md) — Trait (operator-set key-value метки, registry-проекция `soulprint.self.traits`).
