...
| `soulprint.self.traits` | map<string, scalar\|list> | **a Keeper-registry projection** of operator-set key-value labels (ADR-060; not from `SoulprintFacts`, see below). The **effective** set: the host's own `souls.traits` unioned with those of every incarnation it belongs to ([ADR-080](../adr/0080-label-inheritance-union.md)). The keys are dynamic — `soulprint.self.traits.<key>` without a static check of the 3rd segment. |

**The bare form `soulprint.<path>` without `.self`** is a **`soul-lint` validation error**. The canonical form is mandatory.

**`soulprint.self.sid` / `.covens` / `.choirs` / `.traits` / `.role` are a registry projection, available ALWAYS**, regardless of whether Soul has sent a `SoulprintReport`. The source is Keeper's roster (`souls.sid` / `souls.coven[]` / `incarnation_choir_voices` / `souls.traits` / the role from a Voice) — for `covens` and `traits` unioned with the labels of the host's incarnations ([ADR-080](../adr/0080-label-inheritance-union.md)) — not collected facts: `sid` is authoritative via the mTLS peer cert. Keeper mixes them into `soulprint.self` when resolving CEL even with NULL reported facts (a freshly connected host / the collector is not yet implemented). Registry fields **overwrite** identically named reported keys if those happen to be in the push (the registry is the source of truth, [ADR-018](../adr/0018-soulprint-typed.md)). The other branches (`os` / `network` / `kernel` / `cpu` / `memory`) are available only when Soul has sent them — otherwise an access gives the ordinary `no such key`.
...
| `souls.traits` (Postgres, Keeper-side, jsonb) + `incarnation.traits` of the host's incarnations | traits (operator-set key-value labels, unioned — [ADR-080](../adr/0080-label-inheritance-union.md)) | The operator (registry given, like coven; ADR-060) |
...
**The set is EFFECTIVE, not just the host's row** ([ADR-080](../adr/0080-label-inheritance-union.md)): a label attached to an **incarnation** reaches every host in it, and a label attached to a **host** covers that host — neither is copied into the other's storage, and a key held on both sides yields BOTH values (`owner=dba` on the incarnation + `owner=bobik` on the host → `owner` is the list `[bobik, dba]`, so `'dba' in soulprint.self.traits.owner` and `'bobik' in ...` both hold). A predicate written for a scalar key will therefore see a list once that key is also set at the other level — write `in` rather than `==` for keys you set in both places. The same union applies to `soulprint.self.covens` (plus the names of the host's incarnations) and to `soulprint.hosts[].*`, and it is the same union the RBAC scope predicate resolves, so targeting and visibility never disagree about a host.
