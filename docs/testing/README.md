...
| **L4** — manual soak + cloud live-run | — | — | Ручная пред-релизная проверка под нагрузкой **+ повторяемый облачный оркестратор** `scripts/e2e-cloud/` (create / day-2 / destroy через Operator API keeper'а на VM, teleport), см. [e2e-cloud.md](e2e-cloud.md). | пост первой проды / on-demand |
...
- **Живой облачный day-2 / create-destroy на постоянном keeper** → `scripts/e2e-cloud/`
...
- [e2e-cloud.md](e2e-cloud.md) — runbook облачного live-E2E оркестратора
  ...
  (`local` / `tsh`), suites create / create-destroy / day-2, ассерты по apply_run,
