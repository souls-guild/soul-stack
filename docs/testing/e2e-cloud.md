...
(`soul-cloud-wb-linux` / `soul-mod-redis` / `mod-manifest.yaml` - overridden
...
WB-specific (teleport paths `/mnt/c/...`, VM `192.168.2.3`, read `/root/.env`).
...
- `distribute-plugin` - deliver `soul-mod-redis` + `soul-cloud-wb`;
...
| `FQDN_SUFFIX` | `fedorovstepan2-dev.vm.xc.clv3` | stand suffix FQDN |
...
| `PROVIDER` / `PROFILE` | `wb-prod` / `redis-debian-12` | cloud provider / profile (report header) |
