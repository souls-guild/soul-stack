...
| Keeper does not start, **dsn must be postgres:// or postgresql:// URL** in logs | the secret behind `dsn_ref` is in the libpq keyword/value form (`host=… password=…`), which the connection pool accepts and the migration step does not | rewrite the secret as a URL: `postgres://<user>:<password>@<host>:5432/<db>?sslmode=verify-full`. The message names the scheme that arrived and never the DSN — read the value from Vault, not from the log |
| Keeper does not start, **dsn cannot be parsed (contains an invalid percent-escape / contains an invalid character in the host name / cause withheld…)** in logs | a character that is reserved in a URL sits raw in the password or the host of `dsn_ref`: `%`, `#`, `/`, `?`, a space or a control character | percent-encode it in the secret (`%` → `%25`, `#` → `%23`, `/` → `%2F`, `?` → `%3F`, space → `%20`). `@` and `:` inside a password are accepted as they are and need no encoding. `cause withheld` means naming the fault would have quoted the DSN back into the log — read the value from Vault, not from here |
| Keeper does not start, **dsn cannot be parsed (failed to configure TLS (sslmode is invalid) / cannot parse pool_max_conns / …)** in logs | the DSN parses, but one connection *setting* in it is wrong | fix the setting the message names. pgx's own diagnosis is passed through; the connection string pgx wraps it in is dropped, because pgx redacts that string with a pattern that misses `password = …` written with spaces |
...
- **state_schema-incarnation migrations** (ADR-019) is a **separate** operator-initiated operation via the Operator API (`POST /v1/incarnations/{id}/upgrade`), forward-only, does not start automatically when Keeper is restarted. Not to be confused with database schema migrations.
