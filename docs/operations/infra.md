...
| Топология | single instance + AOF | Sentinel или Cluster (зависит от масштаба) |
| Authentication | пароль обязателен (`password_ref` в `keeper.yml`) | ACL-пользователь с минимальным набором команд (post-MVP) |
...
| Решение | Когда |
|---|---|
| **Single instance** + AOF | dev / staging, малые инсталляции. |
| **Sentinel** (1 master + N replica + Sentinel quorum) | Прод on-premise — автоматический failover. |
| **Cluster** (sharded, 3+ master + replicas) | Большие инсталляции (десятки тысяч VM), когда single-node по латентности / памяти не тянет. |
| **Managed**: ElastiCache, Cloud Memorystore, Yandex Managed Redis | Прод в облаке. |

Cluster validated на мега-приёмке 2026-05-25 (3 keeper + 9-нодовый реальный redis-cluster) — leader-failover Reaper-а отрабатывает по TTL без split-brain, presence Souls / Conclave deriv-ятся корректно.

`redis.addr` в `keeper.yml` может быть:

- Single instance: `redis.internal:6379`
- Sentinel master discovery: на момент написания — не нативно поддержано (один TCP-адрес); раскатка через прокси (HAProxy в front-mode-tcp на динамический master) или через [ADR-006](../adr/0006-cache-redis.md#adr-006-кэш-и-координация--redis) extension.
- Cluster: один любой узел (клиент сам обновит топологию через `CLUSTER NODES`).
...
**Восстановление инфраструктуры** — поднять новый Redis, направить туда `keeper.yml::redis.addr` (через hot-reload или systemd reload). Souls продолжают работать, presence пере-фиксируется в новом Redis за TTL (~30s).
...
|---|---|---|---|
| KV v2 | `secret/` | `keeper/postgres` (DSN), `keeper/jwt-signing-key`, `keeper/redis` (password), `keeper/sigil-signing-key`, `keeper/sigil-keys/<key_id>` (R3 multi-anchor), Essence-секреты сервисов | резолв `vault:` ref в конфиге и в CEL `vault(...)` |
...
# KV v2 — обычно по умолчанию активен
...
| Redis password (`secret/keeper/redis`) | По политике | `CONFIG SET requirepass …` в Redis → `vault kv put` → `systemctl reload keeper`. |
