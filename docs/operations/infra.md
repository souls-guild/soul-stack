...
| PG password (`secret/keeper/postgres`) | By policy (once every 90 days) | `ALTER USER keeper PASSWORD …` in PG → `vault kv put secret/keeper/postgres dsn=…` → `systemctl restart keeper` **on every instance**. A `reload` will not do: the DSN is resolved and the pool is opened once at startup (`pg.NewPool`), and nothing re-opens it afterwards. Skipping the restart fails in the nastiest way possible - already open connections keep serving, and only new ones start failing to authenticate, so the breakage looks random and arrives late. |
| Redis password (`secret/keeper/redis`) | By politics | `CONFIG SET requirepass …` in Redis → `vault kv put secret/keeper/redis password=…` (if `redis.password_ref` is vault-ref) → `systemctl restart keeper` **on every instance** - same reason: `password_ref` is resolved from Vault when the client is constructed at startup, a reload does not rebuild it. The Redis password resolution from Vault has been implemented - `password_ref: vault:secret/keeper/redis` is valid. |
...
| Vault AppRole `secret_id` | By TTL `secret_id_ttl` (default 720h in our template) | `vault write -f auth/approle/role/keeper-prod/secret-id` → rewrite `/etc/keeper/vault-secret-id` (mode 0400). **Nothing has to be signalled to the running process**: the AppRole login happens once inside `vault.NewClient` at startup, after which Keeper lives on the issued token, and the new `secret_id` is consumed by the next start. Neither `reload` nor an immediate restart is needed - but the file must be kept current, otherwise the next start (including an unplanned one after a crash) fails to log in and Keeper does not come up. |
