...
| **Sigil trust-anchor** (ed25519 plugin-signing) | private - Vault KV `secret/keeper/sigil-keys/<key_id>`; public recruitment goes to Soul in `BootstrapReply` ([ADR-026(d)/(h)](../adr/0026-sigil.md)) | Signs accepted plugin digests. Compromise = ability to sign an arbitrary plugin binary (RCE on the fleet via a forged plugin binary). |
...
- **`govulncheck` is clean for all modules** - supply-chain scan is embedded in `make check` (see environment requirements, target `make check-vuln`, which since NIM-774 covers every `go.mod` in the tree - twenty modules, where the corpus used to be the eight of `$(MODULES)`).
...
- **Regular `govulncheck`.** Supply-chain scan of dependencies is embedded in `make check` through the target `make check-vuln` (running `govulncheck ./...` across every Go module in the tree - go.work members inside the workspace, `examples/module/*` and the pluginhost fixtures standalone, and the `tests/*` harnesses under the build tag their code lives behind; offline-graceful - `SKIP_VULNCHECK=1` passes without a network). Until NIM-774 the corpus was the eight modules of `$(MODULES)` and the sentence below was true of those eight only: the twelve others were not scanned, not skipped and not named. `make check-vuln-corpus` is the guard that keeps the corpus the whole tree. What the sweep still does **not** see, and prints on every run: `_test.go` files anywhere (`govulncheck -test` defaults to false), and code behind the `smoke` tag. At the time of fixation, govulncheck is clear in all modules. Pre-release launch - part of `make check`; periodic repeat for fresh vuln-DB - operational hygiene.
