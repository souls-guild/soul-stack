...
- **node-exporter** — системные метрики, слушает стандартный `:9100`; бинарь
  `node_exporter` под стабильным system-аккаунтом `node_exporter` (stateful-ветка),
  version-aware install + hardened-unit.
  Коллекторы железа выключены (`node_exporter_collectors: []`) — ставится только ядро.
...
[`redis`](../redis/) (full-stack-эталон apply:destiny): они лежат
в [`examples/destiny/`](../../destiny/) (`redis-single` /
...
- **Checksum tarball-ов — обязательный input** (`node_exporter_sha256`,
  ...
  Передайте хэши из `sha256sums.txt` GitHub-релиза под пару (version, arch);
  `core.url.fetched` верифицирует их ДО публикации файла.
- **Пароль Redis** (`redis_password`, `secret`, обязателен) уходит в обе destiny:
  ...
  в `ps`/journalctl). Отличие от [`redis`](../redis/): там пароль
...
- **Доступ redis_exporter к сокету — через группу `redis`.** Сокет создаётся
  ...
  пакет redis-server из `redis-single`). Ручного `core.user` для redis_exporter
  нет — least-privilege без стабильного uid. node-exporter сокет не нужен, но его
  аккаунт — **стабильный** system-user `node_exporter` (stateful-ветка: textfile-каталог
  железных метрик переживает рестарты), его заводит сама destiny `node-exporter`
  через `core.group.present`/`core.user.present`.
...
destiny (`redis-single` / `redis-exporter` / `node-exporter`),
...
3. **node-exporter** (`apply:destiny`): та же «тройка» (version-aware
   `install` бинаря `node_exporter`) → стабильный system-аккаунт
   `node_exporter` → hardened-unit + `core.service.running`. Коллекторы железа
   выключены (`collectors: []`). От Redis не зависит, идёт последним для
   детерминизма.
...
| `arch` | enum `amd64`/`arm64` | `amd64` | Архитектура релизного tarball-а экспортеров. Объявлен в контракте scenario, но фактически arch берётся напрямую из `soulprint.self.os.arch` (один incarnation может смешивать amd64/arm64-хосты) — этот input в задачи не доезжает. |
...
| `node_exporter_collectors` | array enum `smartmon`/`nvme`/`ipmi` | `[]` | Какие textfile-коллекторы железа node-exporter ставить. `[]` = только ядро. |
| `redis_exporter_listen` | string `host:port` | `:9121` | Listen-адрес redis_exporter. |

Обязательные параметры — `redis_password` и два checksum-а
(`node_exporter_sha256`, `redis_exporter_sha256`, fail-closed по
...
Сервис-аккаунты — по [прод-конвенции §2](../../../docs/destiny/production-conventions.md#2-сервис-аккаунт--гибрид-правило):
**redis-exporter** stateless → `DynamicUser=yes` (ручного `core.user` нет);
**node-exporter** stateful (textfile-каталог железных метрик) → стабильный
`node_exporter` через `core.group.present`/`core.user.present` внутри destiny.

## Валидация

```bash
./soul-lint/bin/soul-lint validate-service  examples/service/redis-monitored/service.yml
./soul-lint/bin/soul-lint validate-scenario examples/service/redis-monitored/scenario/create/main.yml
```
...
```bash
./keeper/bin/soul-trial run examples/service/redis-monitored
```
...
redis-single + redis-exporter + node-exporter со сквозными индексами; node-exporter
раскрывается в полную ленту (бинарь + аккаунт `node_exporter` + unit + все шаги
коллекторов, отбракованные хостом по `when:` при `collectors: []` — в плане они
присутствуют, но при apply дают no-op). Ассертит проброс пути сокета в redis.conf,
сборку `redis_addr=unix://...` для redis_exporter, раскладку бинаря
`node_exporter` (version-aware `unless`), `arch` из soulprint в url-ах экспортеров
и декларативные `core.service.running`-шаги. Даёт `PASS`.
...
> обязательные `redis_password`, `node_exporter_sha256`, `redis_exporter_sha256`.
...
> на linux-стенде через `keeper.push` / pull-агента. `node_exporter_sha256` /
> `redis_exporter_sha256` — обязательные параметры (без дефолта): для реального
> прогона передайте настоящие хэши из `sha256sums` GitHub-релиза, иначе
> `core.url.fetched` упадёт на checksum mismatch.
...
- Инлайн-задачи установки — нет: всё делегировано в три standalone-destiny через
  ...
  дрейфом; см. full-stack-эталон [`redis`](../redis/).
