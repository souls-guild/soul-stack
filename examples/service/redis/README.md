...
> **Состояние (2026-06-22).** Реализованы режимы **`standalone`**, **`cluster`**
> (honest hash-slot Redis Cluster, 16384 слота) и **`sentinel`** (master-replica +
> sentinel-демон) — все три в scenario `create`, выбор через `redis_type`
> ([`sentinel.yml`](scenario/create/sentinel.yml) включён в диспетчер). Реализована и
> day-2 операция cluster — присоединение ноды через отдельный scenario
> [`add_node`](scenario/add_node/main.yml). Нереализован только режим
> **`sentinel_only`**: enum-значение зарезервировано, mode-guard первой задачей
> диспетчера рвёт прогон с понятным сообщением, если он выбран (а не молчаливый
> зелёный no-op). Остальной backlog (cluster day-2 remove/reshard, sentinel
> failover/day-2, TLS/модули) — см. [«В работе»](#в-работе). README описывает только
...
| поле | тип | смысл |
|---|---|---|
| `redis_type` | enum `standalone`/`sentinel`/`cluster`/`sentinel_only` | режим развёртывания (реализованы `standalone`/`cluster`/`sentinel`; `sentinel_only` зарезервирован) |
...
| `redis_sentinel` | object `{master_name, quorum}` | факты sentinel-режима (только `redis_type=sentinel`): имя monitored master + quorum; `quorum` из input или `0` (auto `size/2+1` вычисляется в apply, в state не материализуется). Вне sentinel — пустой объект |
...
| поле | тип | смысл |
|---|---|---|
| `redis_type` | enum, default `standalone` | режим; обрабатываются `standalone`/`cluster`/`sentinel` (выбирает ветку диспетчера); `sentinel_only` рвётся mode-guard-ом |
...
| `sentinel_master_name` | string, опц., default `mymaster` | **(sentinel)** логическое имя monitored master в Sentinel (`SENTINEL MONITOR`, `sentinel.conf`) |
...
прогона вынесено в три ветки, подключаемые через `include:` —
[`standalone.yml`](scenario/create/standalone.yml),
[`cluster.yml`](scenario/create/cluster.yml) и
[`sentinel.yml`](scenario/create/sentinel.yml). `include:` раскрывается **безусловно**
...
standalone, cluster, sentinel: standalone-задачи остаются на прежних индексах плана,
cluster- и sentinel-placeholder-ы добавляются в хвост.
...
поэтому при значении вне `{standalone, cluster, sentinel}` (напр. `sentinel_only`,
запланирован отдельным батчем) **все** ветки гаснут placeholder-skip-ом и `create`
прошёл бы зелёным, ничего не развернув. Guard вычисляет keeper-side булево «режим
реализован»: реализован → no-op; нет → `echo` с конкретным `redis_type` + `exit 1`
рвут прогон fail-fast (кейс [`sentinel-only-not-implemented`](scenario/create/tests/sentinel-only-not-implemented/case.yml)).
...
(из `input.users`), `redis_hosts = []`, `redis_sentinel` (`{master_name, quorum}`
только в режиме sentinel, иначе пустой объект).
...
- [`sentinel-only-not-implemented`](scenario/create/tests/sentinel-only-not-implemented/case.yml)
  — `redis_type=sentinel_only`: mode-guard диспетчера рвёт прогон fail-fast.
...
- режим `sentinel_only` (Sentinel-только хосты без локального redis-инстанса):
  enum-значение зарезервировано, mode-guard диспетчера рвёт прогон fail-fast, если
  он выбран;
- day-2 cluster-операции: `remove-node` / `reshard` (плагин
  `community.redis.cluster` умеет `action: create` + `add-node`; `remove-node`/
  `reshard` в action-enum пока нет);
