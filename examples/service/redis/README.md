# redis - unified Redis service

The "redis" service is **one service for all Redis deployment modes**. The mode is
selected via the `redis_type` field
...
tuning are an **unconditional baseline** (Redis recommendation / hardening), not an
operator choice. These tasks are always active
...
- **redis-server** - `default` is **absent** from `system_acl_users`: it's set via
  `requirepass` in `redis.conf` (`users.acl` doesn't duplicate it).
...
4. **SENTINEL MONITOR reconcile** (`community.redis.sentinel`, on **every** host) -
   ...
   The reconcile algorithm is `MONITOR`/`REMOVE`+`MONITOR`/`SET`/`CONFIG SET`.
   **★ Gate `when: vars.sentinel_monitor_now`** (the deploy-body's
