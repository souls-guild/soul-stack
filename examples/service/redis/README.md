...
Redis Cluster (16384 слота). Три шага:
   ...
   **Render-гарда топологии нет.** Прежний size-guard (`assert:` на число хостов
   roster-а `== shards * (1 + replicas_per_master)`) **удалён**: неверная топология
   **не** отвергается fail-fast при рендере. Несоответствие числа таргетированных souls
   объявленным счётчикам проявится **позже** — при сборке кластера плагином
   `community.redis.cluster` (раскладка слотов по фактическому roster-у), не как
   обрыв render-а. `required_when` на `shards` (input-валидация присутствия) — остался;
   это не size-guard (он про присутствие одного поля, не про сверку с roster-ом).
...
master (`soulprint.hosts[0]`, отсортирован детерминированно). Пять шагов:

   **Render-гарда топологии нет.** Прежний size-guard (`assert:` на число хостов
   roster-а `== 1 + replicas_per_master`) **удалён** (тот же механизм, что в cluster).
   Несоответствие числа таргетированных souls счётчику `replicas_per_master` **не**
   обрывает render fail-fast — проявится позже при привязке реплик/настройке
   sentinel-демонов на фактическом roster-е.
...
— идемпотентно). Следующие батчи эпика redis-консолидации (в этом сервисе
**пока не реализованы**):
