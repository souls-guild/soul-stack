...
>
> Реализованы операционные cluster-операции — присоединение/вывод/решардинг ноды через
...
> [`restart`](scenario/restart/main.yml), а также операционный hot-reload
...
> failover/операции, плагинный `failover`, sentinel-демон TLS) — см.
...
- **destiny [`redis`](../../destiny/redis/)** — режим-агностичный per-host кирпич:
  ...
  отдельные бинари из Nexus), render `redis.conf` из **готового merged**-конфига, render
...
- **scenario сервиса** — трансляция простого ввода в `redis_config` (через
  ...
  cluster-топологии в `nodes`-MAP; в будущих батчах — rolling-restart, операционные сценарии).
...
[`service.yml → state_schema`](service.yml), `state_schema_version: 11`. Цепочка
...
- [`002_to_003.yml`](migrations/002_to_003.yml) — «richer state»: opaque
  `redis_config` дополнен именованными полями намерения (read-model:
...
- [`004_to_005.yml`](migrations/004_to_005.yml) — тайминги failover sentinel:
  `redis_sentinel` дополнен `down_after_ms`/`failover_timeout_ms` (read-model,
...
- [`007_to_008.yml`](migrations/007_to_008.yml) — **cascade-destroy read-model**
  ...
  SID/FQDN VM, поднятых нашим прогоном. Teardown читает их для cascade-чистки
...
- [`008_to_009.yml`](migrations/008_to_009.yml) — **seed-source read-model** (пилот
  ...
  (bool — `false` после migrate, `true` после `detach_source`). Время намеренно не в
...
- [`010_to_011.yml`](migrations/010_to_011.yml) — **редизайн input-контракта `create`**
  (`connection_mode` + `io_threads` + расширение `persistence` + restruct sentinel-таймингов).
  Четыре независимых преобразования state (best-effort — миграция = чистая функция от старого
  state, ADR-019: `essence`/`input`/`vault` в migration-CEL запрещены):
  (1) **persistence REMAP** старого enum `[off, aof, rdb, rdb_aof]` в новый — `aof → aof_1sec`,
  `rdb_aof → rdb_aof_1sec` (старые `aof`/`rdb_aof` = fsync `everysec` бит-в-бит), `off`/`rdb` —
  тождество;
  (2) **`connection_mode`** (новое, enum `tls`/`tls_plain`/`plain`) — обратная свёртка из
  существующего namedfield `tls.{enable,only}`: `!enable → plain`, `enable && only → tls`,
  `enable && !only → tls_plain`;
  (3) **`io_threads`** (новое, int) — best-effort default `0` (исходное число I/O-потоков в
  namedfields не лежало — необратимо из `redis_config`, как `memory_mb` в `002_to_003`);
  (4) **`redis_sentinel` RESTRUCT** (только sentinel-инкарнации): именованные
  `down_after_ms`/`failover_timeout_ms` свёрнуты в `master_settings` (`map<string,string>`
  эффективных per-master директив — зеркало `input.sentinel_master_settings`) +
  `settings: {}` (глобальные sentinel — зеркало `input.sentinel_settings`); cluster
  (`redis_sentinel == {}`) → no-op. Точное значение полей восстанавливается на следующем
  `create`-override.
...
| `redis_version` | string | эффективная версия Redis из input `version` (enum выкаченных в Nexus сборок — `8.6.1`/`8.4.0`/`8.2.2`/`8.0.3`/`7.4.1`/`6.2.21`). `install_method=binary` (default) качает бинари этой версии из Nexus; `install_method=package` ставит `=version`-пин; см. input `version` / `install_method` |
| `connection_mode` | enum `tls`/`tls_plain`/`plain` | **(v11)** режим сетевого канала: `plain` — только plain-порт (TLS off); `tls_plain` — TLS и plain одновременно; `tls` — только TLS (plain закрыт, `port 0`). Read-model (заменил булевы `tls_enabled`/`tls_keep_plain`) |
| `io_threads` | integer | **(v11)** число I/O-потоков Redis (директива `io-threads`). `0` — однопоточный I/O (директива не пишется) |
...
| `redis_sentinel` | object `{master_name, quorum, master_settings, settings}` | факты sentinel-режима: имя monitored master (из `essence.sentinel_master_name`, дефолт `master`) + quorum + `master_settings` (эффективный per-master dict директив sentinel = `essence.sentinel_master_defaults` поверх `input.sentinel_master_settings`, `map<string,string>`; дефолты `down-after-milliseconds`=`5000`/`failover-timeout`=`60000`) + `settings` (глобальные passthrough-директивы `sentinel.conf` из `input.sentinel_settings`). `quorum` всегда `0` (auto `size/2+1` вычисляется в apply, в state не материализуется). **(v11 restruct)**: прежние именованные `down_after_ms`/`failover_timeout_ms` (v5) свёрнуты в `master_settings`. Вне режима `sentinel` — пустой объект |
| `provisioned_vm_ids` | array string | **(v7, cloud-provision read-model, [ADR-061](../../../docs/adr/0061-onboarding-await-and-midrun-reresolve.md))** provider-vm-id VM, поднятых **этим** create-прогоном через `core.cloud.created` (из `register.provision.vm_ids`). Teardown читает их для `core.cloud.destroyed`. Без provision (`input.provision` опущен/`enabled:false`) — `[]` (хосты — declared roster, не провиженные сервисом). См. [«Cloud-provision»](#cloud-provision-create-поднимает-vm-live-ждёт-c1) |
...
[«Трансляция»](#трансляция-простого-ввода-в-redis_config)), а операционные сценарии читают их
...
| `version` | string, **обязателен** (`required`), enum `8.6.1`/`8.4.0`/`8.2.2`/`8.0.3`/`7.4.1`/`6.2.21` | версия Redis из фиксированного набора выкаченных в Nexus сборок (свободный distro-пин заменён закрытым enum — редизайн). `install_method=binary` (default) качает бинари этой версии из Nexus; `install_method=package` ставит `=version`-пин тем же значением. Произвольная версия не принимается |
| `install_method` | enum `package`/`binary`, default `binary` | способ установки redis. `binary` (DEFAULT) — отдельные бинари `redis-server`/`redis-cli`/`redis-benchmark`/`redis-sentinel` из Nexus (`essence.binary_base_url` + `arch`/`Debian`/`distro_ver`/`version` per-host из `soulprint.self.os`; качаются по content-идемпотентности SHA-256, без integrity-verify). `package` — distro-пакет `redis-server` (`=version`-пин). URL/версия бинаря — НЕ operator-input: они в `essence` (`binary_base_url`/`binary_version`, author-context), оператор переопределяет в `spec.essence`. Прежний object-input `install` сведён к одному enum-полю |
| `memory_mb` | integer, опц., min `64` | бюджет памяти под Redis на хосте, МБ; `maxmemory` = доля от него |
| `io_threads` | integer, опц., default `0`, min `0` | число I/O-потоков Redis (директива `io-threads`). `0` (дефолт) — однопоточный I/O (директива не пишется); `>0` — Redis распараллеливает обработку сокетов (типично = числу ядер − 1). Пишется в `redis.conf` только при `>0` |
| `persistence` | enum `off`/`aof_1sec`/`aof_always`/`rdb`/`rdb_aof_1sec`/`rdb_aof_always`, default `rdb` | режим durability; транслируется в `save`/`appendonly`/`appendfsync`. `off` — без сохранения; `aof_1sec`/`aof_always` — только AOF (fsync `everysec` / `always`); `rdb` — только снапшоты; `rdb_aof_1sec`/`rdb_aof_always` — снапшоты + AOF (двойная durability). Расширено с `[off, aof, rdb, rdb_aof]` (редизайн: явный выбор fsync-частоты `1sec`/`always`) |
| `maxmemory_policy` | enum eviction-политик | политика вытеснения при достижении `maxmemory` |
| `replicas_per_master` | integer, опц., default `2`, min `0`, max `8` | **(оба режима)** реплик на каждый master. `cluster` — реплик на шард (roster = `shards * (1 + replicas_per_master)`); `sentinel` — реплик master-а (roster = `1 + replicas_per_master`). default `2` (HA-набор: master + 2 реплики); `0` — без реплик (`sentinel` с `0` = standalone-эквивалент); потолок `8`. Унифицировано 2026-06-25 (заменило прежние `replicas` + `replicas_per_shard`) |
| `shards` | integer, обяз. при `cluster` (`required_when`), min `3` | **(cluster)** число master-шардов; 16384 hash-слота делятся поровну между мастерами. `min 3` — нижний предел honest Redis Cluster (из `<3` мастеров не собирается). Обязателен при `redis_type=cluster` через `required_when: "input.redis_type == 'cluster'"` — пропуск рвёт input-валидацию ДО рендера |
...
| `cluster_topology` | array of array of string, опц. | **(cluster)** явная раскладка шардов: список шардов, каждый шард — список SID `[master, replica, …]` (первый = master). Оператор сам распихивает VM по зонам / кодирует anti-affinity. Не задан → плагин раскладывает роли автоматически по sort SID (поведение до фичи бит-в-бит). Каждый SID обязан быть среди таргетированных souls ровно один раз; все хосты roster-а покрыты |
...
| `redis_settings` | object (passthrough, key→value строки) | произвольные директивы `redis.conf` key→value; **бьют всё** в итоговом merge. **Имена** директив version-aware-валидируются `assert`-ом против каталога валидных директив выбранной версии Redis (см. [«Валидатор имён директив»](#валидатор-имён-директив-redis_settings)) — опечатка / директива из чужой версии рвёт прогон на render (422) ДО applying |
| `connection_mode` | enum `tls`/`tls_plain`/`plain`, default `plain` | **(сетевой канал)** режим канала Redis (заменил булевы `tls_enabled`/`tls_keep_plain` — редизайн). `plain` (дефолт) — только plain-порт 6379 (TLS off); `tls_plain` — TLS и plain одновременно; `tls` — только TLS (plain-порт закрыт, `port 0`). enum **структурно** исключает «нет ни одного listener-а». Технические параметры TLS (порт, Vault-пути `cert`/`key`/`ca`) — НЕ operator-input: они в `essence` (`tls_port`/`tls_cert_ref`/`tls_key_ref`/`tls_ca_ref`, author-context). destiny читает PEM через `vault(ref)` в ячейке `content` (seal-маскинг) |
| `sentinel_settings` | object (passthrough, key→value строки), опц. | **(sentinel)** глобальные (не per-master) passthrough-директивы `sentinel.conf` (`resolve-hostnames`, `announce-port`, …), не покрытые `sentinel_master_settings`. Сливаются в `sentinel.conf` вместе с per-master директивами. Без version-валидации (sentinel-каталога нет). Новое поле (редизайн) |
| `sentinel_master_settings` | object (passthrough, key→value строки), опц. | **(sentinel)** per-master директивы Sentinel: ключ — имя опции `SENTINEL SET` без префикса и master-name (`down-after-milliseconds`, `failover-timeout`, `parallel-syncs`, …); значение — строка. Перекрывают дефолты `essence.sentinel_master_defaults` (`down-after-milliseconds`=`5000`, `failover-timeout`=`60000`). **Поглотило** прежние именованные поля `sentinel_down_after_ms`/`sentinel_failover_timeout_ms` (редизайн) |
...
- `sentinel_down_after_ms` / `sentinel_failover_timeout_ms` — **поглощены** dict-ом
  `sentinel_master_settings` (ключи `down-after-milliseconds` / `failover-timeout`);
  их дефолты `5000`/`60000` переехали в `essence.sentinel_master_defaults` (подложку под
  этот dict). Редизайн: именованные поля заменены passthrough-dict-ом.
- `conf_dir` / `data_dir` — переехали в `essence` (`conf_dir`=`/etc/redis`,
  `data_dir`=`/var/lib/redis`, author-context; оператор переопределяет раскладку в
  `spec.essence` под свой storage-layout, не в Run-форме). В `state` они остаются
  (`add_node` читает layout из `incarnation.state`) — источник правки сместился
  input→essence, форма state не меняется.
- технические параметры TLS (`tls_port`, Vault-пути `tls_cert_ref`/`tls_key_ref`/`tls_ca_ref`)
  — в `essence` (author-context), не operator-input: оператор выбирает только режим канала
  через `connection_mode`, а порт `7379` и пути PEM переопределяет в `spec.essence`.
- `tls_enabled` / `tls_keep_plain` — булевы галочки **удалены**, заменены enum
  `connection_mode` (`tls`/`tls_plain`/`plain`); `compute.tls_on`/`tls_only_on` выводятся
  из enum.
...
> **Два прежних `validate`-правила удалены редизайном.** (1) «хотя бы один listener»
> (`tls.only` требовал `tls.enable`) — `connection_mode` enum `[tls, tls_plain, plain]`
> **структурно** исключает «нет ни одного listener-а» (нельзя выбрать «TLS off + plain
> закрыт»). (2) «`install.method=binary` требует `base_url`/`version`» — URL и версия
> бинаря переехали из operator-input в `essence` (`binary_base_url`/`binary_version`,
> author-context), а `validate`-контекст input-only (essence недоступен); пустой
> `binary_base_url` при `install_method=binary` проявится при render-е destiny, не на
> input-валидации.
...
Во **всех** задачах, рендерящих `users.acl` (create cluster/sentinel + операционные
...
в операционных сценариях / `compute.users_new` в `add_user`). Порядок аргументов `merge()` = приоритет:
...
### ★ Долив в операционных сценариях (инвариант)
...
и сломал бы в операционных сценариях репликацию / sentinel↔redis / health-check. Системные **не
...
4. **SENTINEL MONITOR reconcile** (`community.redis.sentinel`, на **каждом** хосте) —
   ...
   `REPLICAOF NO ONE` (см. [«`migrate_cluster`»](#migrate_cluster-миграция-с-внешнего-кластера)).
...
- **Series-skip:** `input.version` (enum `8.6.1`/`8.4.0`/`8.2.2`/`8.0.3`/`7.4.1`/`6.2.21`)
  → серия `X.Y` извлекается регуляркой; серии **нет** в каталоге → `assert` пропускается
  (валидируются только известные серии, не-каталожная не блокируется). Каталог покрывает
  `6.2`/`7.0`/`7.2`/`7.4`/`8.0`/`8.2`; серии `8.6`/`8.4` пока **не** в каталоге → для версий
  `8.6.1`/`8.4.0` валидатор делает series-skip (добавить серию — перегенерировать каталог).
  **Empty-catalog-skip:** нет `essence.redis_directives` (старый essence) → проверка
  пропускается (back-compat).
...
- **Только в `create`:** `update_config` (тоже принимает `redis_settings`) этого
  валидатора пока **не несёт** — расширение follow-up.

### `add_node` (присоединить ноду к кластеру)
...
2. **`apply: destiny: redis`** на **новой** ноде (`where: soulprint.self.sid == input.new_node_sid`)
   ...
   **источник истины**, зафиксированный `create`, не перевычисляется → нет drift) + systemd.
   ...
   примет её как реплику (см. [«★ Долив в операционных сценариях»](#-долив-в-операционных-сценариях-инвариант)).
...
### `remove_node` (вывести ноду из кластера)
...
### `reshard` (перенести N слотов между мастерами)
...
> **★ `reshard` НЕ идемпотентен** (осознанно, exec-style): повторный apply
...
### `restart` (безопасный rolling-restart)
...
> **★ Источник истины операционных сценариев = `incarnation.state`, не `essence`** ([production-conventions §7a](../../../docs/destiny/production-conventions.md)).
...
> выбрал режим канала через `input.connection_mode` (`tls`/`tls_plain`/`plain`), и эта
> развёрнутая конфигурация зафиксирована только в `state`. Смотреть на `essence` дало бы
...
> применяют операционные hot-reload-сценарии ниже (`update_config`/`add_user`/`rotate_tls`).
...
### `update_config` (hot-reload директив `redis.conf`)
...
развёрнутое — источник истины операционных сценариев = `state`, [§7a](../../../docs/destiny/production-conventions.md#7a-операционные-сценарии-источник-истины--incarnationstate)). Два шага:
...
1. **re-render** `redis.conf` на диск с новым merged config (полный файл — desired
   ...
   [«★ Долив в операционных сценариях»](#-долив-в-операционных-сценариях-инвариант)).
...
### `add_user` (добавить ACL-пользователя через `ACL LOAD`)
...
`AclUser`**, что в create (закрывает операционный мусор: невалидная ACL-строка отсекается на
...
[`update_users`](#update_users-массовое-редактирование-набора-acl-юзеров)
...
1. **re-render** `users.acl` на диск с новым набором: **системные** служебные юзеры
   ...
   [«★ Долив в операционных сценариях»](#-долив-в-операционных-сценариях-инвариант)). per-user пароли из Vault, `.tmpl`
...
### `update_users` (массовое редактирование набора ACL-юзеров)
...
- **системные не трогаются** — `replica`/`monitoring`/`sentinel`/`haproxy` **не** в
  ...
  [«★ Долив в операционных сценариях»](#-долив-в-операционных-сценариях-инвариант)). Bulk-replace operator-extra их **не**
...
### `rotate_tls` (ротация TLS cert/key/CA без рестарта)
...
### `migrate_cluster` (миграция с внешнего кластера)
...
(`REPLICAOF NO ONE`) делает отдельный [`detach_source`](#detach_source-отцепить-внешний-источник)
...
### `detach_source` (отцепить внешний источник)
...
**Диспетчер по `incarnation.state.redis_type`** (операционный сценарий знает режим из state, не из input —
...
- **Guard на не-мигрированную инкарнацию:** `detach` — обычный операционный сценарий, оператор может
...
- [`tls-enabled`](scenario/create/tests/tls-enabled/case.yml)
  — `connection_mode: tls` (только TLS): TLS-PEM-задачи активны, `tls-port`/
  `tls-cert-file`/… в merged `redis.conf`, `port 0` (plain закрыт).
- [`tls-enabled-no-only`](scenario/create/tests/tls-enabled-no-only/case.yml)
  — `connection_mode: tls_plain`: TLS-порт открыт, plain-порт остаётся (без `port 0`).
- [`tls-essence-refs`](scenario/create/tests/tls-essence-refs/case.yml)
  — `connection_mode: tls`, технические TLS-параметры из essence (нестандартный
  `tls_port` 7400, refs `secret/ops/redis/tls`) — оператор задаёт только режим канала.
- [`tls-disabled`](scenario/create/tests/tls-disabled/case.yml)
  — `connection_mode` не задан (default `plain`): TLS off, PEM-задачи placeholder-skip,
  директив TLS нет.
- [`tls-cluster`](scenario/create/tests/tls-cluster/case.yml) — `connection_mode: tls`
  + cluster: `tls-replication`/`tls-cluster yes` в merged config.
- [`install-package`](scenario/create/tests/install-package/case.yml) —
  `install_method: package` (задан явно — default теперь `binary`): distro-пакет с
  `=version`-пином.
- [`install-binary`](scenario/create/tests/install-binary/case.yml) —
  `install_method: binary` (default): **четыре** `core.url.fetched` качают отдельные
  бинари `redis-server`/`redis-cli`/`redis-benchmark`/`redis-sentinel` из Nexus (путь
  `<base_url>/<arch>/Debian/<distro_ver>/<version>/…`) + distro-юзер/группа + свой
  systemd-юнит + симлинки `redis-check-aof`/`redis-check-rdb`; package-ветка
  placeholder-skip.
...
- [`empty-modules-base-url`](scenario/create/tests/empty-modules-base-url/case.yml)
  — пустой `essence.modules_base_url` → vanilla redis (modules.yml group-drop, без
  `loadmodule`/fetch `.so`).
- [`with-modules-base-url`](scenario/create/tests/with-modules-base-url/case.yml)
  — непустой `essence.modules_base_url` на Redis < 8 → полный набор `.so` качается.
...
Операционный hot-reload реализован: `update_config` (live `CONFIG SET` дельты, → state
...
[`migrate_cluster`](#migrate_cluster-миграция-с-внешнего-кластера) (sentinel +
cluster) + [`detach_source`](#detach_source-отцепить-внешний-источник)
...
- эксплуатация sentinel: failover (switchover) и прочие операции sentinel-топологии;
...
- TLS sentinel-демона (`:26379`): data-плоскость redis-server TLS уже реализована
  (operator-enum `connection_mode` ∈ `tls`/`tls_plain`/`plain`; технические параметры —
  в essence), TLS для sentinel-демона — follow-up;
- **cloud-provision (`input.provision`) — keeper-side реализован, live ждёт C1**: доставка
  ...
  (реализован);
- **version-aware валидатор `redis_settings` в `update_config`** — пока только в
