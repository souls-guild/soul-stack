...
| `master_ip` | string, опц. | **(sentinel_only)** IP/адрес **внешнего** redis-master, который мониторит sentinel-кластер. Обязателен при `sentinel_only` — пропуск рвёт master_ip-guard (`redis_type=sentinel_only требует input.master_ip`). В прочих режимах не используется (master выбирается из roster-а) |
...
1. **master_ip-guard** (`core.cmd.shell`, `run_once`, fail-fast ДО любого install) —
   `master_ip` обязателен в этом режиме, но на schema-уровне остаётся
   `required: false` (условную по режиму обязательность схема не выражает). Пропуск
   ловит guard: keeper-side `has(input.master_ip)` встраивается в команду — задан →
   `test "true" = "true"` (no-op), не задан → `echo "redis_type=sentinel_only требует
   input.master_ip ..."; exit 1` рвёт прогон с понятным сообщением (вместо криптичного
   CEL «no such key» ниже).
2. **`apply: destiny: redis`** с `deploy_redis: false` — пакет `redis` ставится
...
3. **SENTINEL MONITOR reconcile** (`community.redis.sentinel`, на **каждом** хосте) —
...
4. **health-gate PONG** (`community.redis.command`, `:26379`) — каждый sentinel-демон
   обязан ответить `PONG`. Строго same-task `register.self` (НЕ cross-passage
...
- [`sentinel-only-create`](scenario/create/tests/sentinel-only-create/case.yml)
  — `redis_type=sentinel_only`, `master_ip` задан: mode-guard и master_ip-guard
  no-op (`true`), пакет `redis` ставится при `deploy_redis=false`, `sentinel.conf`
...
- [`sentinel-only-requires-master-ip`](scenario/create/tests/sentinel-only-requires-master-ip/case.yml)
  — `redis_type=sentinel_only` без `master_ip`: mode-guard no-op, master_ip-guard
  FAIL-ветка (`test "false" = "true"` → `exit 1`).
