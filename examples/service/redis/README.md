...
| `connection_mode` | enum `tls`/`tls_plain`/`plain` | **(v11)** режим сетевого канала: `plain` — только plain-порт (TLS off); `tls_plain` — TLS и plain одновременно; `tls` — только TLS (plain закрыт, `port 0`). Read-model day-2 (заменил булевы `tls_enabled`/`tls_keep_plain`) |
...
- `conf_dir` / `data_dir` — переехали в `essence` (`conf_dir`=`/etc/redis`,
  ...
  (day-2 `add_node` читает layout из `incarnation.state`) — источник правки сместился
