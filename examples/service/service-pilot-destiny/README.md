# service-pilot-destiny — пилот `apply:destiny` (слайс A) + `include:` (слайс B)
...
- **within-destiny include:** `destiny-pilot-flat/tasks/main.yml` подключает
...
(`destiny-pilot-flat/`) — герметичный L0-прогон грузит её из локального дерева,
...
```sh
cd keeper
go run ./cmd/soul-trial run ../examples/service/service-pilot-destiny/scenario/create/tests/render-flat/case.yml
```
