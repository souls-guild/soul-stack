...
func TestResolveEffectiveTelemetry(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:        "empty vars -> manifest values",
		...
		{
			name:        "vars overrides interval and collectors",
			manifest:    tel(nil, ptrStr("60s"), []string{"cpu"}),
			vars:        map[string]any{"telemetry_interval": "120s", "telemetry_collectors": []string{"mem", "disk"}},
			wantEnabled: true,
			wantSec:     120,
			wantCollect: []string{"mem", "disk"},
		},
		{
			name:        "vars collectors as []any (YAML form)",
			manifest:    nil,
			vars:        map[string]any{"telemetry_collectors": []any{"cpu", "load"}},
			wantEnabled: true,
			wantSec:     30,
		...
		},
		{
			...
			manifest:    nil,
			vars:        map[string]any{"telemetry_interval": "5s"},
			wantEnabled: true,
			wantSec:     10,
			wantCollect: allDefault,
		},
		{
			...
			manifest:    nil,
			vars:        map[string]any{"telemetry_interval": "2h"},
			wantEnabled: true,
			wantSec:     3600,
			wantCollect: allDefault,
		},
		{
			...
			manifest:    nil,
			vars:        map[string]any{"telemetry_collectors": []string{"cpu", "bogus", "mem"}},
			wantEnabled: true,
			wantSec:     30,
			wantCollect: []string{"cpu", "mem"},
		},
		{
			name:        "broken vars-interval -> default 30s (not floor)",
			manifest:    tel(nil, ptrStr("60s"), nil),
			vars:        map[string]any{"telemetry_interval": "not-a-duration"},
			wantEnabled: true,
			wantSec:     30,
			wantCollect: allDefault,
		},
		{
			name:        "vars does NOT override enabled (manifest false stays)",
			manifest:    tel(ptrBool(false), nil, nil),
			vars:        map[string]any{"telemetry_interval": "20s"},
			wantEnabled: false,
			wantSec:     20,
			wantCollect: allDefault,
		},
		{
			name:        "vars collectors not a string list -> fallback to manifest",
			manifest:    tel(nil, nil, []string{"cpu"}),
			vars:        map[string]any{"telemetry_collectors": []any{"cpu", 42}},
			wantEnabled: true,
			wantSec:     30,
			wantCollect: []string{"cpu"},
		},
		{
			name:        "all vars-collectors unknown + manifest nil -> fallback to default (all 6, NOT empty)",
			manifest:    nil,
			vars:        map[string]any{"telemetry_collectors": []string{"bogus", "nope"}},
			wantEnabled: true,
			wantSec:     30,
			wantCollect: allDefault,
		},
		{
			name:        "all vars-collectors unknown + manifest set -> fallback to manifest",
			manifest:    tel(nil, nil, []string{"cpu", "mem"}),
			vars:        map[string]any{"telemetry_collectors": []string{"bogus"}},
			wantEnabled: true,
			wantSec:     30,
			wantCollect: []string{"cpu", "mem"},
		},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := ResolveEffectiveTelemetry(tc.manifest, tc.vars)
...
func TestUnknownTelemetryCollectors(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := UnknownTelemetryCollectors(tc.vars); !reflect.DeepEqual(got, tc.want) {
