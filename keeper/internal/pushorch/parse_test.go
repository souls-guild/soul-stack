...
func TestParseDestinyRef_Valid(t *testing.T) {
	cases := []struct {
		...
		// Multiple `@` takes LAST as separator (git-ref may contain `@`,
		// destiny name does not).
...
func TestParseDestinyRef_Invalid(t *testing.T) {
	cases := []struct {
		...
		// kebab-case: `_` not allowed in name (regex reDestinyName).
		{"underscore_name", "redis_base@v1.4.0"},
		// kebab-case: leading dash not allowed in name.
		{"leading_dash_name", "-redis@v1.4.0"},
		// kebab-case: trailing dash not allowed in name.
		{"trailing_dash_name", "redis-@v1.4.0"},
		// kebab-case: double-dash not allowed in name.
		{"double_dash_name", "red--is@v1.4.0"},
		// kebab-case: uppercase not allowed in name.
