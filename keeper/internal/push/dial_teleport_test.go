...
func TestApplyProxyTLSTrust_SystemTrust(t *testing.T) {
	...
	applyProxyTLSTrust(cfg, true, "teleport.example.com")

	if cfg.RootCAs != nil {
		t.Error("RootCAs must be nil (system trust store) under use_system_trust")
	}
	if cfg.ServerName != "teleport.example.com" {
		t.Errorf("ServerName = %q, want host from proxy_addr %q", cfg.ServerName, "teleport.example.com")
	}
...
func TestApplyProxyTLSTrust_Default(t *testing.T) {
	...
	applyProxyTLSTrust(cfg, false, "teleport.example.com")
...
func TestNewTeleportDialer_SystemTrustProxyAddr(t *testing.T) {
	...
	t.Run("malformed proxy_addr fails", func(t *testing.T) {
		d, err := NewTeleportDialer(TeleportDialerConfig{
			ProxyAddr:      "teleport.example.com", // no port → SplitHostPort error
			IdentityFile:   id,
			Cluster:        "c1",
			UseSystemTrust: true,
		})
...
func TestBuildProxyClientConfig_AlpnUpgrade(t *testing.T) {
	tlsCfg := &tls.Config{ServerName: "teleport.example.com"}
	sshCfg := apissh.ClientConfig{User: "root"}

	t.Run("enabled", func(t *testing.T) {
		got := buildProxyClientConfig(
			TeleportDialerConfig{ProxyAddr: "teleport.example.com:443", AlpnUpgrade: true},
		...
		if got.ProxyAddress != "teleport.example.com:443" {
			t.Errorf("ProxyAddress = %q, want teleport.example.com:443", got.ProxyAddress)
		}
	...
	t.Run("disabled is default", func(t *testing.T) {
		got := buildProxyClientConfig(
			TeleportDialerConfig{ProxyAddr: "teleport.example.com:443"}, // AlpnUpgrade zero-value
			tlsCfg, sshCfg, 5*time.Second,
		)
		if got.ALPNConnUpgradeRequired {
...
func TestBuildProxyClientConfig_ResetsNextProtos(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			got := buildProxyClientConfig(
				TeleportDialerConfig{ProxyAddr: "teleport.example.com:443", AlpnUpgrade: tc.alpn},
