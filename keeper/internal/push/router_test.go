...
// returned from the corresponding field; covens are the host's own tags
// (NIM-281: the only ones there are), from map[sid][]string.
...
// TestPGRouter_Level2_MembershipDoesNotRoute — GUARD (NIM-281): a host that
// merely BELONGS to incarnation `redis-prod` does not route on
// `coven_default_providers: {redis-prod: bastion-eu}`. Belonging attaches no
// label, so Level 2 finds nothing and the host falls through to the cluster
// default.
//
// Routing a whole incarnation behind one bastion is done by tagging its hosts —
// the same explicit act as routing any other group. A route inferred from
// membership would move a host onto another bastion the moment it was bound,
// silently changing its auth perimeter (ADR-032: no silent provider fallback).
func TestPGRouter_Level2_MembershipDoesNotRoute(t *testing.T) {
	r := &fakeRouterReader{
		target: map[string]*soul.SSHTarget{"sid-1": {SSHPort: 22, SSHUser: "root", SoulPath: "/usr/local/bin/soul"}},
		covens: map[string][]string{"sid-1": {"db"}}, // member of redis-prod, not tagged with it
	}
	cfg := NewStaticRouterConfigSource(RouterConfig{
	...
	if name != "static-fallback" || src != SourceCluster {
		t.Errorf("got (%q, %v), want (static-fallback, SourceCluster) — only a tag an operator attached routes", name, src)
	}
}

// TestPGRouter_Level2_UnmappedTagFallsThrough — a host carrying tags that are
// simply absent from the map resolves on Level 3, not on an error.
func TestPGRouter_Level2_UnmappedTagFallsThrough(t *testing.T) {
	r := &fakeRouterReader{
		target: map[string]*soul.SSHTarget{"sid-1": {SSHPort: 22, SSHUser: "root", SoulPath: "/usr/local/bin/soul"}},
	...
	if name != "static-fallback" || src != SourceCluster {
		t.Errorf("got (%q, %v), want (static-fallback, SourceCluster)", name, src)
	}
}
...
func TestDispatcher_SendApply_UnknownProvider(t *testing.T) {
	...
	_, err := disp.SendApply(context.Background(), "host-1.example.com", Route{Provider: "ghost"}, req, nil)
