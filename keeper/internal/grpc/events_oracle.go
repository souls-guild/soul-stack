...
// souls + membership). db must satisfy [oracleDB] — *pgxpool.Pool does.
...
func (h *eventStreamHandler) evaluateDecree(
	...
	// the Decree's target incarnation. Not a member → skip + warn; fire is NOT
	// recorded, oracle.fired audit is NOT written: this forbids enqueueing an
	...
	// on behalf of an untrusted event.
	member, err := incarnation.IsMember(ctx, deps.DB, decree.IncarnationName, sid)
	if err != nil {
