...
func TestRegisterGRPCMetrics_RegistersFamilies(t *testing.T) {
	...
	// streams_active — gauge, виден сразу со значением 0. CounterVec без
	// первого WithLabelValues sample не публикует — проверим их после Observe.
...
func TestGRPCMetrics_NilReceiver_NoOp(t *testing.T) {
	// EventStream/Outbound/Bootstrap могут подниматься без obs-стека (unit-
	// тесты, dev-сборка). Все методы на nil-получателе — no-op без паники.
	var m *GRPCMetrics
	m.IncStreams()
	m.DecStreams()
	m.ObserveMessage(directionFromSoul)
...
// bootstrapSpanProbe — вспомогательная проверка span grpc.bootstrap, вызываемая
// из общего span-теста пакета (см. ниже). Bootstrap и dispatch делят один
// пакетный tracer "keeper/grpc"; otel-global делегат привязывается к первому
// установленному provider-у (internal/global), поэтому несколько тестов с
// собственным SetTracerProvider на одном tracer-name несовместимы — оба span-а
// проверяются под ОДНИМ recorder-ом.
...
// TestBootstrap_NoTracer_NoPanic — при no-op глобальном провайдере (OTel
// disabled) Bootstrap не падает, span бесплатен; nil Metrics — no-op.
...
// TestSendApply_DispatchSpan_Created проверяет, что dispatch порождает span
// с атрибутами sid/apply_id при enabled провайдере. SpanRecorder ставится
// глобальным TracerProvider-ом — пакетный `tracer` (otel.Tracer) делегирует
// к нему лениво.
func TestSendApply_DispatchSpan_Created(t *testing.T) {
	...
	// Soul не подключён — deliver вернёт ErrSoulNotConnected. Span обязан
	// создаться в любом случае (он оборачивает попытку dispatch-а).
	...
	// grpc.bootstrap делит тот же пакетный tracer "keeper/grpc"; otel-global
	// делегат привязывается к первому установленному provider-у, поэтому span
	// Bootstrap-RPC проверяется здесь же, под этим recorder-ом (отдельный тест
	// со своим SetTracerProvider отравил бы делегат — см. bootstrapSpanProbe).
	bootstrapSpanProbe(t, rec)
}

// TestSendApply_NoTracer_NoPanic — при отсутствии явного провайдера (no-op
// глобальный) dispatch не падает, span бесплатен. Симулирует OTel disabled.
