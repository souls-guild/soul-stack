...
func TestKeeperSideModules_IndexesOnlyKeeperSide(t *testing.T) {
	r := NewKeeperSideModules(&fakeSoulModuleSpawner{}, []Discovered{
		keeperSideEntry("wb-cloud", "vm", schema.SideKeeper),
		keeperSideEntry("redis", "acl", schema.SideSoul),
		keeperSideEntry("legacy", "thing", ""),
	}, nil)

	if _, ok := r.LookupKeeperSide("wb-cloud.vm"); !ok {
		t.Error("LookupKeeperSide(wb-cloud.vm): not found, want the side: keeper module")
	}
	...
	if got := r.Names(); len(got) != 1 || got[0] != "wb-cloud.vm" {
		t.Errorf("Names() = %v, want exactly [wb-cloud.vm]", got)
	}
}
...
func TestKeeperSideModules_ApplySpawnsForwardsAndCloses(t *testing.T) {
	...
	mod, ok := r.LookupKeeperSide("wb-cloud.vm")
	if !ok {
		t.Fatal("LookupKeeperSide(wb-cloud.vm): not found")
	}
	...
	if spawner.lastDiscovered.Module != "vm" || spawner.lastDiscovered.Alias != "wb-cloud" {
		t.Errorf("spawned %s.%s, want wb-cloud.vm", spawner.lastDiscovered.Alias, spawner.lastDiscovered.Module)
	}
