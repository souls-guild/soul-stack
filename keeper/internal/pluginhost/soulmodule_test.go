...
func TestSpawnSoulModule_RefusesUnrecognisedSide(t *testing.T) {
	...
	if _, err := h.SpawnSoulModule(context.Background(), keeperSideEntry("wb-cloud", "vm", "Keeper")); err == nil {
...
func TestDeclaredSide(t *testing.T) {
	if got := DeclaredSide(keeperSideEntry("wb-cloud", "vm", schema.SideKeeper)); got != schema.SideKeeper {
