...
// nameGuardProvisionScenarios are provision bodies where NIM-58 added guard-assert.
// Path relative to keeper/internal/coremod/cloud package (4 levels to repo root).
...
// TestProvisionScenariosPinVMNameBasePattern — pin-guard NIM-58: each provision body
// MUST validate incarnation.name exactly against [VMNameBasePattern] (single source of
// pattern). Catches YAML-literal vs Go-const drift — main risk of variant B
// (assert-string and driver drift silently).
func TestProvisionScenariosPinVMNameBasePattern(t *testing.T) {
	...
	for _, path := range nameGuardProvisionScenarios {
		...
		if !strings.Contains(string(body), want) {
			t.Errorf("%s: guard-assert %q missing — YAML literal drifted from cloud.VMNameBasePattern (NIM-58 pin-drift); sync assert in provision body with Go const", path, want)
		}
	}
}
...
func TestVMNameBaseDomainMismatch(t *testing.T) {
	...
	long63 := strings.Repeat("a", 63)       // max incarnation name length
	good48 := "r" + strings.Repeat("a", 47) // exactly 48 → ok for both domains
	...
	cases := []struct {
		...
		// Aligned: ok as incarnation name and as VM base.
