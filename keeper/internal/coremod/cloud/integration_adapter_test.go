...
// Integration tests of `core.cloud.provisioned` via [PluginAdapter] over
// keeper/internal/pluginhost. Full chain: discovery → adapter →
// Module.Apply → Spawn → CloudDriver.Create → INSERT into souls +
// bootstrap_tokens. Mimics production end-to-end scenario.
...
// buildFqdnPlugin builds testdata/cloud-plugin-fqdn under separate go.mod
// (with GOWORK=off, same as pluginhost/integration_test.go).
...
// shortDir is short temp-directory in /tmp; on darwin Unix socket sun_path
// limited to ~104 bytes, t.TempDir under /var/folders/... exhausts limit.
...
// setupAdapter brings up host + discovery + adapter with fqdn-plugin
// registered under name "fqdn".
...
`), 0o644); err != nil {
	...
	// for discovered-plugin, else Spawn fail-closed (no_trust_anchor / no_sigil).
	// Signature via same BuildSigilBlock as keeper-Signer (symmetry sign↔verify).
...
// TestIntegration_Apply_Created is happy path: adapter → plugin → souls/bootstrap_tokens.
func TestIntegration_Apply_Created(t *testing.T) {
	...
	m := cloud.New(
		...
		nil, // audit optional — not testing audit-pipeline here
	)
...
// TestIntegration_Apply_UnknownProvider is failed-event on unknown provider.
...
// TestIntegration_Apply_Destroyed_Cascade is ADR-017 cascade end-to-end:
// after successful PluginHost.Destroy, single PG-tx transitions souls→destroyed,
// active soul_seeds→orphaned, active bootstrap_tokens→burned.
// Revoked seeds NOT overwritten (precedence revoked > orphaned).
func TestIntegration_Apply_Destroyed_Cascade(t *testing.T) {
	resetCloud(t)
	ctx := context.Background()

	// First create 2 hosts via Apply(created) — gives full set:
	// souls(pending) + bootstrap_tokens(active) + soul_seeds(0).
	adapter := setupAdapter(t)
	m := cloud.New(adapter,
		&fakeResolver{},
		cloud.NewSoulPG(integrationPool),
	...
	// Simulate issuing active-seed for host-1: cascade should transition it
	// to `orphaned`, host-2 has no active seed (cascade ignores without error).
	_, err := integrationPool.Exec(ctx,
		...
		"host-1.example.com",
		// 64 lower-hex, valid fingerprint-format.
	...
	// For host-2 — revoked-seed (must not be overwritten by orphaned).
	_, err = integrationPool.Exec(ctx,
		`INSERT INTO soul_seeds (sid, fingerprint, serial_number, expires_at, status)
	...
	// Apply(destroyed) with both SIDs.
	...
	// bootstrap_tokens: both → burned (used_at != NULL, used_by_kid = system-cloud-destroy).
	...
	// Module output — contains cascade-counts.
	out := ev.Output.AsMap()
	...
	// seeds_orphaned = 1 (only active host-1; revoked host-2 skipped).
...
// TestIntegration_Apply_Destroyed_Atomicity tests if PluginHost.Destroy
// returns error, registries remain untouched (souls.status NOT changed to
// destroyed). Tests "cascade AFTER Destroy" invariant.
func TestIntegration_Apply_Destroyed_Atomicity(t *testing.T) {
	...
	// Destroy with unknown provider → PluginHost.Destroy fails.
	destroyStream := internaltest.NewApplyStream()
	if err := m.Apply(&pluginv1.ApplyRequest{
		State: cloud.StateDestroyed,
		Params: mustStructIT(t, map[string]any{
	...
	// souls.status should remain pending (cascade not executed).
	var status string
