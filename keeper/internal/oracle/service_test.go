...
func TestService_CreateVigil_OK(t *testing.T) {
	...
	if v.ID != "web-conf" || v.CheckAddr != "core.beacon.file_changed" {
		t.Errorf("vigil = %+v", v)
	}
}
...
func TestService_CreateVigil_ValidationBeforeDB(t *testing.T) {
	...
	cases := []struct {
		...
		{"bad name", CreateVigilInput{ID: "BAD", Subject: covenSel("web"), Interval: "30s", Check: "core.beacon.file_changed"}},
		{"bad interval", CreateVigilInput{ID: "x", Subject: covenSel("web"), Interval: "nope", Check: "core.beacon.file_changed"}},
		{"unknown check", CreateVigilInput{ID: "x", Subject: covenSel("web"), Interval: "30s", Check: "core.beacon.bogus"}},
		{"subject two dimensions", CreateVigilInput{ID: "x", Subject: subject.Selector{Covens: []string{"web"}, SIDs: []string{"h1"}}, Interval: "30s", Check: "core.beacon.file_changed"}},
		{"subject none", CreateVigilInput{ID: "x", Interval: "30s", Check: "core.beacon.file_changed"}},
		{"subject half-written incarnation", CreateVigilInput{ID: "x", Subject: subject.Selector{Incarnation: "redis-prod"}, Interval: "30s", Check: "core.beacon.file_changed"}},
		{"subject half-written trait", CreateVigilInput{ID: "x", Subject: subject.Selector{TraitKey: "tier"}, Interval: "30s", Check: "core.beacon.file_changed"}},
	}
	for _, c := range cases {
...
func TestService_CreateDecree_ValidationBeforeDB(t *testing.T) {
	...
	cases := []struct {
		...
		{"bad incarnation", CreateDecreeInput{ID: "x", OnBeacon: "db-svc", Subject: covenSel("db"), IncarnationName: "BAD..NAME", ActionScenario: "restart_service"}},
		{"bad scenario", CreateDecreeInput{ID: "x", OnBeacon: "db-svc", Subject: covenSel("db"), IncarnationName: "prod-db", ActionScenario: "Bad-Scenario"}},
		{"subject neither", CreateDecreeInput{ID: "x", OnBeacon: "db-svc", IncarnationName: "prod-db", ActionScenario: "restart_service"}},
		{"bad cooldown", CreateDecreeInput{ID: "x", OnBeacon: "db-svc", Subject: covenSel("db"), IncarnationName: "prod-db", ActionScenario: "restart_service", Cooldown: "nope"}},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
