...
// validateInput calls [ValidateInput] with the zero incarnation context — the
// input-only stance. The cases in this file and in validate_input_types_test.go
// are about the input SCHEMA; the incarnation half of the context has its own
// guard (validate_scope_guard_test.go), and running these through a stance would
// only make every one of them assert something it is not about.
func validateInput(ctx context.Context, loader InputScenarioLoader, ref artifact.ServiceRef, scenarioName string, provided map[string]any) (InputGate, error) {
	return ValidateInput(ctx, loader, ref, scenarioName, provided, config.ValidateContext{})
}
