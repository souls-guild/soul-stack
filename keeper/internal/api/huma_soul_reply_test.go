...
func TestGoldenWire_SoulReply(t *testing.T) {
	...
	// --- SoulListEntry: full (all nullable set, covens array) ---
	aid := "archon-alice"
	kid := "keeper-01"
	...
	// --- SoulListEntry: nullable nil → `null` (WITHOUT omitempty), covens/traits empty (handler
	// coalesce → `[]`/`{}`) ---
	...
	// --- SoulHistoryReply: scenario-item (incarnation/scenario set, module nil) ---
	inc := "web-prod"
	scen := "deploy"
	voy := "voy-123"
	...
	// --- errand-item (module set, scenario/incarnation/finished_at nil — running) ---
	mod := "core.cmd.shell"
...
// TestGoldenWire_SoulProjection checks, which projection of domain handlers.Soul*View → native
// preserves byte-exact wire against golden snapshot. Catches regression in mapping fields.
func TestGoldenWire_SoulProjection(t *testing.T) {
	...
	// transport=ssh: token/expires nil → key omitted; covens empty (handler gives coalesce → `[]`).
	emptyCovens := []string{}
	...
	// covens/traits empty (handler coalesce → `[]`/`{}`); projection byte-passthrough preserves form.
	...
	// nil-Items branch: handler gives non-nil [], but projection must preserve nil → `null`.
