...
func TestGoldenWire_RoleReply(t *testing.T) {
	...
	goldenRoleWire(t, "RoleView/full",
		...
		`{"builtin":true,"default_scope":"coven=prod,eu","description":"ops role","effective_permissions":["role.list","service.list"],"effective_scope":"coven=prod,eu","inert_permissions":null,"name":"ops","operators":["archon-alice"],"permissions":["role.list","service.list"]}`)
	...
	goldenRoleWire(t, "RoleView/nil_scope_empty_lists",
		...
		`{"builtin":false,"effective_permissions":[],"inert_permissions":null,"name":"viewer","operators":[],"permissions":[]}`)
	...
	goldenRoleWire(t, "RoleView/nil_lists",
		...
		`{"builtin":false,"effective_permissions":null,"inert_permissions":null,"name":"x","operators":null,"permissions":null}`)
	...
	goldenRoleWire(t, "RoleView/derived",
		RoleView{Builtin: false, DefaultScope: &delta, EffectivePermissions: []string{"incarnation.get on coven=dba AND trait.project=aboba"}, EffectiveScope: &effective, Name: "dba-aboba", Operators: []string{}, ParentRole: &parent, Permissions: []string{"incarnation.get"}, ScopeMode: &mode},
		`{"builtin":false,"default_scope":"trait.project=aboba","effective_permissions":["incarnation.get on coven=dba AND trait.project=aboba"],"effective_scope":"coven=dba AND trait.project=aboba","inert_permissions":null,"name":"dba-aboba","operators":[],"parent_role":"dba","permissions":["incarnation.get"],"scope_mode":"track"}`)
	...
	goldenRoleWire(t, "RoleListReply/items",
		...
		`{"items":[{"builtin":true,"effective_permissions":[],"inert_permissions":null,"name":"ops","operators":[],"permissions":[]}]}`)
