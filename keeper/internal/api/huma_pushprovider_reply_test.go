// GOLDEN byte-exact wire-guard для NATIVE wire-DTO PUSH-PROVIDER-домена (handler-native T5d).
// push-provider больше НЕ зависит от legacy-генерата — golden сверяет json native-значения с
// ЗАФИКСИРОВАННОЙ строкой-эталоном (pinned). Покрыты обе ветки updated_by_aid (nil/non-nil) и
// params ({}-нормализация). Мутация формы native-struct (убрать omitempty / сменить json-тег /
// тип поля / порядок) краснит соответствующий case.
package api
...
func TestGoldenWire_PushProviderReply(t *testing.T) {
	...
	// --- PushProvider: updated_by_aid set + params наполнен ---
	...
	// updated_by_aid nil → ключ опущен (omitempty); params пустой {} (handler нормализует).
	...
	// --- PushProviderListReply (envelope как top-level reply-DTO) ---
