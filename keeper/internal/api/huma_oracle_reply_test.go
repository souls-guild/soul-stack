// GOLDEN byte-exact wire-guard для huma-native reply-DTO ORACLE-домена (handler-native
...
//
//   - категория A (date-time): created_at/updated_at → RFC3339Nano-байт;
//   - категория B ([]-vs-null): items БЕЗ omitempty (nil → null, [] → []) — обе ветки envelope;
//   - категория C (omitempty): coven/sid/created_by_aid/where — ключ опущен при nil;
//   - категория D (byte-passthrough): VigilView.params / DecreeView.action_input —
//     json.RawMessage as-is (nil → null);
//   - FIELD-ORDER: порядок ключей под прежний oapi byte-order (лексикографический по json-тегу).
//
...
// goldenOracle маршалит native-значение и сверяет байты против ожидаемой golden-строки.
...
func TestGoldenWire_OracleReply(t *testing.T) {
	...
	// --- VigilListReply: items non-nil / nil (категория B) ---
