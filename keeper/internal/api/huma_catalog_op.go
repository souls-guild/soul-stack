...
func permissionsListOperation() huma.Operation {
	return huma.Operation{
		...
		Description:   "Машиbutчитаемый каталог `<resource>.<action>` (источник rbac.catalog.go), сгруппированный по resource. Auth-only, без отдельbutй permission (само-описывающий). Read-only, no audit.",
...
func eventTypesListOperation() huma.Operation {
	return huma.Operation{
		...
		Method:        http.MethodGet,
		Path:          "/event-types",
		Summary:       "Каталог event-types for Tiding-подписки",
		Description:   "Допустимые for подписки Tiding типы: areas (area-glob `<name>.*`) + точечные point_events (источник herald/eventtypes.go). Auth-only, без отдельbutй permission (само-описывающий). Read-only, no audit.",
...
func heraldTypesListOperation() huma.Operation {
	return huma.Operation{
		...
		Method:        http.MethodGet,
		Path:          "/herald-types",
		Summary:       "Каталог типов Herald-каonла",
		Description:   "Типы каonла увеtoмлений и их config-поля (webhook/telegram/slack/mattermost/discord/custom/email): name/label/required/secret/kind. Источник — herald.TypeCatalog (тот же, which валидирует CRUD). Auth-only, без отдельbutй permission (само-описывающий). Read-only, no audit.",
		Tags:          []string{"herald"},
		DefaultStatus: http.StatusOK,
		Errors:        []int{http.StatusInternalServerError},
	}
...
func myPermissionsListOperation() huma.Operation {
	return huma.Operation{
		...
		Method:        http.MethodGet,
		Path:          "/me/permissions",
		Summary:       "Эффективные права текущits Архонта",
		Description:   "Подмbutжество каталога, реальbut выданbutе текущему оператору (AID from JWT-claims). Auth-only (свои права видит любой аутентифицированный; чужие не отgives). Read-only, no audit.",
		Tags:          []string{"permission"},
		DefaultStatus: http.StatusOK,
		Errors:        []int{http.StatusInternalServerError},
	}
