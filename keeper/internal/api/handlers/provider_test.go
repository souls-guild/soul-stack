...
func TestProviderHandler_CreateGetListDelete(t *testing.T) {
	...
	if reply.AuditPayload()["credentials_ref"] != "vault:secret/cloud/example" {
		t.Errorf("audit credentials_ref = %v", reply.AuditPayload()["credentials_ref"])
	}

	got, err := h.GetTyped(ctx, "example-cloud")
	if err != nil {
		t.Fatalf("GetTyped: %v", err)
	}
	if got.Type != "example" || got.Region != "ru-1" {
		t.Fatalf("get = %+v", got)
	}
	...
	del, err := h.DeleteTyped(ctx, "example-cloud")
	if err != nil {
...
func TestProviderHandler_DeleteBlockedByProfiles(t *testing.T) {
	...
	pool.hasProfiles["example"] = true
	_, err := h.DeleteTyped(ctx, "example")
