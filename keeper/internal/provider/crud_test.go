...
func TestValidFQDNSuffix(t *testing.T) {
	good := []string{"example", "vm.example", "ns.vm.example", "a.b.c"}
	bad := []string{"", ".example", "example.", "vm..example", "with_underscore.example", "UPPER.example", "-lead.example"}
	for _, s := range good {
...
func TestInsert_FQDNSuffix(t *testing.T) {
	t.Run("valid suffix passed as args[5]", func(t *testing.T) {
		...
		p := validProvider()
		suffix := "ns.vm.example"
		p.FQDNSuffix = &suffix
		if err := Insert(context.Background(), f, p); err != nil {
			t.Fatalf("Insert: %v", err)
		}
		if f.queryRowArgs[5] != "ns.vm.example" {
			t.Errorf("args[5] fqdn_suffix = %v, want ns.vm.example", f.queryRowArgs[5])
		}
	})
