...
type CertRotatorConfig struct {
	...
	// KVMount — keeper.yml `vault.kv_mount`, the mount the rotated cert+key are
	// WRITTEN to (certissue.VaultPath). Distinct from DefaultPKIMount, which is where
	// the CSR is signed. Empty → the default KV mount; hot-reloaded like the rest of
	// this struct, so a mount change reaches the next tick.
	KVMount string
}
...
func (r *CertRotator) rotateOne(ctx context.Context, c dueCert, cfg CertRotatorConfig) (bool, error) {
	...
	// kvMount travels from the SAME cfg snapshot resolvePKI took mount/role from, rather
	// than being re-read here: a hot-reload landing between the two reads would sign
	// against one keeper.yml and write against another.
	material, ierr := r.issueMaterial(ctx, c, pol.Service, mount, role, cfg.KVMount)
	if ierr != nil {
...
// <kv_mount>/<service>/<inc>/tls/{cert,key}).
//
...
func (r *CertRotator) issueMaterial(ctx context.Context, c dueCert, service, mount, role, kvMount string) (*certissue.Material, error) {
	...
	return certissue.Issue(ctx, r.signer, r.vault, r.csrgen, certissue.Params{
		...
		Mount:      mount,
		Role:       role,
		CertPath:   certissue.VaultPath(kvMount, service, c.incarnationID, keepercert.KindCert),
		KeyPath:    certissue.VaultPath(kvMount, service, c.incarnationID, keepercert.KindKey),
	})
}
