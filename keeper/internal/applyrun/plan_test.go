...
// selectRunPlanByApplyIDSQL columns (plan_index/name/module/passage). Scan reuses
...
// the same order as tasks. It catches column order drift and a lost passage.
func TestInsertRunPlan_BulkArrays(t *testing.T) {
	f := &fakeDB{}
	tasks := []RunPlanTask{
		{ApplyID: "AP", PlanIndex: 0, Name: "install redis", Module: "core.pkg.installed", Passage: 0, Params: []byte(`{"name":"redis"}`)},
		{ApplyID: "AP", PlanIndex: 1, Name: "set password", Module: "core.exec.run", Passage: 0, Params: nil},
		{ApplyID: "AP", PlanIndex: 2, Name: "restart", Module: "core.service.running", Passage: 1, Params: []byte(`{"unit":"redis"}`)},
	}
	...
	// args: $1 apply_id, $2 plan_index[], $3 name[], $4 module[], $5 passage[], $6 params[].
	if len(f.execArgs) != 6 {
		t.Fatalf("execArgs len = %d, want 6", len(f.execArgs))
	}
	if strings.Contains(f.execSQL, "no_log") {
		t.Errorf("SQL still writes no_log, removed with the DSL key by ADR-0083 §8: %q", f.execSQL)
	}
	if f.execArgs[0] != "AP" {
	...
	passages, ok := f.execArgs[4].([]int)
	if !ok || passages[2] != 1 {
		t.Errorf("args[4] passage[] = %v, want [0 0 1]", f.execArgs[4])
	}
	// params[] is a parallel *string array: JSON or nil (NULL) per position.
	params, ok := f.execArgs[5].([]*string)
	if !ok || len(params) != 3 {
		t.Fatalf("args[5] params[] = %v, want []*string len 3", f.execArgs[5])
	}
	if params[0] == nil || *params[0] != `{"name":"redis"}` {
		t.Errorf("params[0] = %v, want {\"name\":\"redis\"}", params[0])
	}
	if params[1] != nil {
		t.Errorf("params[1] = %v, want nil (task without params → NULL)", *params[1])
	}
...
// passage/params -> RunPlanTask. ApplyID is set by caller from the
// argument. params: row 0 carries jsonb -> []byte, row 1 is NULL -> nil.
func TestSelectRunPlanByApplyID_Scan(t *testing.T) {
	f := &fakeDB{
		queryFunc: func(_ string) (pgx.Rows, error) {
			return &planRowsStub{rows: [][]any{
				{0, "install", "core.pkg.installed", 0, []byte(`{"name":"redis"}`)},
				{1, "secret", "core.exec.run", 1, nil},
			}}, nil
		},
	}
	...
	if got[0].ApplyID != "AP" || got[0].PlanIndex != 0 || got[0].Module != "core.pkg.installed" {
		t.Errorf("row0 = %+v", got[0])
	}
	if string(got[0].Params) != `{"name":"redis"}` {
		t.Errorf("row0 Params = %s, want {\"name\":\"redis\"}", got[0].Params)
	}
	if got[1].Passage != 1 || got[1].Name != "secret" {
