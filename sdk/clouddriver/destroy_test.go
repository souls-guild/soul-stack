...
// TestReportDestroy_PerVMOutcomes pins the reporting contract: only a
// confirmed teardown is a success, and an unconfirmed one is loud.
func TestReportDestroy_PerVMOutcomes(t *testing.T) {
	boom := errors.New("read denied")
	results := []DestroyResult{
		{VMID: "vm-gone", Gone: true},
		{VMID: "vm-err", Err: boom},
		{VMID: "vm-present"},
	}
	s := &fakeDestroyStream{}
	if err := ReportDestroy(s, results, nil, func(error) FailClass { return FailAuth }); err != nil {
		t.Fatalf("ReportDestroy: %v", err)
	}
	if len(s.sent) != 3 {
		t.Fatalf("sent %d events, want 3", len(s.sent))
	}
	if s.sent[0].Failed || s.sent[0].Message != "destroyed" {
		t.Errorf("gone VM event=%+v, want a clean success", s.sent[0])
	}
	if !s.sent[1].Failed || !strings.Contains(s.sent[1].Message, "read denied") {
		t.Errorf("errored VM event=%+v, want failed with the cause", s.sent[1])
	}
	if !s.sent[2].Failed || !strings.Contains(s.sent[2].Message, "not confirmed") {
		t.Errorf("still-present VM event=%+v, want an explicit not-confirmed failure", s.sent[2])
	}
	for i, e := range s.sent {
		if e.VmId == "" {
			t.Errorf("event %d has no vm_id — the caller cannot act on it", i)
		}
	}
}

// TestReportDestroy_PhaseErrorAppended: the budget diagnosis reaches the
// operator instead of dying inside the driver.
func TestReportDestroy_PhaseErrorAppended(t *testing.T) {
	results := []DestroyResult{{VMID: "vm-1"}}
	phaseErr := &DestroyDeadlineError{
		Attempts: 3, Elapsed: 5 * time.Second, Total: 1,
		Pending: []PresentVM{{VMID: "vm-1", LastState: "DELETE_FAILED", DeleteRejected: true}},
	}
	s := &fakeDestroyStream{}
	if err := ReportDestroy(s, results, phaseErr, nil); err != nil {
		t.Fatalf("ReportDestroy: %v", err)
	}
	last := s.sent[len(s.sent)-1]
	if !last.Failed || !strings.Contains(last.Message, "DELETE_FAILED") {
		t.Errorf("final event=%+v, want the deadline diagnosis", last)
	}
}
