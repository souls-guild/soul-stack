...
First tag of the closed beta. One git tag = version of all 7 go.work modules ([ADR-011](docs/adr/0011-go-layout.md)); the version is injected into binaries via `-X main.<var>`, printed by `keeper version` / `soul version` / `soulctl version`. Beta distribution is build-from-source ([CONTRIBUTING.md](CONTRIBUTING.md)); the release procedure is [RELEASING.md](RELEASING.md).
...
- Event-driven monitoring ([ADR-030](docs/adr/0030-vigil-oracle.md#adr-030-vigil--oracle--event-driven-monitoring-beacons--reactor)): a SaltStack beacons+reactor analog.
